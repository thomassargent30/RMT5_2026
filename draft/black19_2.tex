...
\section{Introduction}
   ...
   This chapter presents various equilibrium models of
the labor market. %search and matching.
We
describe (1) Lucas and Prescott's version of search in an island model; (2) some matching
models in the style of Mortensen, Pissarides, and Diamond; and (3)
a model of employment lotteries as formulated by Rogerson and Hansen.

%a search model of money along the lines of Kiyotaki and Wright.

Chapter \use{search1} studied the optimization problem of a single
unemployed worker who searches for a job by drawing from an
exogenous wage offer distribution.  We now turn to a model with a
continuum of workers who interact across a large number of
spatially separated labor markets. Phelps (1970, introductory
chapter)
\auth{Phelps, Edmund S.}%
recommended  such an ``island economy'' as a good model of labor market frictions.
\index{island model}%
We present an analysis in the spirit of  Lucas and Prescott's (1974) version of such an economy.
\auth{Lucas, Robert E., Jr.}\auth{Prescott, Edward C.}%
Workers  on an island can choose to work at
...
A distinct  approach to modeling unemployment is the matching framework
...
This framework  postulates  a matching function that
...
As a comparison point for  models with search and matching frictions, we also study a
...
written as \index{Bellman equation}%
...
$$
Given a value for $v_u$, this functional equation
has a unique solution $v(\theta, x)$ that is
...
with solution
$$
E-U \;=\; \phi S \quad  \hbox{and} \quad J \;=\; (1-\phi) S\,. \EQN split
$$
...
\index{employment lottery}
\section{Employment lotteries}\label{Empl_lottery}%
...
differences between Prescott's (2005b) example and ours, but none
...
care whether it continues or ceases  to exist. \newpage \noindent Our
...
of employment determination.}\auth{Ljungqvist, Lars}%
...
according to a Markov process, and a bad enough
realization triggers a layoff. The government imposes
a  tax $\tau$ on each layoff. \newpage \noindent  The tax revenues
...
employee with productivity $p$: \index{Bellman equation}%
$$\eqalign{
V(p) &\;=\; \max \, \Bigl\{p \,-\, w \,+\,
...
We let
$\psi^*$ denote the optimally chosen probability of working.  It is equal to $N$ in  equilibrium;
the corresponding
...
$$
N(\tau) = { 2 w(\tau) \over A \left[ 2 \bar p(\tau) + 1
- \bar p(\tau)^2 \right] }
$$
with its derivative
$$
...
A \left[ 2 \bar p(\tau) + 1 -  \bar p(\tau)^2 \right]^2 }\,.
$$
...
Layoff taxes also cause employment  to rise in the general
version of the island model. For a given expected value of
arriving on a new
...
$$
and equation \Ep{value2aa} changes to
$$\theta \, f'(n) \,+\, \beta E\left[v(\theta',n) \vert \theta \right]
      \;=\; \beta v_u \,-\, \tau \,.             \EQN Case1_lay
$$
Lucas and Prescott (1974, p.\ 205) describe how a mobility cost like $\tau$
dampens the reallocation of labor and shifts the curve $\bar x(v_u)$
in Figure \Fg{fig191f} downward. The result is a decrease in
unemployment and a decrease in the equilibrium value of $v_u$.
\auth{Lucas, Robert E., Jr.}\auth{Prescott, Edward C.}%
Hence, Lucas and Prescott note ``that lower
average unemployment is not, in general, associated with higher
welfare for workers.'' They also suggest that the analysis could
conceivably ``help to account for the observed differences
in average unemployment across occupations and among countries.''


%An implication here is that $x^+(\theta)$ rises in response to a
%higher layoff tax. The unchanged expression \Ep{value2bb} means that
%$x^-(\theta)$ falls as a result of the preceding drop in the value
%function. In other words, the range of an island's employment levels
%characterized by no labor movements is enlarged. This effect will shift
%the curve in Figure \Fg{fig191f}  %%19.1
%downward and decrease the equilibrium value of $v_u$. Less labor
%reallocation maps directly into a lower unemployment rate.
...
(1-\phi) S_b(p_0)\,-\, \phi \tau \;&=\; {c \over \beta q(\theta)} \,,
                                                         \EQN noprofit_b \cr}
$$
for Nash products \Ep{Nash_a} and \Ep{Nash_b}, respectively.
...
%Agents meet pairwise and at random.
In each period, an agent meets
one other agent with probability $\theta \in (0,1]$; he meets no other agent
with probability $1-\theta$. Two agents who meet  trade if there is
...
respectively. The Bellman equations can be written \index{Bellman equation}%
$$\EQNalign{
...
As a work horse for frictional unemployment in macro labor research,
we continue to explore the mechanics of the matching framework in
chapter \use{mechanics_matching}. In chapter \use{macrolaborII}, we study
how ``time averaging'' has replaced employment lotteries as a
theoretical foundation of  aggregate labor supply.
...
\noindent{\bf b.} How would an increase in $\mu$ affect  an unemployed  worker's
behavior?
...
firms have to pay a cost of $c>0$ units of output per period.  Jobs are exogenously destroyed
...
function $C(v)$ is strictly convex, twice differentiable and increasing.
  Jobs are exogenously destroyed with probability $s$.
\medskip
\noindent
