...
interprets covariations  among    all of these components
...
of {\it functions}: a transition function mapping the {\it state} today into the state tomorrow,
 and another function mapping the state today into
the other endogenous variables of the model today.
...
other chapters from earlier editions. 
 New to this edition
are chapter
\use{optaxrecur} on recursive formulations of optimal taxation problems and chapter
 \use{mechanics_matching}
...
Chapter \use{mechanics_matching} studies the mechanics of
matching models and sheds light on them by exploring  two substantive issues,
the `Shimer puzzle' and the role of heterogeneity.
% regarding the sensitivity of unemployment to
%variations in productivity, and a recipe of proliferating matching
%functions to gain analytical tractability.
The ``Shimer puzzle'' is  the finding that common calibrations of the standard
matching model do not generate fluctuations in unemployment rates
nearly as large as those observed during business cycles.\NFootnote{A puzzle is always relative to a  model. A `puzzle' is a
%L%feature or
prediction of a model that
is contradicted by data.} \index{puzzle!definition}%
The chapter looks under the hoods of various  matching models that
have been reconfigured to generate big responses in unemployment to
movements in productivity. An outcome of this cross-model investigation is the
discovery of a single channel that we call
``fundamental surplus'' that is common across all of the models.
%These disparate models are found to operate through the same channel
%that we call the ``fundamental surplus.''
%One way to handle heterogeneity is to specify a matching function
%for each type of firm-worker pairs.
%The chapter studies how it can lead to a ``block recursive''
%equilibrium in the sense that value functions and measures of
%market tightness are independent of distributions of agents.
Turning to  heterogeneity,  we study a class of models with multiple   type-specific
matching functions whose equilibria have a block recursive structure in which value
functions and market tightnesses are independent of
distributions of agents. Such models deliver significant analytical tractability; but the
 ``directed search'' that prevails within them  also attenuates the  congestion externalities that
are at the center of other kinds of  matching models.


%L%The ``Shimer puzzle'' is  that standard calibrations of matching models do not generate fluctuations in unemployment rates nearly as large as those observed over business cycles.\NFootnote{A puzzle is always relative to a  model. A `puzzle' is a feature or  prediction of a model that is contradicted by data.} \index{puzzle!definition}%
%L%Chapter \use{mechanics_matching} looks under the hood of a variety of matching models that have been reconfigured in attempts to confront the Shimer puzzle. The chapter examines a variety of different reconfigurations of the standard model and identifies  a single common economic force that each  of these diverse specifications activates in order address the Shimer critique.
...
for opportunities to buy or sell goods or factors of production, opportunities that might
...
into opportunities to  form matches.  Models with matching functions
...
as the fraction of people that a planner assigns to work by using  a lottery
...
Browning, Hansen, and Heckman (1999) describe two justifications
...
to transport parameters from one data set and model specification
...
Chapters \use{timeseries}  and \use{timeseriesapp} describe   two basic models of a time series:
...
describes some numerical techniques to  solve dynamic programs based
on Markov chains.  Chapter \use{dplinear} describes linear
quadratic dynamic programming and some uses  of it. 
This chapter also tells how the Kalman
filter from chapter \use{timeseries} is
mathematically equivalent to
\newpage
\noindent the linear quadratic dynamic
...
`feedforward' and `feedback' components of mathematical
\newpage
\noindent
 formulas  for equilibrium outcomes.
...
some fundamental forces common to a variety of otherwise quite disparate matching models.   Chapter \use{macrolaborII}
