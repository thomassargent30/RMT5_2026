...
recursive in  natural state variables and makes  the government's optimal
...
are augmented with state variables that are simultaneously  `forward looking' and `backward looking', this class of problems
...
In some examples, the  Stackelberg leader is  a government or Pareto planner and
the Stackelberg follower is a representative agent or
private sector.  In section \use{sec:monopfringe},   we'll give industrial organization
 application with another  interpretation of the leaders and followers.
 We'll begin with another industrial organization example, namely,
 the duopoly model that we used as a laboratory  in chapter \use{recurpe} for illustrating
 the formulation and computation of a Markov perfect equilibrium.
 \index{Markov perfect equilibrium}%



\section{Duopoly example}\label{sec:duopoly}%
...
By incurring a cost of change $ \gamma v_{it}^2  $,
...
\subsection{Stackelberg leader and follower}
In contrast to the timing protocol within the Markov perfect equilibrium studied in chapter \use{recurpe}, in the setting to be analyzed here
each firm $i=1,2$ chooses a sequence $\vec q_i \equiv \{q_{it+1}\}_{t=0}^\infty$ once and for all at time $0$.
We  let firm 2 be a {\it Stackelberg leader\/} and firm 1 be a {\it Stackelberg follower}.
...
In choosing $\vec q_2$, firm 2 takes into account that firm 1 will base its choice of
$\vec q_1$ on firm 2's choice of $\vec q_2$.



We begin with a concise  statement of the Stackelberg leader's and follower's problems.
We can express firm 1's problem as
$$ \max_{\vec q_1} \Pi_1(\vec q_1; \vec q_2)  $$
where the appearance behind the semi-colon indicates that  $\vec q_2$ is taken as given.
...
whose maximizer is a sequence $\vec q_2$ that depends on the initial conditions $q_{10}, q_{20}$ and the parameters of
the model $a_0, a_1, \gamma$.


While our abstract formulation reveals the timing protocol and equilibrium concept well, it obscures details
that must be addressed when we want to compute and interpret a Stackelberg plan and the follower's best response to it
To gain insights about these things, we study them in more detail

\subsection{The   firms' problems}
...
Firm 1 seeks a maximum with respect to $\{q_{1t+1}, v_{1t} \}_{t=0}^\infty$ and a minimum with respect
to $\{ \lambda_t\}_{t=0}^\infty$.
We approach this problem using the lag operator  methods described in chapter \use{timeseries}, appendix A and Sargent (1987a,chapter IX).
...
These first-order   conditions and the constraint $q_{1t+1} = q_{1t} + v_{1t} $
...
$$ \eqalign{ v_{1t} & = \beta v_{1t+1} + {\frac{\beta a_0}{2 \gamma}} - {\frac{\beta a_1}{\gamma}} q_{1t+1} -
             ...
             q_{t+1} & = q_{1t} + v_{1t} \cr } \EQN eq_duop0$$
...
$$ - q_{1t} + (1+\beta + c_1) q_{1t+1} - \beta q_{1t+2} = c_0 - c_2 q_{2t+1} \EQN eq_duop1 $$
Equation (1) is a second-order difference equation in the sequence $\vec q_1$ whose solution we want to satisfy two boundary conditions, namely,
an initial condition that $q_{1,0}$ is given and a  terminal condition requiring that $\lim_{T\rightarrow + \infty} \beta^T q_{1t}^2 < + \infty $.

Using the lag operators described in appendix A of chapter \use{timeseries} and  chapter IX of Sargent (1987a, chapter IX), difference equation \Ep{eq_duop1} can be written as
$$ \beta (1 - {\frac{1+\beta + c_1}{\beta}} L + \beta^{-1} L^2 ) q_{1t+2} = - c_0 + c_2 q_{2t+1} \EQN eq_duop2 $$
The polynomial in the lag operator on the left side of equation \Ep{eq_duop2} can be {\it factored\/} as
$$ (1 - {\frac{1+\beta + c_1}{\beta}} L + \beta^{-1} L^2 )  = ( 1 - \delta_1 L ) (1 - \delta_2 L) $$
where $0 < \delta_1 < 1 < {\frac{1}{\sqrt{\beta}}} < \delta_2 $.
Because $\delta_2 > {\frac{1}{\sqrt{\beta}}}$
the operator $(1 - \delta_2 L)$ contributes an {\it unstable\/} component if solved {\it backwards\/} but a
{\it stable\/} component if solved {\it forwards}.
To see this, mechanically  write
$$ (1- \delta_2 L) = -\delta_{2} L (1 - \delta_2^{-1} L^{-1} ) $$
and compute the following inverse operator
$$ \left[-\delta_{2} L (1 - \delta_2^{-1} L^{-1} )\right]^{-1} = - \delta_2^{-1} (1 - {\delta_2}^{-1} L^{-1} )^{-1} L^{-1} $$
Operating on both sides of equation \Ep{eq_duop2} with $\beta^{-1}$ times  this inverse operator gives the follower's decision rule for setting  $q_{1t+1}$
in the  {\it feedback-feedforward\/} form
...
\EQN eq_duop3 $$
This representation summarizes the follower's best response function and  shows how the continuation $\{q_{2s}\}_{s=t+1}^\infty$  of the leader's setting of $\vec q_2$ influences the follower's choice of $q_{1t+1}$.


The problem of the Stackelberg leader firm 1 is to choose the sequence $\{q_{2t+j+1}\}_{t=0}^\infty$ to
maximize its discounted profits
$$ \sum_{t=0}^\infty \{ (a_0 -  a_1 (q_{1t} + q_{2t})) q_{2t}  - \gamma (q_{2t+1} - q_{2t})^2 \} $$
subject to the sequence of constraints \Ep{eq_duop3} for $t \geq 0$.
We can attach a sequence $\{\theta_t\}_{t=0}^\infty$ of Lagrange multipliers to  the sequence of equations \Ep{eq_duop3}    and formulate the following Lagrangian for  the Stackelberg leader firm 2's problem:
$$ \eqalign{ \tilde L & = \sum_{t=0}^\infty  \beta^t\{( (a_0 -  a_1 (q_{1t} + q_{2t}))q_{2t}  - \gamma (q_{2t+1} - q_{2t})^2 \} \cr
  &  + \sum_{t=0}^\infty \beta^t \theta_t \{ \delta_1 q_{1t} +  c_0 \delta_2^{-1} \beta^{-1} -  c_2 \delta_2^{-1} \beta^{-1}
    \sum_{j=0}^\infty \delta_2^{-j} q_{2t+j+1} - q_{1t+1} \} } \quad \EQN eq_duop4 $$
subject to initial conditions for $q_{1t}, q_{2t}$ at $t=0$.

The max-min problem  associated with Lagrangian \Ep{eq_duop4}  is unpleasant because the time $t$ component of  firm $1$'s payoff function
depends on the entire future of its choices of $\{q_{1t+j}\}_{j=0}^\infty$.
This feature renders a direct attack on the problem cumbersome.
It is more convenient to formulate a Stackelberg leader's problem recursively, as we do in a more general linear-quadratic setting next.




\section{Recursive representation of Stackelberg problem}

Let $z_t$ be  an $n_z \times 1$  vector of natural state variables,
$x_t$ an $n_x \times 1$  vector of  endogenous forward-looking variables, and $u_t$ a vector of  variables chosen by the Stackelberg leader.
...
The vector $x_t$ is a function of  {\it future} values of $z$ and $u$.
Nevertheless,   at $t \geq 1$, $x_t$ is  inherited from the past  because
values of how $z$ and $u$ for all $ t \geq 0$ are set  in a Stackelberg plan devised at time $0$.
%   The model determinesunder a Stackelberg plan,
%  the ``jump variables'' $x_t$ at time
% $t$.

\specsec{Remark:}  For $t \geq 1$, $x_t$ will turn out to be both a {\it forward-looking\/} and
a {\it backward-looking\/} variable.  It is forward looking because it depends on forecasts of future actions of the Stackelberg leader.
It is backward looking because it is a promise about time $t$ continuation policies  that was  chosen  by the Stackelberg leader at time $0$.

\newpage

Let $ y_t = \left[\matrix{z_t \cr x_t \cr} \right]$. Let $u_t$ be a vector of variables chosen by
the Stackelberg leader at $t$.
Define the
leader's one-period loss function\NFootnote{The
...
$x_0$, a Stackelberg leader at time $0$ wants to % chooses $(x_0, \{u_t, y_t\}_{t=0}^\infty)$  to
 maximize %  chooses $\{y_{t+1}, x_t, u_t \}_{t=0}^\infty$  to maximize
$$ -  \sum_{t=0}^\infty \beta^t r(y_t, u_t) \EQN new1 $$
in light of  the structural economic model
$$ \left[\matrix{I & 0 \cr
...
At time $0$, the leader maximizes \Ep{new1} by choosing
...
for Stackelberg followers' optimization problems (i.e., their Euler equations). These
summarize the forward-looking aspect of followers' behavior.
We shall provide an example in section \use{sec:monopfringe} in which
the last $n_x$ equations
of \Ep{new3} or \Ep{new30} constitute {\it implementability constraints} consisting of followers' Euler equations.
When combined with a stability condition to be imposed below, these
Euler equations
summarize the followers' best responses to the
sequence of actions chosen  by the Stackelberg leader. The leader uses its understanding of these best responses
to manipulate the followers' choices.
...
\noindent Condition A makes equation \Ep{eqn:xlawforward} explosive if ``solved backward'' but stable if ``solved forward''.\NFootnote{See
appendix \use{appa1} of chapter \use{timeseries} and   chapter \use{dplinear}. Also, see Sargent (1987a, chapter IX).}$^,$\NFootnote{Equation
\Ep{eq_duop3} in the duopoly model is an instance of equation \Ep{eqn:xlawforward}.} So we solve
...
In choosing $u_t$ for $t \geq 1$ at time $0$, the leader  takes into account how future $z$'s and $u$'s affect earlier
...
\specsec{Remark:}
We can regard $x_t$ in equation \Ep{eqn:xlawforward} or \Ep{bell101} as summarizing the optimal behavior of the followers
...
stated in chapter \use{dplinear} motivates us
...
\subsection{Duopoly example}
To illustrate our structure, let's map the duopoly model into it.
We begin with firm 1's problem.
Use  firm 1's
%
...
%             \lambda_{t-1} = 0 , \quad t \geq 1 \cr
%             \frac{\partial L}{\partial v_{1t}} & = -2 \gamma v_{1t} +  \lambda_t = 0 , \quad t \geq 0  \cr }$$
first-order order  conditions \Ep{eq_duop0}
%% and the constraint $q_{1t+1} = q_{1t} + v_{1t} $
%% to take the form
%
%$$ \eqalign{ v_{1t} & = \beta v_{1t+1} + \frac{\beta a_0}{2 \gamma} - \frac{\beta a_1}{\gamma} q_{1t+1} -
%                       \frac{\beta a_1}{2 \gamma} q_{2t+1} \cr
%             q_{t+1} & = q_{1t} + v_{1t} } $$
as components of the following  linear system that confronts the Stackelberg leader firm $2$:
$$ \bmatrix{       1 & 0 & 0 & 0 \cr
                        0 & 1 & 0 & 0 \cr
                        0 & 0 & 1 & 0 \cr
        \frac{\beta a_0}{2 \gamma} & - \frac{\beta a_1}{2 \gamma} & -\frac{\beta a_1}{\gamma} & \beta }
        \bmatrix{ 1 \cr q_{2t+1} \cr q_{1t+1} \cr v_{1t+1} }
        = \bmatrix{ 1 & 0 & 0 & 0 \cr
                          0 & 1 & 0 & 0 \cr
                           0 & 0 & 1 & 1 \cr
                           0 & 0 & 0 & 1 } \bmatrix{ 1 \cr q_{2t} \cr q_{1t} \cr v_{1t} }
        + \bmatrix{ 0 \cr 1 \cr 0 \cr 0 } v_{2t} $$
Time $t$ revenues  of  firm 2 are $\pi_{2t} = a_0 q_{2t} - a_1 q_{2t}^2 - a_1 q_{1t} q_{2t}$ which evidently
equal
$$ z_t' R_1 z_t \equiv \bmatrix{ 1 \cr q_{2t} \cr q_{1t}  }'
   \bmatrix{ 0 & \frac{a_0}{2}& 0 \cr
                   \frac{a_0}{2} & -a_1 & -\frac{a_1}{2}\cr
                   0 & -\frac{a_1}{2} & 0 }
\bmatrix{ 1 \cr q_{2t} \cr q_{1t}  } $$
If we set $Q = \gamma$, then firm 2's period $t$ profits can then be written
$$ y_t' R y_t - Q v_{2t}^2  $$
where $y_t = \bmatrix{ z_t \cr x_t } $ with $x_t = v_{1t}$ and
$R = \bmatrix{ R_1 & 0 \cr 0 & 0 }. $
...
\section{Recursive formulation}\label{recurstack}%
 To solve  a Stackelberg problem recursively,
 we formulate two Bellman equations in two sets of  state variables.

\subsection{Two Bellman equations}
Given $y_0$, define a feasible set of $(\vec y_1, \vec u_0)$ sequences
% $$
...
We  express a Stackelberg problem in terms of  two subproblems:
...
\noindent Working backwards, we solve subproblem 1 first, then subproblem 2.
...
$$     y^* = A y + B u,
  \EQN bell2 $$
and $y^*$ denotes next period's value of $y$.
...
% $$ y_{t+1} = A y_t +   B   u_t . \EQN new30a  $$
 The problem is an instance  of the linear quadratic dynamic programming problem studied in
...
\subsection{Representation of Stackelberg plan}

In order to attain an appropriate representation of the Stackelberg leader's history-dependent plan, we will employ what amounts to a version
 of the  ``Big $K$, little $k$'' device encountered in chapter \use{recurpe}  by  distinguishing $z_t$, which depends partly  on decisions $x_t$
of the followers, from $\check z_t$, which does not.  We will use $\check z_t$ and its history $\check z^t = [\check z_t, \check z_{t-1}, \ldots,
\check z_0] $ to describe the sequence of the Stackelberg leader's  decisions  that the Stackelberg follower takes as given.
Thus, we let $\check y_t' = \bmatrix{\check z_t' & \check x_t'}$ with initial condition $\check z_0 = z_0$ given.
...
From this representation we can deduce the sequence of functions $\sigma = \{\sigma_t(\check z^t)\}_{t=0}^\infty$
that comprise a Stackelberg plan.
For convenience, let $\check A \equiv A -  BF$ and partition $\check A$ conformably to the partition
$y_t = \bmatrix{\check z_t \cr \check x_t  }$ as $\bmatrix{\check A_{11} & \check A_{12} \cr \check A_{21} & \check A_{22} } $.
Let $H^0_0 \equiv - P_{22}^{-1} P_{21}$ so that   $\check x_0 = H^0_0 \check z_0$.  Then
iterations on $\check y_{t+1} = \check A \check y_t$ starting from initial condition $\check y_0 = \bmatrix{\check z_0 \cr H^0_0 \check z_0}$
imply that for $t \geq 1$
$$ x_t = \sum_{j=1}^t H_j^t \check z_{t-j} \EQN stackelbergsigma1 $$
where
...
%$$\eqalign{ H^t_1 & = \check A_{21} \cr
%            H^t_2 & = \check A_{22} H^{t-1}_1 \cr
%           \ \   \vdots \  \  &  \  \ \quad \vdots \cr
%            H^t_t & = \check A_{22} H^{t-1}_{t-1} . }  $$
An optimal decision rule for the Stackelberg's choice of $u_t$ is
$$ u_t  = - F \check y_t \equiv - \bmatrix{ F_z & F_x \cr } \bmatrix{\check z_t \cr x_t \cr } $$
or
$$   u_t  = - F_z \check z_t - F_x \sum_{j=1}^t H^t_j z_{t-j} = \sigma_t(\check z^t) \EQN  stackelbergsigma2 $$
Representation \Ep{stackelbergsigma2} confirms that whenever $F_x \neq 0$, the typical situation, the time $t$ component $\sigma_t$ of a Stackelberg plan
is {\it history dependent\/}, meaning that the Stackelberg leader's choice $u_t$  depends not just on $\check z_t$ but on components of $\check z^{t-1}$.



\subsection{Backwards-looking representation of $x_t$}

We have noted that equation \Ep{eqn:xlawforward}  portrays $x_t$ as a {\it forward-looking\/} variable that depends on the continuation $\vec u_{t}$ chosen
by the Stackelberg leader. We have now discovered that equation \Ep{stackelbergsigma1} represents $x_t$ as a {\it backward-looking variable} that
depends only on the history of $\check z^t$.  That $x_t$ is both backward and forward  looking reveals essential features of a Stackelberg plan.
The backward looking representation \Ep{stackelbergsigma2} tells how a Stackelberg leader's decisions $u_t$ as functions of histories $\check z^t$ are designed in a way to affect what can be interpreted as a promised value in the form of an  $x_t$ to be delivered by the Stackelberg policy $\sigma$.
We shall use this notion of a promised value often in subsequent chapters.
...
It is convenient temporarily to proliferate notation by writing history $\check z^t = \check z^t_0$ to emphasize that it starts at $0$ and ends at $t$.
More generally, let $\check z^t_s = [\check z_t, \check z_{t-1}, \ldots, \check z_s] $. A policy $\sigma$ for the Stackelberg leader is the sequence
$$ \sigma =\{\sigma_t(\check z^t_0)\}_{t=0}^\infty .$$   A one-period  continuation of policy $\sigma$ is

$$\sigma_{+1} = \{\sigma_t(\check z^t_0)\}_{t=1}^\infty .$$
Notice that in general
$$ \{ \sigma_1(\check z_0^1), \sigma_2(\check z_0^2), \ldots \} \neq \{\sigma_1(\check z_1^1), \sigma_2(\check z_1^2), \ldots \} .$$
This inequality evidently holds for the representation \Ep{stackelbergsigma2} of the Stackelberg plan derived above.
We conclude that in general a one-period continuation of policy $\sigma$ is not a Stackelberg plan starting from the value of
$\check z_1$ implied by  $\sigma_0(\check z_0)$. A concise way of saying that a one-period continuation of a Stackelberg plan is not a Stackelberg plan is
the statement that  a Stackelberg plan is {\it time inconsistent\/} or {\it dynamically
inconsistent\/}.




\subsection{Recursive representation again}
Recalling that $y_t = \bmatrix{z_t \cr x_t }$, we conclude that, given $z_0$, we also recall that a recursive representation of a Stackelberg plan is
$$\EQNalign{ x_0 & = - P_{22}^{-1} P_{21} z_0 \cr
             u_t & = - F y_t \quad t \geq 0 \cr
             y_{t+1} & = (A - BF) y_t \quad t \geq 0   . \cr}  $$
%
%Equations \Ep{bell5} and \Ep{king6x0} form a  recursive representation of a Stackelberg plan
It is sometimes helpful to adopt the artificial device of imagining that the Stackelberg leader implements the plan with the assistance of
  a sequence of ``continuation Stackelberg leaders.'' These continuation Stackelberg leaders are bound to respect the following rules and
   and   the following timing protocol:\NFootnote{Think of a continuation Stackelberg leader as a computer program that executes the time $t$ component
    of the  plan designed  by the Stackelberg leader at time $0$.}
\smallskip
...
\noindent Notice how we have distinguished the problems confronting
  continuation Stackelberg leaders who choose at times $t \geq 0$ and a Stackelberg leader who
chooses at time $t=0$.
...
\subsection{Time inconsistency again}
We can use the  two subproblems in section \use{recurstack} to glean further insights about  the time inconsistency of the optimal decisions of a Stackelberg leader.
In the recursive representation of
 a Stackelberg plan, different state variables confront a Stackelberg leader at $t=0$, on the one hand, and continuation Stackelberg leaders at dates $t \geq 0$, on the other.
At $t =0$, the leader faces $z_0$ as a state vector and  chooses the forward looking
vector $x_0$ as well as the forward looking vector $x_1$ that will confront the continuation
leader at time $1$.  At dates $t \geq 0$,  a continuation leader confronts the state vector $x_t$  as  values promised at time $t-1$ that must
...
Define $\vec a_1$ as the continuation of the sequence $\vec a_0$. A
 Stackelberg plan is a triple $(\vec u_0, \vec x_0, \vec z_1)$ that solves the Stackelberg problem starting from a given $z_0$.
...
a Stackelberg plan. Why? Because $x_1$ does not solve subproblem 2 at initial condition $z_1$ generated by a Stackelberg plan.
...
\specsec{Remark about shadow prices:}
From chapter \use{dplinear}, recall the formula $\mu_t = P y_t$ for the vector of
shadow prices $\mu_t = \bmatrix{ \mu_{zt} \cr \mu_{xt} }$ on the time $t$  transition equations.
...
The Lagrange multiplier $\mu_{xt}$ measures the cost to the leader at $t \geq 0$
of confirming expectations about its time $t$ action that the followers held
at dates $s < t$.
Setting
$\mu_{x0}=0$ reflects the situation that at time $0$ there are no prior expectations to confirm.  But
when $\mu_{xt} \neq 0 $ for $ t \geq 1$, it indicates that  it is costly to the original (time $0$) Stackelberg leader for a time $t$ continuation leader to confirm  the followers'  expectations about
 the leader's time $s \geq t$ actions.
 %The  Stackelberg leader takes these costs into account when at time $0$  it weighs the costs and benefits
%of using its choice of $ u_{t}, t \geq 1$ to influence $\{x_s\}_{s=0}^{t-1}$.
...
We have indicated how the duopoly model of section \use{sec:duopoly} fits within the  frameork of this chapter.
As an additional example, this section studies the equilibrium of an industry with
a large firm that acts as a Stackelberg leader with respect to a competitive
fringe.\NFootnote{Sometimes we call this the large firm  `the monopolist' even though there are
...
in capital.}
  Shift  \Ep{oli1} forward one period, replace conditional
...
appropriately augmenting the natural state variables with
 forward-looking variables chosen by Stackelberg followers.\NFootnote{For another application of the techniques
...
\noindent{\it Exercise \the\chapternum.3} \quad {\bf Verification}


\medskip

\noindent Please verify the formulas for
where for $t\geq 1$
$$\eqalign{ H^t_1 & = \check A_{21} \cr
            H^t_2 & = \check A_{22} \check A_{21} \cr
           \ \   \vdots \  \  &  \  \ \quad \vdots \cr
...
that we use to construct the representation  in equation \Ep{stackelbergsigma2} for
$  u_t  = - F_z z_t - F_x \sum_{j=1}^t H^t_j z_{t-j} = \sigma_t(z^t)$  for $\sigma_t(z^t)$.
\medskip
\noindent{\it Hint:} Construct a recursion starting from $t=1$.

\medskip


\noindent{\it Exercise \the\chapternum.4} \quad {\bf Big $K$, little $k$ again}


\medskip

\noindent Please provide a recursive formulation of a follower's problem in the duopoly model described in the text of this chapter.
...
$$
The  challenge is to represent $\{q_{2t}\}_{t=0}^\infty $ as a given sequence that  firm 1 believes it cannot affect.
Please feel free to use the following hints.

\medskip
\noindent {\bf a.} Recall that under the Stackelberg plan, firm 2 sets output according to the $q_{2t}$
component of $$y_{t+1} = \bmatrix{  1 \cr q_{2t} \cr q_{1t} \cr x_t }$$
which is governed by
$$ y_{t+1} = (A - BF) y_t .$$


\medskip
\noindent{\bf b.} Define a state vector
$$ \tilde y_t = \bmatrix{  1 \cr q_{2t} \cr \tilde q_{1t} \cr \tilde x_t }$$
...
\medskip
\noindent {\bf c.} Let firm 1's state vector be
$$ X_t = \bmatrix{ \tilde y_t \cr q_{1t}  } $$
Then argue that  the follower firm 1 faces law of motion
$$
\bmatrix{ \tilde y_{t+1} \cr
q_{1t+1} } = \bmatrix{ A - BF & 0 \cr
0  & 1 }  \bmatrix{ \tilde y_{t} \cr
q_{1t} } + \bmatrix{ 0 \cr 1 } x_t
$$
Argue that this specification assures that from the point of the view of  firm 1, $ \{q_{2t}\} $ is an exogenous process.


\medskip
\noindent{\bf d.} Argue that  $ \tilde q_{1t}, \tilde x_t$ play the role of ``Big $K$'' and that  $q_{1t}, x_t$   play the role of ``little $k$''.


\medskip
\noindent{\bf e.} Argue that the time $t$ component of firm 1's objective  is
$$ \tilde X_t' \tilde R x_t - x_t^2 \tilde Q = \bmatrix{ 1 \cr q_{2t} \cr \tilde q_{1t} \cr \tilde x_t \cr q_{1t} }'
  \bmatrix{ 0 & 0 & 0 & 0 & \frac{a_0}{2} \cr
                  0 & 0 & 0 & 0 & - \frac{a_1}{2} \cr
                  0 & 0 & 0 & 0 & 0 \cr
                  0 & 0 & 0 & 0 & 0 \cr
                  \frac{a_0}{2} &  -\frac{a_1}{2} & 0 & 0 & - a_1 }
  \bmatrix{ 1 \cr q_{2t} \cr \tilde q_{1t} \cr \tilde x_t \cr q_{1t} } - \gamma
     x_t^2 $$

\medskip
\noindent {\bf f.} Argue that firm 1's optimal decision rule is
$ x_t = - \tilde F X_t $
