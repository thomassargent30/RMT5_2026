...
markets provide.   This chapter and chapters \use{socialinsurance}
and \use{socialinsurance2}
...
holdings.  In chapters \use{socialinsurance}
and \use{socialinsurance2}, history dependence
...
some borrowing (letting $b > 0$),  $E a(r) $
...
Price levels $p_t$ for all $ t \geq 0$  increase proportionately with  $M_0$.
...
that  support this selection rule.

  Alterations in the debt limit $\phi$ affect the
...
Equality of rates of return implies $e_t = e_{t-1} =e$ for all
$t$ and therefore
$ p_{1,t} = e p_{2,t}$ for all $t$.


Country $i$ finances a fixed
...
Prescott, Fitgerald, and Alvarez (1992) for  explanations and
applications of back-solving.}
...
becomes \index{Bellman equation}%
...
then analyzed.  In chapters \use{socialinsurance}
and \use{socialinsurance2}, we study a class of  models
for similar environments that, like the models of this chapter,
make consumption allocations history dependent. But
the spirit of the models in chapters \use{socialinsurance}
and \use{socialinsurance2}  differs from
those in this chapter in requiring that the trading structure
be more firmly motivated by the environment.  In particular,
the models in chapters \use{socialinsurance}
and \use{socialinsurance2}
...
cost $m>0$ to change jobs.  That cost is paid at the beginning of the
period that a worker decides to move.  A worker's period $t$ budget constraint
is
