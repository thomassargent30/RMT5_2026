...
and Judd's (1985b) results about an asymptotically zero tax rate on
...
\auth{Straub, Ludwig}\auth{Werning, Ivan}}
...
perform numerical simulations and conclude
 that an optimal {\it ex ante\/} capital tax rate is
approximately zero.
\auth{Zhu, Xiaodong}%
\auth{Chari, V.V.}%
...
model. Aiyagari et.\ al.'s suspension of complete markets  goes a long way  toward rationalizing
outcomes Barro had described.\auth{Aiyagari, Rao} \auth{Marcet,
Albert} \auth{Sargent, Thomas J.} \auth{Sepp\" al\" a, Juha}


\auth{Chari, V.V.}\auth{Christiano, Lawrence J.}\auth{Kehoe, Patrick J.}%
In a monetary economy in which  a government  can trade only
nominal debt bearing a risk-free nominal interest rate,
Chari, Christiano, and Kehoe (1996)
construct an optimal monetary policy that implements the same Ramsey allocation that
would prevail if the government were also  able to  issue state-contingent real debt.
An optimal monetary policy accomplishes that by engineering state-contingent inflation that  transforms
 non-state-contingent nominal  debt into
state-contingent real debt. Systematic variations in the nominal
price level rearrange real government obligations across states in the following ways.
In bad times that are associated with high government expenditures, it is
optimal to raise the price level so that real returns on nominal debt are relatively small;
while for symmetric reasons it is  optimal to lower the price level in
good times that are  associated with low government expenditures. We also
use the Chari, Christiano, and Kehoe (1996) framework to compare   views about the
`fiscal theory of the price level.' Because it  spells out all of the details, the model of Chari el al.\ delivers a
coherent general equilibrium analysis of the determinants of the
nominal price level and real government indebtedness at each node
of an Arrow-Debreu event tree. We return to the
fiscal theory of the price level in chapter \use{fiscalmonetary}.


\auth{Jones, Larry E.} \auth{Manuelli, Rodolfo} \auth{Rossi, Peter E.}%
Jones, Manuelli, and Rossi (1997)
augment a nonmonetary, nonstochastic growth model by allowing human capital accumulation. They
...
government should resort to a consumption tax. But for a particular class of preferences,
even this
consumption tax, and therefore all taxes, should be zero in the
limit  when it is optimal
during a transition period
for the government to amass enough
claims on the private economy that  interest earnings suffice
...
nonstochastic steady state is an  implication of
time-additively separable   utility, \newpage \noindent  a
...
\vfil\break
\noindent A stepwise summary of the primal approach is:
\medskip
...
\section{Indeterminacy of debt and capital taxes}
...
can be offset by restructuring the 
portfolio of government-issued state-contingent debt. Zhu (1992)
shows how such
portfolios  can be constructed.\auth{Zhu, Xiaodong}
...
leaves the following objects unaffected:
\smallskip
\item{1.} The household's intertemporal consumption choice, governed by
first-order condition \Ep{TS_focX;c}.
\smallskip
\item{2.} The current market value of all government debt issued at time $t$,
when discounted with the equilibrium expression for
$p_t(s_{t+1}|s^t)$ in equation \Ep{TS_focX;b}.
\smallskip
\item{3.} The government's revenue from capital taxation net of maturing
government debt in any state $s^{t+1}$.
\medskip

\noindent Thus, the alternative policy is budget-feasible and leaves the
...
We can check that equations
 \Ep{TS_rfbond1} and \Ep{TS_rfbond2} describe a budget-feasible policy by
...
one period in advance is implemented  by setting
$$
\epsilon_{t+1}(s^{t+1}) = \bar \tau^k_{t+1}(s^t) - \tau^k_{t+1}(s^{t+1}).
$$
\index{Ramsey problem!uncertainty}
\section{A Ramsey plan under uncertainty}
We now ask what competitive allocation  a benevolent
government should choose. That is, we pose  the Ramsey problem for the stochastic economy.
Our computational strategy follows the same  recipe that we used when we studied 
...
First-order conditions for the Ramsey problem are
            ...
            k_{t+1}(s^t)\rm{:}& \ \ \theta_t(s^t) = \beta \sum_{s^{t+1}\vert s^t}
                         ...
                         \theta_{t+1}(s^{t+1}) \cr
                          & \hskip3cm \cdot[ F_k(s^{t+1}) + 1 -\delta],
                                                     \ \ \  t \geq 0.  \cr}$$
We have omitted  the conditions for $c_0$ and $n_0$. They differ from the above conditions because they include terms related to the initial stocks of
capital and bonds.
First-order conditions for  for $t\geq1$ imply 
$$\EQNalign{
...
distortionary taxation, we could  include the
...
Indeterminacy of state-contingent government debt and
capital taxes implies that  it is not possible uniquely to characterize
a stationary distribution of realized capital tax rates. But we can study
an  {\it ex ante} capital tax rate
$$
\bar \tau^k_{t+1}(s^t) = {\sum_{s_{t+1}} p_t(s_{t+1}|s^t)
\tau^k_{t+1}(s^{t+1}) r_{t+1}(s^{t+1}) \over
\sum_{s_{t+1}} p_t(s_{t+1}|s^t) r_{t+1}(s^{t+1}) }            \EQN TS_etau
$$
defined as the ratio of the present 
value of taxes on next period's capital income to the present  value of next period's 
capital income. After invoking equation
\Ep{TS_focX;b} for the equilibrium price, 
we get an an 
expression that is identical to equation \Ep{TS_fixtau}. Recall that equation \Ep{TS_fixtau}
resolved the indeterminacy of a Ramsey
plan by pinning down a unique fixed capital tax rate for period $t+1$
conditional on information at time $t$. It follows that 
 $\bar \tau^k_{t+1}(s^t)$ in equation \Ep{TS_etau} can be interpreted as an  {\it ex ante\/} capital tax
rate that is uniquely pinned down despite  the multiplicity of budget-feasible capital tax
schedules under a Ramsey plan. Moreover, it is quite intuitive that one
...
a constant tax rate exactly equal to that fraction.
...
that the {\it ex ante\/} capital tax rate in a stationary equilibrium 
...
\medskip\noindent
We provide a  sketch of the proof  in the next subsection.
 We note here that the  two
possibilities that the proposition outlines with respect to the {\it ex ante\/} capital tax rate are genuine.
...
solve numerically for Ramsey plans when  preferences violate 
...
steady state.  For some utility functions and a stationary equilibrium of a stochastic economy,
Proposition 2 extends this result:  the
...
maximum $H^+$ in the stationary equilibrium.\NFootnote{See Step 3 On pages 286-287, where  Zhu(1992) describes continuity requirements
on component functions.} That is, where
$H^-=H(s^-,k^-)$ and
$H^+=H(s^+,k^+)$, there exist
...
inconsistent, as
indicated by the preceding discussion about taxation of initial capital and discussed further   in chapter \use{fiscalmonetary}.
 In a model without physical capital Lucas and Stokey (1983) provide an examples in which  an  optimal tax policy
 can be rendered  time consistent
if the government can issue and each period appropriately reschedule an appropriate term structure of debt. 
Restructuring a term
structure of history-contingent appropriate provides incentives for continuation Ramsey planners to ratify
continuations of the 
Ramsey allocation
$\{c_t(s^t), n_t(s^t); \forall s^t\}_{t\geq0}$
as the Ramsey allocation for the continuation economy. 
Alvarez, Kehoe and Neumeyer (2004)
apply the argument to the maturity structure of both real and
{\it nominal\/} bonds in a monetary economy. For a class of economies in which  the
Friedman rule that sets  nominal interest rates to zero is optimal, they show that optimal monetary and fiscal policies are time consistent.
When the Friedman rule is optimal, households are satiated with money balances, making  the economy 
imitate a real
...
&\hskip3cm  + \theta_t(s^t) = 0, \hskip1cm t \geq 1;
                                                            \EQN TSs_foc;b \cr
c_0(s^0)\rm{:}& \ \ \ (1+\Phi) u_c(s^0) + \Phi \left[u_{cc}(s^0) c_0(s^0)
        -  u_{\ell c}(s^0) n_0(s^0) \right]                        \cr
&\hskip3cm  - \theta_0(s^0)
        - \Phi u_{cc}(s^0) b_0 = 0;
                                                            \EQN TSs_foc;c \cr
n_0(s^0)\rm{:}& \ \ -(1+\Phi) u_{\ell}(s^0) - \Phi \left[u_{c\ell}(s^0) c_0(s^0)
        -  u_{\ell \ell}(s^0) n_0(s^0) \right]                        \cr
&\hskip3cm  + \theta_0(s^0)
        + \Phi u_{c \ell}(s^0) b_0 = 0.
                                                            \EQN TSs_foc;d \cr}
$$
%L%In equations \Ep{TSs_foc;c} and \Ep{TSs_foc;d} it is to be understood
%L% that $\ell(s^0)$ and $c(s^0)$ are  functions of both $s^0$ and $b_0$.
%L% We retain our assumption that the government  commits to a policy at
%L%time $0$.
...
to eliminate the multiplier $\theta_t(s^t)$:
\offparens
$$\EQNalign{
...
Still taking  $\Phi$ temporarily as given, it remains to compute $c_0(s^0)$
and $n_0(s^0)$ from  first-order conditions \Ep{TSs_foc;c} and \Ep{TSs_foc;d}, respectively. Again,  eliminating the multiplier
$\theta_0(s^0)$ and invoking the resource constraint yields an
expression analogous to \Ep{TS_barg}:
\offparens
$$\EQNalign{
(1+\Phi) &u_c(c_0,1-c_0-g_0) + \Phi \bigl[c_0 u_{cc}(c_0,1-c_0-g_0)   \cr
 & -(c_0+g_0) u_{\ell c}(c_0,1-c_0-g_0) \bigr]
   - \Phi u_{cc}(c_0,1-c_0-g_0)  b_0        \cr
=(1+\Phi) &u_{\ell}(c_0,1-c_0-g_0)
         + \Phi \bigl[c_0 u_{c\ell}(c_0,1-c_0-g_0) \cr
 & -  (c_0+g_0) u_{\ell \ell}(c_0,1-c_0-g_0)  \bigr]
   -  \Phi u_{c \ell}(c_0,1-c_0-g_0)  b_0 .   \hskip1.5cm \EQN TS_barg_0     \cr}
$$
\autoparens
While for $t \geq 1$, $c$ and $n$ depend on  the time $t$ realization of $g$ only,
for $t=0$, $c$ and $n$ also depends on the government's initial debt $b_0$.
...
Using equations \Ep{TSs_tech} and \Ep{TSs_cham15}, we arrive at
$$\EQNalign{
& \Phi Q + \sum_{t=0}^\infty \ \sum_{s^t} \beta^t \pi_t(s^t)
...
\item{1.} Guess a value for $\Phi$.  Then use the first-order conditions   \Ep{TSs_foc} and the
feasibility conditions \Ep{TSs_tech}  to compute $c_t(s^t), n_t(s^t)$ for $t \geq 0$ and all $s^t$.
%L%for $t, s^t$ for all $t \geq 1$ and $c_0(s^0,b_0)$ and $n_0(s^0, b_0)$.


\medskip
\item{2.} Use the first-order conditions \Ep{TSs_foc} to solve for $\theta_t(s^t)$ for $t \geq 0$ and all $s^t$,
%L%and $\theta_0(s^0)$
as functions of $\Phi$ and  the candidate Ramsey allocation at $\Phi$ computed in step 1.
...
$$ 1 - \tau_t^n(s^t) = {\frac{u_\ell(s^t)}{u_c(s^t)} }  \EQN LSA_map;a $$
...
$$ \eqalign{ u_c(s^t) [ n_t(s^t) - g_t(s_t)] &+ \beta \sum_{s_{t+1}} \Pi (s_{t+1}| s_t) u_c(s^{t+1}) b_{t+1}(s_{t+1} | s^t)
 \cr & - u_\ell (s^t) n_t(s^t) = u_c(s^t) b_t(s_t | s^{t-1}) . } \EQN LSA_budget1 $$
...
$$  u_c(s)\left(n(s) - g(s)\right) - u_\ell(s) n(s)  + \beta \sum_{s'} \Pi(s' | s) x'(s')  =  x(s), \EQN LSA_budget2 $$
...
$$ \vec u_c(\vec n - \vec g) -\vec u_\ell \vec n  + \beta \Pi \vec x = \vec x . \EQN LSA_budget200 $$ %\EQN LSA_budget200 $$
...
$$ \vec x= (I - \beta \Pi )^{-1} [ \vec u_c (\vec n-\vec g) - \vec u_\ell \vec n] . \EQN LSA_xsoln $$
...
$c_0(s_0)$ and $n_0(s_0)$, given $\Phi$.  These are $2  (S+1)$ equations in $2  (S+1)$ unknowns.
...
$$   u_{c}(s_0) b_0 = u_{c}(s_0) (n_0 - g_0) - u_{\ell}(s_0) n_0  + \beta \sum_{s=1}^S \Pi(s | s_0) x(s) \EQN Bellman2consr $$
...
\specsec{Definition:}  Let $\{\tau^n_t(s^t),\,
 b_{t+1}(s_{t+1}|s^t);\, \forall s^t, s_{t+1}\}_{t=0}^\infty$
%$\{\tau_t^n(s^t)\}_{t=0}^\infty, \{b_{t+1}(s_{t+1}| s^t)\}_{t=0}^\infty$
be a  time $0$, state $s_0$ Ramsey plan.
Then $\{\tau^n_j(s^j),\,
 b_{j+1}(s_{j+1}|s^j);\, \forall s^j, s_{j+1}\}_{j=t}^\infty$
%$\{\tau_j^n(s^j)\}_{j=t}^\infty, \{b_{j+1}(s_{j+1}| s^j)\}_{j=t}^\infty$
is a  time $t$, history $s^t$
...
\specsec{Key outcome:}  For the Lucas-Stokey model, a time $t$, history $s^t$ continuation  of a time $0$, state $s_0$ Ramsey planner is
...
\noindent We shall explore the time inconsistency  of a  time  $0$, state $s_0$ Ramsey plan in more depth  % \use{Ramsey_appa1}
 when we formulate
...
We assume that $b_0=0$.


\subsection{Example 1: $g_t=g$ for all $t\geq0$}
\label{sec:LucasStokey_ex1}%
...
to redistribute tax distortions over time. With the same tax
...
The argument in example 2 shows that tax revenue is positive. Debt issues are as follows.

At $t=0,1, \ldots ,T-2$, the government
runs a surplus and uses  it to accumulate risk-free one-period bonds issued by the
...
\subsection{Time $0$ is special with $b_0\not= 0$}
\label{sec:LucasStokey_ex1_b0}%
To illustrate how period $0$ is special, we revisit example 1
with $b_0\not= 0$. We assume preferences
$u(c,\ell)=\log(c) + \kappa \ell$ with a value of $\kappa>0$ that is
large enough to guarantee an interior solution to labor,
$n=1-\ell\in(0,1)$.\NFootnote{A sufficiently large
parameter value $\kappa$ ensures that $n <1$, and because the utility
function satisfies an Inada condition $u_c \rightarrow \infty$ as
$c \rightarrow 0$, consumption will be strictly positive in an
equilibrium and hence, $n>0$.} Since government purchases are
deterministic and constant over time in example 1, a Ramsey plan
brings a time-invariant allocation $c,n$ for $t\geq 1$, determined
by condition \Ep{TS_barg}, and possibly different values $c_0, n_0$
in period $0$, determined by condition \Ep{TS_barg_0}.
In section \use{sec:LucasStokey_ex1},
the initial condition was $b_0=0$ and hence, the two conditions
\Ep{TS_barg} and \Ep{TS_barg_0} were the same and the Ramsey plan
prescribed a time-invariant allocation $\hat c = c_0 = c$ and
$\hat n = n_0 = n$.

Given $b_0 \not= 0$ and the assumed preference specification,
expressions \Ep{TS_barg} and \Ep{TS_barg_0} become:
$$\EQNalign{
(1+\Phi) {\frac{1}{c}}  - \Phi c  {\frac{1}{c^2}}
    =(1+\Phi) \kappa \quad \Longrightarrow \qquad
 {\frac{1}{c}}   &= (1+\Phi) \kappa   \hskip3cm \EQN TS_barg_ex;a  \cr
 {\frac{1}{c_0}}    + \Phi {\frac{1}{c_0^2}} b_0
    &= (1+\Phi) \kappa .  \EQN TS_barg_ex;b  \cr}
$$
Since the multiplier $\Phi$ is strictly positive in a Ramsey plan
with distortionary taxation, it follows from expressions
\Ep{TS_barg_ex} that $c_0 > c$ ($c_0<c$) when $b_0>0$ ($b_0<0$).
Thus, according to first-order condition \Ep{LSA_map;a},
$\tau^n_0 < \tau^n$ ($\tau^n_0 > \tau^n$) when $b_0>0$ ($b_0<0$),
where $\tau^n$ is the labor tax rate for $t\geq1$, and
$\tau^n_0$ is the tax rate in period $0$ (or a subsidy rate
if $\tau^n_0<0$).

Consider first the case with initial government debt $b_0>0$, why
does the Ramsey plan depart from the policy of perfect tax
smoothing in section \use{sec:LucasStokey_ex1}? If the government
so wanted, it could roll over a constant debt level forever and
in each period, finance a constant interest payment, in
addition to the government expenditure $g$, which would
allow for a time-invariant tax rate $\hat \tau^n$ to be levied
in all periods, just as in section \use{sec:LucasStokey_ex1}. But the
Ramsey planner does not choose that feasible policy. Instead, the
Ramsey planner trades off the welfare cost of a time-varying labor
tax wedge, $\tau^n_0 < \tau^n$, against a welfare gain from
manipulating an asset price. Using expression \Ep{LSA_map;b}, the
price of an asset paying one good next period under the Ramsey plan
in this nonstochastic economy is
$$
p_0=\beta {\frac{u_c(c,n)}{u_c(c_0,n_0)}} = \beta {\frac{c_0}{c}}
 > \beta = p\,,
$$
where $p_0$ is the price in period $0$, and $p$ is the price in
all future periods $t\geq 1$ when the allocation $c,n$ stays
constant. Hence, the lower labor tax rate $\tau^n_0$ induces
higher consumption $c_0$ in period $0$ and therefore increases
the asset price $p_0$, i.e., lowers the interest rate,
which reduces the burden of an initially indebted government.
And the higher the initial debt $b_0>0$ is, the more the Ramsey
planner would choose to raise $c_0$ relative to $c$ according to
expressions \Ep{TS_barg_ex}.\NFootnote{A higher $b_0$ is associated
with a higher multiplier $\Phi$, so the product $\Phi b_0$ in
expression \Ep{TS_barg_ex;b} is unequivocally increasing in $b_0$.}
As one would expect, reductions in the interest rate become
more valuable when applied to an implied larger stock of government
debt carried over between periods $0$ and $1$, while the welfare
loss of too low of a labor tax wedge $\tau^n_0$ at time $0$ applies
to a single period. By so manipulating the asset price $p_0$, the
Ramsey planner {\it de facto} reduces government indebtedness at
the beginning of period 1 and hence, lowers future debt burden.

For symmetric reasons, given initial government assets $b_0<0$, the
Ramsey planner reduces consumption in period $0$,
$c_0 < c$, so as to lower the asset price in period $0$, $p_0<p$,
i.e., increase the interest rate, in order to improve the return on
the implied stock of government assets carried over between periods
$0$ and $1$. The welfare loss of too high of a labor tax wedge in
period $0$, $\tau^n_0 > \tau^n$, is outweighed by the increase in the
value of government assets at the beginning of period $1$, which
implies a lower distortionary tax rate $\tau^n$
relative to what it would have had to be without the manipulation
of the asset price $p_0$.

Now, since there are such welfare gains from manipulating the
price $p_0$, why do first-order conditions \Ep{TSs_foc} of the
Ramsey plan not involve yet more similar manipulations of future
asset prices?
The simple answer is that the Ramsey problem solves for an optimal
plan under commitment. That is, the Ramsey planner is not allowed
to reoptimize in the future to debase government debt (or enhance
its assets) because that would amount to reneging on rates of
return in the original time-$0$ Ramsey plan.
This reasoning is elucidated in
chapter \use{optaxrecur} when the Ramsey problem is formulated
recursively. Then, there is a Ramsey planner at time $0$ with
state variable $b_0$, and in each future period $t$ and history
$s^t$, there is a ``continuation Ramsey planner'' with state
variable $x_t(s^t) = u_c(s^t) b_t(s_t | s^{t-1})$, who is
constrained to satisfy an implementability condition in the form
of expression \Ep{LSA_budget1}. An implication being that the
continuation Ramsey planners are prevented from manipulating
asset prices in the way that is available to the Ramsey planner
in period $0$, as exemplified in this subsection.
...
Returning to Lucas and Stokey's (1983) model without physical capital,
we follow Aiyagari, Marcet, Sargent, and Sepp\"al\"a (2002)
and study optimal taxation without state-contingent debt.
As in our earlier Lucas and Stokey analysis, we do this by deleting
capital tax revenues and imposing the equilibrium wage rate
$w_t(s^t)=1$ in government budget constraint \Ep{TS_gov}. Now we also  replace
...
$$
 b_t(s^{t-1}) =    \tau^n_t(s^t) n_t(s^t) - g_t(s_t) - T_t(s^t)
              ...
              \EQN TS_gov_wo
$$
where we have included the possibility of a nonnegative lump-sum
transfer $T_t(s^t) \geq 0 $ to the representative household.
 In an  allocation that solves the Ramsey problem and  that
levies distorting  taxes on labor, why would the government ever want to
   hand revenues back to the representative household? That would not  happen in an economy
...
transfers. But  without state-contingent debt,   there are circumstances that induce  a government  to make lump-sum
transfers.

We proceed to eliminate  taxes and prices so that we can   express
government budget sets solely in terms of allocations and
any nonnegative lump-sum transfers.
Note that, given equilibrium wage $w_t(s^t)=1$, expression \Ep{TS_focX;a} for
the representative household's optimal marginal rate of substitution
between consumption and leisure is
$${ u_\ell(s^t) \over u_c(s^t) } = 1-\tau^n_t(s^t)  \EQN TS_gov_wo_a
$$
and that the consumption Euler equation for the representative
household able to trade risk-free
debt with one-period gross interest rate $R_t(s^t)$ is
$$
{1 \over R_t(s^t)} = \sum_{s^{t+1}\vert s^t} \beta  \pi_{t+1}(s^{t+1} | s^t)
                   { u_c(s^{t+1}) \over u_c(s^{t}) }. \EQN TS_gov_wo_b
$$
We substitute resource constraint \Ep{TSs_tech} into the
the net-of-interest government surplus,
$\tau^n_t(s^t) n_t(s^t) - g_t(s_t) - T_t(s^t)$, and use
first-order condition \Ep{TS_gov_wo_a} to eliminate the labor tax
rate and so express the net-of-interest government surplus as
$$
\left[1 - {u_{\ell}(s^t) \over u_c(s^t)}\right]
\left[c_t(s^t)+g_t(s_t)\right] -g_t(s_t) - T_t(s^t) \;\equiv\;
z[c_t(s^t)\!,\, g_t(s_t)\!,\, T_t(s^t)].          \EQN AMSS_z
$$
Adopting the same convention as used for the utility function,
we let $z(s^t)$ denote the value of the net-of-interest
government surplus function at time $t$ history $s^t$, evaluated
at an allocation (and any lump-sum transfer) to be understood
from the context.


Substituting expressions \Ep{TS_gov_wo_b} and \Ep{AMSS_z} into
the government's budget constraint \Ep{TS_gov_wo} yields:
$$
 b_t(s^{t-1}) =  z(s^t) + b_{t+1}(s^t) \sum_{s^{t+1}\vert s^t}
                 \beta  \pi_{t+1}(s^{t+1} | s^t)
               { u_c(s^{t+1}) \over u_c(s^{t}) } \; .  \EQN TS_gov_wo2
$$
In order to solve the government's budget constraint forward,
we move the multiplicative factor $b_{t+1}(s^t)$ inside of the
summation sign in \Ep{TS_gov_wo2}.
Next, for each summation term indexed by $s_{t+1}$ in \Ep{TS_gov_wo2},
we use period $t+1$ history $s^{t+1}$ version of \Ep{TS_gov_wo}
to substitute out for $b_{t+1}(s^t)$:
$$\EQNalign{
...
of government indebtedness, and by invoking versions of the
natural debt limits defined in chapters \use{recurge} and
\use{incomplete}, we arrive at a final expression:
$$
 b_t(s^{t-1}) =  \sum_{j=0}^\infty \sum_{s^{t+j} | s^t} \beta^j
         \pi_{t+j}(s^{t+j} | s^t)
        { u_c(s^{t+j}) \over u_c(s^{t}) } \;z(s^{t+j}).  \EQN TS_gov_wo3
$$

Equation \Ep{TS_gov_wo3} at time $t=0$ and initial state $s^0$
becomes the   implementability condition on a Ramsey allocation  that  Lucas and Stokey (1983) imposed:
$$
 b_0(s^{-1}) = E_{0} \sum_{j=0}^\infty \beta^j
         { u_c(s^{j}) \over u_c(s^{0}) } \;z(s^{j}).    \EQN TS_gov_wo4
$$
Lucas and Stokey's implementability condition applies to our model with only risk-free debt.
But while equation \Ep{TS_gov_wo4} is the only implementability
condition  in Lucas and Stokey's complete markets model,
many more implementability conditions must be satisfied in the
economy with only one-period risk-free debt. In particular,
according to equation \Ep{TS_gov_wo3},
we must now also impose  that
$$
...
sides of \Ep{TS_gov_wo32} must be measurable with  respect to $s^{t-1}$ for all $t, s^{t}$.
%
%
%
...
%side of \Ep{TS_gov_wo32} with respect to $s^{t-1}$.}

Having deduced the implementability conditions, we can now pose the  Ramsey  problem
as
$$\EQNalign{
...
$$
Here we have substituted the resource constraint \Ep{TSs_tech} into the
utility function, and to simplify the exposition, we temporarily proceed under
the presumption  that non-negative lump-sum transfers are not part
of the Ramsey plan. Subsection \use{sec:Ex3revist} offers an example
where lump-sum transfers are needed along some equilibrium
paths.

To form a Lagrangian for the Ramsey problem, we let  $\gamma_0(s^0)$ be a nonnegative Lagrange multiplier on constraint
\Ep{AMSS_st;a}. As in the  economy with state-contingent debt,
...
and  constraints \Ep{AMSS_st;b} by
...
iterated expectations, Abel's summation
...
&+ \Psi_t(s^t)\left\{ \left[
             ...
             &  - \gamma_t(s^t)\left[
                       u_{cc}(s^{t}) - u_{c\ell}(s^{t})\right]b_t(s^{t-1}) =0 \,,
                                                      \EQN AMSS_foc;a \cr
\noalign{\vskip.3cm}
\noalign{\noindent \rm and with respect to $b_{t+1}(s^t)$ as}
\noalign{\vskip.3cm}
...
we  notice just two differences from
condition \Ep{TS_barg} that characterized a Ramsey  optimal allocation in Lucas and Stokey's  economy with
...
expression \Ep{TS_barg}.  This term again comes  from  the constraint that
...
Among other things, we want  to learn
...
debt no longer binds.  We shall investigate this question in the next subsection.
% and
%again in subsection \use{sec:specialcaseAMSS} of chapter \use{optaxrecur}.
...
variable $\Psi_\infty$ would then play the role of the single multiplier
in an economy with state-contingent debt because, as noted above,
the first-order condition \Ep{AMSS_foc;a} would then be the same
...
$g_t(s_t)$), the government would have accumulated high debt, and
convergence would occur to a contingent-debt economy with
...
implies convergence of $\Psi_t(s^t)=\Psi_\infty$, and we return  to our earlier logic where expression \Ep{TS_barg}
...
class of utility functions that make the covariance term in equation
\Ep{AMSS_Psi} identically equal to zero, in particular, utility functions that are linear in consumption and additively separable from
the utility of leisure. (See the preference specification in
 subsection \use{sec:Ex3revist}.) That makes  the marginal utility of consumption be
constant so that  expression \Ep{AMSS_Psi} reduces to
$$
...
utility attained by the Ramsey planner. The cumulative multiplier
$\Psi_t(s^t)$ remains strictly
positive so long as the government must resort to distortionary
taxation either in the current period or for a possible realization of the
...
condition \Ep{AMSS_foc;a}, it follows that a Ramsey tax policy
...
are returned to the households as positive lump-sum transfers. Such transfers
  occur when government expenditures fall below
...
periods and thereby reducing the \newpage \noindent  deadweight loss of
...
``asset limit.'' But we doubt this argument, since a government
...
example 3 in sub\-section \use{sec:LucasStokey_ex3}
%above of government expenditures that was taken from
...
in subsections \use{sec:LucasStokey_ex1} and \use{sec:LucasStokey_ex2},
respectively, involve no uncertainty, so the issue of state-contingent debt
...
With these preferences, the first-order condition \Ep{AMSS_foc;a}
...
As in our  analysis of this example when there are complete state-contingent debt markets, we assume that
...
It is evident from this example that a government with access
...
expenditures. Starting from period $T+1$, the  tail of the Ramsey
allocation coincides  with the allocation of an economy with
state-contingent debt that would have occurred under the same
shocks, but for different initial debt, either a strictly
negative debt level associated with $\Phi=0$, if $g_T=0$, or a
strictly positive debt level that would correspond to
$\Phi = \Psi_0+\gamma_T$, if $g_T=g>0$.

It is instructive to consider two realizations of such a  state-contingent-debt
economy   in which  time-invariant multipliers satisfy  $\Phi=0$ and
$\Phi = \Psi_0+\gamma_T$, respectively.  These are  two economies whose tail allocations
after period $T$ coincide with those of our economy without
state-contingent debt under the realizations $g_T=0$ and $g_T=g>0$,
respectively. A state-contingent-debt economy with multiplier $\Phi=0$
means that its government has never had to rely on any
distortionary taxation, and hence, the government must initially
have owned enough claims against the private sector to finance
the stochastic government expenditure in period $T$. Thus, by using
the equilibrium prices of state-contingent claims at a first-best
allocation, i.e., when there is never any distortionary taxation,
we can compute the strictly negative initial debt level that would
suffice to finance a government expenditure of $g>0$ with
probability $\alpha$ in period $T$. If the government's initial
claims against the private sector exceed that critical number,
 excess government assets would be handed back lump-sum to
the representative household.

Now consider the state-contingent-debt economy with multiplier
$\Phi = \Psi_0+\gamma_T$. Its initial government debt level must
be strictly positive for the following reasons. Recall that the
economy without state-contingent debt starts with a zero initial
debt level, and as we have shown, its Ramsey plan involves an increase
in the tax rate after the unfortunate realization of $g_T=g>0$.
In contrast, the government in the state-contingent-debt economy
levies the same tax rate for all $t\not=T$, and for it to have
chosen in all those periods the proposed  eventual high tax rate of
the economy without state-contingent debt, the initial government
debt level of the state-contingent-debt economy must have been
larger than zero.

\newpage 

We end with some reflections on welfare and commitment. The
representative household is {\it ex ante} better off when the
government can use state-contingent claims because the
Pareto problem for that economy has fewer constraints than it does in
an economy in which  the government is constrained
to use only risk-free bonds. It is also true that the representative
household of the risk-free bonds only economy is {\it ex post} better off under the
tail allocation after period $T$ if the state $g_T=0$ is realized,
after which there would never be any distortionary taxation in the economy
without state-contingent debt. A tail allocation without distortionary
taxation would tempt a  government facing that tail government expenditure sequence but
finding itself
indebted after period $T$. Since the government will never again
need access to financial markets to finance any government
expenditures, if it were offered a choice to do so, the government would want  to renege on any
existing government debt in order to
make the representative household  better off. Yes,  the representative
household would lose from not receiving payments on its
holdings of government debt, but that loss would be outweighed by the
gain from never again having to pay any distortionary taxes. This mental experiment is simply not allowed here because the Ramsey
problem has been structured from the beginning so that a government can never renege on its liabilities, and the household's
behavior relies on that. If households had anticipated that a government would ever renege on
its liabilities, it  would not have bought
the government debt in the first place. This circle of concerns will be the topic of
chapters \use{credible} and \use{chang} when we study governments
that lack the ability to commit and must therefore rely on
`credible government policies.' A government  policy is credible if the
government has  incentives to adhere to it in all periods and
...
Our strategy is to follow Chari, Christiano, and Kehoe.  First, in subsection \use{sec:opt_nonmonetary},  we find a Ramsey plan and an associated Ramsey allocation for a nonmonetary
economy with state-contingent government debt. Then, in subsection \use{sec:map_mon_into_nonmonet},  we state   conditions on fundamentals that  allow that  same allocation to  be supported by a Ramsey plan for
a monetary economy having %without real state-contingent government debt but
 only nominal non-state-contingent debt.

A key outcome here is that the Ramsey equilibrium in the monetary economy makes
the price level fluctuate in response to  government expenditure shocks. The price level adjusts  to deliver history-contingent returns to
...
$$
where $u_j$ is the derivative of $u$ with respect
...
aspect of Aiyagari's et al.'s  (2002) calculations for an economy with no
...
Returning to a nonmonetary, nonstochastic growth model, Jones, Manuelli, and Rossi (1997)
...
that raw labor vanishes from the agent's present-value
budget constraint.\NFootnote{One
...
Our optimal steady-state tax analysis
is silent about how long it takes to reach
the zero tax on capital income and how taxes and
 redistributive transfers are set during the transition
to a steady state. These issues have been  studied numerically
by Chari, Christiano, and Kehoe (1994),
though their paper  involves no redistributional
concerns because they assume a representative agent.
...
Throughout this chapter we have assumed that a government can commit to future tax rates  at time 0. As noted earlier,
taxing the capital stock at time 0 amounts to lump-sum
taxation and therefore dispenses with
...
\medskip
\noindent Given an exogenous sequence of government purchases $\{g_t\}_{t=0}^\infty$,
