...
\centerline{\bf  Tom's Qualifier Question}
...
An  ``inverse problem''  asserts an  answer and invite you to state an  associated question.
...
\medskip
\noindent{\bf Part A.} Where $x \in {\bf R}$, $f >0, d >0 $,   $\beta \in (0,1)$, and $g : {\bf R} \rightarrow {\bf R}$, for what intertemporal problem is the following  a Bellman equation?
$$ w(x) =  \{- f x^2 - d(g(x)  - x)^2 + \beta w(g(x)) \} \eqno(1) $$


\medskip
\noindent{\bf Part B.}   Where $x \in {\bf R}$, $f >0, d >0 $,    $\beta \in (0,1)$, and $w(x)$ solves functional equation (1),  for what intertemporal problem is the following  a Bellman equation?
...
\noindent {\bf Part C.} Where $x \in {\bf R}$, $f >0, d >0 $, and   $\beta \in (0,1)$, for what intertemporal problem is the following  a Bellman equation?
$$ v(x) = \max_{x'} \{ - f x - d(x' - x)^2 + \beta v(x') \}
\eqno(3) $$

\medskip
\noindent{\bf Part D.} Could  the Bellman equations in parts A and B provide a useful  approach to solving the Bellman equation in Part C?
