...
Besides trading the  risky asset just described, we
...
Aall agents know the same pricing   function that maps the state $s_t$  into the equilibrium price $p(s_t)$.
...
the subjective probability $P_a(1,1)$ ($P_b(2,2)$), 
 agents of type $a$ ($b$) earn an expected rate
...
subjective beliefs as follows. {\bf Lars -- please read the following sentence that I edited a lot and may
have made worse!} We suppose that each  type of agent enters the economy owning one half of the Lucas tree and arrives at a moment when 
...
Excluding  dividends that had arrived  in the first period, under the true distribution the
...
purchases less than $\beta^{-1}$. So under the true distribution
 it seems that purchasing assets is 
a losing proposition for everyone. But of course, 
the true unconditional expected present value of
the asset must be received by someone in the
economy, as determined by how the subjective
...
asset in state $2$ ($1$). Because in that state, their
subjectively expected gross rate 
of return on the asset is less than $\beta^{-1}$. An
agent going short can be thought of as creating shares to
a `synthetic' Lucas tree that mimic the payoffs of 
the real Lucas tree. Of course, the agent would have
...
streams as collateral. It follows that allowing for 
...
such complications, the literature building on the
Harrison-Kreps analysis commonly assume that agents
have infinite wealth. And given that assumption, it
follows that short sales cannot be allowed since there 
would not exist an equilibrium. Pessimistic agents 
...
described in appendix \use{app:compB} of chapter \use{recurge}. There,  both risk aversion and  heterogeneous beliefs 
motivate trade in Arrow securities in a 
sequential-trading competitive equilibrium.
The Arrow securities are in zero net supply, so 
 trade in them means that some agents must be selling short. 
...
chapter \use{recurge},  volumes eventually
approach zero as agents with the most
accurate beliefs eventually  consume the 
