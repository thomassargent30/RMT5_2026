...
% ====================================================================
%  SUGGESTED ROAD MAP (draft for editing)
%
%  NOTE: Four sections currently lack \label{} tags and therefore
%  cannot yet be referenced by \use{}.  Before using \use{} you may
%  wish to add labels as follows (suggested names shown):
%
%    \section{Interpretation of risk-aversion parameter}
%      ->  add  \label{sec:riskparam}
%
%    \section{Market price of risk}
%      ->  add  \label{sec:marketpriceofrisk}
%
%    \section{Risk-neutral probabilities}
%      ->  add  \label{sec:riskneutral}
%
%    \section{Distorted beliefs}
%      ->  add  \label{sec:distortedbeliefs}
%
% ====================================================================

This chapter is organized as follows.
Section \use{sec:riskparam} uses Pratt's (1964) local analysis of the insurance premium
for a fair gamble to explain economists' consensus that plausible values of the coefficient
of relative risk aversion $\gamma$ are no greater than $2$ or $3$.

Section \use{sec:equitypremium} documents the equity premium puzzle using the log-normal
framework of Hansen and Singleton (1983): matching the observed 6 percent gap between
U.S.\ equity and Treasury-bill returns requires an implausibly large $\gamma \approx 27$.

Section \use{sec:marketpriceofrisk} uses the Cauchy-Schwarz inequality to bound expected
returns on any asset by the risk-free rate plus the market price of risk times the
conditional standard deviation of the return.  It reframes the equity premium puzzle
as the observation that aggregate consumption data imply a market price of risk far
below the one inferred directly from asset prices and returns.

Section \use{sec:HJbounds} follows Hansen and Jagannathan (1991) in deriving a
nonparametric lower bound on the volatility of any stochastic discount factor consistent
with the law of one price.  The bound is traced out as a parabola in the
mean--standard-deviation plane and is computed from observed asset prices and returns
alone, without specifying preferences.

Section \use{sec:failureHJ} shows that the time-separable CRRA stochastic discount
factor $m_{t+1}=\beta(C_{t+1}/C_t)^{-\gamma}$ cannot attain the Hansen-Jagannathan
bounds as $\gamma$ varies: pushing $\gamma$ high enough to achieve sufficient volatility
of $m_{t+1}$ simultaneously drives the risk-free rate to implausibly low values,
a second failure that Weil (1990) called the risk-free rate puzzle.  The root cause
is that a single parameter $\gamma$ conflates atemporal risk aversion and aversion to
variation in intertemporal consumption paths.

Section \use{sec:recursivepref} studies the Kreps-Porteus (1978) / Epstein-Zin (1989)
recursive non-expected utility model as applied by Tallarini (2000).  By setting the
intertemporal substitution parameter $\eta=1$, Tallarini assigns $\gamma$ the sole task
of governing risk aversion, thereby allowing high $\gamma$ to push the stochastic
discount factor toward the Hansen-Jagannathan bounds without generating the risk-free
rate puzzle.

Section \use{sec:reinterpret} responds to Lucas's (2003) criticism of Tallarini's
high $\gamma=50$ by showing that the risk-sensitive recursion can equivalently be
interpreted as expressing aversion to model misspecification (robustness) rather than
atemporal risk aversion.  It develops the connection among the penalty parameter
$\theta$, relative entropy, and detection-error probabilities, and shows that moderate
amounts of model-uncertainty aversion can substitute for extremely high risk aversion
in approaching the Hansen-Jagannathan bounds.

Section \use{sec:costsfluctuations} revisits Lucas's (1987, 2003) calculation of the
welfare cost of aggregate consumption fluctuations.  Under risk-sensitive preferences,
the cost is proportional to $\gamma$.  Under the robustness reinterpretation, most of
what Tallarini labeled the market price of risk becomes a market price of model
uncertainty, implying much larger welfare costs of aggregate risk than Lucas's original
small estimate.

Section \use{sec:ConsDuff} describes how Constantinides and Duffie (1996) reverse
engineer a pattern of stochastic cross-sectional dispersion in household consumption
growth that, under incomplete markets and individual CRRA preferences with a modest
$\gamma$, reconciles individual Euler equations with the aggregate asset-pricing
restrictions implied by the observed equity premium.

Section \use{sec:expaffine} offers a different resolution by positing an
exponential-quadratic stochastic discount factor with risk prices $\lambda_t$ that are
affine in a state vector $z_t$.  This model is not derived from a consumer's preference
ordering; instead it is calibrated directly to asset returns and delivers tractable
closed-form formulas for expected excess returns and the term structure of yields.

Section \use{sec:riskneutral} derives the risk-neutral probability measure induced by
the affine stochastic discount factor of section \use{sec:expaffine}, and shows that
under the risk-neutral measure every asset earns the risk-free return, expressing
asset pricing in compact form.

Section \use{sec:distortedbeliefs} follows Piazzesi, Salomao, and Schneider (2015) in
using survey evidence to show that expert forecasts embody beliefs that are
systematically biased relative to the physical (data-generating) measure.  When a
representative agent with these distorted beliefs prices assets, what a
rational-expectations econometrician labels risk prices actually reflects a mixture of
genuine risk compensation and systematic belief distortions.

Appendix \use{appRiesz} states and proves the Riesz representation theorem, which
establishes that any linear pricing functional (implied by the law of one price) can be
represented as an inner product with some stochastic discount factor.  When markets are
incomplete there are many stochastic discount factors that price assets correctly.

Appendix \use{sec:sdf_risk_sens} shows how to compute stochastic discount factors for
both ordinary time-separable preferences and for risk-sensitive preferences by
suitably differentiating value functions with respect to current and future
state-contingent consumption.

Appendix \use{sec:backus} applies the affine term structure framework of section
\use{sec:affine_term_structure1} to the Backus-Zin (1994) log-normal bond pricing
model.  Using state-space methods from chapter \use{timeseries}, it derives formulas
for the mean term structure and impulse response functions of yields, and explains how
an upward-sloping yield curve on average reveals negative serial correlation in the log
stochastic discount factor.

\section{Interpretation of risk-aversion parameter}\label{sec:riskparam}%
    To  understand why
...
of Euler equations under the assumptions that returns are  log normal.\NFootnote{See Hansen and Singleton (1982) for a study of the equity premium puzzle without the log normal assumption. It instead uses a generalized methods of moments approach to estimate the parameters of the representative consumer's preference ordering.}
...
\section{Market price of risk}\label{sec:marketpriceofrisk}%
...
The ratio $\left({\sigma_t (m_{t+1}) \over E_t m_{t+1} }\right)$ is called the
market price of risk.  \index{market price of risk}% %  The bound on the right side of  \Ep{hansen3} is attained by securities that are on the efficient
...
${\frac{E_t R_{t+1} - R_f,{t+1}}{\sigma_t(R_{t+1})}}$, i.e., its excess return relative to its standard deviation.
% A Sharpe ratio measures the
%excess return relative to the standard deviation.
Inequality \Ep{hansen3a} implies that   
${\frac{E_t R_{t+1} - R_f,{t+1}}{\sigma_t(R_{t+1})}} \leq \left({\sigma_t (m_{t+1}) \over E_t m_{t+1} }\right)$ so that the market price
...
the  portfolio and  assets of comprising it  should cost the same.  Technically,  equation \Ep{eqn:linearfunctional}  asserts that
...
\subsection{Inner product representation of  price functional}
...
discount factor\/} is a scalar random variable $y$ that verifies:
$$ \phi(p) = E(y p) \  \forall p \in P. \EQN riesz $$
Equality \Ep{riesz} implies that  the vector  $q$ of prices of the primitive securities satisfies
$$ q  = E (y x).  \EQN price1 $$
The law of one price
implies that   a pricing functional is linear and that therefore  there exists a stochastic discount factor.
When markets are not complete, there exist
...
Notice that $\phi(1) = E (y) $ so that  the expected value of the stochastic discount factor is simply the
...
\subsection{Admissible stochastic discount factors}

   In previous sections we constructed structural models of the stochastic
discount factor. Our
theories often implied that
...
tomorrow.  For a  CRRA utility function $U(C) = (1-\gamma) C^{1-\gamma}$, this specification
sets the stochastic discount factor to $\beta \left(\frac{C_{t+1}}{C_{t}}\right)^{- \gamma}$.
 Hansen and Jagannathan wanted to impose less and to approach the data with a {\it class\/}
of stochastic discount factors.\NFootnote{Hansen and Jagannathan note
...
equation \Ep{price1} and confirming that $q = E(y^* x)$. Thus, $E x y^* = E [x x'] [E x x']^{-1} q = q$.
Besides $y^*$, % in equation \Ep{stochdisc1},
...
$ y = y^* + e $
is also a stochastic discount factor, where $e$ is orthogonal to $x$. Let $y_1$ and $y_2$ be two
stochastic discount factors, so that $E y_1 x = E y_2 x$, which implies that $E (y_1 - y_2) x = 0$.  Thus,
the difference between two stochastic discount factors is orthogonal to $x$.}
%Let ${\cal Y}$ be the set of all stochastic discount factors.
%\subsection{A Hansen-Jagannathan bound}
  Given data on prices $q$ and  payoffs $x$,
Hansen and Jagannathan inferred properties
...
They  used a variance decomposition associated with a linear regression   to construct their bounds.

Hansen and Jagannathan assume that they observe $q$ and $E x x'$ and
know the theoretical restriction $E y x = q$.
   %Let $y$ be an unobserved stochastic discount factor.
Though a stochastic discount factor  $y$ is unobservable, we can represent it in terms
...
but not everything about ${\rm cov}(x,y)$.  Notice that
...
That the residuals in the projection equation \Ep{regress2} are orthogonal to $x$ induces the variance decomposition
...
random variable $y$.
The left side of \Ep{HJbound1} is a lower bound on the standard deviation of all stochastic discount factors with
assumed mean $E(y)$ used to compute $b$ in equation \Ep{regress}.\NFootnote{The stochastic
discount factors are not
necessarily positive.  Hansen and Jagannathan (1991)
derive another bound that imposes positivity.}
...
We focus on the case in which  no risk-free
asset is included among the basis securities underlying $x$.  For this case Hansen and Jagannathan  calculate a lower bound on $\sigma(y)$ as a function of
an unknown value
of $E(y)$.
...
for every  $E(y)$ of interest.
Thus, we can compute the bound for various values
...
\Ep{bound4} is a parabola as a function of $E(y)$.\NFootnote{Recall that a (gross) return
...
constant.  So when one of the assets bears a risk-free return,  only one point on the frontier  is of
interest, the one with the known $E(y)$.}




We shall use  quarterly data on  returns on two basic assets, namely, the real return on a
...
to the data is the following model that makes the log of per capita consumption a
...
To compute the mean and standard deviation of   stochastic discount factor \Ep{CRRA3}, we use:

\medskip
\specsec{Property of log normal distribution:}  If $\log X \sim {\cal N}(\mu_x, \sigma_x^2)$, then $E (X )= \exp\left( \mu_x + {\frac{1}{2}} \sigma_x^2\right)$ and
${\rm std}(X) = E(X) \sqrt{\left( \exp(\sigma_x^2) - 1 \right)} $. Here ${\rm std}$ denotes a standard deviation.
\medskip

\tag{lognormalpropertypage}{\the\pageno}%
...
are\NFootnote{Let log consumption growth be a random variable $g$ with  probability density  $\phi(\cdot)$ having  finite moments
 ...
 is a {\it \idx{moment generating function}} with power series
 ...
 all orders.
 Stanley Zin (2002) named this a `never a dull moment'  and indicated how one could adjust higher moments of log consumption growth to fit asset pricing observations while
...
\centerline{\epsfxsize=2.75truein\epsffile{timeseparableCRRApreferences_gray.eps}}
...
The coefficient of relative risk aversion, $\gamma$ takes
values 1, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50 and the discount
factor $\beta$=0.995.}
\endfigure

Another convenient way to express the stochastic discount factor $m_{t+1}$ given by \Ep{CRRA3} is first to represent it in terms of a discount rate $\rho$, namely,  $\beta = \exp(-\rho)$. Then equation \Ep{CRRA3} is
equivalent with
$$ \EQNalign{ m_{t+1} & = \exp ( - r_t  - \gamma \sigma_c \varepsilon_{t+1}) \EQN affine101a \cr %- {\frac{1}{2}} \sigma_c^2 \gamma^2 removed
...
often appropriately called the short rate.  Equation \Ep{affine101b} shows how discounting in preferences ($\rho$), consumption growth ($\mu$), taste for
...
called the price of consumption growth risk. That loading   equals the market price of risk computed above.


 Figure
 \Fg{CRRAtimeseparablepreferences} plots Hansen-Jagannathan bounds.  They form  the
parabola in the upper right corner and  were constructed using quarterly data on two returns, the real return on a
...
The  risk free rate puzzle  % that impedes the CRRA stochastic discount factor \Ep{CRRA3} from attaining the Hansen-Jagannathan bounds as $\gamma$ is increased
  focuses on equations \Ep{EsdfCRRA} and \Ep{sigmasfCRRA}.  The time separable CRRA specification causes two conceptually distinct consumer attitudes  to find expression   through a single parameter $\gamma$.
...
the representative consumer's distaste for deviations of consumption from a smooth path across time.
 The consumption growth rate parameter $\mu$ induces  deviations from intertemporal consumption smoothness,
while $\gamma$ multiplicatively  affects an adjustment of the  risk-free interest needed to compensate the consumer for accepting paths that are not smooth intertemporally, regardless of
...
The parameter $\gamma$ also affects $E(m)$ through the term $\exp\left({\frac{\sigma_c^{2}\gamma^2 }{2}}\right)$, which reflects a \idx{precautionary savings} motive to be analyzed in chapter \use{selfinsure} that comes from the consumer's dislike of risky consumption streams, a dislike that increases with $\gamma$ and is  compensated for by a {\it higher} $E(m)$.

 \medskip

 Empirically, the estimates of $\mu$ and $\sigma_c$  in  \Tbl{table:parameterestimates}  are the same order of magnitude. Thus, $\mu$ is two orders of magnitude larger than $\sigma_c^2$, which makes increases in $\gamma$ drive $E(m)$ {\it down\/} through its precautionary-savings effect on the term $\exp(- \gamma \mu)$ much faster
...
we  describe how Tallarini (2000) made progress by assigning to $\gamma$   only the single job of describing  risk aversion while using a new parameter $\eta$ to describe attitudes toward intertemporal substitution.
...
risk aversion.}
In the special case that $\gamma=1$ (or  $\theta = +\infty$), application of L'H\^opital's rule verifies that recursion \Ep{Trecur2} becomes a
...
We continue to assume that the log  consumption  process is a random walk with drift  \Ep{TallRW} and seek to solve \Ep{Trecur2}.  Guess a
...
care not only about the conditional expectation of  continuation utility but also its  conditional variance.

 According to \Ep{U_var_mult1},  when $\theta < +\infty$
...
This means that %That the consumer dislikes conditional volatility of continuation utility
% when $\theta < + \infty$,
the consumer cares about  both  the timing of the resolution of uncertainty
and  the persistence of risk. When $\theta = + \infty$, he cares about neither.
Figures \Fg{treeAB} and \Fg{treeCD} show  payoffs (displayed above the nodes) and transition probabilities (the fractions above the lines connecting nodes) for four plans.
...
\midfigure{tallarini}
\centerline{\epsfxsize=2.75truein\epsffile{Tallarinisfigure2_gray.eps}}
\caption{Solid line:
...
Applying  the properties of a log normal random variable described on page \use{lognormalpropertypage}  to  formula \Ep{stdfTall} for the stochastic discount factor gives
...
to avoid the risk-free rate  puzzle and to approach the Hansen-Jagannathan bounds as the  risk aversion parameter $\gamma$ increases.

To connect to  the exponential quadratic stochastic discount factor models of section \use{sec:expaffine}, we can  represent  Tallarini's stochastic discount factor \Ep{stdfTall} as
...
bounds required that Tallarini set the risk-aversion parameter $\gamma$ to such  a  high value, namely 50,  that it provoked   Robert E. Lucas, Jr.,   to disregard
...
estimates of risk aversion to do it.} {``Macroeconomic Priorities,'' 2003}

%Relying on  what he regarded to be  more pertinent evidence about risk aversion,
To  measure the costs of aggregate fluctuations
...
the $\gamma$ of $50$ that Tallarini required to reconcile his model of preferences with  both the consumption data  and the asset returns data
...
his attitude about model misspecification is about his reaction to facing  an {\it unknown\/} probability distribution.
...
The minimization problem on the right  side of \Ep{Tminimization} has the following interpretation.
  {\it Ex post}, a decision maker values random outcomes  $\varepsilon$ according to the value function
...
Performing the minimization on the right side of \Ep{Tminimization} gives the following minimizing value of $g(\varepsilon)$:\NFootnote{Equation \Ep{Tminimization} implies the
 following useful bound on the expected value of $Z( \varepsilon)$ under distorted distribution $ \phi(\varepsilon) g (\varepsilon)$:
 $ \int Z(\varepsilon)  \phi(\varepsilon) g(\varepsilon) d \varepsilon \geq {\bf T}(Z) - \theta {\rm ent}(g (\varepsilon)).$}
...
$\phi(\varepsilon)$.
An application of L'H\^opital's rule shows that when $\theta= +\infty$, ${\bf T}(Z) = \int Z(\varepsilon) \phi(\varepsilon) d \varepsilon $, which is expected utility under complete trust
in the density $\phi$.\index{L'H\^opital's rule}
...
\section{Costs of aggregate fluctuations}\label{sec:costsfluctuations}%
 ...
 Lucas wanted to measure  the gains to eliminating further unpredictable fluctuations in aggregate U.S.\ per capita consumption beyond the reductions  that had
 already been achieved by post World War II aggregate stabilization policies. His method was to find an upper bound on   possible   gains by computing the reduction in initial consumption that a representative consumer with time-separable
 preferences would be willing to accept in exchange for  eliminating {\it all\/} unpredictable fluctuations that post WWII consumption has exhibited.  In this section, we describe Tallarini's version
...
process with the same expected growth rate as the process $\{C_t\}$ is 
$$ C_{t+1}^d = \exp( \mu + {\frac{1}{2}} \sigma_c^2) C_t^d \EQN TallRWdet $$
because $E \exp( \mu + \sigma_c \varepsilon_{t+1}) = \exp( \mu + {\frac{1}{2}} \sigma_c^2)$ according to a  property of a log normal random variable described on page \use{lognormalpropertypage}.
...
to be proportional to $\gamma$.
 Pertinent sources of evidence about the magnitude of  $\gamma$ and whether they should  be  interpreted as  measures of attitudes toward risk, or maybe something else,
...
\centerline{\epsfxsize=2.75truein\epsffile{proportions1_gray.eps}}
...
Section \use{sec:MPU}  argued that most of what Tallarini interpreted as the market price of {\it risk\/} should instead be interpreted
...
Figure \Fg{BHS_punchline} shows Barillas, Hansen, and Sargent's (2009) measures of the costs of removing random fluctuations in aggregate consumption per capita (the dotted line) as well as costs of removing
...
\index{reverse engineer!Constantinides and Duffie}%
...
This model of the stochastic discount factor asserts that exposure to the  random part of  consumption growth, $\sigma_c  \varepsilon_{t+1}$, is the only possible source discrepancies among assets' expected rates of returns: % on different securities:
$\varepsilon_{t+1}$
...
$R_{j,t+1}, i=1, \ldots, N$,  to let the data reveal  risks and their prices.


The model has two components.  The first  is a vector autoregression that describes
...
The second component is a vector of risk prices  $\lambda_t$ and an associated stochastic discount factor $m_{t+1}$ defined
...
Here $\lambda_0$ is $m \times 1$ and $\lambda_z$ is $m \times m$. The  entries of  $\lambda_t$ that multiply corresponding entries of the risks $\varepsilon_{t+1}$ are  called
...
\section{Risk-neutral probabilities}\label{sec:riskneutral}%
...
\section{Distorted beliefs}\label{sec:distortedbeliefs}%
...
We have described two broad lines of response to findings of Hansen and Singleton that the standard time-separable CRRA model of preferences fails to explain observed expected returns on assets.  The first line of response  alters  the representative agent's model  of preferences by tampering 
either with its utility function or its statistical model of consumption growth dynamics. We described examples of this approach in sections \use{sec:recursivepref} and \use{sec:reinterpret}.  The second line of response  divorces  the stochastic discount factor  from consumer preferences and instead directly positing a statistical model of a stochastic discount factor. We illustated that approach when we studied the affine model of the stochastic discount factor in section \use{sec:expaffine}.
\auth{Singleton, Kenneth J.}\auth{Hansen, Lars Peter}%
...
\use{socialinsurance},
we describe ingredients of such models.



\appendix{A}{Riesz representation theorem}\label{appRiesz}%
The version of the {Riesz representation theorem}  used in this chapter is yet
another ramification of population least squares regression.\NFootnote{See appendix \use{appc1} of chapter \use{timeseries} for basic
mechanics.}
...
Therefore, $ \phi(x) = a \phi(y) = <x, y>. $ % \EQN eqn:Riesz4 $$
  \endproof
...
%%% Resize this one
\midfigure{backus1f}
\centerline{\epsfxsize=2.25true in\epsffile{backus1_gray.eps}}
...
\midfigure{backus2f}
\centerline{\epsfxsize=2.25true in\epsffile{backus2_gray.eps}}
...
\midfigure{backus3f}
\centerline{\epsfxsize=2.25true in\epsffile{backus3_gray.eps}}
 \caption{Impulse response of log of stochastic discount factor.}
\infiglist{Beware of free parameters.}
\endfigure

\midfigure{backus4f}
\centerline{\epsfxsize=2.25true in\epsffile{backus4_gray.eps}}
...
\midfigure{backus5f}
\centerline{\epsfxsize=3true in\epsffile{backus5_gray.eps}}
