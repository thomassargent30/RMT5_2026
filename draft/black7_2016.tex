...
the time $t$ consumption good {\it at all nodes\/} $s^t$  and for all $t \geq 1$. After
...
allocations share the notable property that they are  functions only of
...
endowment realization at time $t$, and  not in addition  on the particular history that led to that aggregate endowment.
...
\medskip
 ...
 To compute an optimal allocation, first solve  \Ep{planner4}
...
When we introduce  those imperfections  into the environment, the time invariance of  Pareto
weights evaporates.
...
consumer has its own Lagrange multiplier $\mu_i$ that is independent of time. With specification \Ep{eq0} of the utility functional,
we have
...
\subsection{Interpretation of trading arrangement}
  In the  competitive   equilibrium with Arrow-Debreu timing, all trades occur at $t=0$ in
...
that each consumer faces one  budget constraint that restricts  trades across all dates and histories.
...
the consumer at date $t$ and history $s^t$. We find this state by taking an
...
equilibrium where all trading takes place at time $0$, what is the implied continuation wealth of consumer $i$ at time $t$ after history $s^t$?
The answer is obtained by summing up the value of the consumer's
holdings of claims to current and future consumption at  time $t$ and
history $s^t$. Since history $s^t$ has been realized, we discard all
claims contingent on time $t$ histories $\tilde s^t \neq s^t$ that
were not realized. Hence, the implied wealth is  determined simply
by the trades that were undertaken by consumer $i$ at the outset of a
time $0$ trading equilibrium, when the consumer can be thought of as
having sold  the entire endowment stream on the right side of budget
...
one-period securities are traded period by period, and that consumers
...
wealth of consumer $i$ at a point in time can be decomposed into  {\it financial wealth} and
...
`human wealth'.}
Financial wealth at time $t$ after history $s^t$ is the consumer's
...
in a sequential trading equilibrium should be equal to its
continuation wealth in a time $0$ trading equilibrium minus the continuation value of
its current and future endowment
(i.e., its non-financial wealth), also evaluated in terms of prices for a time-$0$-trading competitive equilibrium. Thus, the
...
each consumer starts  with zero financial wealth at time $0$,
...
$s^\tau$, starting from a distribution of   wealth  implied by the tails of each consumer's
...
the consumer  in the sequential economy with the equilibrium  tail wealth
...
$$
for all $s_{t+1}$, $t$, $s^t$. The natural debt limits \Ep{cecm_debt2} do  not
...
take the consumer's first-order conditions \Ep{eq3} for the Arrow-Debreu
...
endowment stream to finance consumption, in the same way that consumers are
...
in the debt limit restrictions to which the consumer must adhere. In
particular,
if the consumer wants to ensure that consumption plan
$\{c^i_\tau(s^\tau)\}$
can be attained starting next period in all possible future states,
the consumer should subtract the value of this commitment to future
consumption
from the natural debt limit in \Ep{cecm_debt1}.
 Thus, the consumer is facing a
...
for $s_t$ imparts further structure to  equilibrium prices and quantities.
...
limits \Ep{cecm_debt1} and consumers' wealth levels \Ep{cecm_wealth}
...
borne by individual consumers. In particular, each consumer
...
endowment. And from that very same insurance motive, the consumer
now enters the present period with a wealth level that is a
function of the current state $s_t$. It is a state-contingent
wealth level that was chosen by the consumer in the previous
...
be shown that a consumer especially wants  higher wealth levels for
...
to  coordinate consumers'
transactions at time $t-1$  in such a way  that consumers bear  only aggregate
risk.
...
are both functions only of a Markov state $s$ motivates us to seek a
recursive formulation of the consumer's optimization problem.
Consumer $i$'s state at time $t$ is its wealth $a^i_t$ and the
...
The Bellman equation for the consumer's problem is
...
the state at date $t+j$ being $ s_{t+j}$. The consumer's
...
hold for all consumers, the characterization of pricing kernels
...
In appendix \use{app:compB}, we show that when consumers have heterogeneous beliefs,  as  time passes  equilibrium allocations assign   less
and less  to those
...
The likelihood ratios $ \pi_t^i(s^t) / \pi_t^1(s^t) $, % ${\frac{\pi_t^i(s^t)}{\pi_t^1(s^t)}}$,
 which are identically unity when there are common beliefs, now influence optimal allocations.
...
for all $\pi^i \not= \pi$.}
Relative entropy measures the divergence of probability distribution $\pi^i$ from probability
 distribution $\pi$.\NFootnote{Relative entropy also governs statistical properties of likelihood ratio tests for
...
\noindent{\it Exercise \the\chapternum.8} \quad {\bf Optimal
taxation}
\medskip
\noindent
