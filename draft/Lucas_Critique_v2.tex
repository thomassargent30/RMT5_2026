...
of state $i$ at $t$.  Below, we'll suppress the $t$ and just write $\pi_i$ with $t$ being understood, depending on the
context of our application.


We have the following  normal distributions  conditional on $x_t, s_t$:
...
But distributions conditional on $x_t$ are {\it mixtures of normals}, with $\pi_i$ being mixing probabilities.               
Where
$$ \eqalign{ \bar A & = \pi(1) A_1 + \pi(2) A_2   \cr
                    \bar G & = \pi(1) G_1 + \pi(2) G_2 \cr
                    \bar H & = \pi(1) H_1 + \pi(2) H_2 }$$ 
we have
$$ \eqalign{ E x_{t+1} | x_t & = \bar A x_t \cr
                    E y_t | x_t & = \bar G x_t      \cr
                    E z_t | x_t & = \bar H x_t }$$ 
Now let's compute conditional covariances.  
Since
$$ x_{t+1} - \bar A x_t = \cases{ (A_1 - \bar A) x_t  + C_1 w_{t+1} & if $s_t =1$ \cr
                                                    (A_2 - \bar A) x_t  + C_2 w_{t+1} & if $s_t =2$ } $$
it follows that 
$$ E (x_{t+1} - \bar A x_t ) (x_{t+1} - \bar A x_t ) ' | x_t = \sum_i \pi_i [ (A_i - \bar A) x_t x_t' (A_i - \bar A)' + C_i C_i' ] $$
And since                  
$$ y_t = \cases{ G_1 x_t & if $s_t =1$ \cr
                           G_2 x_t & if $s_t =2 $ } $$                     
and 
$$ y_t - \bar G x_t = \cases{ (G_1 - \bar G) x_t & if $s_t =1$ \cr
                           (G_2  - \bar G) x_t & if $s_t =2 $ } $$                             
it follows that 
$$ E (y_t - \bar G x_t) (y_t - \bar G x_t)' | x_t = \sum_i \pi_i (G_i - \bar G) x_t x_t' (G_i - \bar G)'.  $$   
and that
$$ E (z_t - \bar H x_t) (z_t - \bar H x_t)' | x_t = \sum_i \pi_i (H_i - \bar H) x_t x_t' (H_i - \bar H)'.  $$                                                    
...
$$ \mu_{t+1} = \bar A \mu_t$$
and
$$\Sigma_{t+1} = \pi_1 (A_1 - \bar A) \Sigma_t (A_1 - \bar A)' + \pi_2 (A_2 - \bar A) \Sigma_t (A_2 - \bar A)' + \bar C \bar C ' $$
and that iterations on these two recursions will converge, respectively, to 
a stationary mean vector $\mu$  and covariance matrix $\Sigma$ that
satisfy
$$ \mu  = \bar A \mu $$
and
...
Let $p_t$ be the rate  of inflation and $m_t$ the rate of growth of the money supply, where the rate of inflation is the first difference in the logarithm of the price level
