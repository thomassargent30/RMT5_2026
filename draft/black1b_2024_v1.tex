...
\NFootnote{Our specification of the problem allows consumption to exceed bliss consumption and  be negative.  Similar features characterize  a wide  class
...
assume that their initial financial wealths $b^i_0$  equal 
zero  for all $i$. 
...
vectors $z_0$ are independent draws from the stationary distribution  of the common statistical model \Ep{sprob15}. Let  $z^i_0$ denote the initial vector of consumer $i$ with
an associated initial non-financial income of 
$y^i_0 = \check G  z^i_0$. 
Future realizations of consumer $i$'s non-financial income are driven by
 idiosyncratic draws $w^i_{t+1}$ of a vector of shocks.
...
the vectors $z^i_0$ implies that the cross-section average of non-financial income equals the mean of its  stationary distribution each period, which we denote  $Y$. Now  integrate over all agents' consumptions  given
...
non-financial income, which  equals the constant $Y$ in 
this stationary environment. Equality \Ep{eq:c_marketclear_0} asserts that the goods market clears in period $0$l
To confirm that the goods market in all future 
periods also clears, we use expression  
...
$$ 
where the second equality returns the zero mean 
...
applying this same computation recursively 
...
While we have shown that the cross-section mean of 
...
$$ \eqalign{ E_0 c^i_t & = c^i_0 \cr
              E_0(c^i_t  - c^i_0)^2 & = t h h' . } 
                                 \EQN eq:spreadcons $$
Hence, with the intitial conditions that we have specified, each consumer $i$'s consumption follows a random walk and his/her financial wealth is co-integrated with his/her own consumption. But average debt in the cross-section of all consumers stays at zero.   Each period, the market for one-period risk-free loans clears at the same 
constant gross interest rate $R = \beta^{-1}$. 


Thus, we have described a collection of  people who are  borrowing and lending
...
So the risk-free loans are being made from one consumer to another  consumer who is also within our closed set of agents.
In this way, we have dispensed with the ``small open economy'' assumption and created our 
first ``Bewley model''.  We'll encounter many more Bewley models in chapter \use{incomplete}.
...
%eq:spreadcons $$
Because each consumer $i$ dislikes variance in consumption, he would prefer a completely smoothed   consumption stream  in which $c^i_t = c^i_0$ for all $t \geq 0$, where
$c^i_0$ is given by \Ep{eq:c_null}.
Furthermore, such an allocation 
...
\use{sec:borrowlender}. It will provide  
...
the  problem solved by a typical consumer. 
...
Following the section \use{sec:LQmodel} approach  of
...
We can use  the pricing kernel to  price ``redundant securities'' that are  bundles of 
...
It is a hallmark of risk sharing in 
...
\Ep{eq:c_null}. The cross-section distribution of consumption is time-invariant, in contrast to the
incomplete markets economy where it spreads out over time.  And now an individual $i$'s location in the cross-section distribution of consumption is forever fixed.  He is destined to be in the same quantile forever. 
%  Instead of
...
The composition of 
consumer $i$'s optimum  portfolio is time-invariant: the same bundle $b(z^i_{t+1}, \bar c^i)$
...
Given their initial draws of $z^i_0$,
 all consumers   
...
since their their
concave utility functions tell them to   dislike conditional 
variance in consumption.
%Lars2024%Consumers in the 
%incomplete markets economy attain  lower
%expected utility than they do in the complete markets economy 
%since, while  their consumption varies
%around the same  mean as in the complete markets economy,
%the  conditional  variance of their consumption  grows
...
\use{incomplete}.  In those chapters, among
the objects to be  determined within an  equilibrium are  interest rates and state-contingent prices.
