...
\subsection{Example 5: A Markov version of 
...
satisfying $\sum_{s'} \pi(s'\vert s)=1$.
%% for $s\in\{0,1\}$. 
...
consumers start with no beginning-of-period wealth, 
$[\bar a^1(1)\; \bar a^2(1)] = [0\; 0]$.
According to proposition 5, this initial wealth
distribution will prevail at the beginning of
...
consumption as he would  if he were  instead not to  ``roll over'' 
...
allow consumer 2 to susstain a constant 
...
After seeing these equilibrium outcomes that are  implied by proposition 5, we can derive the equilibrium
...
a = {1 \over 1+Q(0 \vert 1)-Q(0 \vert 0)} 
  = {1 \over 1+\beta 
     \bigl[\pi(0 \vert 1)-\pi(0 \vert 0)\bigr]}\,, \EQN aREC
$$
...
expressions \Ep{allocREC} are equal to \Ep{alloc} 
of example 3 in section \use{sec:example3}.


\subsection{Example 6: Example 5 with initial redistribution}
...
for some feasible value of $a_0$. Feasibility of an initial redistribution imposes a 
bound on $a_0$ that is determined by the borrowing limit
...
any one of the budget constraints of consumer $i$, 
we can solve for the consumption level $\bar c^i$:
$$ 
\EQNalign{ \bar c^1 &= { 1-\beta\,\pi(0 \vert 0) \over
    1 + \beta \bigl[\pi(0 \vert 1)-\pi(0 \vert 0)\bigr]}
...
state variable $v$ that appears in the Bellman equation
for $P(v)$  will itself satisfy a Bellman equation.
...
\subsection{Recursive version of Pareto problem}
 ...
 We 
revisit the section \use{sec:RecursivePareto} recursive  formulatation of 
Pareto problem, but now add heterogeneous beliefs.
In addition to Inada condition \Ep{eq_Inada}, we 
assume that the utility functions satisfy 
$\lim_{c \rightarrow 0} u_i(c) = -\infty$, a condition that facilitates 
 our analysis because it brings the following 
...
is consumes almost nothing , so further withdrawals of utility from that person can bring  only miniscule
more goods  to
...
approaches $u_2(1/(1-\beta))$, i.e., the expected
discounted utility $P(v)$ of a type 1 person
plummets toward $-\infty$ and the slope $P'(v)$
converges to $-\infty$.
...
$$ u_2(c) + \beta \sum_{s'} w_{s'} \pi^2(s') \geq v .
                                      \EQN heterogbel13 $$
First-order conditions $P'(w_{s'}) \pi^1(s') =P'(v) \pi^2(s')$  with respect to $w_{s'}$ for all $s'$
and the envelope condition $P'(v) = -\theta$ no longer
imply that $P'(w_{s'})=P'(v)$.  
...
to be more (less) likely to happen than a type 1 person had. 

We now study the  relative
Pareto weight  time goes to
infinity under a true set of probabilities.
Let $\pi(s)$ denote the true probability of 
state $s$ being realized, which satisfy
...
(equaling $\pi^1(s')$) and summing over $s'$, 
we obtain
$$
...
The reason is that under our assumption that
a type 1 person knows the true probabilities,
such a person will almost surely not become
impoverished  as
time goes to infinity and 
hence, the relative Pareto weight of the other 
...
at time $0$ there are complete markets for claims to the consumption good at times $t+j$ for $j = 0, 1, \ldots$.
