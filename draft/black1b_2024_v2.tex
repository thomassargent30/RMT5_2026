...
In light of our chapter \use{dynamicdp1}, \use{practical}, and \use{dplinear} applications of  dynamic programming, this 
 chapter circles back and applies time series  tools from chapter \use{timeseries}.
...
and public finance, the linear-quadratic model of consumption smoothing.  We also use this  model as a vehicle  to introduce the complete and incomplete markets models that we shall
encounter  later chapters. Thus, section \use{sec:borrowlender} exploits the section \use{sec:LQmodel} mathematics to construct a closed-economy 
incomplete markets model in the spirit of the Bewley models to be studied in more general settings in chapter \use{incomplete}.  
Section \use{sec:LQcomplete} then uses closely related mathematics to construct a complete markets general equilibrium model that anticipates 
the more general analysis of chapter \use{recurge}.
 Section \use{sec:lucascritique} uses  chapter \use{timeseries} methods to present Robert E. Lucas, Jr.'s (1976)
criticism of  econometric policy evaluation procedures widely practiced in the 1960s and 1970s, while section \use{sec:lucascritique2} describes   responses
...
\caption{Graph of $f(\Sigma) = {
 {\Sigma(R+Q) + Q R} \over \Sigma + R }$ against $\Sigma$ when  $Q= R=1$.  The 45-degree
line is also plotted. Iterations on the Riccati equation for $\Sigma_t$ converge
to the intersection.}
\infiglist{Muth}
\endfigure
...
Figure \Fg{muth1f}, it can be verified that the positive solution is $\Sigma = 1.6180339$,
...
$t \rightarrow \infty$.  Evidently, $\Sigma_t \rightarrow \Sigma_\infty$,
 the fixed point portrayed in  Figure \Fg{muth1f}.
...
\auth{Burnside, Craig} \auth{Rebelo, Sergio}%r
\auth{Canova, Fabio}%
\auth{DeJong, David}%
\auth{Dave, Chetan}%
...
$$ c_t + b_t = R^{-1} b_{t+1}  + y_t , \quad t \geq 0 ,\EQN sprob2 $$
...
To make  fit into a linear-quadratic permanent income structure,
 we now  assume
...
activities, see Kiyotaki and  Wright's (1993)
...
consumers want but that the producer himself or herself does not.  
...
called money.} %\tag{ftnt5LQ}{the\pageno}%
...
%To assist us 
%will exploiting the fact that the solution to 
This linear quadratic
...
where  the last equation comes from applying to decision rule \Ep{sprob9} the appropriate version  our formula \Ep{discount1} for forecasting geometric sums of future values of the random variables $\{y_{t+j}\}_{j=0}^\infty$.
...
If we think of the initial distribution as describing a joint distribution of $c_0, b_0$ for a cross section
of {\it ex ante} identical households born at time $0$, then our  formulas  describe the evolution of  cross section distributions of  $b_t, c_t$ as the population of households ages.  Cross-section consumption distributions  spread out as time passes.\NFootnote{See Deaton
and Paxton (1994) and Storesletten, Telmer, and  Yaron (2004) for evidence that cross section distributions of consumption spread out with age.}$^,$\NFootnote{Our specification of the problem allows consumption to exceed bliss consumption and  be negative.  Similar features characterize  a wide  class
of linear-quadratic models analyzed, for example, by Holt, Modigliani, Muth, and Simon (1960) and Hansen and Sargent (2013).
Earlier in footnote  5,  %\use{ftnt5LQ}, 
we interpreted negative consumption as a 
consumer's production of the good at a disutility cost, 
something that a consumer would like to optimally undertake.
In contrast, consumption in excess of the bliss 
point is something undesirable to the consumer.}
We'll reconsider this outcome in   sections \use{sec:borrowlender} and \use{sec:LQcomplete},  and also in chapters \use{recurge}, \use{selfinsure}, \use{incomplete}, \use{socialinsurance} and
...
Up to now, we have set $R = \beta^{-1}$  and  taken as   
determined ``outside'' the model.  Saying that we are analyzing a ``small open economy''  is a common excuse  for this assumption.
\auth{Bewley, Truman F.}%
By adapting ideas of   Truman Bewley (1977, 1980) to the present  setting, we can  fill out 
the  consumer's environment  to make  $R = \beta^{-1}$ become an equilibrium outcome.  
To do this, we imagine that our consumer is one of a  continuum of unit measure of {\it ex ante} identical but {\it ex post} 
heterogeneous consumers who trade a  risk-free one-period 
bond whose price is $\beta$.  
We index individual consumers  by $i\in [0,1]$ and 
assume that   for all $i$  initial financial wealths $b^i_0$  equal 
zero. 
...
In particular, we assume that their initial
vectors $z_0$ are independent draws from the stationary distribution  of the common statistical model \Ep{sprob15}. Let  $z^i_0$ denote the initial exogenous state vector of consumer $i$ that drives the consumer's non-financial income process, so that the consumer's initial non-financial income is $y^i_0 = \check G  z^i_0$. 
Future realizations  of consumer $i$'s non-financial income are driven by
 idiosyncratic draws  of a vector $w^i_{t+1}$ of shocks.
 
Given initial condition $b^i_0 = 0$,  use  expressions
\Ep{sprob8} and \Ep{eqn:lucascritiquecons} to deduce that  agent $i$'s
...
the vectors $z^i_0$ implies that the cross-section average of non-financial incomes equals the mean of its  stationary distribution each period, which we denote  $Y.$ Now  integrate over all agents' consumptions  given
by expression \Ep{eq:c_null} to get 
$$ \int_0^1 c^i_0 \, di 
= (1-\beta) \sum_{j=0}^\infty \beta^j 
             E_0 \int_0^1 y^i_{j} \, di
= (1-\beta) {\frac{E_0 Y}{1-\beta}} = Y ,
                                    \EQN eq:c_marketclear_0
$$
where we have passed the integral inside the
conditional expectation $E_0$; 
integration of $y^i_{j}$ over the continuum of agents on
the unit interval yields the cross-section average of 
non-financial income, which  equals a constant $Y$ in 
this stationary environment. Equality \Ep{eq:c_marketclear_0} asserts that the goods market clears in period $0$.
To confirm that  goods markets in all future 
periods also clear, we use expression  
\Ep{sprob16;a} for an agent's optimal 
consumption choice,
$$ c^i_{t+1} = c^i_t + h w^i_{t+1} ,  \EQN eq:c_future
$$
where $h = (1-\beta) \check G (I - \beta \check A)^{-1} 
\check C $. 
Integration of $c^i_{t+1}$ over all
agents yields
$$ \int_0^1 c^i_{t+1} \,di 
= \int_0^1 c^i_t \,di + h \int_0^1 w^i_{t+1} \,di
= \int_0^1 c^i_t \,di = \int_0^1 c^i_0 \,di =Y,
$$ 
where the second equality recognizes the zero mean 
of the i.i.d.~process for $w_{t+1}$ after 
integrating over the continuum of agents on the 
unit interval. The third equality follows from 
applying this computation recursively 
until reaching period $0$, at which the resulting 
integration of $c^i_0$ over all agents equals $Y$ 
according to equation \Ep{eq:c_marketclear_0}. 
%Given that the goods market clears in every period 
%$t \geq 0$,\NFootnote{The cross-section mean of 
%consumption appropriately accounts for  any negative 
%consumption rates  as being production.} 
%the market for borrowing and lending will also 
%clear in period $t$ by Walras Law. 

%At this interest rate,  appropriate weighted sums of 
%one-period risk-free borrowing equal  appropriate weighted 
%sums of one-period risk free lending at each $t \geq 0$. 


We have shown that the cross-section mean of 
consumption equals the stationary mean of non-financial 
income each period.\NFootnote{The cross-section mean of 
consumption appropriately accounts for  any negative 
consumption rates  as being production.} 
What about higher moments of the 
cross-section distribution of consumption? 
From equation \Ep{eq:c_future}, we know that
$$ \eqalign{ E_0 c^i_t & = c^i_0 \cr
              E_0(c^i_t  - c^i_0)^2 & = t h h' . } 
                                 \EQN eq:spreadcons $$
Hence, with the intitial conditions that we have specified, each consumer $i$'s consumption follows a random walk and his/her financial wealth is co-integrated with his/her own consumption. The gross quantities  traded each period increase as time passes. Nevertheless,  average debt across  all consumers stays at zero.   Each period, the market for one-period risk-free loans clears at the same 
constant gross interest rate $R = \beta^{-1}$. 


Thus, we have described a collection of  people who   borrow and lend
with each other at a  constant gross risk-free interest rate of
$R = \beta^{-1}$.
...
Risk-free loans are being made from one consumer to another  consumer who is also within our closed set of agents.
In this way, we have dispensed with the ``small open economy'' assumption and created a 
 ``Bewley model''.  We'll encounter  more Bewley models in chapter \use{incomplete}.
\auth{Bewley, Truman F.}%
...
%Lars2024%Given initial condition $b^i_0 = 0$ and using 
%expressions
...
%so that
%$$ \eqalign{ E_0 c^i_t & = c^i_0 \cr
%              E_0(c^i_t  - c^i_0)^2 & = t h h' . } \EQN 
%eq:spreadcons $$
Because each consumer $i$ dislikes variation of  consumption over time, he would prefer a completely smoothed   consumption stream  in which $c^i_t = c^i_0$ for all $t \geq 0$, where
$c^i_0$ is given by \Ep{eq:c_null}.
Such an allocation 
is  feasible since the cross-section average of our continuum of consumers  equals the mean of the
stationary distribution of non-financial income.
If at time $0$ everyone could get together and agree to
pool all   non-financial incomes  and  have each person consume $Y$, each 
person would prefer that allocation to the 
risky allocation that prevails  in our closed one-period risk-free bond economy.   In section \use{sec:LQcomplete}, we describe an environment that our consumers would prefer to  live in, one  with the same physical opportunities but with markets in more financial assets.  We shall include  enough securities  so that our consumers can  completely insure their consumption processes   against random 
fluctuations in their non-financial incomes. 
%We do this  in section \use{sec:LQcomplete}.
...
% to avoid all fluctuations in consumption by trading state-contingent claims to future consumption.
 % As in section \use{sec:LQmodel} 
 We temporarily  suppress the 
index $i$ that identifies  a particular person  and begin by formulating 
the decision  problem solved by a typical   consumer. 
We replace our earlier budget constraint \Ep{sprob2}  by
...
due at time $t+1$ that are chosen at time $t$. The 
function $b_{t}(z_{t+1})$ describes the consumer's financial portfolio, chosen  at time $t$,  of one-period state-contingent
securities  that promise to pay  $b_{t}(z_{t+1})$ units 
of time $t+1$ consumption at time $t+1$ in Markov state 
$z_{t+1}$. The time $t$ portfolio function    
$b_{t}(z_{t+1})$
%Lars2024%: \bbR \rightarrow \bbR$ 
is measurable at time $t$ and maps next period's 
$z_{t+1}$
%Lars2024% \in \bbR$ 
into a $z_{t+1}$-contingent payment 
at time $t+1$.

In the spirit of our section \use{sec:borrowlender} approach  that 
arbitrarily set $R^{-1}=\beta$ and then verifying % as we did  in section \use{sec:borrowlender} 
that this is an equilibrium outcome, here we guess that the equilibrium 
pricing kernel is 
...
Pricing kernel \Ep{kernel101} discounts state-contingent claims on consumption at date $t+1$ by the product of the time  discount factor $\beta$ and  conditional probabilities of    events
at which  claims pay off. 
Sometimes the kernel $q(z_{t+1} | z_t)$ is called  a state-price density,
while  the function $b_t(z_{t+1})$ is said to describe a bundle of one-period ahead Arrow securities.


We can use  the pricing kernel to  price ``redundant securities'' consisting of   bundles of 
...
at time $t+1$ when realization $z_{t+1} \in A$
%Lars2024% \subset \bbR$ 
is 
$$
...
so that  the gross interest rate on one-period risk free loans is  $R=\beta^{-1}$. This outcome indicates a sense in which our guess of pricing kernel \Ep{kernel101} seems to  
correspond to its ``incomplete markets'' counterpart  in our  section  \use{sec:borrowlender} model. 


We conjecture that the equilibrium allocation to a typical person  is a time-invariant consumption level, 
...
To construct a financial plan that supports this
 consumption level, we revisit consumption
function \Ep{eqn:lucascritiquecons} in the 
incomplete markets economy. Recall that in that economy  
the consumer enters period $t$ in Markov state $z_t$
with non-contingent debt $b_t$ and chooses a 
consumption level $c_t$ that makes conditional 
expectations of  future consumption levels  satisfy 
$E_t c_{t+j} = c_t$ for all $j>0$, which are 
first-order necessary conditions in the consumer's
optimization problem. As a counterpart 
in the present complete markets 
economy,  we assume  that the consumer enters period $t$
with a state-contingent debt $b_{t-1}(z_t)$ at which the  conjectured constant 
consumption level $\bar c$ can be financed. Thus, 
...
Before confirming the conjectured debt function $b(z_{t}, \bar c)$, 
note  that indebtedness  entering
period $t$ depends %does not depend on history but 
only on 
the realization of $z_{t}$, not on earlier $z$'s.\NFootnote{Financial 
...
The absence of history dependence in the debt function $b(z_{t}, \bar c)$
reflects the   complete risk sharing attained within 
our conjectured complete markets equilibrium.
% that idiosyncratic risks
%are not born by anyone.

We now confirm that the conjectured portfolio choice
attains the constant consumption level $\bar c$ and therefore  the consumer can afford it. Substituting conjectures 
...
E \left[\sum_{j=0}^\infty \beta^j y_{t+j+1} 
            \Bigg| z_{t+1}\right] 
             - {\frac{1}{1-\beta}} \bar c )d z_{t+1}  +y_t \cr
...
\bar c &= \int q(z_{1} | z_0 ) b(z_1, \bar c) d z_{1}  
                       + y_0 \,,                  \cr}
$$
where we  again have used the law of iterated 
...
in the incomplete markets economy, and that the portfolio
choice $b(z_{1},\bar c)$ is affordable at time $0$.

Having analyzed the situation and choices of a typical consumer living 
in our complete-markets environment, we now restore  superscripts $i$ to pertinent 
objects and analyze a stationary cross section of such consumers.
To make things fit together, we  guess a distribution of consumers'
initial endogenous states, their  $z^i_0$'s.
We again adopt our  section
\use{sec:borrowlender} assumption that in 
...
$i\in[0,1]$. We also assume that consumers have 
%Under that assumption,
%together with the initial conditions of
zero 
outstanding debts at the beginning of period $0$, 
so that $b_{-1}^i(z_0) = 0$ for all $i$, in which equation \Ep{eq:c_null} implies that 
$$
c^i_0(z^i_0) = (1-\beta) \check G(I-\beta \check A)^{-1} z^i_0 .
\EQN eq:c_null101
$$
We have  characterized
optimal consumption decisions  in a general equilibrium of the
...
\Ep{eq:c_null101}. The cross-section distribution of consumption is time-invariant, unlike the
incomplete markets economy in which  it spreads out over time.  In our complete markets economy,
 individual $i$'s location in the cross-section distribution of consumption is forever fixed: he is destined to stay  in the same wealth and consumption  quantiles forever. 
%  Instead of
...
of period $0$ manifests itself both in perpetually 
high consumption $\bar c^i$ described  by
\Ep{eq:c_null} and in  
lower indebtedness $b(z^i_{t}, \bar c^i)$ in 
all future Markov states $z^i_{t}$ as described  by
\Ep{guess102}. Evidently, the effect on 
indebtedness is a common  downward
shift in $b(z^i_{t}, \bar c^i)$ across all 
possible Markov states $z^i_{t}$, which provides
the financial resources  to sustain a permanently 
a higher time invariant 
consumption level $\bar c^i$. Differences in 
$b(z^i_{t}, \bar c^i)$ across 
states $z^i_{t}$  serve to insure 
against idiosyncratic shocks $w^i_{t}$ to  
consumer $i$'s non-financial income.
While there is  heterogeneity in  realizations
...
specification of their non-financial income processes described by  \Ep{sprob15}. It follows that the 
optimum decision rule for indebtedness, 
$b(\cdot, \cdot)$, is the same for all 
consumers so that it need  bear no index $i$.

The composition of 
consumer $i$'s optimum  portfolio is time-invariant: the consumer purchases the 
same bundle $b(z^i_{t+1}, \bar c^i)$
at every date $t\geq 0$ regardless
of the present Markov state $z^i_t$. Of course, the
purchase price of that portfolio depends on 
the pricing kernel $q(z^i_{t+1}|z^i_t)$ that 
is a function of the Markov state $z^i_t$ at
the time of purchase. As confirmed above,
the  state-dependent purchase price of the optimum 
portfolio, namely,  
$$
\int q(z^i_{t+1}|z^i_t) b(z^i_{t+1}, \bar c^i) d z^i_{t+1} \,,
$$
is affordable to each consumer $i$ in every 
period $t$ and all  Markov states  $z^i_t$.

Given  initial draws of $z^i_0$,
 all consumers   
attain  higher expected utilities  under complete 
markets by consuming $\bar c^i$ forever. 
In the incomplete markets economy of section \use{sec:borrowlender} in which  
consumer $i$'s time $0$ expectations of 
future consumption  equal 
$E_0 c^i_t = c^i_0 = \bar c^i$ at all forecast
horizons $t$, notice that  conditional {\it means}  equal 
the constant consumption level $\bar c^i$ that
consumer $i$ enjoys  in the complete markets
economy. 
In addition, the conditional variance of 
a consumer's consumption in the incomplete markets
economy grows with the forecast horizon as shown in
\Ep{eq:spreadcons}. It follows that consumers in 
the incomplete markets economy attain lower expected 
utility than they would  in the  complete markets economy 
since their
concave utility functions tell them to   dislike 
fluctuations in consumption around the  constant permanent consumption level they attain 
in the complete markets economy.
%Lars2024%conditional variance in consumption.
...
\section{The Lucas Critique}\label{sec:lucascritique}%
 We can use our chapter \use{timeseries} tools to describe reasoning that led Robert E. Lucas, Jr., (1976) to criticize policy evaluation   procedures that were then  widely used in applied work  (and  occasionally still are), while also showing how those same tools can be used to construct models that incorporate rational expectations in a way that responds to his criticism.
...
where   $A$ is a stable matrix, $\delta \in (0,1)$ is a discount factor,
 and   $x_0 \sim {\cal N}(\mu, \Sigma) $, where $(\mu, \Sigma)$ are the  mean vector and covariance matrix of the time $t$ marginal distribution associated with the  stationary distribution of 
   $\{x_t\}_{t=0}^\infty$ process.\NFootnote{In applications like ones  described earlier in this chapter and in chapter \use{dplinear},  $A$ sometimes depends on deeper parameters.}  Once again, $w_{t+1}$ is the time $t+1$ component of   an i.i.d.~sequence of ${\cal N}(0,I)$ random vectors.
   Here $d_t$ is a vector of decisions that  an econometrician observes. The  presence of the resolvent operator
   $(I - \delta A)^{-1}$  on the right side of \Ep{lucas101;d} indicates  that decisions $d_t$ embed  optimal  forecasts of $x_{t+j}, j \geq 0$.   Equations \Ep{lucas101;c} and \Ep{lucas101;d} assert that $d_t$ equals a  fixed matrix $H$ times a   vector of
...
$$ y_{t+1} = \alpha + \rho_1 y_t + \rho_2 y_{t-2} + \sigma w_{t+1}  \EQN lucas102 $$
 that we can represent with equations \Ep{lucas101;a}-\Ep{lucas101;b}.  Think of
$d_t$ as a time $t$ {\it decision\/}  and $d_t = S x_t$ as a {\it decision rule\/} that tells how a representative  economic agent  sets
$d_t$ as a function of time $t$ information $x_t$ about future $y_{t+j}$'s so that 
$d_t = H E_t \sum_{j=0}^\infty \delta^j y_{t+j}$,  where $E_t$ denotes a mathematical expectation conditional on time $t$ information.
Lucas (1976) offered three examples that   fit within  this framework. In Lucas's  examples,  decisions $d_t$ were
   a consumption rate or  an   investment rate 
or a labor supply (in  his Phillips curve example), and $y_t$ was a stochastic process that embeds  
government polices 
...
= A \Sigma_{xx} A' + C C'$ and $\Sigma_{dx} = S \Sigma_{xx}$, as discussed in chapter 
\use{timeseries}.  Application of the least squares projection formula \Ep{leastsq12} confirms that  $S$  equals a matrix of population regression coefficients of $d_t$ on $x_t$
   and that for each component of $d_t$ the $R^2$ of the linear regression  is $1$.\NFootnote{Apply formula \Ep{leastsq12} for the matrix of regression coefficients
...
Lucas (1976) criticized then contemporary policy evaluation procedures  that had used the following faulty logic.

\medskip


\item{a.}  The researcher gathers   historical observations on the decision vector $d_t$ and the state vector $x_t$ from historical data and estimates % $\{d_t, x_t\}_{t=0}^T$,
 regression  $d_t = \hat S x_t$ % is a
%regression equation, and   estimates    regression coefficients $S$ 
by applying  least squares  equation-by-equation. % to  obtain $d_t = \hat S x_t$.


\medskip

\item{b.} The  researcher  does not realize that $S$ actually emerges from optimality conditions and is of the form
shown in %he decision rule for $d_t$  but  does not know  that  $S$ satisfies equation
 \Ep{lucas101;d}. Not knowing that $S$ satisfies  cross-equation restrictions, the researcher treats $S$ as a list of free 
parameters that will be  invariant to alterations in  policies. 
...
\item{c.}  Wanting to study counterfactuals, the researcher posits a new process
for $\{x_t\}$ that differs from \Ep{lucas101;a} or \Ep{lucas101;b}, denoted $\{\hat x_t\}$.\NFootnote{However, the researcher would be correct if he 
were to restrict himself to studying policies giving rise to  $\{x_t\}$ sequences that
  {\it are \/} described by  system \Ep{lucas101;a}-\Ep{lucas101;b}.}
%  Mistakenly,  the researcher  assumes that the  estimated   decision rule %$d_t = S x_t$ that  estimated
...
%  \Ep{lucas101;a}-\Ep{lucas101;b}.   Consequently, he falsely believes that he can use that estimated  decision rule $d_t = \hat S x_t$ to predict how arbitrary variations in $\{x_t\}$ that  depart  systematically from system  \Ep{lucas101;a}-\Ep{lucas101;b}
%  %variations in $\{x_t\}$ 
%   will induce
...
\item{d.} A researcher who computes $d_t = \hat S \tilde x_t$ with the new process $\{\tilde x_t\}$ but retains the originally estimated decision rule $\hat S$,  mistakenly trusts a misspecified model. 
%ill-posed model.
%  Holding the historically estimated decision rule $\hat S $ fixed,
...
\item{e.} When the researcher simulates outcomes $\{y_t, d_t, x_t\}$ under the new pro-
cess  $\{\tilde x_t\}$  using the original decision rule $\hat S$, outcomes  appears suboptimal according to a social welfare function that depends
on these  paths.
% After
...
% of a state-space system to which he applies the chapter \use{dplinear} linear-quadratic optimal control theory   to  recommend a
...
\item{f.}  After simulating   alternative processes 
$\{\tilde x_t\}$ and  outcomes
$\{y_t,d_t,x_t\}$, the researcher observes a relationship between social welfare
and the alternative policy scenarios and  consequently seels a  welfare
maximizing sequence $\{\tilde x_t\}$. Rather than recognizing that S itself
must change under the newly realized outcomes, the researcher attributes the suboptimal welfare to poor policy design and proceeds to
recommend  a new law of motion $(\tilde A,\tilde G)$ derived  by  holding $S$ fixed and solving a new linear-quadratic
control problem.

\medskip
\item{g.} The researcher is disappointed to find that the new policy $(\tilde A, \tilde G)$ does not yield the desired outcomes $\{y_t, d_t\}$ under the new law of motion $\{\tilde x_t\}$ because the decision rule $d_t = \hat S x_t$  no longer describes the data generated under the new regime $\tilde G, \tilde A$.




% When    decision makers inside the model  who choose $d_t$ recognize that under the new government policy  $(\tilde A, \tilde G)$ %and not $(A, G)$ 
%  describe the $\{y_t\}$ process, they will adjust their  decision rule $S$  to become 
%  $\tilde S = H \tilde G (I - \delta \tilde A)^{-1}$. The  policy maker will be disappointed to find that $d_t = \hat S x_t$  no longer  describes data generated
% under the new regime $\tilde G, \tilde A$.

\medskip

Lucas (1976) asserted  that the  line of reasoning in points (a)-(g)    characterized an influential  1940s-1960s econometric policy evaluation
approach  that used  optimal control techniques  to propose  improved  macroeconomic policies. 
Point (g) emphasizes  that the method was flawed because it  ignored
 the key cross-equation restrictions in equations \Ep{lucas101;d}. Altering the policy-generating
matrices from the $(G, A)$ pair that prevailed historically to the new pair  $(\tilde G, \tilde A)$ that describes a  hypothetical policy ``experiment'' 
 ...
 would   systematically  alter how purposeful  private decision makers would choose  make the  $\{y_t\}$ process depend on the  $\{x_t\}$ process, as
 described by
...
under the new  regime. Structure \Ep{lucas101}, and in particular the set of cross-equation restrictions \Ep{lucas101;d},
implies  that $S$  varies {\it systematically\/} across   hypothetical changes
in  $(\tilde G, \tilde A)$, belying an  assumption that $S$ is invariant to the hypothetical policy interventions being investigated.
According to this reasoning, the joint probability distribution of the $\{d_t, x_t\}$ process and the implied
  probability distribution
of $d_t$ conditional on  $x_t$ process  depend on the  triple of matrices $(A, C, G)$.

Lucas (1976) summarized his critique  as follows:
...
that any change in policies will systematically affect the structure of econometric models.}{``Econometric Policy Evaluation: A Critique,'' 1976}
 ...
 series relevant to the decision makers.'' In
 section \use{sec:LQmodel}, we   encountered an example of   restrictions like \Ep{lucas101;d}  on optimal decision rules  when we noticed that the LQ permanent income model of consumption smoothing implies an optimal decision rule
   for consumption of the form \Ep{eqn:lucascritiquecons}. We encountered other  examples  in chapter \use{dplinear}.
    Lucas (1976) inspired  a  reconstruction of   econometric models  and policy evaluation procedures  that  incorporated
      the kinds of cross-equation
...
early papers about  rational expectations econometrics.    See Sargent (1981)  Hansen and Sargent (1991a) for further
...
\section{Another Response to the Lucas Critique}\label{sec:lucascritique2}%
Section \use{sec:lucascritique} concluded by describing  one  response to the Lucas Critique,  namely,
...
changes in  government  decision rules  or ``regimes''  typically  
unfold in ways that private agents understand.  %don't occur in a vacuum but were
...
%like \Ep{lucas101;d}.} 
Thus, 
Sargent and Wallace (1976) asserted that 
...
determined by a well understood stochastic  process  leaves  econometricians and policy analysts with
 no ``free parameters'' to change, giving them no scope  to offer policy advice.
 Christopher A. Sims (1982) and Sargent (1984) expressed  this viewpoint.\NFootnote{Musto and Yilmaz (2003) used this  approach 
 to model a  political-economic equilibrium  with complete financial markets.}
 \auth{Musto, David K.}%
 \auth{Yilmaz, Bilge}%
...
\noindent Here we'll use some  chapter \use{timeseries} tools to  study   several   parts of Sims and Zha's analysis:
...
\medskip
\item{b.} How agents make decisions when they  know   the current regime and regime transition probabilities.
...
nonlinearities discussed  by Sims and Zha   with  a finite set  of linear state space models with
...
Assume that $\{x_{t}, y_t\}_{t=0}^\infty$ is governed by  the linear state-space systems  
$$ \eqalign{ x_{t+1} & = A_{s_t} x_{t} + C_{s_t} w_{t+1} \cr
...
Markov state $s_t$.    
We  want to compute conditional  mathematical expectations
...
We want to  compute  $z_t$, the  expectation of the geometric sum of future $y_{t+j}$'s conditional on both $x_t$ and $s_t$ defined 
...
to get
$$  H_i  = G_{i}   + \delta \sum_{j} H_j A_i P_{ij}, \EQN guessH  $$ % \EQN Hformula101 $$
which implies  that
$$ \eqalign{ H_1 & = G_1 + \delta [ H_1 A_1 P_{11} + H_2 A_1 P_{12} ] \cr
                   H_2 & = G_2 + \delta [ H_1 A_2 P_{21} + H_2 A_2 P_{22} ]. }$$
Stacking these equations and solving for matrices $H_1, H_2$, we obtain 
$$ \bmatrix{H_1 & H_2 }  = \bmatrix{G_1 & G_2}             (I - \delta \bmatrix {A_1 P_{11} & A_2 P_{21} \cr
                                                   A_1 P_{12} & A_2 P_{22} }  )^{-1}. \EQN Hformula101 $$
%We have thus computed $z_t$ defined in \Ep{SimsZhaz_t}.

\subsection{Time-invariant VAR}
 There exists a  first-order
  {\it time-invariant\/} vector autoregression that is implied by   \Ep{MSLQ} and that takes the form
  ...
  when they say of a time-varying system that ``its behavior could be well approximated as linear and non-time-varying.'' Or perhaps they had in mind higher than first-order vector autoregressions 
  that involve projecting $x_{t+1}$ not just on $x_t$ but on lags $x_{t-j}, j= 1, \ldots, p$.
To examine these issues, we can compare three  conditional expectations, namely,
...
where $x^t = x_t, x_{t-1}, \ldots, $. 

While the joint process $\{x_t, s_t\}$ is Markov, except for special $s_t$ processes,  the process $\{ x_t \}$ is not. In general then,  $ E [ \ \cdot \ | x_t ] \neq E [ \ \cdot \ | x^t ]$
...
In the remainder of this section, we restrict ourselves to situations in which $s_t$ is i.i.d.  Appendix  \use{appendixSimsZha} of this chapter discusses  consequences of $s_t$ not being i.i.d.



\subsection{I.I.D. $s_t$ process}

We now assume that the transition matrix $P$ is 
$$ P = \bmatrix{.5 & .5 \cr
               .5 & .5},  $$
              so that the stationary distribution $\pi_0$ is $\pi_0 = \bmatrix{.5 & .5}$. 
...
$$
E x_{t+1} | x_t = (.5 A_1 + .5 A_2 ) x_t.
$$
where $\pi_1 = .5$ and $\pi_2 =.5$ are  stationary probabilities that $s_t = 1$ and that  $s_t = 2$, respectively. % so that 
%$\pi_i = E {\bf 1}_{s_t = i}$.
We can then represent $(x_{t+1} - E(x_{t+1}| x_t))(x_{t+1} - E(x_{t+1}| x_t))^T$ as 
$$ 
({\bf 1}_{s_t = 1}  C_1 w_{t+1} + {\bf 1}_{s_t =2}  C_2 w_{t+1})({\bf 1}_{s_t = 1}  C_1 w_{t+1} + {\bf 1}_{s_t =2}  C_2 w_{t+1})^T .
$$
Taking mathematical  expectations with respect to the stationary distribution of $s_t$ gives
$$ E (x_{t+1} - E(x_{t+1}| x_t))(x_{t+1} - E(x_{t+1}| x_t))^T = \sum_i .5 C_i C_i^T .
$$
...
\subsection{Example}
Let's construct  an example fashioned after a rational expectations version of Philip Cagan's (1956) model of
hyperinflation.\NFootnote{Sargent and Wallace (1973) and Sargent (1977) formulated   rational expectations versions of  Cagan's model.}   The rate of inflation is the first difference of the logarithm of the price level
...
Setting  $x_t= \bmatrix{m_t & m_{t-1} }'$   lets us represent a  stochastic process for $\{m_t\}_{t=0}^\infty$ with a state-space form \Ep{MSLQ }:
...
as a vehicle to illustrate Sims and Zha's remark about how  a linear time-invariant autoregression can approximate
...
\subsection{An associated time-invariant autoregression}\label{sec:cond_dist_102}%
We use some of the formulas above and our example in  equation \Ep{second_order_AR} to revisit Sims and Zha's (2006)
comments about how in some circumstances   a linear time invariant autoregression can do a good job of  approximating a Markov switching autoregression.
  We set $\delta = \lambda = .95$, $(\rho_{1,1}, \rho_{2,1}, c_{1}) =
...
$$ \eqalign{ x_{t+1}  & = \bar A x_t + v_{t+1} \cr 
              z_t & = \bar H x_t } $$
where $ E v_t x_{t-j} = 0 \ \forall j \geq 0$ and where 
$$ \bar H = \bar G (I - \delta \bar A)^{-1}  , \EQN hall_mark_restr 
$$
 where 
 $$ \eqalign{\bar G & = .5 G_1 + .5 G_2 \cr 
            \bar A & = .5 A_1 + .5 A_2  .} $$
The formula for $\bar H$ is a  special case  of equation \Ep{Hformula101} 
In this special  case in which  $s_t$ is i.i.d.,
% in which $s_t$ is independently and identically distributed, 
cross-equation rational expectations restrictions of the form 
...
$.64$ in state $s_t = 2$.  
For this same example, it is instructive to set  $\delta =.95$ and  $G_i =
\bmatrix{1 & 0 \cr}'$ for $i = 1, 2$ so that $G_i$ does {\it \/} not depend on the Markov state $s_t$. 
In this special case,  
$$ \bar H =  \bmatrix{5.39 & -.77 }.  $$ 

Appendix \use{appendixSimsZha} of this chapter describes how higher-order VARs arise when the Markov-switching $s_t$ is  temporally dependent. 
...
Applications described  in this chapter are  indicative  of much theoretical and empirical work
...
Chapters \use{stackel} and \use{optaxrecur} study formulations of macroeconomic control problems that are designed to be immune to the 
Lucas Critique.
\auth{Quah, Danny}%
...
by applying ideas that we described in section \use{lagrangianformulation} of  chapter \use{dplinear}.
...
variables $[b_t, z_t]$.
A powerful way to  determine this function is to apply the  invariant subspace method of chapter \use{dplinear}.
...
This appendix describes an infinite order VAR associated with the  section \use{sec:lucascritique2}  Markov switching model.

\subsection{Conditioning on  history  $x^t$}
Consider  the Markov switching VAR model studied in section \use{sec:lucascritique2}. 
When the Markov state $s_t$ is not i.i.d., the stochastic process $\{x_t\}_{t=0}^\infty$ is no longer Markov.
But  under appropriate  restrictions on  eigenvalues of the $A_i$ matrices, it is possible that the doubly infinite  stochastic process $\{x_t\}_{t=-\infty}^\infty$ is  stationary. 
In this situation,   an infinite-order vector autoregression exists. We can construct it by  computing the mathematical expectation of $x_{t+1}$ conditional
on $x^t \equiv \{x_s\}_{s=-\infty}^t$. % of $x$'s up to time $t$.
We can  also  compute a mathematical expectation of a geometric sum of future
...
%We let $\pi_t$ be the distribution over the finite Markov state $s_t$. Note that the
 We start by noting that the probability distribution of $x_{t+1}$  conditional on $(x_t, s_t)$ is multivariate normal:
...
Let $\psi_i(x^*| x_t)$ be the Gaussian density  ${\cal N}( A_i x_t, C_{i}C_{i} ' )$ in Markov state
$s_t = i$. Evidently,   $x_{t+1}$ is distributed according to  a mixture of normals
...
At time $t$,   we know  $x_t$ and also   $x_{t-1}, x_{t-2}, \ldots, $.
Past values of $x$   typically  
 contain  information about   $x_{t+1}$. % is contained in the preceding mixture of normals.

We  remarked earlier  that although  the joint process $\{x_t, s_t\}$ is 
...
History dependence emerges  through  dependence of the time $t$ mixing probabilities $q_t$ on $x^t$.
...
Formula \Ep{mixture103} shows how dependence of $z_t$ on   $ \{x_s\}_{s=0}^t$ enters because this history   contains information
about the unobserved state (or ``regime'') $s_t$. 
...
%It is useful to restrict  $q_0$ 
 By   imagining that the system has been running 
  long enough, we can pretend that there has been  an ``infinite past''. 
 Then, under appropriate restrictions on  eigenvalues of the $A_{s_t}$ matrices, the transition matrix $P$, and an  initial probability vector $q_0$, the probability model
provided by \Ep{bayes_nonlinstate}, \Ep{mixture102}, and  \Ep{mixture103} describe a  stationary stochastic process 
for   $\{x_t, y_t, z_t, q_t \}$.  Since the process is stationary, the marginal joint process of  $\{x_t, y_t, z_t\}$  is  covariance
stationary.  That means that population vector autoregressions  of the form
$$ y_{t+1} = \sum_{j=0}^m b_j y_{t-j} + \epsilon_{t+1}^m $$
with $E \epsilon_{t+1}^m y_{t-j}' =0 $ for $j =0, 1, \ldots, m$ are well defined, as are  the linear-least-squares 
projections 
$$ z_t = \sum_{j=0}^m h_j x_{t-j} + u_t^m $$
where $E u_t^m x_{t-j}' = 0$.  
Such autoregressions and  linear-least-squares projections are examples of 
the linear models that Sims and Zha (2006) might have had  in mind as  being   good approximations despite 
...
$$ c_t^i + b_t^i = R^{-1} b_{t+1}^i  + y_t^i , \quad t \geq 0 ,  $$
where $y_t$ is an exogenous
 stationary endowment process, $R$ is a constant gross
