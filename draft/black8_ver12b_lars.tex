...
with valued fiat currency, government debt, intergenerational
transfers, and fiscal policy. We conclude with the analysis of
a growth model.
...
old that makes them better off.  We shall soon learn how an institution 
of either fiat money or of unbacked, perpetual government debt 
...
durable asset in the form of fiat money or issues
one-period bonds that are rolled over forever, i.e.,
the government debt is unbacked. Under the auxiliary assumption 
...
both transformations  explicitly impose same rates of return, i.e., we
...
This equation lends credence to our earlier analogy that 
...
system behaves under two different types of imperfect foresight, adaptive dynamics.
...
$w_2(j)$ when old.  
For a consumer group $j$ born at time $t$, each young person's 
...
tree bears no fruit, $d=0$, but the tree itself is durable.
...
of the tree would be governed by the very same calculations of 
valuation. Though, please note that the price level in  
section \use{sec:Samuelson101} is the price of goods in terms of fiat
money, which in our current context would be the price of goods in 
terms of Lucas tree. Hence, the price of Lucas tree (in terms of
...
people at time $t$, with $n > 0$. For example, the extended 
...
As a comparison, recall the high-interest-rate stationary
...
fiat money in section \use{sec:Samuelson101} has now
...
to receive in period $T$ and, hence, it would have altered 
...
By so relaxing our earlier assumptions $n=\delta=1$, we 
can prove the proposition on suboptimal steady states
in subsection \use{sec:dynineff}, i.e., a steady state with
a capital-labor ratio $k^\star$ satisfying
$F_1(k^\star,1) < \delta +n-1$ is Pareto suboptimal.

The earlier resource constraint \Ep{olg_res} in period $t\geq 0$
is replaced by
...
deducting for depreciation). Hence, the inputs in the 
...
subsection \use{sec:dynineff}, we describe a
...
suboptimal, $F_1(k^\star,1) < \delta +n-1$, is about
...
Second, the cutoff for suboptimality is further
raised if the population grows, $n>1$. This is also 
a kind of required replacement as seen from the planner's 
...
young to old agents, which can be 
done with unbacked government debt or fiat money.
