...
\ruledtable
Object | Formula \cr
unconditional mean   |  $ \mu_{t+1} = A \mu_t$ \nr
unconditional covariance | $\Sigma_{t+1} = A \Sigma_{t} A' + C C' $ \cr
...
\endruledtable
\medskip
\noindent The accompanying table summarizes some formulas for various conditional and unconditional first and second moments of
the state $x_t$ in a stationary distribution for the  linear stochastic state space system with parameters  $A, C, G$.
...
Evidently,
$$ E_t \sum_{j=0}^\infty \beta^j y_{t+j} = G(I -\beta A)^{-1} x_t,
\EQN discount1 $$
...
In this application,
 the constant $\sigma$  functions as a
...
This section explains the notion of a population linear regression
equation.  \index{regression!population}% \index{regression equation}%
...
%space systems under assumptions described above.
 Although
 the notion of a population linear regression applies to a much wider class of probability distributions,  until  the end of this section
...
\noindent where $\mu = E z = \int z f(z | \mu, \Sigma) \,  d  z $ is the mean of $z$ and $\Sigma = E (z - \mu) (z-\mu)' =
...
Then the marginal densities of the random vectors $z_1$ and $z_2$ are
...
$$\beta = \Sigma_{12} \Sigma_{22}^{-1} \EQN leastsq12 $$ is an $(N- k) \times k$ matrix
 of population {\it regression coefficients \/} of $z_1 -\mu_1$ on $z_2- \mu_2$ and
$\hat \Sigma_{11}$ is the covariance matrix $E \epsilon \epsilon'$  of least-squares errors.
...
where the random  vector of least squares error $\epsilon$ is statistically independent of $z_2- \mu_2$. Such independence implies (but is not implied by)
 the (weaker)  linear least squares orthogonality
condition
$$ E \epsilon (z_2 - \mu_2)' = 0 ,  \EQN leastsq2  $$
an equation that we shall refer to in subsection \use{sec:nonnormal} when we consider random vectors $z$ that are non multivariate normal.





To apply the concept of  population regression, we require  a Gaussian random vector $z$ and  its mean vector $\mu$
and covariance matrix $\Sigma$. As one possible source, we can take an instance of the state-space  a state-space system \Ep{statesp1}, perhaps with
initial conditions that make the state covariance matrix constant over time. We can use
the formulas summarized in sections \use{sec:linearsysmoments} and \use{sec:summarymoments} to compute first and second moments of any pair
of random variables.  We can assemble these appropriately to construct $\mu$ and $\Sigma$ for a vector $z$ assembled from components of
...
$z_1, z_2$.   Thus, we have shown how a model (e.g., a triple $A, C, G$,
...
function} completely describes what a given data set says about  model parameters that pin down the matrices $A, C, G$. % in a way that  is straightforward to decode.

%

\subsection{Bayesian interpretation of conditional probability}
Here we provide a brief digression about Bayesian inference, an approach that interprets jointly distributed random vectors $z_1, z_2$ and
associated conditional distributions in a particular way.
Leaving the pertinent mean and variance parameters implicit for convenience and  using our  formulas, it can be verified that the joint density
$f(z) \equiv f(z_1, z_2)$ (to emphasize that $f(z)$ is the joint density of the two vectors $z_1 $ and $z_2$)  satisfies
...
By a similar logic, the conditional distribution $f(z_2|z_1)$ satisfies
$$ f(z_2 | z_1) = {\frac{f(z_1, z_2)}{f(z_1)} } . \EQN eqn:Bayes101a $$
Equations \Ep{eqn:Bayes101} and \Ep{eqn:Bayes101a} are ``just mathematics'', being  equations that define  conditional distributions according to the
laws of probability.
...
A Bayesian statistician might interpret
...
the density of a potentially observed  random vector $z_2$ conditional on parameter vector or hidden state  $z_1$,  the density $f(z_1)$ as a ``prior'' probability density
...
A Bayesian statistician has available an interesting interpretation of equation \Ep{condmeansurprise}. The equation
 asserts that the conditional mean $E z_1 | z_2 \equiv \hat \mu_1$ equals the unconditional mean $\mu_1$ before observing $z_2$
plus  $\beta (z_2 - \mu_2)$, the systematic part of the regression  of the surprise $z_1 - \mu_1$ in $z_1$ on the surprise
$z_2 - \mu_2$ in $z_2$.
A Bayesian statistician would interpret $\mu_1$ as the mean of her prior distribution for $z_1$ and $\hat \mu_1$ as the mean of her
...
and  it minimizes the mean squared error.  Now let's make the weaker assumption that $z$ is an $N \times 1$ random vector
with mean $\mu$ and covariance matrix $\Sigma$, but that it is not necessarily normal.  In this case, the linear least squares
projection of $z_1$ on $z_2$ is given by
$$ \hat E z_1 | z_2 = (\mu_1 - \beta \mu_2) +  \beta z_2  \EQN regresslinearo $$
where $\beta$ continues to be given by formula \Ep{leastsq12} and the covariance matrix of linear  least squares errors
continues to be given by the formula for $\hat \Sigma_{11}$. These assertions follow from the least-squares orthogonality conditions
...
We have shown  how to map the matrices $A, C$
into all of the second moments of the
stationary distribution of the stochastic process $\{x_t\}$.
Linear economic models typically give $A, C$ as functions
...
i.i.d.\ vector sequence of normal random variables with mean zero and
...
We use the insight that the information in $y_0$ that is new relative to the information  $(\hat x_0, \Sigma_0)$ that we knew before observing $y_0$
...
We  compute the  distribution of $x_0$ conditional on $y_0$.  The conditional  mean $E [x_0| y_0] =  \hat x_0 + L_0 (y_0 - G \hat x_0)$
satisfies
 an appropriate version of the population regression  formula \Ep{leastsq12}, namely,
...
$L_0$ as population least squares regression coefficients.  The least
...
Equation \Ep{kalf10new} is known as a matrix Riccati difference equation; it restricts a sequence of
...
The Kalman filter in effect uses a sequence of least squares projections called a  Gram-Schmidt process to  construct an orthogonal
basis for the information set $y_{t-1}, y_{t-2}, \ldots, y_0$.  Instead of computing $E [x_t | y_{t-1}, y_{t-2}, \ldots, y_0]$ from
one big linear least squares regression, the Gram-Schmidt process computes a sequence of much smaller regressions on successive components
$a_s$ of an orthogonal basis $[a_{t-1}, a_{t-2}, \ldots, a_0]$ for the linear space spanned by $[y_{t-1}, y_{t-2}, \ldots, y_0]$.
...
\medskip
\noindent {\it Exercise \the\chapternum.33}\quad  {\bf From shocks to innovations}
...
and $\{v_t\}{t=0}^\infty$ is an i.i.d. sequence with $v_t \sim N(0, R)$ for all $t$; and $E w_t' v_s = 0$
for all $t$ and $s$.
