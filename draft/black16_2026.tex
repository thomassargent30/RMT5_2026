...
then  many  first-period payoffs can be sustained as equilibria of  a repeated version of a game.
  A main purpose of this chapter is to study how discounting of future payoffs
...
\topfigure{game1f}
\centerline{\epsfxsize=2.0truein\epsffile{game1.ps}}
...
\midfigure{game2f}
\centerline{\epsfxsize=2.0truein\epsffile{game2.ps}}
...
A state  variable that encodes a ``reputation'' is both
...
Abreu, Pearce, and    Stacchetti that \newpage \noindent astutely exploits
...
a current value $v$ that obeys the Bellman equation \index{Bellman equation}%
...
\medskip\noindent
{\sc Lemma 1:}  Consider a strategy profile $\sigma$, and let
...
(x^N,y^N)$ for all $t$ and for all $(x^{t-1}, y^{t-1})$, and value
$V_g(\sigma^N)$ and  continuation values $V_g(\sigma^N\vert_{(x^t,y^t)})$
...
objects:\NFootnote{In the example, objects 3 and 4 are equated.}
\medskip
...
outcome is not low enough  that the prospect of reverting to it can support repetition of Ramsey.
In that case, the prospect of reverting 
to repetition of Nash after a deviation   can at best support
a lower value for the government  than can be attained by repeating the Ramsey outcome.
In this case, the prospect of reverting to repetition of Nash might  support something better than repetition of Nash, but it won't
be as good as repeating the  Ramsey outcome. % by although perhaps its is
%better than repeating the Nash outcome.

In this circumstance it is natural to ask: is there  a better SPE than  the best one that can be supported by anticipating reversion to repetition
of the one-period Nash outcome?
...
and computing  values for
 {\it all\/} the subgame perfect equilibria.\NFootnote{Relative to earlier editions, this section has been expanded and revised to take account 
 of contributions by Barth\'elemy and Mengus (2020).}%
 \auth{Barth\'elemy, Jean}\auth{Mengus, Eric}%  
 APS build on the way that the lemma
...
conditions of  Lemma 1 and is therefore an SPE with
...
We want to find the set $V$. In Definition~7, $W$ serves as a
set of candidate continuation values. The idea is to pick an $(x,y)\in
C$, then to check whether we can find a function $w(\cdot):Y\to W$ that
would make the government want to adhere to the $y$ component if
$w(\cdot)$ could be used to assign continuation values---$w(y)$ for
adhering to $y$, and $w(\eta)$ for deviating to any $\eta\in Y$. If the
answer is yes, we say that the triple $(x,y,w(\cdot))$ is ``admissible
with respect to~$W$.'' Because we have verified that the incentive
constraints are satisfied, a yes answer allows us to calculate the value
$
  (1-\delta)\,r(x,y)+\delta\,w(y)
$
that can be supported with $w(\cdot)$ as the continuation-value
function. Thus, the idea is to use \Ep{og7}  to define a mapping
from values tomorrow to values today, like that used in dynamic
programming. In the next section we define $B(W)$ as the set of possible
values attained with admissible triples whose continuation-value
functions map into~$W$. Then we view $B$ as an operator mapping sets of
continuation values $W$ into sets of values $B(W)$. This operator is the
counterpart to the $T$ operator associated with ordinary dynamic
programming.

% \bigskip\noindent
% Here \eqref{eq:adm} refers to the admissibility condition of
% Definition~7, i.e.,
% \begin{equation}\label{eq:adm}
%   (1-\delta)\,r(x,y)+\delta\,w(y)\;\geq\;
%   (1-\delta)\,r(x,\eta)+\delta\,w(\eta),\qquad\forall\,\eta\in Y.
% \end{equation}

%  We want to find the set $V$. In Definition 7, $W$ serves as a set of candidate
% continuation values. The idea is to pick an $(x,y)\in C$,  then 
% to check whether we can find a function $w(\cdot):Y\to W$ that would make the government want to adhere to the yy
% y component. If the answer is yes, we say that the
% triple (x,y,w(⋅))(x,y,w(\cdot))
% (x,y,w(⋅)) is 'admissible with respect to WW
% W'. Because we have verified that the incentive constraints are satisfied, a yes answer allows us to calculate the value 
% (1−δ)r(x,y)+δw(y)(1-\delta)r(x,y)+\delta w(y)
% (1−δ)r(x,y)+δw(y) that can be supported with w(⋅)w(\cdot)
% w(⋅) as continuation values.  
...
that the Bellman equation is $$Q  = T(Q). \EQN mccallbell $$
%This equation
%is
% solved by iterating to convergence  from any initial $Q$.

  Just as the right  side of \Ep{mccallbell} takes a candidate continuation value
 $Q$ for tomorrow
and maps it  into a value $T(Q)$ for today,
APS define a mapping $B(W)$ that, by considering only
admissible {\bf triples}, maps a set of values $W$ tomorrow into a new
set $B(W)$ of values today.
Thus, APS use admissible {\bf triples} to map candidate
continuation values tomorrow into new candidate values today.
%  APS define a mapping
...
admissible with respect to~$W'$ and $w\in B(W')$.\qed
...
Compactness of~$V$ follows by an
...
\medskip\noindent
{\sc Corollary:} (Two-value characterization.)
...
define $v(y)=v_1$ and $v(\eta)=v_2$ for $\eta\neq y$; then~\Ep{eq:cl2} holds,
so Lemma 2 applies.

\medskip
{\sl Necessity.\/}
Since $(x,y)$ is supported by an SPE, Lemma 2 yields a
function $v(\cdot):Y\to V$ satisfying~\Ep{eq:cl2}.  Set $v_1=v(y)$.  The
set $\{v(\eta):\eta\in Y\}$ is a subset of~$V$; since $V$ is compact
 and bounded, its infimum is attained in~$V$.
Set
$$  v_2 \;=\; \inf_{\eta\in Y}\,v(\eta) \;\in\; V .  $$
Then $v(\eta)\geq v_2$ for all $\eta\in Y$, so~\Ep{eq:cl2} implies
$$  (1-\delta)\,r(x,y)+\delta v_1
    \;=\; (1-\delta)\,r(x,y)+\delta v(y)
...
\medskip
\item{1.} $V\subseteq B(V)$ (by the lemma).
\smallskip
\item{2.} If $W\subseteq B(W)$, then $B(W) \subseteq V$ (by theorem 1).
\smallskip
\item{3.}  $B$ is monotone and maps compact sets into compact sets.
\medskip

\noindent Facts 1 and 2 imply that $V=B(V)$, so that the set of
...
$W_0$ for which $V \subseteq B(W_0)  \subseteq W_0$,  and to iterate
 on $B$. By the two-value Corollary above, it suffices to search over pairs
$(w_1,w_2)\in W\times W$ rather than  functions $w(\cdot):Y\to W$.
...
\item{1.}  Start with a set $W_0 = [\underline{w}_0,\overline w_0]$ that
...
\item{2.}  Compute the boundaries of the set $B(W_0) = [\underline{w}_1,
...
\item{3.}  Having constructed $W_1 = B (W_0) \subseteq W_0$, continue to
...
\medskip\noindent
{\sc Proposition 3:}  A subgame perfect equilibrium $\sigma$
...
Recall from Proposition 3 that the worst equilibrium is self-enforcing,
and here we repeat
...
the government's \newpage \noindent strategy plays a dual role, as it does
...
\smallskip
\noindent{\bf g.} Find the value to the government associated with the best
SPE.
\smallskip
\noindent{\bf h.}  Find the outcome path associated with the worst SPE.
\smallskip
