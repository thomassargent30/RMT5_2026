...
In this  Samuelson (1958) setup, 
at time $0$ the government gives  initial old agents a total of  $M$ units of currency. Consequently, the  government budget constraint
at time $0$ becomes 
...
of the new offer curve and the feasibility line in 
...
This is the same equilibrium condition as in the economy with
...
agents as a benchmark against which we can compare  inefficient equilibrium
allocations that  entail overaccumulation of capital.


\subsection{Technology and input prices}
...
As discusssed  in chapter \use{dynamicdp1}, the optimal solution
...
$$F_2(1,k_\infty) = {1 \over \beta}.$$

% Analogous to what we showed for the endowment economy in 
% chapter \use{recurge},
 A competitive 
...
and rent it to firms at a rental rate $\tilde r_t$, 
and sell their labor services at a wage rate 
$\tilde w_t$. Profit maximization by firms implies
...
% in an equilibrium:
$$\EQNalign{ F_1(l_t,k_t) - \tilde w_t &= 
  (1-\alpha) A \, \left(k_t \over l_t\right)^{\alpha}
 - \tilde w_t =0,                        \EQN firm_FOC;a \cr
             F_2(l_t,k_t) - \tilde r_t &= 
  \alpha A \, \left(k_t \over l_t\right)^{-(1-\alpha)}
 - \tilde r_t =0,                        \EQN firm_FOC;b \cr}$$
...
$$F(l,k) = F_1(l,k)\,l+F_2(l,k)\,k=\tilde w l+\tilde r K
...
$$ \max_{\{c^t_t, c^t_{t+1}, k^t_{t+1}, l^t_t\}} 
      u(c^t_t) + \beta \,u(c^t_{t+1})     \EQN olgmax $$
subject to
$$\EQNalign{&c^t_t + k^t_{t+1} \leq \tilde w_t \, l^t_t, \EQN growth_bc;a \cr
            &c^t_{t+1} \leq \tilde r_{t+1} \, k^t_{t+1}, \EQN growth_bc;b \cr
...
$$ 1 = \beta \,{u'(c^t_{t+1})\over u'(c^t_t)}\, \tilde r_{t+1} 
...
Suppose that $u(c)=\ln(c)$. The saving function is
$\tilde s(\tilde w_t, \tilde r_{t+1}) = 
\tilde w_t - c^t_t=\tilde w_t - {1 \over 1+\beta}\tilde w_t
= {\beta \over 1+\beta}\tilde w_t$.\NFootnote{With logarithmic
...
capital $\tilde r_{t+1}$ do not affect optimal savings.}
In an equilibrium, saving of the representative agent
$t\geq 0$  equals to next period's capital stock $k_{t+1}$:
$$k_{t+1} = \tilde s(\tilde w_t, \tilde r_{t+1})
            = {\beta \over 1+\beta}\tilde w_t
...
allowing for population growth, $N_{t+1}=n N_t$ for $n>0$, and 
...
$$ \hat k > \bar k \quad \Longrightarrow \quad 
   {\beta \over 1+\beta} \, {1-\alpha \over \alpha} > 1.
$$
As one might have anticipated, parameterizations that would
...
A proof of the above  proposition is relegated to 
appendix X. But here we use Figure X
for a graphical exposition of possible Pareto improvements 
that move the economy from an inefficient steady state 
to a Pareto optimal steady state in just one transition 
period. 

\noindent
{\bf [To be completed in one paragraph]}
