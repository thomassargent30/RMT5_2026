...
$$ \pi_t = s_t n_t - {\frac {f_2}{2}} n_t^2 - {\frac{d}{2}} (n_{t+1} - n_t)^2 - w_t n_t, $$
  $s_t$ is an exogenous productivity shock, $w_t$ is an exogenous wage rate, $f_2 > 0$, and $d > 0$.  The $\{s_t, w_t\}_{t=0}^\infty$ process is governed by
...
\medskip
\noindent{\bf b.} Please formulate the firm's optimum problem as a stochastic discounted optimal linear regulator problem. Define the firm's control
  as $u_t = n_{t+1} - n_t$. Please describe the form of the firm's demand curve for labor.
...
is a given initial condition,
$$ \upsilon_t =  - b_t n_t - {\frac {h_2}{2}} n_t^2 - {\frac{q}{2}} (n_{t+1} + n_t)^2 , $$
 $w_t$ is an exogenous wage, $h_2 > 0$,  and $q > 0$.  The $\{b_t, w_t\}_{t=0}^\infty$ process is governed by
$$\eqalign{ z_{t+1}  & = A_{22} z_t + C_2 \epsilon_{t+1} \cr
...
$$\eqalign{ b_{t+1} & = \alpha_s + \rho_1 s_t + \sigma_s \epsilon_{1,t+1} \cr
           w_{t+1} & = \alpha_w + \gamma_1 w_t + \sigma_w \epsilon_{2,t+1} ,}$$
where $\{\epsilon_{i,t}\}$ are each i.i.d.~sequences of ${\cal N}(0,1)$ random variables.  Show how to map these processes into system
(1).
\medskip
...
\noindent{\bf b.} Please formulate the worker's optimum labor supply problem as a stochastic discounted optimal linear regulator problem.
  Define the control as $u_t = n_{t+1} - n_t$. Define the state vector.  Please describe the form of the worker's supply curve for labor and represent it in state-space form.
...
$$\eqalign{ c_t &  =  s_t n_t - {\frac {f_2}{2}} n_t^2 - {\frac{d}{2}} (n_{t+1} - n_t)^2 \cr
            \upsilon_t & =   - b_t n_t - {\frac {h_2}{2}} n_t^2 - {\frac{q}{2}} (n_{t+1} + n_t)^2 } $$
where $f_2 > 0, h_2 >0, d > 0, q > 0$, $s_t$ is a productivity shock,  $b_t$ is a preference shock, $c_t$ is a worker's consumption, and $\upsilon_t$
...
\medskip
\noindent{\bf a.} Please formulate the planner's  optimum labor supply problem as a stochastic discounted optimal linear regulator problem.  Please describe the form of the planner's rule for setting $n_{t+1}$  and represent it in state-space form.

\medskip

\noindent
{\bf Hints:} Set $u_t =  n_{t+1} - n_t$, $x_t = \bmatrix{z_t \cr n_t}, C = \bmatrix{ C_2 \cr 0}$. The optimal decision rule is $u_{t} = - F x_t$ and the optimal closed loop system
...
Formulate the representative firm's problem as a discounted dynamic programming problem in which the control is $\check u_t = \check u_t$ and the state is
