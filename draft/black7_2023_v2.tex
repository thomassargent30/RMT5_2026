...
These  entail different assets  and timings of
...
event $s_t \in {\bf S}$. Let the history of events up and until time $t$
be denoted $s^t = [s_0, s_{1}, \ldots, s_{t-1}, s_t]$.
%Lars2023%
%\NFootnote{Later in this chapter and in other chapters too,
%we shall reverse the recording order and  denote a history as 
%$s^t = [s_t, s_{t-1}, \ldots, s_1, s_0]$.} 
The unconditional
...
of observing $s^t$ conditional on the realization of $s^\tau$
as $\pi_{t,\tau}(s^t\vert s^\tau)$.
...
There are $I$ consumers named $i=1, \ldots , I$. Consumer $i$
owns a stochastic endowment of one good
$y_t^i(s^t)$ that depends on the
history $s^t$.
The history $s^t$ is publicly observable.
Consumer $i$
purchases a history-dependent  consumption plan ${\bf c}^i =
 \{c_t^i(s^t)\}_{t\geq0,\,s^t}$ and
orders these
consumption streams by
\auth{Blume, Lawrence}%
\auth{Easley, David}%
$$ U_i({\bf c}^i) =
  \sum_{t=0}^\infty \sum_{s^t} \beta^t u_i\left(c_t^i(s^t)\right)
   \pi_t(s^t),
 \EQN eq0 $$
 where $0 < \beta < 1$.
...
$$ \lim_{c \downarrow 0} u'_i(c) = +\infty.  \EQN eq_Inada $$
Throughout the body  of this chapter, we adopt the assumption, routinely
employed in much of macroeconomics,
that  consumers share   probabilities $\pi_t(s^t)$ for all $t$ and $s^t$.\NFootnote{Exercises  \the\chapternum.15 -  \the\chapternum.19
study examples in which we replace \Ep{eq0} with
$$ U_i({\bf c}^i) =
  \sum_{t=0}^\infty \sum_{s^t} \beta^t u_i\left(c_t^i(s^t)\right)
   \pi_t^i(s^t), $$
where $\pi_t^i(s^t)$ is a personal probability distribution specific to consumer $i$.  Blume and Easley
(2006) studied such settings and investigated which consumers'  beliefs ultimately influence  
allocations and prices. We discuss related consequences of heterogeneous beliefs and   heterogeneous discounting in
...
a list $\{{\bf c}^i\}_i=\{\{c_t^i(s^t)\}_{t\geq0,\,s^t}\}_i$ of each agent $i$'s consumption at every date $t$ and 
all histories $s^t$. A {\it feasible allocation} satisfies
...
that correspond, respectively,  to  two views of an economy provided by
Figures \Fg{tree1f} and \Fg{tree2f}. %7.1 and 7.2.
One is
what we shall call the Arrow-Debreu structure.  Here  people meet
at time $0$ to  trade claims to consumption at all times $t >0$
and that are contingent on all possible histories up to $t$,
$s^t$. In that economy, at time $0$, consumers trade claims on
the time $t$ consumption good {\it at all nodes\/} $s^t$   for all $t \geq 1$. After
time $0$, no further trades occur, but deliveries agreed to at time $0$ are made.
...
arrangements support identical equilibrium allocations. 
Those
allocations share the notable property that they are  functions only of   time-invariant
parameters describing the initial distribution of wealth and  
the realization of
the {\it aggregate\/} endowment $\sum_{i=1}^I y_t^i(s^t)$.
...
%Before trading, the situation of consumer $i$ at time $t$ depends on the history $s^t$.
  A natural measure of consumer
$i$'s luck in life is  $\{y^i_0(s_0), y^i_1(s^1), \ldots \}$, whose time $t$ component
evidently in general depends on the history $s^t$. A
question that will occupy us in this chapter and in chapters
\use{incomplete} and
\use{socialinsurance} is whether, after trading, the consumer's
...
allocation  at time $t$  depends only on some time-invariant parameters that describe the time $0$  initial distribution of wealth and
on the realization of the aggregate
endowment  at time $t$,  not  on the  history that led to that aggregate endowment.
 The market incompleteness of chapter
...
said to be efficient if it is Pareto optimal, meaning that  it has the property
that any reallocation  that makes one consumer strictly better
off also makes one or more other consumers worse off. We can find
...
chooses allocations ${\bf c}^i, i=1, \ldots, I$ to maximize
$$ W = \sum_{i=1}^I \lambda_i U_i({\bf c}^i)  \EQN planner1  $$
subject to \Ep{eq4}.
...
side of \Ep{planner4} is the  aggregate endowment at $s^t$, so the
left side is  also a function only of that same aggregate endowment. Thus, given $\{\lambda_i\}_{i=1}^I$,
...
preceding it  or  the cross-section distribution of individual
...
preceding it or  the cross-section distribution of individual
endowments realized at $t, s^t$: $c_t^i(s^t)=c_\tau^i(\tilde
s^{\tau})$ for $s^t$ and $\tilde s^{\tau}$ such that $\sum_j
y_t^j(s^t) = \sum_j y_\tau^j(\tilde s^{\tau})$.
\medskip

 To compute an optimal allocation, we can first solve  \Ep{planner4}
...
\use{socialinsurance}, we shall see that  constancy through
time of Pareto weights
 $\{\lambda_j\}_{j=1}^I$ is a telltale sign that there are no
enforcement- or information-related  incentive problems
in this economy. %In chapter \use{socialinsurance},
When we introduce  those imperfections, the time invariance of  Pareto
weights will evaporate.\NFootnote{In  appendices to this
chapter, we will learn that Pareto weights also change 
over time when agents have different beliefs or  discount factors.}
\index{Pareto weights!time invariance}%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\section{Time $0$ trading: Arrow-Debreu securities}\label{sec:ADtime0}%
We now describe how an optimal allocation can be attained by a
competitive equilibrium with the Arrow-Debreu timing. Consumers
trade  a complete set of dated history-contingent claims to consumption. Trades occur at time $0$, after $s_0$
has been realized. At $t=0$, consumers can exchange claims on
time $t$ consumption, contingent on history $s^t$ at price
$q_t(s^t)$, measured in some unit of account. 
%Lars2023% The superscript $0$ refers to the date at which
%trades occur, while 
Hence, the subscript $t$ refers to the date that
deliveries of goods are to be made.  The consumer's budget 
constraint is
$$ \sum_{t=0}^\infty \sum_{s^t}  q_t(s^t)  c_t^i(s^t)   \leq
   \sum_{t=0}^\infty \sum_{s^t}  q_t(s^t) y_t^i(s^t).   \EQN eq2 $$
The consumer's problem is to choose  
${\bf c}^i =\{c^i_t(s^t)\}_{t\geq0,\,s^t}$
...
Upon completing trades at time $0$, the consumer's
{\it single\/} budget constraint \Ep{eq2} will hold
with equality. 
%Lars2023%
%Underlying the {\it single\/}  budget constraint \Ep{eq2} is the
%fact that multilateral   trades occur through a clearing
%operation that keeps track of net claims.\NFootnote{In
% the language of modern payments systems, this is a system
%with net settlements, not gross settlements.}
All trades occur at time $0$.  After time $0$, trades that
were agreed to at time $0$ are executed, but no more trades occur.


Attach a Lagrange multiplier $\mu_i$ to each consumer's  budget constraint  \Ep{eq2}.
We obtain the
first-order conditions for the consumer's problem:
$$  {\partial U_i({\bf c}^i) \over \partial c_t^i(s^t)} = \mu_i q_t(s^t), \EQN eq3b $$
for all $i, t, s^t$.
The left side is the derivative of total utility with respect to
the time $t$, history $s^t$ component of consumption.  Each
consumer has its own Lagrange multiplier $\mu_i$ that is independent of $t, s^t$. With specification \Ep{eq0} of the utility functional,
we have
$$ {\partial U_i({\bf c}^i) \over \partial c_t^i(s^t)} =   \beta^t
 u'_i\left(c_t^i(s^t)\right) \pi_t(s^t) .  \EQN eq3a $$
This expression  implies that equation \Ep{eq3b} can be written
$$ \beta^t u'_i\left(c_t^i(s^t)\right) \pi_t(s^t) 
= \mu_i q_t(s^t). \EQN eq3 $$


We use the following  definitions:
\medskip
\noindent{\sc Definition:} A {\it price system} is a list of
prices $\{q_t(s^t)\}_{t\geq0,\,s^t}$. It contains  a price $q_t(s^t)$ for each 
time $t$, history $s^t$ good. 
%\noindent{\sc Definitions:}   A {\it price system} is a sequence 
%of functions $\{q_t(s^t)\}_{t=0}^\infty$.  
%An {\it allocation} is a list of sequences of functions 
%${\bf c}^i=\{c_t^i(s^t)\}_{t=0}^\infty$,
%one for each $i$.
\medskip
\noindent{\sc Definition 1:} A {\it competitive equilibrium} is
...
$$ {u'_i\left(c_t^i(s^t)\right) \over
 u'_j\left(c_t^j(s^t)\right) }
...
$$ c_t^i(s^t) = u^{\prime -1}_i  
  \left\{u'_1\left(c_t^1(s^t)\right) {\mu_i \over \mu_1}
 \right\}. \EQN eq7 $$
Substituting this into equation \Ep{eq4} at equality gives
$$ \sum_i
u^{\prime -1}_i  \left\{u'_1\left(c_t^1(s^t)\right) 
{\mu_i \over \mu_1}\right\} =
...
\medskip
\noindent{\sc Proposition 2:}  A competitive equilibrium allocation
is a function of the realized aggregate endowment and does not depend
on time $t$ or the specific history $s^t$ or on
...
\subsection{Equilibrium price system}
  Suppose that ${\bf c}^i$, $i=1, \ldots, I$  is an equilibrium allocation.
Then    marginal condition \Ep{eq3b} or \Ep{eq3} restricts
the price system $q_t(s^t)$ as a function of the equilibrium allocation assigned
to any consumer $i$. But to exploit this fact in computation, we need  first to find a way  to compute
an equilibrium allocation before  computing prices.  As we shall see soon, solving the planning problem provides a convenient way
to do that.


  Because  units of a
price system are arbitrary, one price
can be  normalized
at any positive value.  We shall set $q_0(s_0)= 1$,
thereby casting a  price system in units of time $0$
  goods, which we denote as price system
$\{q^0_t(s^t)\}_{t\geq0,\,s^t}$.\NFootnote{Later in this 
chapter, after  the economy has reached time $\tau$
after history $s^\tau$, we will renormalize 
price system and  cast it in units of
time $\tau$, history $s^\tau$ goods, which
we will denote $\{q^\tau_t(s^t)\}_{t\geq \tau,\,s^t}$,
where all histories $s^t$ are to be understood to 
share the same initial sequence of events $s^\tau$.}
This choice implies that
 $\mu_i  =  u'_i(c_0^i(s_0)) $ by equation \Ep{eq3}.
An alternative  normalization would set  one of the
multipliers $\{\mu_i\}_i$, say $\mu_1=1$, as
we do 
in section \use{sec:Negishi}. Evidently, the price 
system and the multipliers are interrelated,
as  marginal condition \Ep{eq3b} 
or \Ep{eq3} shows. On the one hand, fixing 
units of the price system also determines  
absolute values of the multipliers, i.e., 
marginal utilities from relaxing the budget
constraints expressed in that particular price 
system. On the other hand, fixing 
absolute values of the multipliers would 
also determine  which units in which a  price system is 
expressed, as would be required for those multipliers to 
emerge as the marginal utilities from relaxing 
the budget constraints expressed in that
price system. 


The basic reason that we are free to choose a numeraire is that equilibrium quantities satisfy conditions that depend only  on {\it relative}
prices.  A related fact is that   only relative Pareto
weights matter for a Pareto problem.
...
\subsection{Optimality of equilibrium 
                          allocation}\label{sec:PO}%
  A competitive  equilibrium allocation   is a particular Pareto
optimal allocation, one that sets the Pareto weights $\lambda_i =
\mu_i^{-1}$.  These Pareto weights are unique up to multiplication by a positive scalar. Furthermore, at a
competitive equilibrium allocation,  {\it shadow prices\/}
$\theta_t(s^t)$ for the associated planning problem equal  competitive equilibrium
prices $q_t(s^t)$ for goods to be delivered at date $t$
at  history $s^t$. % associated with the Arrow-Debreu
 That  allocations for a planning
problem and a competitive equilibrium are identical reflects 
...
The second welfare theorem states that there exist a price system and  an initial distribution of wealth that can support an
  efficient allocation as a competitive equilibrium allocation.
 \auth{Mas-Colell, Andreu}
\auth{Whinston, Michael D.}\auth{Green, Jerry R.}

To illustrate the second welfare theorem in our economy, 
consider the following initial redistribution scheme.
At the beginning of the economy, each agent $i$
receives a lump-sum transfer $a_0^i(s_0)$ expressed in 
terms of the good at time 0 in the given initial state
$s_0$. The redistribution scheme satisfies
$\sum_i a_0^i(s_0) = 0$, and the budget constraint of 
agent $i$, using price system $q^0_t(s^t)$,
 becomes 
$$ \sum_{t=0}^\infty \sum_{s^t}  q_t^0(s^t)  c_t^i(s^t)   \leq  
a_0^i(s_0) \,+\,
   \sum_{t=0}^\infty \sum_{s^t}  q_t^0(s^t) y_t^i(s^t).   \EQN Redist_eq2 $$
For the redistribution scheme to be feasible,
negative transfers cannot be too large:
$$ - a_0^i(s_0) \leq 
 \sum_{t=0}^\infty \sum_{s^t}  q_t^0(s^t) y_t^i(s^t)
  \equiv A_0^i(s_0),                    \EQN Redist_A
$$   
that is, the absolute value of a negative transfer 
cannot be larger than the value of an agent's total
endowment. If $ - a_0^i(s_0) = A_0^i(s_0)$, 
agent $i$ has no lifetime wealth on the 
right side of \Ep{Redist_eq2} and therefore  cannot
purchase any consumption. This would correspond to a zero 
Pareto weight and hence, agent $i$ would not
enter objective function \Ep{planner1} of the 
Pareto problem. The efficient allocation then  has 
agent $i$ consuming nothing and allocates  his entire 
endowment  to other agents.


%Lars2023%
%\subsection{Interpretation of trading arrangement}
%  In the  competitive   equilibrium with Arrow-Debreu timing, 
%all trades occur at $t=0$.  Deliveries occur after $t=0$, but no 
%more trades.
%  A vast clearing or credit system operates at $t=0$.
%It ensures that condition \Ep{eq2} holds for each consumer
%$i$.   A symptom of the once-and-for-all and net-clearing 
%trading arrangement is
%that each consumer faces a single  budget constraint that 
%restricts  trades across all commodities indexed by  dates and 
%histories.
%
%  In section \use{sec:arrowsecurities}, we describe another 
%trading arrangement with
...
\subsection{Equilibrium computation}\label{sec:Negishi}%
...
Negishi algorithm\/} is one way to accomplish this.\NFootnote{See Negishi
(1960).}
\medskip
\noindent{\bf 1.}  Fix a positive value for one $\mu_i$, say $\mu_1=1$,
throughout the algorithm.  Guess  positive values for the
remaining  $\mu_i$'s. 
\medskip
\noindent{\bf 2.}  
Solve  equations \Ep{eq7} and \Ep{eq8} for
a candidate consumption allocation ${\bf c}^i, i=1, \ldots, I$.
\medskip
\noindent{\bf 3.}  Use \Ep{eq3} for any consumer $i$ to solve for the
price system $q_t(s^t)$.
\medskip
\noindent{\bf 4.}  For $i =2, \ldots, I$, check the budget constraint
...
\medskip \noindent{\bf 5.} Iterate to convergence on steps 2-4.
\medskip
\noindent Multiplying all of the $\mu_i$'s by a positive scalar
simply  changes the units of the price system. That is why we
are free to normalize as we have in step 1. After the
algorithm has converged to an equilibrium and given our
normalization $\mu_1=1$, we can simply multiply all
$\mu_i$'s by  $u'_1(c_0^1(s_0))$ to make the good in
period 0 to be the numeraire, $q^0_0(s_0)= 1$,
...
The preference specification in the following example enables us to avoid iterating on Pareto weights as we have to do with 
the Negishi algorithm.
\subsection{Example 1: risk sharing}\label{sec:ex1_risksharing}%
\noindent Suppose that the agents have identical preference orderings so that $u_i = u$ and that
$u$ is of the constant
...
\use{socialinsurance}, we study how some impediments to
trade might explain these rejections. } \auth{Cochrane, John
H.} \auth{Mace, Barbara} \auth{Townsend, Robert M.}

Fractions  of the aggregate endowment assigned to each
...
and (2) an identical  homothetic one-period utility function for all agents.
...
\Ep{eq3} imply that an equilibrium price system satisfies
$$ q_t(s^t) = \mu_i^{-1} \alpha_i^{-\gamma} \beta^t (Y_t(s^t))^{-\gamma} \pi_t(s^t) , \EQN price_precompute$$
%$$ q_t^0(s^t) = \mu_i^{-1} \beta^t \left(\alpha_i \overline y_t(s^t)\right)^{-\gamma} \pi_t(s^t)  $$
where $c_t^i(s_t) = \alpha_i Y_t(s^t)$, $Y_t(s^t) = \sum_i y_t^i(s^t)$, and $\alpha_i$ is consumer $i$'s
fixed consumption share of the aggregate endowment.
By picking the good in period $0$ to be the numeraire and 
using expression \Ep{price_precompute}, the price
system becomes
$$ q^0_t(s^t) = {\frac {q_t(s^t)} {q_0(s_0)} } 
= \beta^t {\frac {(Y_t(s^t))^{-\gamma}} {(Y_0(s_0))^{-\gamma}} }
 \pi_t(s^t) .   \EQN price_precompute_norm$$  
Alternatively, we could normalize the price system by setting 
$\mu_i$ for one consumer to an arbitrary positive number.
For example, by setting 
$\mu_i = \alpha_i^{-\gamma}$ in \Ep{price_precompute}
we obtain another normalized 
price system that is expressed solely in terms of
...
\medskip
\item{1.} Use \Ep{price_precompute} to compute an equilibrium price system that is expressed solely in terms of aggregate 
endowments, let say, price system \Ep{price_precompute_norm}.

\medskip
\item{2.} Use this price system and consumer $i$'s budget 
constraint to compute
$$ \alpha_i = {\frac {\sum_{t=0}^\infty \sum_{s^t} q^0_t(s^t) y_t^i(s^t) } {\sum_{t=0}^\infty \sum_{s^t} q^0_t(s^t)  Y_t(s^t)  } } .$$
This formula asserts that consumer $i$'s fixed consumption share $\alpha_i$ equals its share of aggregate wealth evaluated at the competitive  equilibrium price system.
...
$[0,1]$.  There are two consumers, with $ y_t^1(s^t) =s_t$ and
...
$$ q_t(s^t) = \beta^t \pi_t(s^t) {u'_i(\bar c^i) \over \mu_i}, \EQN ADprice $$
for all $t$ and $s^t$, for $i=1,2$.  Consumer $i$'s budget constraint
implies
...
$$ \bar c^i =\left({r \over 1+r}\right) E_0 \sum_{t=0}^\infty
     (1+r)^{-t}  y_t^i(s^t) .
$$%%      \EQN eq20new $$
Hence, equation \Ep{eq20} is a version of Milton Friedman's permanent income
model,  which asserts that a consumer with zero financial assets
consumes the annuity value of its human wealth defined as the
expected discounted value of its labor income (which here we take to be $ y^i_t(s^t)$).  In the present
example, the consumer completely  smooths its consumption across
time and
  histories,
something that the consumer in Friedman's model typically cannot
do. See chapters \use{timeseriesapp} and \use{selfinsure}.}


\subsection{Example 3: periodic endowment processes}
\label{sec:example3}%
 Consider the special case of the previous example in which
 $s_t$ is deterministic in the sense that it alternates between the values 1 and 0;
...
Let $\tilde s^t$ be the history  $(1, 0, 1, \ldots)$ up to
...
$$ q_t^0(s^t) =\cases{ \beta^t, & if $s^t = \tilde s^t$; \cr
                       0, & otherwise.\cr}  $$
Here we are using the time $0$ good as numeraire, $q^0_0(\tilde s_0)=1$.
...
\subsection{Example 4: only initial
                       uncertainty}\label{sec:example4}%
Consider another special case of example 2 from
subsection \use{sec:example2GG} in which  the event space 
is $S = \{0, .5, 1\}$ and  
uncertainty is temporary in the following sense.  
The event $s_t$ is nonstochastic with a value of $.5$ 
for $t=0,1$, whereas $s_2$ will take the value of 
either $0$ or $1$, each with probability $.5$. 
Conditional on the realization of 
$s_2$, all future events will be $s_t=s_2$ for $t>2$. 
So the unconditional probabilites of possible 
histories are $\pi_0(.5) = \pi_1(.5, .5) =1$,
$\pi_2(.5, .5, 0)=\pi_2(.5, .5, 1) =.5$,
$\pi_t(.5, .5, 0, \ldots,  0, 0)=
 \pi_t(.5, .5, 1, \ldots,  1, 1) = .5$ 
for all $t > 2$. Thus, for $t \geq 2 $, there are just two possible paths of $s^t$, each with probability
$.5$, namely, $s^t = [ .5, .5, 0, \ldots, 0, 0]$ and 
$s^t = [.5, .5, 1, \ldots, 1, 1]$.  Along the first 
path, the endowment of agent $1$ is 
$y_t^1 = .5 $ for $t=0, 1$ and $y_t^1 = 0$ for $t \geq 2$.
Along the second path, the endowment of agent $1$ is
$y_t^1 = .5 $ for $t=0, 1$ and $y_t^1 = 1$ for $t \geq 2$.


We can apply the method of subsection \use{sec:example2GG} to compute an equilibrium.  The aggregate endowment is $Y_t(s^t)=1$
for all $t$ and all $s^t$ that occur with positive probability.  Therefore, an equilibrium price system, expressed in terms of 
the $t=0$ good, is $q^0_1(.5, .5)= \beta$ and 
$q_t^0 (s^t) = .5 \beta^t$ for $t \geq 2$ and for both 
possible $s^t$ sequences listed above.
If we use these prices to compute the value of an agent's endowment, we obtain $.5 (1 + \beta) +.5 
\sum_{t=2}^\infty \beta^t = .5 \sum_{t=0}^\infty \beta^t 
 = {\frac{.5}{1-\beta}}$ for both agents $i=1,2$.
The budget constraint of either agent $i=1,2$ is 
satisfied when he consumes a constant
consumption of $.5$ each period at  each possible history: $c_t^i(s^t) = .5$ for all $t$ for all $s^t$.
...
a  new  candidate asset by breaking its payouts  into a sequence of history-contingent
claims, multiplying each  component of that sequence with the
relevant ``state price deflator'' $q_t^0(s^t)$, then adding up
those values. The  asset is  {\it redundant\/}, in the
sense that it constitutes  a bundle of history-contingent dated claims that are already available to agents: 
adding a redundant  asset to 
the economy does not change  agents'  consumption sets.  Removing a redundant
asset from an  economy would  not change a competitive equilibrium allocation. 
%each component of which has a competitive equilibrium price.  
...
\subsection{Pricing redundant assets}
  Let $\{d_t(s^t)\}_{t\geq 0,\,s^t}$ be a stream of claims
...
$$
If this equation did not hold, anyone could make unbounded profits
...
payouts on the riskless consol.  The time $t$ strip is just the payout
...
Return to the stream of dividends $\{d_t(s^t)\}_{t\geq 0,\,s^t}$ 
generated by the asset   priced in equation \Ep{asset1}. For $\tau
\geq 1$, suppose that we strip off the first $\tau-1$ periods of
the dividend and want  the time $0$ value of the remaining 
dividend stream.
%Lars2023% $\{d_t(s^t)\}_{t \geq   \tau}$.   
...
the owner to the remaining dividend stream
%Lars2023%  $\{d_t(s^t)\}_{t \geq   \tau}$ 
at time $\tau$ if history $s^\tau$ is realized,
...
$\tilde s^\tau=s^\tau$.
Under the price system $q^0_t(s^t)$ the
units of the price $p^0_\tau(s^\tau)$ are time $0$, 
state $s_0$ goods.
...
$$
where\NFootnote{Because the marginal conditions hold for all
consumers, this condition holds for all $i$.}
$$ \eqalign{  
{q_t^0(s^t) \over q_\tau^0(s^\tau)} 
      & = {\beta^t u'_i\left(c_t^i(s^t)\right) \pi_t(s^t)
               \over \beta^\tau u'_i\left(c_\tau^i(s^\tau)\right)
                                       \pi_\tau(s^\tau) } \cr
        & = \beta^{t - \tau} {u'_i\left(c_t^i(s^t)\right) 
                 \over u'_i\left(c_\tau^i(s^\tau)\right)}
          \pi_{t,\tau}(s^t|s^\tau) \equiv q^\tau_t(s^t). \cr}
    \EQN qdef $$
Here
$q^\tau_t(s^t)$ is the price of one unit of consumption delivered
at time $t$, history $s^t$ in terms of the time $\tau \leq t$,
history $s^\tau$ consumption good, for any history $s^t$ that 
starts with the initial sequence $s^\tau$; 
$\pi_{t,\tau}(s^t|s^\tau)$ is the
probability of history $s^t$ conditional on history
$s^\tau$ at time $\tau$.  %%%, as given by \Ep{cecm_pi}.
...
at time $\tau$ entitles the owner to  dividends from time
...
$$ 1 = E \left[\beta{u'(c_{\tau+1}) \over u'(c_\tau)} R_{\tau+1} \right]
    ...
    \EQN oneperiodr2 $$
In chapters \use{assetpricing1} and \use{assetpricing2}, we shall see many more instances of this
equation.
...
trading of which  substitutes perfectly for  comprehensive trading
...
We build on an insight of Arrow (1964).
% that one-period securities
...
At
each date  $t \geq 0$,  only at the history $s^t$ actually realized,  trades occur in a complete set of claims   to
one-period-ahead state-contingent consumption. A
competitive equilibrium of this sequential-trading economy
  attains the same allocation
as the competitive  equilibrium with all trades at time $0$ that we described earlier.
 \index{Arrow securities}

\auth{Arrow, Kenneth J.}


\subsection{Financial wealth as  endogenous state variable}
...
a consumer at date $t$ and history $s^t$. We can discover this state by taking an
...
We begin by asking the following key question. In the competitive
equilibrium where all trading takes place at time $0$, what is the implied continuation financial wealth of consumer $i$ at time $t$ after history $s^t$?
The answer is obtained by summing up the value of the consumer's
holdings of claims to current and all future consumptions at  time $t$ and
history $s^t$. Since history $s^t$ has been realized, we discard all
claims contingent on time $t$ histories $\tilde s^t \neq s^t$ that
were not realized. Hence, the implied financial wealth is  determined simply
by the trades that were undertaken by consumer $i$ at the outset of a
time $0$ trading equilibrium, when the consumer  sold  the entire endowment stream on the right side of budget
constraint \Ep{eq2} in order to acquire the contingent consumption claims
on the left side.

With sequential trading,   
one-period Arrow securities are traded period by period and  consumers
retain the ownership of their endowment processes throughout time.
In this setting, the total
wealth of consumer $i$ at a point in time can be decomposed into  {\it financial}  and
 {\it non-financial} wealth.\NFootnote{In some applications,  financial wealth
is also called `non-human wealth' and non-financial wealth is called
`human wealth'.}
Financial wealth at time $t$ after history $s^t$ consists of the consumer's
beginning-of-period holdings of Arrow securities that are contingent on
the current state $s_t$ being realized,
while the present value of the consumer's current and future endowment constitutes non-financial
wealth. From Arrow's (1964) insight that the two trading arrangements
yield  identical equilibrium allocations, a consumer's financial wealth
in a sequential trading equilibrium should  equal  its
continuation wealth in a time $0$ trading equilibrium minus the continuation value of
its current and future endowment
(i.e., its non-financial wealth), also evaluated at the equilibrium prices for a time-$0$-trading competitive equilibrium. Thus, the
financial wealth of consumer $i$ at time $t$ after history $s^t$,
...
each consumer starts  with zero financial wealth at time $0$, i.e., 
$\Upsilon^i_0(s_0)=0$ for all $i$. At $t >0$, financial wealth
$\Upsilon^i_t(s^t)$ typically differs from zero for consumer $i$,
...
That is, the  Arrow securities that make up financial
wealth are in zero net supply -- positive holdings of some consumers
constitute indebtedness of the other consumers who have issued those
securities.
...
%A key step in constructing a sequential-trading arrangement is to
%identify a variable to serve as the state in a value function for
%the consumer at date $t$. We find this state by taking an
%equilibrium allocation and price system for the (Arrow-Debreu)
%time $0$ trading structure and applying a guess-and-verify method.
...
%continuation wealth of consumer $i$ at time
...
%$s^t$, we sum up the value of the consumer's purchased claims to
...
%Consumer $i$'s net claim to delivery of goods in a future period $\tau\geqt$
%contingent on history $\tilde s^\tau$ whose time $t$ partial history
%$\tilde s^t=s^t$ is  $[c^i_\tau(\tilde s^\tau)-y_\tau^i(\tilde s^\tau)]$.
%Thus, the consumer's financial wealth, or the value of all its
...
commodities whose markets can be regarded as having all been reopened at date $\tau$, history
$s^\tau$, and being restarted from a distribution of  financial wealth  implied by  tails of each consumer's
%Lars2023%endowment and 
consumption streams for a complete markets economy that originally had convened at $t=0$.
...
$\tau$, history $s^\tau$ financial wealth that is implicit in a time $0$ Arrow-Debreu
equilibrium. If markets are reopened at date $\tau$ after history
$s^\tau$, no trades  occur: facing   price system
\Ep{qdef}, all consumers choose to continue the  tails of their
...
trading, we propose to  match  the time $t$, history $s^{t}$ 
financial wealth of
the consumer  in the sequential economy with  
%Lars2023%equilibrium  tail wealth
...
limits, objects that were  absent from  the time $0$ budget constraint \Ep{eq2} in the Arrow-Debreu economy.
...
synthesize restrictions that work by starting from an equilibrium
allocation of the Arrow-Debreu economy (with time $0$ markets) and then construct  state-by-state debt limits that support
the equilibrium allocation that emerged from the Arrow-Debreu economy under a sequential trading arrangement.
 We'll refer to these weakest possible
debt limits as  ``\idx{natural debt limit}s.'' These limits come
from  requiring that it has to be {\it
feasible\/} for a consumer to repay his state contingent debt in
...
%Lars2023% Let $q_\tau^t(s^\tau)$ be an Arrow-Debreu price, 
%denominated  in units of the date $t$, history
%$s^t$  consumption good.
Consider the value of the tail of  agent $i$'s endowment sequence
at time $t$ at history $s^t$:
$$ A^i_t(s^t) =
   \sum_{\tau=t}^\infty \; \sum_{s^\tau \vert s^t}
...
repay starting from  period $t$, assuming that his consumption is
zero forever after.
...
With sequential trading, we shall require that consumer $i$ at time $t-1$ and
history $s^{t-1}$ cannot promise to pay more than $A^i_t(s^t)$
conditional on the realization of $s_t$ tomorrow, because it will
not be feasible to repay more. Consumer $i$ at
time $t-1$ and history $s^{t-1}$ faces  such  a borrowing constraint for each possible
realization of $s_t$.
...
consumers trade claims to date $t+1$ consumption, whose payment
is  contingent
on the realization of $s_{t+1}$.   Let $a_t^i(s^t)$ denote the
claims to time $t$ consumption, other than its time $t$  endowment $y_t^i(s^t)$,
that consumer $i$ brings into time $t$ in history $s^t$. Let
$ Q_t(s_{t+1} | s^t)$
be a {\it pricing kernel},  to be interpreted as follows:
$Q_t(s_{t+1} | s^t)$ is the price of one unit of time $t+1$ consumption,
contingent on realization $s_{t+1}$ at $t+1$, when the
history at $t$ is $s^t$.
 %We are guessing
...
$$ \tilde c_t^i(s^t) + \sum_{s_{t+1}} a^i_{t+1}(s^t, s_{t+1}) Q_t(s_{t+1} | s^t)
     \leq  y_t^i(s^t) + a_t^i(s^t) . \EQN eq10   $$
...
At time $t$ after history $s^t$, a consumer chooses 
$\tilde c^i_t(s^t)$
and $\{a^i_{t+1}(s^t, s_{t+1})\}_{s_{t+1}}$, where
$\{a^i_{t+1}(s^t, s_{t+1})\}_{s_{t+1}}$,   is a vector of claims on time $t+1$ consumption,
there being one component of the vector for each  value of the time $t+1$ realization of
$s_{t+1}$.    To rule out Ponzi schemes,
we impose the state-by-state borrowing
constraints
$$ - a^i_{t+1}(s^{t+1}) \leq  A^i_{t+1}(s^{t+1}),        \EQN cecm_debt2$$
where $A^i_{t+1}(s^{t+1})$ is computed in equation \Ep{cecm_debt1}.


Let $\eta^i_t(s^t)$ and $\nu^i_t(s^t,s_{t+1})$ be  nonnegative
Lagrange multipliers on  budget constraint \Ep{eq10} and 
borrowing constraint \Ep{cecm_debt2}, respectively, at time $t$
and history $s^t$ for all $s_{t+1}$. Form the Lagrangian  \offparens
$$ \eqalign{ L^i =
 &\sum_{t=0}^\infty \sum_{s^t}
   \Bigr\{  \beta^t u_i(\tilde c_t^i(s^t)) \pi_t(s^t)  \cr
 & +\eta^i_t(s^t) \Bigl [y_t^i(s^t) + a_t^i(s^t) - \tilde c_t^i(s^t)
       - \sum_{s_{t+1}} a^i_{t+1}(s^t,s_{t+1}) Q_t(s_{t+1} | s^t) \Bigr] \cr
 & +\sum_{s_{t+1}} \nu^i_t(s^t,s_{t+1}) \left[ A^i_{t+1}(s^{t+1}) + a^i_{t+1}(s^{t+1}) \right]
                                                             \Bigl\}, \cr}        \EQN seqLagrangean $$
for a given initial wealth $a^i_0(s_0)$.
First-order conditions for maximizing $L^i$ with respect to
$\tilde c^i_t(s^t)$ and $\{a^i_{t+1}(s^t,s_{t+1})\}_{s_{t+1}}$ are
$$\EQNalign{
\beta^t u'_i(\tilde c_t^i(s^t)) \pi_t(s^t) -\eta^i_t(s^t) &=0\,,\EQN seqFOC;a \cr
\noalign{\vskip.2cm}
-\eta^i_t(s^t) Q_t(s_{t+1} | s^t) + \nu^i_t(s^t,s_{t+1})
+ \eta^i_{t+1}(s^t,s_{t+1}) & =0\,, \hskip1cm \EQN seqFOC;b \cr}
$$
for all $s_{t+1}$, $t$, $s^t$. At an optimal plan,  natural debt limits \Ep{cecm_debt2} do  not
bind and the Lagrange multipliers $\nu^i_t(s^t,s_{t+1})$
 all equal  zero for the following reason: if there were any
history $s^{t+1}$ leading to a binding natural debt limit, the
consumer would from then on have to set consumption equal to zero
in order to honor its debt. Because the consumer's utility
function satisfies the Inada condition
$\lim_{c \downarrow 0} u'_i(c) = +\infty$, that would mean that all
future marginal utilities would be infinite. By postponing earlier consumption to periods
after such a binding constraint,  it is easy
to find alternative affordable allocations that yield higher
expected utility. %, i.e., alternative preferable
...
\pi_{t+1,t}(s^{t+1} | s^t),                       \EQN seqFOCc
$$
for all $s_{t+1}$, $t$, $s^t$.


As evident from Lagrangean \Ep{seqLagrangean}, a
consumer's optimization problem under sequential trading
with Arrow securities is as challenging as it is with time 0 trading with Arrow-Debreu
securities. While there are fewer securities traded at 
each date under sequential trading, choosing   
one-period claims today requires 
simultaneously choosing all future portfolios of
Arrow securities. At time 0, let consumer $i$'s 
choice of all asset portfolios be denoted 
${\bf a}^i =
\{\{a^i_{t+1}(s^t,s_{t+1})\}_{s_{t+1}}
                            \}_{t\geq0,\,s^t}$
and the associated borrowing limits are
${\bf A}^i =\{A^i_{t+1}(s^{t+1})\}_{t\geq0,\,s^{t+1}}$.

 \medskip
\noindent{\sc Definition:}  A {\it distribution of wealth} is
a vector $\vec {a}_t(s^t) = \{a_t^i(s^t)\}_i$ satisfying
$\sum_i a_t^i(s^t) = 0$.
\medskip


\noindent{\sc Definition 2:} A {\it competitive equilibrium with sequential trading of one-period Arrow securities} is
an initial distribution of wealth $\vec { a}_0(s_0)$, 
borrowing limits $\{{\bf A}^i\}_i$ satisfying 
\Ep{cecm_debt1}, 
%%for all $i$, for all $t$, and for all $s^t$,
an allocation $\{{\bf c}^i\}_i$, asset portfolios 
$\{{\bf a}^i\}_i$, and pricing
kernels $Q_t(s_{t+1} | s^t)$ such that

\medskip
\noindent
{\bf (i)} For all $i$,
given $ a^i_0(s_0)$, the borrowing 
limits ${\bf A}^i$ and the pricing kernels, 
the allocation ${\bf c}^i$ and asset portfolios
${\bf a}^i$ solve the consumer's problem.

\medskip
\noindent
{\bf (ii)} For all realizations of $\{s^t\}_{t\geq0}$,
allocations and   portfolios
$\{c^i_t(s^t), $

\noindent
$\{a^i_{t+1}(s^t, s_{t+1})\}_{s_{t+1}}\}_i$
satisfy
$\sum_i c^i_t(s^t) = \sum_i y_t^i(s^t)$ and
$\sum_i a_{t+1}^i(s^t, s_{t+1}) = 0$ 
for all $s_{t+1}$.
...
distribution of wealth. We'll say more about it  soon.  %The Arrow-Debreu equilibrium with
%complete markets at time $0$ in effect pinned down a {\it
%particular\/} distribution of wealth.
\index{competitive
equilibrium!sequential trading}

Recall that  the  equilibrium of a complete markets economy with time $0$ trading described in Definition 1 consists of just two
objects, namely, a price system and an allocation.  Evidently,  the  equilibrium of a sequential-trading economy with one-period Arrow securities
described in Definition 2 consists of more objects: an initial distribution of wealth, borrowing limits, a collection of one-period pricing kernels, and a collection of portfolio and consumption choices. In thinking about  relationships between  the two equilibrium concepts, 
it will be useful to keep in mind both their different timing protocols and the dimensions of the objects that comprise them.



\subsection{Equivalence of allocations}
By making an appropriate guess about the form of  pricing
kernels, we can show that a competitive equilibrium
...
suppose that the pricing kernel $Q_t(s_{t+1} | s^t)$ makes
the following recursion true:
$$\EQNalign{
 q^0_{t+1}(s^{t+1}) &= Q_t(s_{t+1} | s^t) q_t^0(s^t), \cr
\noalign{\hbox{\rm or}}
 Q_t(s_{t+1} | s^t) &= q^t_{t+1}(s^{t+1}).       \EQN eq16 \cr}
$$
%where recall that $q_{t+1}^t(s^{t+1}) 
%  ={\frac{q_{t+1}^0(s^{t+1})}{q_t^0 (s^t)}}$.
Hence, our guess is 
that $Q_t(s_{t+1} | s^t)$ equals
the time-0 trading price $q^0_{t+1}(s^{t+1})$ after
a change in numeraire for the latter; in particular, 
since the price $Q_t(s_{t+1} | s^t)$ 
is expressed in terms of time $t$ history $s^t$ goods,
we divide $q^0_{t+1}(s^{t+1})$ by $q_t^0(s^t)$ to
adopt that  numeraire. 

Let
$\{{\bf c}^i \}_i$ be a competitive equilibrium allocation
in the Arrow-Debreu economy.
If equation \Ep{eq16} is satisfied, that allocation is also a
sequential-trading competitive equilibrium allocation.  To verify this claim,
take the consumer's first-order conditions \Ep{eq3} in the Arrow-Debreu
...
We conjecture that the initial wealth distribution
$\vec { a}_0(s_0)$ of the sequential-trading economy should be a zero vector, a natural guess because
it means that each consumer must rely on its own
endowment stream to finance consumption, just as consumers are
constrained at time $0$ to finance their history-contingent purchases
for the infinite future  in the Arrow-Debreu economy.
To  verify  the conjecture, we must show that the zero
initial wealth vector enables consumer $i$ to finance $\{c^i_t(s^t)\}_{t \geq 0, s^t}$
and leaves no room to increase consumption in any period
 after any history.


The proof proceeds by guessing that, at time $t\geq 0$ and history
$s^t$, consumer $i$ chooses a portfolio given
by $a^i_{t+1}(s^t,s_{t+1})=\Upsilon^i_{t+1}(s^{t+1})$ for all
$s_{t+1}$.  The value of this  portfolio expressed
in terms of the date $t$, history $s^t$ consumption good is
$$\EQNalign{
  \sum_{s_{t+1}} a^i_{t+1}(s^t,s_{t+1})  Q_t(s_{t+1} | s^t)
...
$$}
To demonstrate that consumer $i$ can afford this portfolio strategy, we now
...
$a^i_0(s_0)=0$, substitution of equation \Ep{cecm_Wcost} into
budget constraint \Ep{eq10}
at equality yields
$$ \tilde c_0^i(s_0) + \sum_{t=1}^\infty \; \sum_{s^t}
    q_t^0(s^t) \left[ c^i_t(s^t) - y^i_t(s^t) \right]
   =  y^i_0(s_0) + 0\,.  $$
This expression and budget constraint \Ep{eq2} at
equality imply $\tilde c_0^i(s_0) = c_0^i(s_0)$. Thus,
...
periods $t > 0$ and histories $s^t$, we replace $a^i_t(s^t)$ 
in constraint \Ep{eq10} by $\Upsilon^i_t(s^t)$, and
after noticing that the value of the asset portfolio in
\Ep{cecm_Wcost} can be written as
$$
\sum_{s_{t+1}} a^i_{t+1}(s_{t+1},s^t) Q_t(s_{t+1} | s^t)
...
We have shown that the proposed portfolio strategy attains the equilibrium
consumption plan
 of the Arrow-Debreu economy, but what
precludes consumer $i$ from further increasing current consumption by
reducing some component of the asset portfolio? Debt limits prevent it.  
 To ensure that consumption plan
$\{c^i_{t+j}(s^{t+j})\}_{j\geq 1,s^{t+j}}$
can be attained starting next period in all possible future states,
we must subtract the value of commitments to future
consumption
from the natural debt limit in \Ep{cecm_debt1}.
 Thus, the consumer faces a
state-by-state borrowing constraint that is more
 restrictive than restriction \Ep{cecm_debt2}:
for any $s^{t+1}$
$$  - a^i_{t+1}(s^{t+1}) \leq  A^i_{t+1}(s^{t+1} )
- \sum_{\tau=t+1}^\infty \; \sum_{s^\tau \vert s^{t+1}}
  q_\tau^{t+1}(s^\tau) c^i_\tau(s^\tau)
  = - \Upsilon^i_{t+1}(s^{t+1})$$
or
$$   a^i_{t+1}(s^{t+1})  \geq  \Upsilon^i_{t+1}(s^{t+1}). $$
Hence, consumer $i$ does not want to increase consumption 
$c^i_t(s^t)$ at time $t$ by
reducing next period's wealth below $\Upsilon^i_{t+1}(s^{t+1})$
for some realization of $s_{t+1}$ because
...
\subsection{Computing an equilibrium with sequential
            trading}\label{sec:example4cont}%
We  recommend using the following indirect method to compute 
an equilibrium  with sequential trading of a complete set of Arrow securities:

\medskip
\item {1.}  Compute an equilibrium of the corresponding  Arrow-Debreu economy with time $0$ trading.
\medskip
...
\medskip
\item {3.} Compute equilibrium prices from formula \Ep{eq17}.
\medskip
\item{4.} Compute debt limits from \Ep{cecm_debt1}.
\medskip
\item {5.}  Compute portfolios of one-period Arrow securities  by first computing implied time $t+1$, history $s^{t+1}$
wealth $\Upsilon^i_{t+1}(s^{t+1}) $ from \Ep{cecm_wealth} evaluated at the Arrow-Debreu equilibrium prices,
then set $a^i_{t+1}(s^t,s_{t+1}) 
          = \Upsilon^i_{t+1}(s^{t+1})$.

\medskip


We can apply the algorithm to compute an equilibrium of the economy described in  example 4 from subsection \use{sec:example4}. 
The price system is $Q_0(.5\big|.5) =\beta$, 
%$Q_0(0\big|1) =0$, 
$Q_1(1\big|.5,.5) = Q_1(0\big|.5,.5) = .5 \beta$  and
$Q_t(1\big|.5,.5,1,\ldots,1) = Q_t(0\big|.5,.5,0,\ldots,0)$ 
$= \beta $ for $t \geq 2$.
Also, $\Upsilon^i_1(.5,.5)= 0$ for $i=1,2$.  For $t \geq 2$,
$\Upsilon_t^1 (.5,.5,1, \ldots,1)$$= \sum_{\tau \geq t} \beta^{\tau - t}(.5 -1) = {-.5 \over 1-\beta}$ and
$\Upsilon_t^2(.5,.5,1, \ldots,1) = \sum_{\tau \geq t} \beta^{\tau - t}(.5 -0) = {.5 \over 1-\beta}$; and
the reverse outcomes  
$\Upsilon_t^1 (.5,.5,0, \ldots,0)= {.5 \over 1-\beta}$ and
$\Upsilon_t^2 (.5,.5,0, \ldots,0)= {-.5 \over 1-\beta}$.
Therefore, in period $1$, the first  consumer trades Arrow securities in amounts $a_2^1(.5,.5,1) = {-.5 \over 1-\beta}$,
$a_2^1(.5,.5,0) = {.5 \over 1-\beta}$, while the second consumer trades  Arrow securities in amounts 
$a_2^2(.5,.5,1) = {.5 \over 1-\beta}$,
$a_2^2(.5,.5,0) = {-.5 \over 1-\beta}$   After period $2$, the consumers perpetually roll over their debts or assets of either
...
in an Arrow-Debreu economy with  complete markets in dated
...
events $s^t$, which in turn are governed by joint
probability measure $\pi_t(s^t)$. At this level of generality, both 
 the pricing kernels $Q_t(s_{t+1} | s^t)$ and the
wealth distributions $\vec {a}_t(s^t)$ in the
sequential-trading economy  depend on the history $s^t$, so both
 are time-dependent functions of all past events
$\{s_\tau\}_{\tau=0}^t$. This level of generality  makes it  difficult to
formulate an economic model that can be used to confront
observations. For empirical work, we want  a framework in which economic outcomes
...
%       \pi(s_1 | s_0)   . \EQN eq1a  $$
The Markov property implies that the conditional probability
$\pi_{t,\tau}(s^t\vert s^\tau)$ for $t>\tau$ depends only on the state
$s_\tau$ at time $\tau$ and does not depend on the history
before $\tau$:
$$\pi_{t,\tau}(s^t|s^\tau) = \pi(s_t\vert s_{t-1}) \pi(s_{t-1} \vert s_{t-2})
          \ldots\pi(s_{\tau+1} \vert s_\tau).\EQN cecm_pi$$


Next, we assume that consumers' endowments in period $t$ are
time invariant measurable functions of $s_t$,
$y^i_t(s^t)=y^i(s_t)$ for each $i$. %This assumption means that
%each consumer's endowment follows a Markov process, since $s_t$
%itself is governed by a Markov process.
This assumption and  the Markov assumption about  $s_t$ put
 further structure on  equilibrium prices and quantities.
...
individual's consumption is  a function  of only the current
realization of the aggregate endowment and does not depend on the
specific history preceding it.\NFootnote{Of course, the equilibrium allocation also depends on the distribution
...
\Ep{seqFOCc} shows that the one-step-ahead pricing kernel in the sequential-trading
equilibrium is   a function  of only the current state:
$$
Q_t(s_{t+1} | s^t) = \beta
...
we can also
establish history independence of  relative prices in the
...
\noindent
Using this proposition, we can verify that consumers' 
continuation wealth levels \Ep{cecm_wealth} and
the natural debt limits \Ep{cecm_debt1} both
exhibit history independence:
$$\EQNalign{
 \Upsilon^i_t(s^t) &= \bar \Upsilon^i(s_t)\,, \EQN MarkovU 
\cr
 A^i_t(s^t) &= \bar A^i(s_t) \,,              \EQN MarkovA 
\cr}
$$
where the latter satisfy the recursion
$$ \bar A^i(s) = y^i(s) + \sum_{s'} Q(s'|s) \bar A^i(s'),  \EQN borrowingconA $$
with the additional notation 
$\bar A^i = \{\bar A^i(s)\}_s$.


These results hold stark implications for asset holdings
and the evolution of financial wealth in a 
sequential-trading competitive equilibrium:

\medskip
\noindent{\sc Proposition 5:} If time $t$ endowments are 
a function of a Markov state $s_t$, 
the beginning-of-period wealth distribution in a
sequential-trading competitive equilibrium is
a function only of the realized state $s_t=s$ and
$\vec {\bar a}(s)$ satisfies $\sum_i \bar a^i(s)=0$.
Thus, for each state $s'$ that can be reached next 
period, i.e., $\pi(s'|s)>0$, $\{\bar a^i(s')\}_i$ is the 
end-of-period asset holdings of the Arrow security that 
pays off in period $t+1$ contingent on state $s_{t+1}=s'$
being realized. Furthermore, the initial distribution of 
wealth $\vec {a}_0(s_0)$ is revisited whenever 
the Markov state $s_t$ returns to $s_0$, so that 
$\vec {\bar a}(s_0) = \vec { a}_0(s_0)$.
\medskip


The evolution of the wealth distribution conveys an
insight about how a sequential-trading competitive equilibrium
attains a first-best outcome in which no idiosyncratic risk is
borne by individual consumers. Thus, each consumer
enters every period with a wealth level that is independent of
past realizations of his endowment: past trades have
fully insured him against  idiosyncratic realizations of his
endowment process. And from the way he manages  that very same insurance motive, the consumer
now enters the present period with a wealth level that is a
function of the current state $s_t$. It is a state-contingent
wealth level that  the consumer chose at $t-1$, a level  just sufficient to
continue  a trading strategy that completely  insures against future
idiosyncratic risks. The optimal holding of wealth is  a function
 of $s_t$ alone because the current state $s_t$ determines both the
current endowment and the current pricing kernel  and so contains all information
relevant for predicting
future realizations of the consumer's endowment process as well as future prices.

%But aggregate shocks cannot be diversified away. An 
%``invisible hand''
%uses the pricing kernel
%$Q(s_t\vert s_{t-1})$ and  cleared  markets
%to  arrange consumers'
%transactions at time $t-1$ so   that they end up  bearing  
%only aggregate risk.

Thus, in a sequential-trading competitive 
equilibrium when endowments are governed by a 
Markov process, the beginning-of-period wealth
distribution at time $t$ in state $s_t=s$
is $\vec {\bar a}(s)$ and consumers receive current
endowments $\{y^i(s)\}_i$. Given Arrow security
prices $\{Q(s'|s)\}_{s'}$, consumers choose
consumption $\{\bar c^i(s)\}_i$ and portfolios
of Arrow securities $\{\{\bar a^i(s')\}_{s'}\}_i$ 
that satisfy borrowing constraints 
$\{\{\bar A^i(s')\}_{s'}\}_i$.
Here it is to be understood that for any state $s'$
that cannot be realized next period, i.e.,
$\pi(s'|s)=0$, there is no trade in an Arrow
security that pays off next period $t+1$ 
contingent on the impossible state realization
$s_{t+1}=s'$. 
Hence, end-of-period asset portfolios will differ 
across two time periods $t$ and $t+k$ in states 
$s_t$ and $s_{t+k}$, respectively, only with
respect to Arrow securities that pay off in
some state $s'$ that can be realized
after one of the two states $s_t$ and
$s_{t+k}$ but not after the other state, i.e.,
if there is any state $s'$ such that
either $\pi(s'|s_t)=0$ and $\pi(s'|s_{t+k})>0$, 
or $\pi(s'|s_t)>0$ and 
$\pi(s'|s_{t+k})=0$.

 
We conclude that equilibrium outcomes 
inherit the Markov property since the 
quantities and prices are functions only of 
the realized state $s$, or in the case of 
end-of-period asset portfolios, are even 
time invariant (but for the above caveat with 
respect to any next period state $s'$ that 
cannot be realized after some current 
state $s$).\NFootnote{When we compute equilibrium
beginning-of-period wealth distributions
$\{ \vec {\bar a}(s) \}_s$,
we can proceed as if the end-of-period 
asset portfolios are identical across
all current Markov states $s$. The justification for proceeding in this way is that
if there is any state $s'$ that
cannot be realized after a current
state $s$, i.e., $\pi(s'|s)=0$, the
equilibrium price for the associated Arrow
security is zero, $Q(s'|s)=0$. Consequently,
it would have no effect on an equilibrium  allocation if we 
were to impose that even in such a current state $s$ consumers
have end-of-period holdings $\{\bar a^i(s')\}_i$ 
of that particular Arrow security: 
at a zero price, there would be neither a
cost to the purchasers nor a payment
to an issuer of that security.}
What we have suppressed in these functions is 
that the outcomes are of course also functions 
of the implicit Pareto weights associated
with this efficient competitive equilibrium 
allocation, which in turn are determined by 
individual consumers' endowment processes, 
$\{\{y^i(s)\}_s\}_i$, the initial Markov 
state $s_0$, and the initial wealth distribution
$\vec { a}_0(s_0)$. Given those primitives 
and circumstances at time 0, 
consumers ``play the hands that they are dealt'' 
by trading sequentially in complete markets for 
Arrow securities. An ``invisible hand''
uses the pricing kernel $Q(s'|s)$ and cleared 
markets to arrange consumers' transactions 
in any state $s$ so that they end up bearing   only aggregate risk
and 
no idiosyncratic risk.
An implication of such an efficient risk sharing
arrangement is that whenever the Markov state $s_t$ 
returns to the initial state $s_0$, each consumer 
returns  to exactly same situation that 
prevailed in the first period, including a
beginning-of-period wealth distribution that  equals 
the initial wealth distribution
$\vec {a}_0(s_0)$. The following example
illustrates the recursive behavior of the wealth
distribution.


%The opposite 
%supposition of an individual reaching a particular
%state with different wealth at differen times would
%immediately lead to a contradiction. Because any such
%two instances of an individual reaching the same state
%but with different assets would contradict that this
%individual is implementing the same consumption plan 
%for the future. 


\subsection{Example 5: Markov version of 
                           example 2}\label{sec:example5}%
Consider a two-state Markov version of example 2 in 
section \use{sec:example2GG}.
Let the stochastic event $s_t$ take values 
$s\in\{0,1\}$, 
with initial state $s_0=1$ and transition 
probabilities $\pi(s'\vert s)\in [0,1]$
satisfying $\pi(s|s)<1$ and $\sum_{s'} \pi(s'\vert s)=1$,
for all $s$ and $s'$.
There are two consumers with endowments $y^1(s)=s$ and 
$y^2(s)=1-s$. Since the aggregate endowment is constant,
$\sum_i y^i(s)=1$, equilibrium consumption of each
consumer stays constant over time, 
$\bar c^1$ and $\bar c^2$, as in example 2. Hence, 
the pricing kernel in formula \Ep{MarkovQ} becomes
$Q(s'|s) = \beta \,\pi(s'|s)$.

In period 0 when the initial state is $s_0=1$,  
consumers start with no beginning-of-period wealth, so 
$[\bar a^1(1),\, \bar a^2(1)] = [0,\, 0]$.
According to proposition 5, this initial wealth
distribution will recur  at the beginning of
a period whenever a realized state is $s=1$;  furthermore, 
there is another beginning-of-period wealth
distribution when the state is $s=0$:
$[\bar a^1(0),\, \bar a^2(0)] = [a,\, -\!a]$ 
for some value of $a$ and hence,
$\sum_{i} \bar a^i(0) =0$.

These beginning-of-period wealth distributions in
turn imply associated time-invariant end-of-period 
asset portfolios for  the two consumers. 
Specifically,  Arrow
securities that pay off next period contingent on 
the realized state being $s'=1$ are not  issued, since the 
beginning-of-period wealth distribution is the 
zero vector when the realized state is $s'=1$.
Thus, the only possible equilibirum issues of Arrow
securities would be ones that pay off next period
contingent on the realized state being $s'=0$. Thus, there can be non zero
securities $\{\bar a^i(0)\}_i$ traded  at a price 
$Q(0 | s)$ that depends on the 
state $s$ when the security is issued.

Starting in period 0, it is evidently consumer 2
who would like to borrow, i.e.,  $\bar a^2(0)<0$, since he 
has no endowment in period 0, $y^2(s_0)=y^2(1)=0$.
A loan allows consumer 2 to consume
$-Q(0 | 1)\, \bar a^2(0)$ in period 0. It is reasonable 
that  repayment of the one-period loan is
contingent on next period's state being
$s'=0$, i.e., the state in which consumer 2 
receives an endowment $y^2(0)=1$, while the
loan is forgiven if next period the  state remains  $s'=1$. This equilibrium outcome is 
essential for  efficient risk sharing since
otherwise consumer 2 would become ever more
indebted so long as state $s'=1$ persists.
Furthermore, according to proposition 5, 
when state $s'=0$ is eventually realized, 
consumer 2 will keep the same negative 
holding of Arrow security $\bar a^2(0)<0$.
Consumer 2  ``rolls over''
his indebtedness with future payments
contingent on remaining in state $s'=0$, the state
in which consumer 2 receives an endowment. By 
so doing, consumer 2 avoids having to reduce
consumption as he would  if he were  to ``pay off'' instead of  ``roll over'' 
his indebtedness. Eventually, when state $s'=1$ 
is realized, the debt is forgiven and the 
situation returns  to the zero vector beginning-of-period 
wealth distribution. Remarkably, such 
transactions in Arrow securities after period 0 
allow consumer 2 to sustain a constant 
consumption level, $\bar c^2$.

Symmetrically,  consumer 1, who is the counterparty to these past Arrow
security transactions,  sustains  a constant consumption level,
$\bar c^1$. That starts in period 0 with
consumer 1 extending the above state-contingent 
loans to consumer 2, $\bar a^1(0)=- \bar a^2(0)=a >0$.
Any repayments of  that sequence of 
one-period loans are contingent on the
realized state in next period being $s'=0$, the
state in which consumer 1 has no endowment,
$y^1(0)=0$. When $s' = 0$ eventually, 
a loan repayment of $a$ enables consumer 1 
to continue to finance consumption 
$\bar c^1$ so long as state $s'=0$ persists. 
Without having to borrow, consumer 1 does this 
by maintaining an unchanged positive holding of 
Arrow securities, $\bar a^1(0)=a>0$. 

After seeing these equilibrium outcomes that are  implied by proposition 5, we can derive  equilibrium
consumption levels $\bar c^1$ and $\bar c^2$ from
 consumers' budget constraints and the market
clearing condition for the Arrow security,
$\sum_i \bar a^i(0) =0$. As above, let the 
equilibrium quantity of the Arrow security be
denoted $a=\bar a^1(0)=-\bar a^2(0)>0$. In period 0,
and in all subsequent periods when the Markov state
is $s=1$, the budget constraint of consumers 1 and 2, respectively, are:
$$ \EQNalign{\bar c^1 + Q(0|1) a & = 1,   \EQN bc_1REC;a \cr
             \bar c^2 - Q(0|1) a & = 0,   \EQN bc_1REC;b \cr} 
$$
and, when the Markov state is $s=0$, the budget 
constraints are:
$$ \EQNalign{\bar c^1 + Q(0|0) a & = a,   \EQN bc_0REC;a \cr
             \bar c^2 - Q(0|0) a & = 1-a. \EQN bc_0REC;b \cr} 
$$
We can use budget constraints of either 
consumer 1 or 2 to eliminate  
consumption  $\bar c^i$ and solve for the 
quantity  $a$ of the Arrow security:
$$
a = {1 \over 1+Q(0 \vert 1)-Q(0 \vert 0)} 
  = {1 \over 1+\beta 
     \bigl[\pi(0 \vert 1)-\pi(0 \vert 0)\bigr]}
\in \left[ {1 \over 1+\beta},\, {1 \over 1-\beta} \right),
                                                    \EQN aREC
$$
where we use the formula for the  equilibrium price kernel
$Q(0|s) = \beta \,\pi(0|s)$.  The lowest 
value in range for $a$ 
in \Ep{aREC} is attained when $\pi(0|1)=1$ and $\pi(0|0)=0$.
The upper bound is the limit as $\pi(0|1)$ approaches 
zero and $\pi(0|0)$ approaches one.


Next, after substituting expression \Ep{aREC} into 
either budget constraint of consumer $i$, 
we can solve for the consumption level $\bar c^i$:
$$ 
\EQNalign{ \bar c^1 &= { 1-\beta\,\pi(0 \vert 0) \over
    1 + \beta \bigl[\pi(0 \vert 1)-\pi(0 \vert 0)\bigr]}\,,    
                                          \EQN allocREC;a \cr
           \bar c^2 &= { \beta\,\pi(0 \vert 1) \over
    1 + \beta \bigl[\pi(0 \vert 1)-\pi(0 \vert 0)\bigr]}\,.    
                                          \EQN allocREC;b \cr} $$
These expressions have properties  that we would
expect. For example, given any $\pi(0|0)$, as the 
transition probability $\pi(0|1)$ goes to zero,  $\bar c^1$ and $\bar c^2$ approach 1 and 0,
respectively. This follows from the initial Markov 
state being $s_0=1$ in which consumer 1 receives 
the entire endowment. Another example would be deterministic
periodic endowments, $\pi(1|0)=\pi(0|1)=1$, when 
expressions \Ep{allocREC} agree with  \Ep{alloc} 
of example 3 in section \use{sec:example3}.

Lastly, we can confirm that the natural debt
limits are satisfied. Because only consumer 2 is ever
indebted, it suffices to check that consumer 2's 
indebtedness $\bar a^2(0) <0$ does not violate his natural 
debt limit, $-\bar a^2(0) \leq \bar A^2(0)$.
%% where $a^2(0)=-a$. 
According to recursion \Ep{borrowingconA}, 
consumer 2's natural debt limits satisfy
$$ \EQNalign{
\bar A^2(0) & = 1 + \sum_{s'} Q(s'|0) \bar A^2(s'),   
                                     \EQN debtlimREC;a \cr
\bar A^2(1) & = 0 + \sum_{s'} Q(s'|1) \bar A^2(s').  
                                     \EQN debtlimREC;b \cr}
$$
From equation \Ep{debtlimREC;b}, we can solve for
$\bar A^2(1)$
$$
\bar A^2(1) = {Q(0|1) \over 1-Q(1|1) } \bar A^2(0),
%% = {\beta \pi(0|1) \over 1-\beta \pi(1|1) } \bar A^2(0),
                                       \EQN debtlim_1REC
$$
which we substitute into \Ep{debtlimREC;a} to arrive
at an expression for $\bar A^2(0)$:
$$
\bar A^2(0) = {1 \over
     1 - Q(0|0) - Q(1|0) {Q(0|1) \over 1- Q(1|1)} }.
%%   = {1 \over
%%     1 - \beta \pi(0|0) - \beta \pi(1|0) 
%%            {\beta \pi(0|1) \over 1- \beta \pi(1|1)} } > 0,
                                       \EQN debtlim_0REC
$$

Using our formula for the equilibrium price kernel
$Q(s'|s) = \beta \,\pi(s'|s)$, we can now confirm that 
the natural debt limit, $\bar A^2(0) \geq -\bar a^2(0)$
where $\bar a^2(0)=-a$ as given by \Ep{aREC}, holds:
$$ \EQNalign{
\bar A^2(0) &= {1 \over
     1 - \beta \pi(0|0) - \beta \pi(1|0) 
            {\beta \pi(0|1) \over 1- \beta \pi(1|1)} } \cr
&> 
{1 \over 1 - \beta \pi(0|0) + \beta \pi(0|1) } 
= a = -\bar a^2(0).                \EQN OKdebtlimREC  \cr}
$$
The strict inequality in \Ep{OKdebtlimREC} arises
after adding  two strictly positive terms to the 
denominator of the expression for $\bar A^2(0)$,  namely,  $\beta \,\pi(0|1)$ 
and $\beta^2 \pi(1|0) \pi(0|1)$
$/ [1- \beta \pi(1|1)]$.\NFootnote{This argument 
requires that  the initial quantity $\bar A^2(0)$
be strictly positive, which is evidently  true by 
recursion \Ep{debtlimREC} from which 
$\bar A^2(0)$ was deduced.}

%Lars2023%
%which  in turn requires that the sum of
%the two negative terms in the denominator
%of \Ep{debtlim_0REC} is less than one in absolute value.
%Using the equilibrium price kernel
%$Q(s'|s) = \beta \,\pi(s'|s)$, we confirm this to be 
%true:
%$$
%\beta \left[\pi(0|0) + \pi(1|0){\beta [1-\pi(1|1)] \over 
%                              1-\beta \pi(1|1)} \right]
%<  \beta \left[\pi(0|0) + \pi(1|0){1-\beta \pi(1|1) \over 
%                              1-\beta \pi(1|1)} \right]
%=\beta <1,
%$$
%where we started by using $\pi(0|1)=1-\pi(1|1)$.}




\subsection{Example 6: Example 5 after initial redistribution}
As described in section \use{sec:PO}, suppose that section \use{sec:example5}  example 5
economy starts after an initial lump-sum redistribution.
Thus, in period $0$ when the initial state
is $s_0=1$, let the beginning-of-period wealth of 
consumer 1 and 2 be $a_0$ and $-a_0$, respectively,
for some feasible value of $a_0$. Feasibility of an initial redistribution imposes a bound on $a_0$ that is 
determined by the natural debt limit
of the consumer who receives the negative lump-sum
transfer. If consumer $i$ is the one who receives
a negative transfer, the bound on the absolute value
of $a_0$ is $|a_0|\leq \bar A^i(1)$. This inequality states that the initial indebtedness of consumer $i$ cannot 
exceed the maximal value that consumer $i$ can repay 
starting from period $0$ when the Markov state is 
$s_0=1$. This bound on a negative transfer is the
Markov version of constraint \Ep{Redist_A}
in section \use{sec:PO}.

According to proposition 5, the initial wealth
distribution will be revisited at the beginning of
any period in which  the realized state is $s=1$, i.e.,
$[\bar a^1(1),\, \bar a^2(1)] = [a_0,\, -a_0]$.
To compute the beginning-of-period wealth
distribution when the state is $0$,
$[\bar a^1(0),\, \bar a^2(0)] = [a,\, -a]$, we 
proceed exactly as in section \use{sec:example5}.
In period 0,
and in all future periods when the Markov state
is $s=1$, the budget constraint of consumer 1 and 2, respectively, are:
$$ \EQNalign{\bar c^1 + Q(0|1) a + Q(1|1) a_0 & = 1+a_0,
                                        \EQN bc_1Redist;a \cr
             \bar c^2 - Q(0|1) a - Q(1|1) a_0 & = -a_0,
                                         \EQN bc_1Redist;b \cr} 
$$
and, when the Markov state is $s=0$, the budget 
constraints are:
$$ \EQNalign{\bar c^1 + Q(0|0) a + Q(1|0) a_0 & = a,
                                       \EQN bc_0Redist;a \cr
             \bar c^2 - Q(0|0) a - Q(1|0) a_0 & = 1-a.
                                       \EQN bc_0Redist;b \cr} 
$$
Using the two budget constraints of either 
consumer 1 or 2, we can eliminate the 
consumption level $\bar c^i$ and solve for the 
quantity  $a$ of the Arrow security:
$$
a = {1 + \bigl[1 +Q(1 \vert 0)-Q(1 \vert 1)\bigr] a_0
     \over 1+Q(0 \vert 1)-Q(0 \vert 0)} 
  = {1 \over 1+\beta 
     \bigl[\pi(0 \vert 1)-\pi(0 \vert 0)\bigr]}\,+\, a_0\,, 
                                             \EQN aRedist
$$
where we have used the  equilibrium price kernel
$Q(s'|s) = \beta \,\pi(s'|s)$.\NFootnote{The coefficient
on $a_0$  simplifies when probabilities are interchanged:
 $\pi(0|1)-\pi(0|0)
= [1-\pi(1|1)]-[1-\pi(1|0)]
= \pi(1|0)-\pi(1|1)$.}
Next, after substituting expression \Ep{aRedist} into 
either budget constraint of consumer $i$, 
we can solve for the consumption level $\bar c^i$:
$$ 
\EQNalign{ \bar c^1 &= { 1-\beta\,\pi(0 \vert 0) \over
    1 + \beta \bigl[\pi(0 \vert 1)-\pi(0 \vert 0)\bigr]}
    \,+\, (1-\beta) a_0\,,    
                                          \EQN allocRedist;a \cr
           \bar c^2 &= { \beta\,\pi(0 \vert 1) \over
    1 + \beta \bigl[\pi(0 \vert 1)-\pi(0 \vert 0)\bigr]}
    \,-\, (1-\beta) a_0\,.    
                                          \EQN allocRedist;b \cr} $$
Hence, in comparison to the allocation \Ep{allocREC}
in section \use{sec:example5}, any initial redistribution
causes a permanent increase
(decrease) in the consumption of the consumer who receives
the positive (negative) initial transfer; that change
in consumption equals the annuity value of the transfer.



\subsection{Recursive formulation of optimization and equilibrium}
An equilibrium  pricing kernel of the form $Q(s'|s)$ and  endowments $y^i(s)$
that are  functions only of a Markov state $s$ let us to provide a
recursive formulation of the consumer's optimization problem.
Consumer $i$'s state at time $t$ is its wealth $a^i_t$ and Markov state $s_t$.
% We seek a pair of optimal policy
%functions $h^i(a,s)$, $g^i(a,s,s')$ such that the consumer's
...
expected discounted utility  that consumer $i$ with current wealth $a$
can attain in state $s$.
The value function satisfies the Bellman equation % for the consumer's problem is
$$ v^i(a, s) = \max_{\hat c, \{\hat a(s')\}_{s'}} 
\left\{ u_i(\hat c) + \beta
\sum_{s'} v^i\bigl(\hat a(s'),s'\bigr) 
                 \pi (s' | s) \right\} \EQN eq11 $$
where  maximization is subject to the following version
of constraint \Ep{eq10}:
$$ \hat c + \sum_{s'} \hat a(s') Q(s' | s)
     \leq  y^i(s) + a  \EQN eq12   $$
and also
$$\EQNalign{\hat c & \geq 0, \EQN eeq12;a \cr
           -   \hat a(s') & \leq \bar A^i(s'), \hskip.5cm \forall s',   \EQN eeq12;b \cr}$$
where $\{\bar A^i(s')\}_{s'}$ satisfy recursion
\Ep{borrowingconA}. 
Optimum decision rules take the forms
$$\EQNalign{\hat c & =  h^i(a,s), \EQN policy2;a \cr
  \hat a(s') & = g^i(a,s,s'), \hskip.5cm \forall s'. 
                                  \EQN policy2;b \cr}$$


\medskip\noindent{\sc Definition:} A {\it  recursive competitive equilibrium} is
an initial distribution of wealth $\vec a_0$, 
a pricing kernel $Q(s' | s)$, 
a set of borrowing limits $\{\bar A^i \}_i$ that
satisfy \Ep{borrowingconA},
sets of value functions $\{v^i(a,s)\}_i$, and
decision rules $\{h^i(a,s), g^i(a,s,s')\}_i$ such
that

\medskip
\noindent{\bf (i)} For all $i$, given
$a^i_0$, $\bar A^i$, the pricing kernel $Q(s' | s)$, and
the value function $v^i(a,s)$, the decision rules
$h^i(a,s)$ and $g^i(a,s,s')$ solve the right side
of Bellman equation \Ep{eq11} subject to constraints
\Ep{eq12} and \Ep{eeq12}.

\medskip
\noindent{\bf (ii)} For all $i$, the value function
$v^i(a,s)$ and the decision rules
$h^i(a,s)$ and $g^i(a,s,s')$ satisfy
$$ v^i(a, s) = u_i\bigl(h^i(a,s)\bigr) + \beta
\sum_{s'} v^i\bigl(g^i(a,s,s'),\,s'\bigr) 
                 \pi (s' | s).  \EQN eq11_confirm $$

%\medskip
\noindent{\bf (iii)} For all realizations of 
$\{s_t\}_{t=0}^\infty$, consumption and asset
portfolios $\{\{\hat c^i_t,$

\noindent
$\{\hat a^i_{t+1}(s')\}_{s'}\}_t\}_i$,
generated by the decision rules
$h^i(a,s)$ and $g^i(a,s,s')$,  
satisfy $\sum_i \hat c^i_t = \sum_i y^i(s_t)$ and
...
%\medskip
%\noindent{\bf (d)} The initial financial wealth vector $\vec %a_0$ satisfies $\sum_{i=1}^I a_0^i = 0 $.


\medskip
\noindent  
Condition {\bf (i)} asserts that the decision 
rules maximize a consumer's utility subject to the
budget constraint and borrowing limits, under the 
assumption that the value function on the right
side of Bellman equation \Ep{eq11} 
 captures future consequences of those 
decisions. Condition {\bf (ii)} confirms that each
consumer uses an appropriate 
value function.
%% to evaluate end-of-period asset 
%%holdings taken into the subsequent period. 
For each $i$, the value function $v^i(a,s)$ 
satisfies the optimality equation and hence, 
the consumer is maximizing discounted
expected utility given  initial wealth
and the process that generates prices
and future endowments. Condition {\bf (iii)} 
requires that markets for goods and Arrow
securites clear at all times.

%Condition ({\bf d }) asserts that the economy is 
%closed and  starts off from a position in which 
%there are  zero net claims in the aggregate.


According to proposition 5,  when endowments are governed by 
a Markov process, a recursive competitive
equilibrium exists. Specifically, given the pricing 
kernel $Q(s'|s)$ in \Ep{MarkovQ},
%% that prevails in the 
%% sequential-trading competitive equilibrium,
the optimum decision rules for consumer $i$ in Markov
state $s$ will, in an equilibrium, be evaluated at
consumer $i$'s state $(\bar a^i(s),\,s)$ and those 
decision rules  prescribe that:
$$\EQNalign{h^i(\bar a^i(s),s) &= \bar c^i(s), 
                                       \EQN EQpolicy2;a \cr
\noalign{\vskip.2cm}
g^i(\bar a^i(s),s,s') &=\cases{\bar a^i(s'), 
                        & if $\pi(s'|s) >0$; \cr
                     0, & otherwise.\cr}  \EQN EQpolicy2;b 
                                                 \cr}$$
In the circular reasoning that accompanies the concept of a  competitive equilibrium, 
this confirms that the equilibrium pricing kernel satisfies \Ep{MarkovQ}. To confirm this claim,  use the 
first-order conditions for the problem on the right side
of Bellman equation \Ep{eq11} and the
\idx{Benveniste-Scheinkman formula} and rearrange to get
$$ Q(s' | s) = \beta {u'_i(\bar c^i(s')) 
                 \over u'_i(\bar c^i(s)) }
                                   \pi(s' | s). \EQN eq14 $$
...
%Note that the solution of the Bellman equation implicitly 
%depends
%on $Q(\cdot \vert \cdot)$ because it appears in the 
%constraint
%\Ep{eq12}. In particular, use the first-order conditions for  
%the problem on the right of equation \Ep{eq11} and the
%\idx{Benveniste-Scheinkman formula} and rearrange to get
%$$ Q(s_{t+1} | s_t ) = {\beta u'_i(c_{t+1}^i) 
%\pi(s_{t+1} | s_t)
...
extensively when  we study  asset pricing in chapter \use{assetpricing1}.
...
%\medskip\noindent{\sc Definition:} A {\it  recursive 
%competitive equilibrium} is
%an initial distribution of wealth $\vec a_0$, a set of 
%borrowing limits $\{\bar A^i(s)\}_{i=1}^I$,
%a pricing kernel $Q(s' | s)$, sets of value functions 
%$\{v^i(a,s)\}_{i=1}^I$, and
%decision rules $\{c^i(s), a^i(s)\}_{i=1}^I$ such
%that

%\noindent{\bf (a)} The state-by-state borrowing constraints 
%satisfy the recursion ....
%$$ \bar A^i(s) = y^i(s) + \sum_{s'} Q(s'|s) \bar A^i(s').  
%\EQN borrowingconA $$
%\medskip
%\noindent{\bf (b)} For all $i$, given
% $a^i_0$, $\bar A^i(s)$,  and the pricing kernel, the value 
%functions and decision rules
%solve the consumer's problem;
%\medskip
%\noindent{\bf (c)} For all realizations of 
%$\{s_t\}_{t=0}^\infty$, the consumption and asset
%portfolios $\{\{c^i_t,$
%$\{\hat a^i_{t+1}(s')\}_{s'}\}_i\}_t$  satisfy $\sum_i c^i_t 
%= \sum_i y^i(s_t)$ and
%$\sum_i \hat a_{t+1}^i(s') = 0$
%for all $t$ and $s'$.
%%%for all $i$,  $\{\theta_t^i\}_{t=0}^\infty$ satisfies
...
%%%(c) for all $t$,  $\sum_i h(\theta_{t}^i,s_t) = \sum_i y^i(s_t)$.
%\medskip
%\noindent{\bf (d)} The initial financial wealth vector 
%$\vec a_0$ satisfies $\sum_{i=1}^I a_0^i = 0 $.
%\medskip
%\noindent \index{recursive!competitive equilibrium}
%\noindent   Condition ({\bf c}) asserts that there are  zero 
%net aggregate claims in all Markov states.
% Condition ({\bf d }) asserts that the economy is closed and  
%starts off from a position in which there 
%are  zero net claims in the aggregate.

%If  an allocation and prices in   a recursive competitive 
%equilibrium are to be consistent
%with the equilibrium allocation and price system that 
%prevail in a  corresponding complete markets economy with
% all trades occurring at time $0$,
%we must impose that $a_0^i = 0$ for $i = 1, \ldots , I$.  
%That  assures that at time $0$ the present value of each 
%agent's consumption equals the present value of his 
%endowment stream,
%the  restriction imposed by equation \Ep{eq2}
%in the section \use{sec:ADtime0}  arrangement with all 
%trades occurring at time $0$.

%\index{state variable degeneracy}%

%%%Starting the system off with $a_0^i =0 \ \forall i$ has 
%%%a striking 
%%%implication 
%The economy has a remarkable property that we can call 
%{\it state variable degeneracy}.
%Thus, note that although there are two state variables in 
%the value function $v^i(a,s)$, within a recursive 
%competitive equilibrium
%starting from $a_0^i = 0 \ \forall i$  at the starting  
%Markov state  $s_0$, two outcomes  prevail:


%\medskip
%\item {(i)} Financial wealth $a$ is an exact function of the 
%Markov state $s$. 


%\medskip
%\item{(ii)} $a_0^i = 0 $ for all $i$ whenever the Markov 
%state $s_t$ returns to   $s_0$. 

%\medskip

%\noindent Finding (i) asserts that within a competitive 
%equilbrium 
%  the exogenous Markov state is all we require to track an 
%individual.  
%Financial wealth turns out to be redundant because it is an 
%exact function of the exogenous state $s_t$. 
% This finding depends critically on there being complete 
%markets in Arrow securities and unfolds because 
% in equilibrium continuation wealth can be expressed as a 
%function of $s_t$. 
%For more about this issue, please see chapters 
%\use{selfinsure} and \use{incomplete} and sections 
%\use{sec:LQmodel} and \use{sec:LQcomplete} 
%of chapter \use{timeseriesapp}. 
%Finding (ii) asserts that each household  revisits the zero 
%financial wealth state 
%with which he began life provided that $s_0$ is a recurrent 
...
$\hat Q_j(s' | s)$ denote the $j$-step pricing kernel to be interpreted
as follows: $\hat Q_j(s' | s)$ gives the price of one unit of
consumption $j$ periods ahead, contingent on the state in that
future period being $s'$, given that the current state is $s$. For
example, $j=1$ corresponds to the one-step pricing kernel 
in \Ep{eq14}, $\hat Q_1(s' | s) = Q(s' | s)$.


With markets in all possible $j$-step-ahead contingent claims,
the counterpart to constraint \Ep{eq12}, the consumer's budget
constraint at time $t$, is
$$ c_t^i + \sum_{j=1}^\infty \sum_{ s_{t+j}} 
   \hat Q_j( s_{t+j} | s_t)
   z^i_{t,j}( s_{t+j}) \leq  y^i(s_t) + a_t^i. \EQN cecm_eq10   $$
Here $z^i_{t,j}( s_{t+j})$ is consumer $i$'s holdings at
the end of period $t$ of contingent claims that pay one unit of
the consumption good $j$ periods ahead when % at date $t+j$, contingent on
the Markov state at date $t+j$ is $ s_{t+j}$. The consumer's
wealth at $t+1$   depends on his time $t$  portfolio
and the realization of $s_{t+1}$,
$$
a^i_{t+1}(s_{t+1}) = z^i_{t,1}(s_{t+1})
   + \sum_{j=2}^\infty \sum_{ s_{t+j}}  
  \hat Q_{j-1}( s_{t+j} | s_{t+1})
           z^i_{t,j}( s_{t+j}) . \EQN cecm_eq10a
$$
The realization of $s_{t+1}$ determines which element 
$z^i_{t,1}( s_{t+1})$ of the portfolio of
one-period-ahead claims pays off at
time $t+1$,
and also the capital gains earned and losses inflicted on  holdings of longer horizon claims 
$\{z^i_{t,j}( s_{t+j})\}_{j\geq 2, s_{t+j}}$
implied by equilibrium prices 
$\{\hat Q_{j-1}( s_{t+j} | s_{t+1})\}_{j\geq 2, s_{t+j}}$.


With respect to $z^i_{t,j}( s_{t+j})$ for $j>1$,
use the first-order condition for maximizing the right side of  \Ep{eq11} subject to 
the revised budget constraints associated with opportunity sets described by
\Ep{cecm_eq10}-\Ep{cecm_eq10a} 
and the Benveniste-Scheinkman formula and then rearrange to get
$$ \hat Q_j( s_{t+j} | s_t ) = \sum_{s_{t+1}}
   {\beta u'_i[c^i_{t+1}(s_{t+1})] \pi(s_{t+1} | s_t) \over u'_i(c_t^i) }
   \hat Q_{j-1}( s_{t+j} | s_{t+1} ).           \EQN Qrecurs $$
When evaluated at the competitive equilibrium
consumption allocation, this expression characterizes two adjacent pricing
...
hold for all consumers,  \Ep{Qrecurs} holds for all $i$.} Together with first-order
condition \Ep{eq14}, formula \Ep{Qrecurs}  implies that the
kernels $\hat Q_j$, $j = 2, 3, \ldots,$ can be computed recursively:
$$ \hat Q_j( s_{t+j} | s_t) = \sum_{s_{t+1}} 
            \hat Q_1(s_{t+1} | s_t)
            \hat Q_{j-1}( s_{t+j} | s_{t+1}). \EQN Qrecurs2 $$
...
prices by manipulating  budget sets that includes opportunities to trade  redundant
assets.
%\NFootnote{Arbitrage pricing theory was advocated by
%Stephen Ross (1976).} \auth{Ross, Stephen}
This provides an alternative way of deriving restriction
\Ep{Qrecurs2} that was established above by using consumers'
...
consumer also trade  a synthetic ex-dividend 
``Lucas (1978) tree''.
\auth{Lucas, Robert E., Jr.}%
 Because markets
are already complete, these additional assets are redundant and must be priced in ways that leave the budget set
...
one unit of consumption  if the state takes value
$s_{t+j}$ at time $t+j$, the consumer also purchases  $N_{t}$
units of  a  ``Lucas tree''. Let the ex-dividend price of the
tree at time $t$  \index{arbitrage}%
 be $p(s_t)$.  This means that at $t+1$  the
...
consumer to a claim on  $N_{t}[p(s_{t+1}) + d(s_{t+1})]$
 units of time $t+1$, state $s_{t+1}$ consumption.\NFootnote{We calculate the price of this asset
using a different method in
chapter \use{assetpricing1}.}
As in the case of the j-step-ahead contingent claims, 
negative holdings $N_{t}<0$ of the synthetic ``Lucas tree''
means that the consumer is issuing the security; a security that
...
$$c_t + \sum_{j=1}^\infty \sum_{s_{t+j}} \hat Q_j(s_{t+j} | s_t)
      z_{t,j}(s_{t+j}) + p(s_t) N_{t} \leq a_t  + y(s_t)
\EQN arbit1;a $$ 
and the law of motion of wealth is
$$\eqalign{ a_{t+1}(s_{t+1}) & = z_{t,1}(s_{t+1}) + \left[p(s_{t+1})
+ d(s_{t+1})\right] N_{t} \cr
  & +  \sum_{j=2}^\infty \sum_{s_{t+j}} 
       \hat Q_{j-1}(s_{t+j} | s_{t+1})
       z_{t,j}(s_{t+j}). \cr} \EQN arbit1;b $$
Multiply equation  \Ep{arbit1;b} by $\hat Q_1(s_{t+1} | s_t)$,
sum over $s_{t+1}$, solve for \hfil\break
 $\sum_{s_{t+1}} \hat Q_1(s_{t+1} | s_t) z_{t,1}(s_t)$,
and substitute this expression into \Ep{arbit1;a} to get
$$\EQNalign{ c_t +& \left\{p(s_t) - \sum_{s_{t+1}} 
      \hat Q_1(s_{t+1} | s_t)
      [ p(s_{t+1}) + d(s_{t+1})]   \right\} N_{t} \cr
      + & \sum_{j=2}^{\infty} \sum_{s_{t+j}} \left\{
     \hat Q_j(s_{t+j} | s_t) - \sum_{s_{t+1}} 
       \hat Q_1(s_{t+1} | s_t) \hat Q_{j-1}(s_{t+j} | s_{t+1})
                      \right\} z_{t,j}(s_{t+j}) \hskip1cm \cr
     + & \sum_{s_{t+1}} \hat Q_1(s_{t+1} | s_t ) a_{t+1}(s_{t+1})
                                 \leq a_t + y(s_t). \EQN arbit2\cr}$$
Please stare at the  two terms in braces. If they  are not zero, the consumer can attain
...
has no satiation point, in a competitive equilibrium, the terms in the
braces must be zero. Thus, we have the  arbitrage-free pricing
formulas \index{arbitrage!no-arbitrage principle}%
$$\EQNalign{ p(s_t) &=  \sum_{s_{t+1}} \hat Q_1(s_{t+1} | s_t)
...
\use{sec:multi-step}.
Recall formula \Ep{Qrecurs2} for the price 
$\hat Q_j(s_{t+j} | s_t)$ of a $j$-period ahead 
contingent claim in Markov state
$s_t$.  At time $t$ in Markov state $s_t$, the price of a risk-free claim on  one unit of consumption at time $t+j$ is\NFootnote{The 
one-period gross {\it return\/} on a risk-free claim at time $t$ on consumption  at time $t+j$ is risky and equals
$$ R_{j,t}(s_{t+1},s_t) = {\frac{p_{j-1,t+1}(s_{t+1}) 
%Lars2023% - p_{j,t}(s_t)
}{p_{j,t}(s_t)}} .$$}
$$ p_{j,t}(s_t) = \sum_{s_{t+j} } \hat Q_j(s_{t+j} | s_t) . $$
At time $t$, the  ``yield to maturity'' (or just ``yield'') of a sure claim to one unit of time $t+j$ consumption is
the scalar $\rho_{j,t}(s_t)$ that satisfies \index{yield!to maturity}
$$ p_{j,t}(s_t) = \exp( -j \rho_{j,t}(s_t)) $$
or
$$ \rho_{j,t}(s_t) = - {\frac{\log p_{j,t}(s_t)}{j}}.  \EQN eq:yield $$
A vector  $\rho_{j,t}(s_t)_{j=1}^T$ is  called a ``term structure of interest rates.''
Theories of  the term structure of interest rates to be outlined in chapters \use{assetpricing1} and \use{assetpricing2}
seek to understand the evolution of $\{\rho_{j,t} \}_{j=1}^T$  across  time and Markov states.

\subsection{Constructing yields}

Often prices $p_{j,t}(s_t)$ are not observed directly because claims to risk-free promises to goods $t+j$ periods ahead are not traded separately.  But when
enough distinct {\it bundles\/} of such claims are traded, there are ways to infer  $p_{j,t}(s_t)$'s by appropriately unbundling a set of bond prices.
  Here is a brief account of how.
Suppose that at time $t$ observations are available on the prices of   bonds $i=1, \ldots, I$. The $i$th bond is a claim
to a stream of risk-free claims to $d_{i,t+\tau}$ units of time $t+\tau$ consumption for horizons  $\tau = 1, 2, \ldots, T$.
Some components of  $d_{i,t+\tau}$ can be zero.  According to our theory,  the observed price $v_{i,t}$ of  bond $i$ at $t$ satisfies
$$ v_{i,t}(s_t) = \sum_{j =1}^T d_{i,t+\tau} p_{j,t}(s_t) . $$  Represent this equation for a collection of bonds  $i = 1, \ldots , I$
as
$$  \bmatrix{ v_{1,t} \cr v_{2,t} \cr \vdots \cr v_{I,t} } =
     \bmatrix{ d_{1,t+1} & d_{1,t+2} & \cdots & d_{1,t+T} \cr
      d_{2,t+1} & d_{2,t+2} & \cdots & d_{2,t+T} \cr
      \vdots & \vdots & \vdots & \vdots \cr
       d_{I,t+1} & d_{I,t+2} & \cdots & d_{I,t+T} }
      \bmatrix{ p_{1,t}(s_t) \cr p_{2,t}(s_t) \cr \vdots \cr p_{T,t}(s_t) }   $$
or
$$ V_t = D_t P_t . $$
...
to infer. After we have constructed $P_t$ or $\hat P_t$, we can construct yields from equation \Ep{eq:yield}. \index{inverse!generalized}%
...
gives a preview of ideas to appear in chapters \use{socialinsurance}
and \use{credible}.
For this purpose, we
consider a special case of the example 2 section \use{sec:example2GG}  
economy with a constant aggregate
endowment and two types of consumer with $y_t^1 = s_t$, $y_t^2 = 1-s_t$.
We  assume that the $s_t$ process
...
utility streams by assigning a consumption
allocation today and a vector of pairs of state-contingent
promised discounted utility streams starting tomorrow,
one pair for each possible realization of tomorrow's state.
We use dynamic programming to characterize the Pareto
frontier. Define $P(v)$ as the maximal expected discounted
utility that can be offered to a type 1 person when 
a type 2 person is offered an expected discounted
utility of at least $v$. 
...
$\sum_{t=0}^\infty \sum_{s^t} \beta^t u_2(c_t^2(s^t)) \pi_t(s^t) \geq v.$
In terms of the competitive equilibrium
allocation calculated for  the section \use{sec:example2GG} example 2  economy above,
let $\overline c= \overline c^2$ be the  constant consumption
allocated to a type 2 person and $1-\overline
c=\overline c^1$ be the constant consumption
allocated to a type 1 person.
Since we have shown that a competitive equilibrium allocation is
a Pareto optimal allocation, we already know one point on the
Pareto frontier $P(v)$. In particular, when a type 2 person is promised
$v= u_2(\overline c) / (1 - \beta) $, a type 1 person attains
life-time utility
$P(v) =  u_1(1-\overline c) / (1-\beta)$. 
When now computing the rest of the Pareto frontier $P(v)$, 
we consider initial values of $v$ that exclude the two
extreme end points at which either a type 1 or a type 2 person 
consumes the entire endowment and the other type consumes nothing.
 Specifically, we consider initial promises $v$ 
of expected discounted utility of a type 2 person 
belonging to the set
$V = [u_2(\epsilon) / (1-\beta),\,  
      u_2(1-\epsilon) / (1 -\beta)]$ 
...
%%We can express the discounted values $v$  and $P(v)$ 
%%recursively\NFootnote{This is our first example
%%of a `dynamic program squared'.  We call it that because the 
%%state variable $v$ that appears in the Bellman equation
%%for $P(v)$ itself satisfies another Bellman equation.}
%%as
%%$$ v = \sum_{s=1}^S \left[ u_1(c_s)+\beta w_s \right]  \Pi_i $$
%%and
%%$$ P(v) = \sum_{s=1}^S \left[ u_2(1- c_s) + \beta P(w_s) 
%%           \right] \Pi_s, $$
%%where $c_s$ is consumption of the type 1 person in state $s$,
%%$w_s$ is the continuation value assigned to the type
%%$1$ person in state $s$; and $1-c_s$ and $P(w_s)$ are the 
%%consumption
%%and the continuation value, respectively, assigned to a type 2 %%person in state $s$.
%%Assume that  the continuation values $w_s \in V$, where $V$ is 
%%a set of admissible discounted values of utility.   
%%In this section,
%%we assume
%%that $V = [u_1(\epsilon) / (1-\beta),\,  u_1(1) / (1 -\beta)]$ 
%%where $\epsilon\in(0,1)$ is an arbitrarily small number.



%%%$V = [0, {u(1) \over 1 -\beta}]$. In effect, a consumption
%%%plan offers a consumer a state-contingent consumption vector in
...
%%In effect, before the realization of the current state, a 
%%Pareto optimal
%%allocation offers the type 1 person a state-contingent vector 
%%of consumption $c_s$ in state $s$ and
%%a state-contingent vector of continuation values
%%$w_s$ in state $s$, with each $w_s$ itself being a present 
%%value of one-period future utilities.

In terms of the pair of values
$(P(v),v)$,
 we can express a Pareto problem recursively as
$$ P(v) = \max_{c, \{w_{s'} \}_{s'} } \left[ u_1(1-c) 
   + \beta \sum_{s'} P(w_{s'}) \pi(s') \right] \EQN paret1 $$
where  maximization is subject to
$$ u_2(c) + \beta \sum_{s'} w_{s'} \pi(s') \geq v \EQN paret2 $$
where $c \in [0,1]$ and $w_{s'} \in W$ for all $s'$.  Here $W$ is a set of
possible continuation values, an object that ought to be determined as part of the analysis.
In this section, we simply set $W = V$, the set described above.\NFootnote{In  chapters \use{uninsur1},
\use{credible}, and \use{chang}, we describe analytic
techniques that can be used to justify this equality in classes of models more general than the one being presented here.}
Thus, in order to deliver promised value $v$ to the type 2
person, the planner assigns current consumption $c$
and continuation values $\{w_{s'}\}_{s'}$, one continuation
value $w_{s'}$ for each possible state $s'$ next period.
The corresponding assignment to a type 1 person is
current consumption $1-c$ and continuation values
$\{P(w_{s'})\}_{s'}$. 
...
%%In this section,
%% we assume
%%that $V = [u_1(\epsilon) / (1-\beta),\,  u_1(1-\epsilon) / %%(1 -\beta)]$ where
%%$\epsilon\in(0,1)$ is an arbitrarily small number.

To solve the Pareto problem, form the
Lagrangian
$$ L = u_1(1-c) + \beta \sum_{s'} P(w_{s'}) \pi(s')
+ \theta \left[ u_2(c) + \beta \sum_{s'} w_{s'} \pi(s') - v \right] $$
where $\theta $ is a Lagrange multiplier on constraint \Ep{paret2}.
First-order conditions
with respect to $c$ and $w_{s'}$ for all $s'$, respectively, are
$$\EQNalign{ - u'_1(1-c) + \theta u'_2(c) & = 0 \EQN paret3;a \cr
        P'(w_{s'}) + \theta & = 0. \EQN paret3;b \cr} $$
The envelope condition is
$ P'(v) = -\theta$.  Thus, \Ep{paret3;b} becomes
$  P'(w_{s'}) = P'(v)   $.  But $P(v)$ happens to be strictly concave,
so  this equality implies
$ w_{s'} = v$.  Therefore,  any solution of the Pareto problem leaves
the continuation value $w_{s'}$ independent  of  the state $s'$.
Equation
\Ep{paret3;a} implies that
$${u'_1(1-c) \over u'_2(c)} = -P' (v) .\EQN paret4 $$
Since $v$ is constant over time (because $w_{s'} = v$ 
for all $s'$),
it follows that $c$ is constant over time.



%%To solve the Pareto problem, form the Lagrangian
...
%%$$\EQNalign{ - u'_2(1-c_s) + \theta u'_1(c_s) & = 0 
%%\EQN paret3;a \cr
%%        P'(w_s) + \theta & = 0. \EQN paret3;b \cr} $$
%%The envelope condition is
%%$ P'(v) = -\theta$.  Thus, \Ep{paret3;b} becomes
%%$  P'(w_s) = P'(v)   $.  But $P(v)$ happens to be strictly 
%%concave, so  this equality implies
%%$ w_s = v$.  Therefore,  any solution of the Pareto problem 
%%leaves
%%the continuation value $w_s$ independent  of  the state $s$.
%%Equation \Ep{paret3;a} implies that
%%$${u'_2(1-c_s) \over u'_1(c_s)} = -P' (v) .\EQN paret4 $$
%%Since the right side of \Ep{paret4} is independent of $s$, so 
%%is the left side, and therefore $c$ is independent of $s$.
%%  And since $v$ is constant over time (because $w_s = v$ for 
%%all $s$), it follows that $c$ is constant over time.


 Notice from
\Ep{paret4} that $-P'(v)$ serves as a   relative Pareto weight on
the type 2 person. Our recursive formulation brings out that,
because $P'(w_{s'}) = P'(v)$,
 the relative  Pareto weight remains constant over time and is independent
of the  realization of $s'$.    The planner dictates
...
to current realizations of shocks to endowments or preferences.\NFootnote{In those settings, the recursive
formulation of the Pareto problem will become 
a `dynamic program squared'.  We call it that because the 
state variable $v$ that appears in the Bellman equation
for $P(v)$  will itself satisfy yet another Bellman equation.
For example, the consumer who  receives a
promised value might be deciding whether  to remain
within an insurance arrangement or to elope into autarky. }
%Thus, the consumer would be solving her own dynamic program
%which would impose an enforcement constraint on the 
%planner's dynamic program.}



\section{Concluding remarks}
  The  framework in this chapter serves as foundation for much of  macroeconomics, including
extensive literatures on asset pricing and risk sharing.
We describe  asset pricing in more
...
that seem inconsistent with complete markets. For models   with exogenously imposed   incomplete markets,
...
required to facilitate bilateral exchanges.   Modern models of
money introduce frictions that impede complete markets.     Some
monetary models (e.g., the cash-in-advance model of Lucas, 1981)
impose minimal impediments to complete markets in ways designed to  preserve many
...
model of chapter \use{townsend} and the Kiyotaki-Wright search
...
\appendix{}{Appendices: Departures from key assumptions}


\noindent
The  finding that the equilibrium consumption of a consumer
...
In appendix \use{app:compAA}, we show that when consumers have heterogeneous
discount factors,  equilibrium allocations eventually entail  efficient
 ``immiserization'': at equilibrium prices,  less patient
consumers choose to consume almost nothing
as time approaches infinity.
In appendix \use{app:compB}, we show that when consumers share the same discount factor but have  heterogeneous beliefs,  as  time passes  equilibrium allocations assign   less
and less  to those
...
\appendix{A}{Heterogeneous discounting\label{app:compAA}}

We now modify \Ep{eq0} to allow for heterogeneous discount
factors.  Let $\beta_i$ denote
the subjective discount factor of consumer $i$, where
...
most patient consumer at the value assigned in the initial time $t=0$
...
Pareto weight on consumer $i$'s utility satisfies
$$
\lambda^t_i =  \left[ \beta_i \over \beta_1 \right]^t \lambda_i,
$$
where $\lambda_i$ is the weight on consumer $i$'s utility in the
...
zero.  The most patient type of consumer eventually
consumes everything.


\appendix{B}{Heterogeneous beliefs \label{app:compB}}
\auth{Debreu, Gerard}%
...
In the body of this chapter, we  assigned the same probabilities to all agents inside the model.  In particular, when we  adopted our restrictive expected utility specification \Ep{eq0}, we
 imputed a common probability specification $\{\pi_t(s^t)\}$   to all consumers $i=1, \ldots, I$.
 At various points in the body of the  chapter and  in  chapters \use{assetpricing1} and \use{assetpricing2} about asset pricing,
 we   go beyond that to impute the same common probability model shared by agents inside the model  both to outside economists and to   `nature'. Nature is
 also often called  the
actual history-generating or data-generating mechanism.  Imputing common beliefs to everyone inside a model, to nature,
...
By equating probability distributions
 ...
 be required to describe heterogeneous beliefs. In doing this it combines and confounds   distinct roles that actual and  perceived probabilities  $\pi_t(s^t)$ play in influencing not only actual outcomes but also   individuals'
...
to allow for different personal probabilities
$$ U_i({\bf c}^i) = \sum_{t=0}^\infty \sum_{s^t} \beta^t 
   u_i\left(c^i_t(s^t)\right)
...
{\frac{u_i^\prime(c^i_t(s^t))}{u_1^\prime(c^1_t(s^t))}}
 = {\frac{\lambda_1}{\lambda_i}} \; .
                                               \EQN heterogbel2 $$
The likelihood ratios $ \pi_t^i(s^t) / \pi_t^1(s^t) $, % ${\frac{\pi_t^i(s^t)}{\pi_t^1(s^t)}}$,
 which were identically unity when there were common beliefs, now influence optimal allocations.
For a given vector of time $0$  Pareto weights $\{\lambda_i\}_{i=1}^I$, the Pareto planner tilts consumption  at a date-history pair toward
people who place higher probabilities on that date-history pair.

As in the case of heterogeneous discounting, expression
\Ep{heterogbel2} implies that ratios of marginal utilities
in an efficient allocation are no longer time invariant.
Once again it is useful to let $\lambda^t_i(s^t)$ be the
Pareto weight attached to consumer $i$'s utility in a
...
\beta^t u_i(c_{\tau}^i(s^\tau)) \pi_{\tau,t}^i(s^\tau | s^t) .$$
Under heterogeneous
beliefs, the allocations chosen by every  continuation  planner  equal  continuations of the allocation chosen by the time $0$ planner provided that
 continuation Pareto weights satisfy
$$
\lambda^t_i(s^t) =  \left[ \pi_t^i(s^t) \over \pi_t^1(s^t) \right]
                    \lambda_i,       \EQN heterogbel3
$$
where we have normalized the Pareto weights
by always assigning consumer $1$ the same Pareto 
weight as in the initial time $0$ Pareto problem, i.e., 
$\lambda^t_1(s^t) = \lambda_1$ for all $t\geq0$ and
all $s^t$.
...
\noindent
The  Pareto problem above does not mention nature's probability specification.
 But sometimes we want to compute
a distribution of consumption implied by
\Ep{heterogbel2} under a probability distribution that we pretend is nature's.
...
for all $\pi^i \not= \pi$.}
Relative entropy measures the divergence of probability distribution $\pi$ from probability
 distribution $\pi^i$.\NFootnote{Relative entropy also governs statistical properties of likelihood ratio tests for
...
a law of large numbers under distribution $\pi$
$$ {\frac{1}{t}}\sum_{\tau=0}^t
...
utilities diverges to $+\infty$ for all $i > 1$. Since the marginal utility in the
denominator is bounded below because of the finite aggregate endowment,
we conclude that the marginal utility of consumer $i \neq 1$ goes to
infinity and hence $\lim_t c^i_t(s^t)\rightarrow 0$.

This example is a special case of
Blume and Easley's (2006) more general proposition that in the limit as $t \rightarrow +\infty$, the  consumer with beliefs closest to the truth, as measured by relative entropy, receives the entire allocation.\NFootnote{See Borovi\u{c}ka (2020) for how outcomes differ when consumers have 
recursive preferences of a type that generalize ones we shall encounter in chapter \use{assetpricing2}.}
\auth{Blume, Lawrence}%
\auth{Easley, David}%
\auth{Borovi\u{c}ka, Jaroslav}%



\subsection{Recursive version of Pareto problem}
 We add heterogeneous beliefs to the section \use{sec:RecursivePareto} recursive  formulation of a
Pareto problem.
In addition to Inada condition \Ep{eq_Inada}, we 
assume that utility functions satisfy 
$\lim_{c \rightarrow 0} u_i(c) = -\infty$, a condition that facilitates 
 our analysis by bringing the following 
properties to  the Pareto frontier. On the left 
side, the Pareto frontier $P(v)$ reaches a plateau 
as it converges to $u_1(1/(1-\beta))$ as 
$v \rightarrow -\infty$, so that the  slope $P'(v)$
converges to zero. Thus, at very  low 
expected discounted utilities $v$, a type 2 person
consumes almost nothing, so further withdrawals of utility from that person can bring 
 very little additional consumption  to
a type 1 person. %So the expected  
%discounted utility $P(v)$ of a type 1 person
%hardly changes in response to an arbitrarily low 
%$v$ declining further toward $-\infty$.
At  
the right side of the Pareto frontier, a symmetric analysis  applies. The Pareto
frontier $P(v)$ exhibits a precipice as $v$ 
approaches $u_2(1/(1-\beta))$, i.e.,  expected
discounted utility $P(v)$ of a type 1 person
plummets toward $-\infty$ and  slope $P'(v)$
converges to $-\infty$.

With heterogeneous beliefs,
Pareto problem \Ep{paret1}--\Ep{paret2}
now becomes
$$ P(v) = \max_{c, \{w_{s'} \}_{s'} } \left[ u_1(1-c) 
   + \beta \sum_{s'} P(w_{s'}) \pi^1(s') \right] 
                                      \EQN heterogbel12 $$
where  maximization is subject to
$$ u_2(c) + \beta \sum_{s'} w_{s'} \pi^2(s') \geq v .
                                      \EQN heterogbel13 $$
First-order conditions with respect to continuation
values are $P'(w_{s'}) \pi^1(s') =- \theta \pi^2(s')$  
for all $s'$ and the envelope condition is 
$P'(v) = -\theta$. Hence, we no longer have the
implication that $P'(w_{s'})=P'(v)$ but rather that
$P'(w_{s'}) \pi^1(s') =P'(v) \pi^2(s')$.
Recalling  that $-P'(v)$ serves as a relative Pareto weight
on the type 2 person 
brings out that now the Pareto weight changes with
the realization of next period's state $s'$:
$$
-P'(w_{s'})= {\frac{\pi^2(s')}{\pi^1(s')}} 
              \left(-P'(v)\right) .     \EQN heterogbel14 $$
This finding is an i.i.d.\ version of the more general updating formula for  Pareto weights under heterogeneous 
beliefs in expression \Ep{heterogbel3}. 
The relative Pareto weight on the 
type 2 person increases (decreases) if a state $s'$
is realized that a type 2 person had thought
to be more (less) likely  than a type 1 person had. 

We now study what happens asymptotically to the relative
Pareto weight as time goes to
infinity under a true set of probabilities.
Let $\pi(s)$ denote the true probability of 
state $s$ being realized, where 
$\sum_{s} \pi(s) =1$. We assume that 
the type 1 person knows the true probabilities, 
i.e., $\pi^1(s)=\pi(s)$ for all $s$, while
the type 2 person has less accurate beliefs, i.e.,
$\pi^2(s) \neq \pi(s)$ for some $s$ and
$\sum_{s} \pi^2(s) =1$. After multiplying both sides 
of expression \Ep{heterogbel14} by $\pi(s')$
(which equals $\pi^1(s')$) and summing over $s'$, 
we obtain
$$
\sum_{s'} \pi(s') (-P'(w_{s'})) = -P'(v) .    
                             \EQN heterogbel15 $$
Thus, the relative Pareto weight $-P'(v)$ is 
a nonnegative martingale. From what we 
learned in section \use{sec:heterogbel_example},
expectations of future values of $-P'(v)$ are
bounded from above, i.e., $E[-P(v_t)]<+\infty$.
This must be true because under our assumption that
a type 1 person knows the true probabilities,
a  type 1 person will almost surely not become
impoverished  as
time goes to infinity and 
hence, the relative Pareto weight of the 
type 2 person cannot go to infinity.

It follows from the supermartingale convergence
theorem (see appendix A of chapter \use{selfinsure}) 
that $-P'(v)$ converges almost surely to a finite
limit. More exactly, we can show that $-P'(v)$ must converge 
to $0$, so that $v$ converges to $-\infty$. 
Suppose to the contrary that $-P'(v)$ converges 
to a nonzero limit, which implies that $v$ 
converges to a finite limit. That would
contradict optimality condition \Ep{heterogbel14}
that dictates how future $w_{s'}$ always 
spread out when the planner maximizes 
expected discounted utilities of agents who
do not share beliefs. This 
contradiction can be avoided only if $v$, the
utility of the type 2 person with the less
accurate beliefs, converges to $-\infty$; 
therefore, the limit of
$-P'(v)$ must be zero.\NFootnote{If it had
been the type 1 person who had less accurate 
beliefs, we would simply have had to exchange
roles of the two types in the
formulation of Pareto problem   
\Ep{heterogbel12}-\Ep{heterogbel13}.}
...
Milton Friedman's assertion that competition and survival of the fittest will eventually align the personal beliefs reflected in competitive equilibrium prices of
risky securities with the objective probabilities that generate the data.


\subsection{Mispricing?}

In our example, what drives the divergence outcome is that the consumer with  less accurate beliefs ``pays too much'' when buying  insurance
...
agents have different priors.  They construct examples in which    agents with either
 a looser  or a less accurate prior receive  equilibrium allocations that approach zero asymptotically.
 Relative entropies again play a key role.
 %in their analysis.
...
homogeneous beliefs  of preference specification \Ep{eq0}.  Equivalence of  Pareto optimal allocations to competitive equilibrium allocations
 also applies to the heterogeneous beliefs setting of this appendix.
...
economies in which it is  consumption of the consumer with {\it less\/} accurate beliefs grows over time.\NFootnote{See
Blume and Easley (2006) and
Cogley, Sargent, and Tsyrennikov (2014). }
...
economies with two consumers, one good, and incomplete markets.
 Beker and Chattopadhyay prove that so long as an equilibrium allocation remains effectively constrained
 by market incompleteness, either (a) the consumption
of both consumers is arbitrarily close to zero infinitely often,
or (b) the consumption of one consumer
converges to zero. Further more, this finding  prevails  whether or not
beliefs are  heterogeneous. Moreover, a consumer whose consumption eventually
vanishes can actually be marginally {\it more}  patient or have  marginally {\it  more}
accurate beliefs than another consumer whose consumption remains positive.  These perplexing outcomes stand in contrast to
...
consumer's consumption from vanishing:  even the most unlucky  run of
...
an incomplete-market economy with  Markov endowments provides comfort  for the way we shall compute
stationary equilibria  of Bewley models of
chapter \use{incomplete}.}

A possible limitation of  Beker and
Chattopadhyay's  analysis  is that they assumed equilibrium allocations in which 
consumers' Euler inequalities  
 always hold with equality. But   in
an incomplete-market economy  in which  Ponzi schemes
are ruled out by imposing ad hoc borrowing limits, a consumer's Euler inequalities 
will occasionally be  strict  (see chapter \use{selfinsure}).
...
economy depend  on worst-case endowmwnt scenarios, i.e., they require  that  if
...
There are equal measures of   representative consumers    of two types, indexed by $i=1,2$.
A type  $i$ agent  has preferences ordered by
$$ U_i({\bf c}^i) =
   \sum_{t=0}^\infty \sum_{s^t} \beta_i^t 
               \,u_i\left(c_t^i(s^t)\right)\,
   \pi_t^i(s^t),  \EQN Beker0 $$
where  $u_1(c)=c^{1-\gamma}/(1-\gamma)$,
for $\gamma\in(0,1)$ and $\gamma> 1$, and  $u_2(c) = \ln (c)$. Thus, depending on the value of
$\gamma$, type 1 consumers are either 
more or less risk-averse than type 2 consumers. 
For all $t\geq0$, and all $s^t$ the true
probabilities $\pi_t(s^t)>0$ and the subjective probabilities
$\pi_t^i(s^t)>0$ for all $i$.
The aggregate endowment is stochastic and always strictly positive:
...
payouts $\mu_{t+1}(s^{t+1})>0$ for all $t\ge0$ and all $s^{t+1}$.
One unit of the asset acquired in period $t$ at history $s^t$ bears
 gross payout $\mu_{t+1}(s_{t+1}, s^t)$ at $t+1$  when
...
of period $t$, history $s^t$, including his endowment, i.e.,
$$ a^i_t(s^t) = \mu_t(s^t) b^i_t(s^{t-1}) + y^i_t(s^t).  \EQN Beker2
$$
(This differs from our definition of $a^i_t(s^t)$
in the body of this chapter
that excluded current period's endowment)
Agent $i$'s choice of asset $b^i_{t+1}(s^t)$ satisfies an Euler equation
$$
p_t(s^t) = \beta_i {\sum_{s^{t+1} | s^t} 
\pi^i_{t+1,t}(s^{t+1}| s^t)
...
are \NFootnote{We shall guess and  verify that consumer $2$'s
...
term, while the second term is a function of prices that  the consumer takes as given. 
After substituting the budget constraint \Ep{Beker1} and the law of motion for wealth
\Ep{Beker2}  into the utility function and  next
period's value function, the Bellman equation becomes
$$\EQNalign{
&v_t(a^2_t(s^t),s^t) = \max_{b^2_{t+1}(s^t)} \Biggl\{
\ln\left(a^2_t(s^t) - p_t(s^t) b^2_{t+1}(s^t)\right)            \cr
& \hskip1cm
  + \beta_2  \sum_{s^{t+1}|s^t}  \pi_{t+1,t}^2(s^{t+1}|s^t)
\left(  { \ln\left(\mu_{t+1}(s^{t+1}) b^2_{t+1}(s^{t})\right) \over 1-\beta_2 }
        + \hat v_{t+1}(s^{t+1}) \right) \Biggr\}.      \hskip.5cm (\star)   \cr}
$$
 Taking a first-order condition
with respect to $b^2_{t+1}(s^t)$ and rearranging, yields
asset decision rule \Ep{Beker4;b}, which when substituted  into
budget constraint \Ep{Beker1}, yields consumption decision rule
\Ep{Beker4;a}. Next, substitute this optimal choice into the right
side of $\rm{(\star)}$  and rearrange to get 
$$\EQNalign{
v_t(a^2_t(s^t),s^t) =& { \ln\left(a^2_t(s^t)\right) 
\over 1-\beta_2 }
+ \ln(1-\beta_2) + \beta_2  \sum_{s^{t+1}|s^t}  
\pi_{t+1,t}^2(s^{t+1}|s^t) \cr
& \cdot \left( {1 \over 1 - \beta_2} \ln\left( \beta_2 {\mu_{t+1}(s^{t+1}) \over p_t(s^t)}\right) + \hat v_{t+1}( s^{t+1}) \right) \equiv
{ \ln\left(a^2_t(s^t)\right) \over 1-\beta_2 } +  \hat v_t(s^t),  \cr}
$$
which is the form of value function that we had guessed. Another way
to demonstrate the optimality of decision rules \Ep{Beker4}, is to consider
a corresponding finite-horizon problem, deploy backward induction, and  compute
horizon-dependent value functions  and  decision rules. As
the horizon  goes to infinity,  decision rules 
...
for next period's consumption $c^2_{t+1}(s^{t+1})$ into consumer $2$'s
Euler equation \Ep{Beker3} and  invoking law of motion \Ep{Beker2} for
wealth  gives
$$ \EQNalign{
p_t(s^t) &= \beta_2 {\sum_{s^{t+1} | s^t} 
\pi^2_{t+1,t}(s^{t+1}| s^t)
\mu_{t+1}(s^{t+1}) [(1 -\beta_2) \mu_{t+1}(s^{t+1}) b^2_{t+1}(s^t)]^{-1}
         \over [c^2_t(s^t)]^{-1} }                                \cr
  &= \beta_2 { [(1 -\beta_2) b^2_{t+1}(s^t)]^{-1}
             \;\sum_{s^{t+1} | s^t} \pi^2_{t+1,t}(s^{t+1}| s^t)
         \over [c^2_t(s^t)]^{-1} }                                 \cr
  &= \beta_2 {\mu_{t+1}(\tilde s^{t+1}) [c^2_{t+1}(\tilde s^{t+1})]^{-1}
         \over [c^2_t(s^t)]^{-1} },                  \EQN Beker5  \cr}
$$
for any $\tilde s^{t+1} | s^t$. Equality \Ep{Beker5} asserts  that
%is constant for all
%$s_{t+1}$, i.e.,
 $\mu_{t+1}(s^{t+1}) [c^2_{t+1}(s^{t+1})]^{-1}$, being the product of two $s^{t+1}$-measurable random variables,  
equals $\beta_2^{-1} p(s^t) c_t^2(s^t)$, an $s^t$-measurable random variable. This ``measureability'' restriction is a tell-tale sign of market incompleteness. %Therefore, we are free to represent
...
To characterize  equilibria, it is useful to
...
\over   \sum_{s^{t} | s^{t-1}} \pi^i_{t,t-1}(s^{t}| s^{t-1})
\mu_{t}(s^{t}) [c^i_{t}(s^{t})]^{-\gamma} },       \EQN Beker8
$$
for $\tilde s^{t} | s^{t-1}$ and $i=1,2$, which evidently all have 
%The random variable $\xi^i_t(\tilde s^{t})$ has 
conditional means equal to  one.
...
\over   \sum_{s^{t} | s^{t-1}} \pi^1_{t,t-1}(s^{t}| s^{t-1})
\mu_{t}(s^{t}) [Y_{t}(s^{t})
          - (1 -\beta_2) \mu_{t}(s^{t}) b^2_{t}(s^{t-1})]^{-\gamma} }
                                                            \EQN Beker9
$$
for $\tilde s^{t} | s^{t-1}$.
We shall use the random variables defined in \Ep{Beker8} to study how beliefs affect allocations
in various settings. 

\subsection{Asset payout correlated with i.i.d.\ aggregate
            endowment}\label{sec:Beker_example}%
\noindent
We now study a special economy in which asset payouts equal  the
aggregate endowment, $\mu_t(s^t)=Y_t(s^t)$ and in which  $Y_t(s^t)$ is
an i.i.d.\ process, so that $Y_t(s^t) = Y(s_t)$ with probability
$\pi_t(s^t)=\pi(s_t)>0$, for $s_t\in S$. We assume that agent $i$ knows that $\mu_t(s^t)$ is an i.i.d.\ process and that his subjective
...
$$
where the second equality  invokes the assumption that  the aggregate
endowment is an i.i.d.\ process.
...
$$ \EQNalign{
&\beta_1 {\sum_{s^{t+1} | s^t} \pi^1_{t+1,t}(s^{t+1}| s^t)
...
$$
where the expression for consumer $2$ is taken from \Ep{Beker5}.  Because the product
$\mu_{t+1}(\tilde s^{t+1}) [c^2_{t+1}(\tilde s^{t+1})]^{-1}$ is
constant across realizations of $\tilde s^{t+1}| s^t$,
 we can select any $\tilde s^{t+1}|s^t$. We 
let $\tilde s_{t+1}$ denote the particular event that happens to be 
 realized at time $t+1$,  so  the realized history
becomes $\tilde s^{t+1}$.
Next, multiply and divide the left side of \Ep{Beker11} by
$Y_{t+1}(\tilde s^{t+1}) [c^1_{t+1}(\tilde s^{t+1})]^{-\gamma}$ to get 
$$
...
{ [c^1_{0}(\tilde s^{0})]^{\gamma} \over c^2_{0}(\tilde s^{0}) },
                                                         \EQN Beker13  \cr }
$$
where the second equality is obtained by iterating. We have again 
invoked the assumption of an i.i.d.\ $\{s_t\}$ stochastic process so that
$\xi^1_t(\tilde s^{t}) = \xi^1(\tilde s_t)$, as defined in \Ep{Beker10}.

Given  preference parameter $\gamma\neq 1$ for consumer $1$,
...
It follows from \Ep{Beker13} that if  $\beta_1=\beta_2$, so that  both types of  consumer share the same
discount factor,  and if both  hold the same true beliefs
...
$c^2_T(s^T) \rightarrow 0$ as $T$ goes to infinity. Thus, 
consumer $2$'s consumption vanishes with probability one. In contrast, in a complete
market economy, a person's consumption at a given point in  time  would  be a fixed fraction of  the aggregate endowment
 and that a  time-invariant Pareto weight associated with a  competitive equilibrium allocation assigns to him.

Actually,  since subjective
probabilities $\{\pi^2(s); s\in S\}$ are absent from equilibrium
expression \Ep{Beker13},  our finding in this example does not depend
on probability  beliefs held by consumer $2$. For the sake of  argument, suppose
that consumer $2$ holds  true beliefs but that consumer $1$ has incorrect
beliefs. As shown above, for
$\{\pi^1(s); s\in S\}$ sufficiently close to $\{\pi(s); s\in S\}$,
consumer $2$'s consumption  eventually vanishes.
This  differs from the outcome in the \use{sec:heterogbel_example} complete market
economy in which  the consumer with the true beliefs  eventually consumes the
entire aggregate endowment.

Using  the same line of reasoning, another implication of \Ep{Beker13}
is that even if consumer $1$ is marginally less patient than consumer $2$, so that 
...
%regardless of whether beliefs are homogeneous or heterogeneous.
...
We now assume that the preference parameter
of consumer $1$
is $\gamma=1$ so that  both consumers have a logarithmic utility function; we also assume  that  discount factors are identical, so that  $\beta_1 = \beta_2 = \beta$.
According to \Ep{Beker10}, $\xi^1(\tilde s)=1$ for all $\tilde s\in S$
and hence, the consumption ratio $c^1_t(\tilde s^t) / c^2_t(\tilde s^t)$
in \Ep{Beker13} remains constant over time. In this special 
...
utility are being  evaluated from the perspective  of true
probabilities.\NFootnote{Calling it `beneficial' is controversial.
A consumer with subjective probabilities
that differ from the true probabilities would at each  point in time
would  make other choices if there were complete rather than
incomplete markets. Thus, that  consumer is perpetually  feels  deprived of preferable options that he could have
chosen under complete markets. An argument  in favor of the
...
consumption that will actually  be realized. That is, there is no loss
of utility {\it per se} from a deluded consumer's holding dismal subjective
beliefs about future consequences of having had to make choices from a
restricted set of options. Market incompleteness is 
beneficial because it  compels the consumer period-by-period to choose a better
allocation, at least as measured by  expected utility under the true probability
distribution.}$^,$\NFootnote{See Blume et al. (2018) for further analysis of situations and senses 
in which market incompletion is beneficial.} 
\auth{Cogley, Timothy}%
\auth{Sargent,  Thomas J.}%
\auth{Tsyrennikov, Viktor}%
\auth{Blume, Lawrence}%
...
correct beliefs. Recalling  our sections
...
$(\lambda_1, \lambda_2)$ constitute an incomplete market equilibrium allocation,
...
i.e., consumer $2$ purchases one unit of the asset with asset payouts
...
assets $b^2_2(s^1)= \beta a^2_1(s^1)/p_1(s^1)=1$. This process  continues.
Each period, consumer $2$ lends
...
$(\lambda_1, \lambda_2)=(\beta, 1-\beta)$. This construction involves
no mathematical expectations, so  consumers' beliefs are irrelevant.\NFootnote{Are  consumers' beliefs really irrelevant here?  Actually, our assertion that no expectations are 
involved  is subject to  a qualification best seen by replacing consumer $2$'s Euler
equation in the construction in the text  with that of consumer $1$  given
in  \Ep{Beker3}. This switch puts mathematical  expectations back on the table, as they are  for consumer $2$ in \Ep{Beker5}.  However,  these
expectations now actually  vanish for consumer $1$ when the product
$\mu_{t+1}(s^{t+1}) [c^1_{t+1}(s^{t+1})]^{-1}$ is  constant across realizations of next period's stochastic
event $s_{t+1}$.}

How can it be, with or without
correct beliefs, that an  equilibrium
allocation of the incomplete market economy equals the allocation  of the complete market economy
with correct beliefs? Starting with  correct beliefs, we
...
implies that an efficient allocation prescribes that each
 person  consumes a constant share of the aggregate endowment
in all periods and all states. Consequently,  it is  important
that the exogenously specified payouts of the single asset in the
incomplete market economy are perfectly correlated with the
aggregate endowment.

What  role is played by the exogenous package  of
Arrow securities composing  the single asset in the
incomplete market economy?  How can that package 
support the same  allocation that would prevail in  a complete
market economy? The answer is that consumer $1$ owns the entire
aggregate endowment after period $0$, so that efficient
risk sharing  here  has  each type of agent  
consume a constant fraction of the aggregate endowment. That  can be achieved
by trading Arrow securities for each possible
state next period in proportion to the realizations of the
aggregate endowment in those states.  Trades  in
  the  bundle
...
\noindent{\it Exercise \the\chapternum.1} \quad {\bf Family economics, I}
\medskip
\noindent There is one consumption good and one input, labor.
 A family has two members named $1$ and $2$.  The family is run by person $1$.  His welfare function
is
$$ \lambda_1 \log c_1 + \lambda_2  [\log c_2 - n_2 ] , $$
where $\lambda_1$ and $\lambda_2$ are positive Pareto weights,  $c_1, c_2$ are  consumption of  person $1$ and person $2$, respectively, and $n_2$ is labor supplied by person $2$. Person $1$ has no  labor to supply  but is endowed
with $s > 0$ units of the consumption good.  Feasible allocations satisfy
$$ c_1 + c_2 \leq n_2 + s .$$

\medskip

\noindent{\bf a.} Formulate the Pareto problem as a Lagrangian.

\medskip
\noindent{\bf b.} Solve the Pareto problem for an optimal allocation and Lagrange multiplier.

\medskip
\noindent {\bf c.}  Interpret  the Lagrange multiplier as a ``shadow price'', and tell the object of which it is the shadow price.

\medskip
\noindent{\bf d.} Describe how the family could be reorganized as a competitive economy, being careful to identify an initial distribution of
property and a price system.

\medskip

\noindent{\bf e.} Compute a competitive equilibrium of the economy that you identified in part {\bf d.}

\medskip
\noindent{\bf f.} Please tell how $n_2$ would respond to different values of $s$.

\medskip

\noindent{\it Exercise \the\chapternum.2} \quad {\bf Family economics, II}
\medskip
\noindent Modify the economy in exercise \the\chapternum.1 in the following way only.  Instead of being endowed with $s$ units of consumption, household
$1$ is endowed with $1$ unit of the consumption good; and one unit of labor now produces $s n_2$ units of the consumption good, where $s >0$.  So a feasible
allocation now satisfies
$$ c_1 + c_2 \leq s n_2 + 1 .$$
Preferences are identical with those described in exercise  \the\chapternum.25.  Please answer counterparts of parts {\bf a} -- {\bf f} for this family.


\medskip

\noindent{\it Exercise \the\chapternum.3}\quad {\bf Existence of
representative consumer}
\medskip
\noindent Suppose consumers 1 and 2 have one-period utility
functions $u(c_1)$ and $w(c_2)$, respectively, where $u$ and $w$
are both increasing, strictly concave, twice differentiable
functions of a scalar consumption rate. Let $c > 0$ be the total amount the single consumption good available to be allocated
between consumers 1 and 2. Where $\theta \in (0,1)$ is a Pareto weight, consider the Pareto
problem:
 $$v_\theta(c) = \max_{\{c_1, c_2\}} \left[ \theta u(c_1)
   + (1-\theta) w(c_2) \right]$$ subject to
the constraint $c_1 + c_2 = c$.  Show that the solution of this
problem has the form of a concave utility function $v_\theta(c)$,
which depends on the Pareto weight $\theta$. Where $\{c_1(c,\theta), c_2(c,\theta)\}$ is a Pareto optimal allocation,
show that
$v_\theta'(c) = \theta u'(c_1(c,\theta)) = (1-\theta) w'(c_2(c,\theta))$.

The function $v_\theta(c)$ is the utility function of a {\it
representative consumer}.  A representative consumer always
...
%\vfil\eject
\medskip
\noindent {\it Exercise \the\chapternum.4} \quad  {\bf Term
...
\medskip
\noindent{\bf a.} Define a competitive equilibrium with once-and-for-all trading of all goods at 
 time $0$,
 being careful
to name all of the objects of which it consists.
\medskip
\noindent{\bf b.}  Tell how to compute a competitive equilibrium with time $0$ trading of all goods.
\medskip
...
\medskip
\noindent{\it Exercise \the\chapternum.5} \ \  An economy consists
...
consumption good   $0, 1, 1, 0, 1, 1, 0, \ldots$.  Assume that there are complete markets with time 0 trading.

%at time $0$ there are complete markets for claims to the 
%consumption good at times $t+j$ for $j = 0, 1, \ldots$.
...
\medskip
\noindent{\it Exercise \the\chapternum.6} \ \  Consider a pure
...
one-period time $t+1$ state-contingent
 Arrow securities in units of time $t$ consumption.)
   Define
 ``natural borrowing limits'' for each consumer in each state.
...
\noindent{\bf i.}  Within the one-period Arrow securities economy,
  a new asset is introduced.     One of the consumers
decides to market a one-period-ahead riskless claim to one unit
of consumption. Compute the equilibrium
price of this security when $s_t = 0$ and when    $s_t =1$.
Justify your formula for these prices in terms of first
principles.

\medskip
\noindent{\bf j.}  Within the one-period Arrow securities
equilibrium,  a new asset is introduced.     One of the consumers
...
\noindent{\it Exercise \the\chapternum.8} \quad {\bf Optimal taxation}
\medskip
\noindent
...
distribution $\pi_0$.   Let $\pi_t(g^t)$ denote the probability of
 history $g^t= g_t, g_{t-1} , \ldots , g_0$, conditioned on
...
contingent on  history $g^t$ at a price $p_t^0(g^t) = \beta^t
...
R_t(g^t) $ at time $t$.  The 
 value of its expenditures
must not exceed  the value of its revenues, where  values are calculated using
Arrow-Debreu prices to evaluate state-contingent streams of consumption goods.
...
\medskip
\noindent{\it Exercise \the\chapternum.9} \quad {\bf A competitive
equilibrium}
...
\noindent{\it Exercise \the\chapternum.10} \quad {\bf Corners}
\medskip
\noindent
...
\medskip
\noindent{\it Exercise \the\chapternum.11}\quad {\bf Equivalent
martingale measure}
\medskip\noindent
...
\medskip\noindent
Show that the asset price  in (1) of  exercise \the\chapternum.11 can also be
represented as
...
\medskip
\noindent{\it Exercise \the\chapternum.13}\quad {\bf Early
resolution of uncertainty}
\medskip\noindent
An economy consists of two consumers named $i=1,2$.   Each
consumer evaluates streams of a single consumption good according
...
y^i(s_t) $.  The consumers' endowments of the one nonstorable
...
stochastic matrix
$$P = \left[\matrix{ 1 & 0 & 0 \cr .5 & 0 & .5 \cr 0 & 0 & 1\cr}\right].$$  The endowments of the two consumers
are
...
of the consumers.  (Note: Usually this would be an impossible
task by hand, but this problem has been set up to make the task
manageable.)

\medskip
\noindent{\it Exercise \the\chapternum.14}\quad donated by Pierre-Olivier
Weill
\medskip
...
\medskip
\noindent{\it Exercise \the\chapternum.15} \quad {\bf Diverse beliefs, I}
\medskip
\noindent
...
\medskip
\noindent{\it Exercise \the\chapternum.16} \quad {\bf Diverse beliefs, II}
\medskip
\noindent
...
\medskip
\noindent{\it Exercise \the\chapternum.17} \quad {\bf Diverse beliefs, III}
\medskip
...
\medskip
\noindent{\it Exercise \the\chapternum.18} \quad {\bf Diverse beliefs, IV}
\medskip
\noindent
...
\medskip
\noindent{\it Exercise \the\chapternum.19} \quad {\bf Diverse beliefs, V}
\medskip
\noindent
 A pure exchange economy is populated by two consumers. Consumer $i$ has preferences over history-contingent
consumption sequences $\{c_t^i(s^t)\}$ that are ordered by
...
\medskip
\noindent{\it Exercise \the\chapternum.20} \quad {\bf Risk-free bonds}
...
\noindent{\it Exercise \the\chapternum.21} \quad {\bf Entropy}
...
\noindent{\it Exercise \the\chapternum.22} \quad {\bf Heterogeneous  beliefs again}
...
\noindent{\it Exercise \the\chapternum.23} \quad {\bf Recursive version of Pareto problem}
...
\noindent{\it Exercise \the\chapternum.24} \quad {\bf Recovering probabilities from prices}
\medskip
...
\medskip
\noindent{\bf Note:} For more about exercises of this type, see exercise 2.2. and  Ross (2015).

\auth{Ross, Stephen A.}%
\medskip

\noindent{\it Exercise \the\chapternum.25} \quad {\bf Recovering probabilities from prices, II}
\medskip
...
\noindent{\it Exercise \the\chapternum.26} \quad {\bf Recovering probabilities from prices, III}
\medskip
\noindent Consider a representative agent economy with an exogenous endowment of a single consumption good governed by a two-state Markov chain
with state $s_t \in \{ 1, 2\}$ having transition  matrix $P$ with typical element $P_{ij} = {\rm Prob} (s_{t+1} = j | s_t = i)$.
...
\noindent{\it Exercise \the\chapternum.27} \quad {\bf Corners}
\medskip


\noindent Consider an economy with  exogenous endowments governed by a Markov chain with  matrix $P = \bmatrix{ \lambda & (1-\lambda) \cr
                                                                                                                 1 - \delta & \delta } $
 that governs transitions of a state $s_t \in \{\bar s_1, \bar s_2\}$, where $\lambda \in (0,1), \delta \in (0,1)$. The initial probability distribution of $s_0$
is $\pi_0$. The Markov chain induces a sequence $\{\pi_t(s^t)\}_{t=0}^\infty$  of distributions over histories $s^t = [s_t, s_{t-1}, \ldots, s_0]$.
There are equal numbers of  two types of consumers. A consumer of type $i$ receives an exogenous endowment $y^i(s_t) \geq 0$ of a nonstorable consumption good at time, Markov state $t, s_t$.
Consumers of type 1 order consumption
streams of the one good according to
$$ V_ 1 = \sum_{t=0}^\infty \sum_{s^t} \beta^t c^1_t(s^t) \pi_t(s^t) $$
and consumers of type 2 order consumption streams according to
$$ V_2 = \sum_{t=0}^\infty \sum_{s^t} \beta^t \ln(c^2_t(s^t)) \pi_t(s^t) $$
where $c_t^i(s^t)\geq 0$ is the consumption of a type $i$ consumer and
$\beta \in (0,1)$ is  a common discount factor. Please note the
nonnegativity constraint on consumption of each person (the force
of this is that $c^i_t$ is {\it consumption\/}, not {\it
production\/}).   The consumption good is tradable but
...
\noindent{\bf a.} A Pareto planner chooses a consumption allocation to maximize the criterion
$$ \alpha V_1 + ( 1 - \alpha) V_2   ,$$
where $\alpha \in (0,1)$.  Maximization is subject to the feasibility restrictions $ \sum_i c_t^i(s^t) \leq \sum_i y^i(s_t)$ for all $t \geq 0$
for all $s^t$.
Please characterize a Pareto optimal allocation and get as far as you can in computing it.
 Under what circumstances is the constraint $c_t^i(s^t) \geq 0$ binding? When it is binding,
how is the optimal  allocation related to the aggregate endowment $y^1(s_t) + y^2(s_t)$?
\medskip

\noindent{\bf b.} Please define a competitive equilibrium with trading each period $t \geq 0$ at each history $s^t$  of a complete set of Arrow one-period-ahead state-contingent securities,
being careful to include descriptions of all objects that comprise a competitive equilibrium.

\medskip
\noindent{\bf c.}  Compute all objects that comprise a competitive equilibrium with trading each period $t \geq 0$ at each history $s^t$  of a complete set of Arrow one-period-ahead state-contingent securities under the following alternative auxiliary assumptions:

\medskip
\noindent {\bf Auxiliary Assumption 1:} The equilibrium allocation is such that the restrictions $c_t^i(s^t)\geq 0$ never bind for any $i$.

\medskip

\noindent {\bf Auxiliary Assumption 2:} The equilibrium allocation is such that the restrictions $c_t^i(s^t) \geq 0$ always bind for some $i$.
...
\noindent{\it Exercise \the\chapternum.28} \quad {\bf Recovering probabilities from prices, IV}
\medskip


\noindent
Consider an economy with an exogenous endowment governed by a Markov chain with transition matrix $P = \bmatrix{ \lambda & (1-\lambda) \cr
                                                                                                                 1 - \delta & \delta } $
 that governs transitions of a state $s_t \in \{\bar s_1, \bar s_2\}$, where $\lambda \in (0,1), \delta \in (0,1)$. The initial distribution over $s_0$
is $\pi_0$. The Markov chain induces a sequence $\{\pi_t(s^t) \}_{t=0}^\infty$  of distributions over histories $s^t = [s_t, s_{t-1}, \ldots, s_0]$.  There is a representative
consumer who orders consumption plans according to
$$ \sum_{t=0}^\infty \sum_{s^t} \beta^t u(c_t (s^t) ) \pi_t(s^t)  $$
where $u(c) = (1-\gamma)^{-1} c^{1-\gamma}$ and $\gamma > 0$ and $\beta \in (0,1)$. There is one consumer called the representative consumer.  The representative
consumer's endowment is $y_t = \bar y_1 >0 $ if $s_t = \bar s_1$ and  $y_t = \bar y_2 >0 $ if $s_t = \bar s_2$.




\medskip
\noindent{\bf a.} Define a   competitive equilibrium with trading at each date, history pair $t, s^t$ of one-period-ahead Arrow state contingent securities.  Please  define all components
of a competitive equilibrium, including the allocation and the price system.

\medskip

\noindent {\bf b.}  Show that equilibrium  one-period Arrow state-contingent security prices can be arranged into a $(2 \times 2)$ matrix $Q^{(1)}$ whose  $(i,j)$ component $Q^{(1)}_{i,j}$
is the price of one unit of consumption when Markov state $s_{t+1} = \bar s_j$   tomorrow  when the Markov state $s_t$ today is in state $\bar s_i$.
Please give formulas for all elements of $Q^{(1)}$ in terms of the fundamental parameters of the economy $\beta, \gamma, \lambda, \delta, \bar y_1, \bar y_2$.

\medskip

\noindent{\bf c.}  Define a competitive equilibrium with trading at each date, history pair $t, s^t$ of both one-period-ahead and two-period-ahead Arrow state contingent securities.
Let  two-period Arrow state-contingent security prices be arranged into a $(2 \times 2)$ matrix $Q^{(2)}$ whose  $(i,j)$ component
$Q^{(2)}_{i,j}$ is the price of one unit of consumption when Markov state $s_{t+2} = \bar s_j$   two periods ahead  when the Markov state $s_t$ today is in state $\bar s_i$.
Please give formulas for all elements of $Q^{(2)}$ in terms of the fundamental parameters of the economy $\beta, \gamma, \lambda, \delta, \bar y_1, \bar y_2$.

\medskip

\noindent{\bf d.} (``inverse problem'')  An outsider observes this economy. The outsider knows the theoretical structure of the economy but does not know the parameter values $[\lambda, \delta, \beta,
\gamma, \bar y_1, \bar y_2]$. But at a particular date at which $s_t = \bar s_1$, the outsider observes
$$[Q^{(1)}(1,1), Q^{(1)}(1,2), Q^{(2)}(1,1), Q^{(2)}(1,2)] .$$
Please interpret these observations.
From these observations
alone, can the outsider infer $\lambda, \delta,  \beta$?   Please explain your answer.

\medskip

\index{law of iterated values}%
\noindent{\it Exercise \the\chapternum.29} \quad {\bf Law of Iterated Values}
\medskip

\noindent Consider a pure exchange  economy in which aggregate endowments are a time-invariant function of a Markov state $s$
governed by a time-invariant Markov chain with transition probability matrix $P$.  Assume a competitive equilibrium  with 
complete markets in one-period Arrow securitites with pricing kernel assembled into an   $n \times n$ matrix


$$
Q_{ij} = {\rm{Price}} \left\{s_{t+1} = \bar{s}_j \mid s_t = \bar{s}_i \right\}.
$$


\medskip
\noindent{\bf a.} Argue that the implied  $(i,j)$ component of  the $k$-step ahead pricing kernel  is 
$$
Q_k(s_{t+k} = \bar s_j | s_t = \bar s_i)   = Q^{k}_{i,j}
$$
where $Q^{k}_{i,j}$ is the $(i,j)$ component of the matrix $Q^k$, the $k$th power of the matrix $Q$.
{\it Hint:} Feel free to deploy an arbitrage-free pricing argument.

\medskip

\noindent The time $t$ {\it value} in Markov state $s_t$  of a time $t+j$  payout $d(s_{t+j})$
is 
$$
...
\medskip
\noindent{\bf b.}
The law of iterated values asserts that
$$
V \left[ V (d(s_{t+j}) | s_{t+1}) \right] | s_t  =   V(d(s_{t+j}))| s_t
$$
Please prove  the law of iterated values.

\medskip
\noindent{\bf c.} Compare the law of iterated values  with the law of iterated expectations.

\medskip

\noindent{\bf d.} Suppose now a pure exchange economy in which endowments are history dependent and  $s_t$
is not Markov. Does a law of iterated values still hold?  Prove your assertion.


\medskip

\index{law of iterated values}%
\noindent{\it Exercise \the\chapternum.30} \quad {\bf Natural borrowing limit?}
\medskip

\noindent  Where $c_t$ is consumption at date $t$ and $b_t$ is one-period debt owed at time $t$,
...
\noindent{\it Exercise \the\chapternum.31} \quad {\bf Alternative borrowing limits}
\medskip
...
\noindent{\it Exercise \the\chapternum.32} \quad {\bf Natural borrowing limits}
\medskip

\noindent This exercise is designed to  indicate why it is {\it not\/} necessary to impose ``natural borrowing limits'' 
...
\noindent{\it Exercise \the\chapternum.33} \quad {\bf Two types}
\medskip

\noindent Consider an infinite-horizon pure exchange economy that exists at $t=0, 1, \ldots $.
...
\medskip
\noindent{\bf c.} Define an equilibrium with sequential trades of a complete set of one-period Arrow  securities.
\medskip
...
\noindent{\it Exercise \the\chapternum.34} \quad {\bf Two types}
\medskip
...
\noindent{\it Exercise \the\chapternum.35} \quad {\bf Markov strikes again}
\medskip

\noindent 
To streamline things, we assume that the aggregate endowment of a nonstorable consumption good is a time invariant function $y_t = y(s_t)$
of a Markov state $s_t \in {\bf S}$ governed by a Markov chain $(P, \pi_0)$.  A representative
consumer considers alternative consumption profiles $\{c_t(s_t)\}_{t=0}^\infty$ that are functions of the Markov state.
...
After $s_0$ has been realized, there are omplete markets at time $0$, in date- and Markov state-contingent
