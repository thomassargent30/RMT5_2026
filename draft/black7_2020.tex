...
The consumer's problem is to choose  $c^i$
...
In this example, we assume that a common  one-period utility function is $\frac{c^{1-\gamma}}{1-\gamma}$.
...
%for $t > 2$.
If we use these prices to compute the value of agent $i$'s endowment, we obtain $.5 \sum_{t=2}^\infty \beta^t + .5 (1 + \beta) = .5 \sum_{t=0}^\infty
...
retain the ownership Often their endowment processes throughout time.
...
that
%\noindent
{\bf (a)} given   the pricing kernels and
 $\tilde a^i_0(s_0)$ and  the borrowing limits $\{A_t^i(s^t)$ for all $i$, the consumption allocation
$\tilde c^i$ and portfolio $\{\tilde a^i_{t+1}(s_{t+1},s^t)\}\}$ solves the consumer's problem for all $i$;
and
%\noindent
{\bf (b)} for all realizations of $\{s^t\}_{t=0}^\infty$,
allocations and   portfolios
$\{\tilde c^i_t(s^t), \{\tilde a^i_{t+1}(s_{t+1},s^t)\}\}_i$
satisfy
...
Using this proposition, we can verify that  the natural debt
limits \Ep{cecm_debt1} and consumers' continuation wealth levels \Ep{cecm_wealth} both
exhibit history independence:
$$\EQNalign{
 A^i_t(s^t) &= \bar A^i(s_t) \,,              \EQN MarkovA \cr
 \Upsilon^i_t(s^t) &= \bar \Upsilon^i(s_t)\,. \EQN MarkovU \cr}
$$
Finding \Ep{MarkovU} about  continuation wealth levels  conveys an
...
future realizations of the consumer's endowment process as well as future prices. It can
be shown that a consumer especially wants  higher wealth levels in
those states next period that  either  make his next period
endowment low or more generally  signal poor future prospects for its endowment in
the more distant future.
%Of course, individuals' desires are tempered by differences in the economy's
%aggregate endowment across states (as reflected in equilibrium
%asset prices).
But aggregate shocks cannot be diversified away. An ``invisible hand''
uses the pricing kernel
$Q(s_t\vert s_{t-1})$ and  cleared  markets
to  arrange consumers'
transactions at time $t-1$ so   that they end up  bearing  only aggregate
risk.
...
\noindent   Condition ({\bf c}) asserts that there are  zero net aggregate claims in all Markov states.
 Condition ({\bf d }) asserts that the economy is closed and  starts off from a position in which there 
...
we must impose that $a_0^i = 0$ for $i = 1, \ldots , I$.  That  assures that at time $0$ the present value of each agent's consumption equals the present value of his endowment stream,
the  restriction imposed by equation \Ep{eq2}
in the section \use{sec:ADtime0}  arrangement with all trades occurring at time $0$.

\index{state variable degeneracy}%

%Starting the system off with $a_0^i =0 \ \forall i$ has a striking 
%implication 
The economy has a remarkable property that we can call {\it state variable degeneracy}.
Thus, note that although there are two state variables in the value function $v^i(a,s)$, within a recursive competitive 
equilibrium
starting from $a_0^i = 0 \ \forall i$  at the starting  Markov state  $s_0$, two outcomes  prevail:


\medskip
\item {(i)} Financial wealth $a$ is an exact function of the Markov state $s$. 


\medskip
\item{(ii)} $a_0^i = 0 $ for all $i$ whenever the Markov state $s_t$ returns to   $s_0$. 

\medskip

\noindent Finding (i) asserts that within a competitive equilbrium 
  the exogenous Markov state is all we require to track an individual.  
Financial wealth turns out to be redundant because it is an exact function of the exogenous state $s_t$. 
 This finding depends critically on there being complete markets in Arrow securities and unfolds because 
 in equilibrium continuation wealth can be expressed as a function of $s_t$. 
For more about this issue, please see chapters \use{selfinsure} and \use{incomplete} and sections 
\use{sec:LQmodel} and \use{sec:LQcomplete} 
of chapter \use{timeseriesapp}. 
Finding (ii) asserts that each household  revisits the zero financial wealth state 
with which he began life provided that $s_0$ is a recurrent state of the Markov chain.
...
then set $a^i(s_t) = \Upsilon^i(s^t)$.

\medskip

We can apply a slight generalization of the algorithm that allows time-dependent functions of the Markov state
to compute an equilibrium of the economy described in  example 4 from subsection \use{sec:example4}. 
(Because that example assumes
a time-dependent one-step transition matrix for the Markov state $s_t$, we have to allow functions 
$(a^{i}_t(s), \Upsilon^{i}_t(s))$ to be time-dependent.)
Applying an appropriately modified version of our five step algorithm to example 4 from section \use{sec:example4} gives us
...
that can be offered to a type 2 person when 
a type 1 person is offered at least $v$. Each of these expected values
is to be evaluated {\it before\/} the realization of the state at an initial
date.
...
of the  realization of $s_t$.    The planner dictates
