...
not a continuous function.\NFootnote{In the $d_2$ metric, the limit point of  the sequence  $\{t^n\}$  is not unique; it also converges to $f=0$, which is in $C[0,1]$.  %In a $d_2$ sense as we showed in the text, this $f=0$
 %limit is equivalent with other functions $g$ that are {\it not\/} in $C[0,1]$. %The sequence $\{t^n\}$  also converges to the function equal everywhere to $0$ in the  $d_2$ metric, since $d_2(t^n,0)=\sqrt{1/(2n+1)}$ approaches  $0$  as $n \rightarrow \infty$.
 This just illustrates that the $d_2$ metric generates an  enormous equivalence class in the space of Riemann integrable
functions that is  a superset of $C[0,1]$. If $f$ and $g$ are Riemann integrable on $[0, 1]$ and $f=g$ almost everywhere, then $d_2(f, g)=0$.
 Because in a $d_2$ sense,  $\{t^n\}$ converges to a  function that  is not in $C[0,1]$,  $ \{C[0,1], d_2\}$ is not a  complete metric space.}
...
\medskip
We want a notion of continuity of an operator.
\medskip
%\specsec{Definition:}
...
define the new operator $T_\mu$ as follows: \index{Bellman equation}%
$$T_\mu (v) = T(v)$$
or
