...
markets models  of chapter \use{incomplete}.  This chapter
provides our first encounter with the powerful supermartingale convergence theorem.

To highlight  effects of uncertainty and borrowing constraints, we shall
...
Components of the sequence of endowments are independently and identically
...
The optimal value function $V(a)$  for an agent with $a>0$  satisfies the Bellman equation
\offparens
$$\EQNalign{
...
``Self-insurance'' occurs when the agent uses savings to insure
 against income fluctuations. On the one hand, in response
...
properties of an optimal ``self-insurance'' plan. Will the
...
\use{socialinsurance}.}  Or might the agent eventually become
...
we will show that neither of these outcomes occurs: consumption  diverges
to infinity!\auth{Chamberlain, Gary} \auth{Wilson, Charles}
\auth{Sotomayor, Marilda A. de Oliveira}

We begin by studying the savings
problem under the assumption that the endowment is  a nonrandom sequence  that
does not grow perpetually.  In this case,  consumption {\it does\/} converge.

\section{Nonstochastic endowment}

It is instructive to study the optimal consumption decisions
...
borrowing constraint on one-period Arrow securities that  are
risk free in the current context.  After that, we'll arbitrarily
tighten  the borrowing constraint and impose a no-borrowing
condition $a_{t+1} \geq y_{t+1}$ in the problem posed in the previous section.
...
stringent no-borrowing constraint,  consumption
...
\EQN borrowcon $$
The right side of the inequality is the present value of the endowment, which is the maximal amount that  it is feasible
to  repay at time $t$
when $c_t \geq 0$.
...
\EQN constraint0 $$
When $c_t \geq 0$,
under the natural borrowing constraints,  inequality \Ep{constraint0}
is the only restriction that the sequence of  budget constraints \Ep{self_bc1}
imposes on the $\{c_t\}$ sequence.
   The first-order necessary conditions  for  maximizing
\Ep{selfobj} subject to \Ep{self_bc1} and \Ep{noborr} are % \Ep{constraint0} are
...
is a constant consumption level chosen to satisfy  \Ep{constraint0}
at equality:
...
and shrinkage impinge on consumption plans in the presence of an ad hoc
...
$(1+r)^{-1} b_{t+1}  \leq 0  \
 \forall t \geq 0$.  To facilitate a transition to our subsequent
...
$a_t$ rather than $b_t$  makes it possible to cast the
consumer's optimal value function in terms of the single state variable
...
\medskip
\itemitem{({\bf a}) } $c^*_{t-1} = c^*_t$; or
\itemitem{({\bf b}) } $c^*_{t-1} < c^*_t$ and $c^*_{t-1} = a^*_{t-1}$,
and hence $a^*_{t} = y_t$.
\medskip
\noindent
Condition ({\bf b}) states that the no-borrowing constraint binds only when
...
According to conditions ({\bf a}) and ({\bf b}), $c_{t-1}$ can never exceed $c_t$.
...
to make $c_{t-1}$ equal to $c_t$ as in condition ({\bf a}). Therefore,
consumption increases from one period to another as in condition ({\bf b})
...
about the  optimal consumption plan.
...
binding at $t-1$.   If the no-borrowing constraint  binds
...
the annuity return on  the present value of the tail (i.e., the continuation) of the
income process is maximized. We state this in the following proposition.

\medskip
\noindent{\sc Proposition 1:} Given a no-borrowing constraint and a nonstochastic
...
that $\bar c > \bar x$. Then conditions ({\bf a}) and ({\bf b}) imply that
...
constraint  binds at date $t-1$, consumption will increase to   satisfy
...
income model of chapter \use{timeseriesapp} as a benchmark for the results to come.  Assume as
...
always consume bliss consumption by accumulating debt at a high rate and without
limit.}
...
not binding. A \idx{Benveniste-Scheinkman formula} implies $u'(c)=V'(a)$,
...
Chamberlain and Wilson (2000) extended the result that consumption diverges to infinity with an i.i.d.\
endowment process 
 to an arbitrary stationary
...
Contributions by
Marcet, Obiols-Homs, and Weil (2007) and Zhu (2020) studied how  adding an endogenous labor supply
...
while labor supply converges to zero.   The Marcet, Obiols-Homs, and Weil (2007) and Zhu (2020) analysis provides valuable additional intuition
...
$h_t \in [0, 1]$, where $c_t$ is  consumption of a single good and
$h_t$ is leisure.
...
Following Zhu (2020), we adopt the assumptions that
...
Following Foley and Hellwig (1975) and Zhu (2020), we solve the problem in two steps. Step 1 solves an intratemporal
...
Under assumptions A1-A3, Zhu (2020) shows that  the indirect utility function
...
\medskip
\item{1.} $V_1(A_t, e_t) = R u_1(c_t, h_t) \rightarrow 0$ while $h_t$ remains uniformly bounded away from  one infinitely often with the
outcomes that
$\lim_{t\rightarrow \infty } A_t = +\infty $ and $\lim_{t\rightarrow \infty} c_t = + \infty$; or
\medskip
\item{2.}  $h_t \rightarrow 1 , A_t \rightarrow \bar A, c_t \rightarrow r \bar A$ almost surely.
\medskip
...
Marcet, Obiols-Homs, and Weil (2007) and Zhu (2020) to make  the labor supply decision  endogenous. What drives case 2 is a wealth effect that causes the household eventually
...
Whether case 1 or 2 prevails depends on the shape of the consumer's utility function $u(c,h)$.  Zhu (2020)
...
Zhu (2020) establishes  the following:
...
\section{Concluding remarks}
  ...
  This chapter has maintained an assumption $\beta (1+r) =1$ that 
  is an  important ingredient in helping to  deliver the divergence
...
will be that a social planner has access to risk-free loans from outside
 and seeks to maximize agents' welfare subject to enforcement
...
and information problems a planner would just redistribute the
...
A consumer has preferences over stochastic processes of a single consumption
...
condition $a_0 = y_0$.
One-period assets $(a_t -c_t)$ carried  over into period
$t+1$ from $t$ must be nonnegative, so that a no-borrowing
constraint is $a_t \geq c_t$. At time $t=0$, after $y_0$ is realized,
the consumer devises an optimal consumption plan.
\medskip
\noindent{\bf a.} Draw a tree that portrays  possible paths for
...
\medskip
\noindent{\bf e.} Discuss whether your   results in part {\bf d} conform to
...
\medskip
\noindent  A  pure exchange  economy consists of two
...
in equilibrium, each household simply consumes its endowment always.
...
\medskip
\noindent{\it Exercise  \the\chapternum.6}  \quad {\bf Stochastic discount factors?}
\medskip
