...
agents as a contrast to possibly inefficient equilibrium
allocations in overlapping generations models, which entail
...
Here we have added labor supply $l_t$ as a choice variable and an
input to the production function $F(l_t,k_t)$ in resource 
constraint \Ep{growth1}. The household is endowed with one unit
of time in each period that can be allocated to working, 
$l_t\in [0,1]$. Since the time spent working does not
enter the period utility function, $\ln(c_t)\equiv u(c_t)$,
the planner will trivially set $l_t$ equal to its highest 
value so as to maximize output.
After plugging that optimal value $l_t=1$ into resource 
constraint \Ep{growth1}, the optimization problem becomes 
identical to the one in 
chapter \use{dynamicdp1}.\NFootnote{As will become
evident, the critical 
assumption of our augmented production function in 
equation \Ep{growth1} is that the exponents on labor
and capital add up to one which makes the 
technology to be constant returns to scale,
...
directly yielded the described simplification of the
production function in equation \Ep{growth1}, where
...
As detailed in chapter \use{dynamicdp1}, the optimal solution
...
constraint \Ep{growth1} and, hence, the Euler equation 
...
chapter \use{recurge}, a competitive 
equilibrium in a production economy with infinitely-lived 
agents is Pareto efficient (see chapter \use{growth1}).
There are alternative ways to decentralize the activities 
of households and firms operating the production technology 
in a market economy. Here we adopt a common
...
that labor and capital are hired such that the marginal
product of each input equals the wage rate and the
rental rate, respectively; that is, the following
...
the two production factors. And given our Cobb-Douglas
production function, those factor shares are constant and
...
in models with infinitely-lived agents, we now seek 
to understand why the equilibrium allocation in a 
growth model with overlapping generations might fail 
...
of time as young that can be devoted to working, while old 
...
in applied work using overlapping generations models, because higher 
weights on future utilities generate larger savings which help to 
match aggregate observations on high capital-output ratios.}
The optimally chosen labor supply is once again $l^t_t=1$ since
there is no disutility from working.

The operation of firms and the characterization of equlibrium 
factor prices in expression \Ep{firm_FOC} remain the same as in
the model with infinitely-lived agents. Even the earlier Euler 
...
labor supply $l_{t+1}=1$. But of course, the important
difference is that Euler equation \Ep{olg_Euler} pertains
to a life-cycle optimization problem where the agent is
only concerned about the two periods $t$ and $t+1$. 

Suppose that $u(c)=\ln(c)$. The savings function is
...
equal to one, meaning that when the rate of return on savings
rises, the incentive to consume later (substitution effect) is
exactly offset by the increased wealth that allows for higher
consumption today (income effect). Thus, the rental rate on
capital $\tilde r_{t+1}$ does not affect optimal savings.}
In an equilibrium, the savings of the representative agent
$t\geq 0$ is equal to next period's capital stock $k_{t+1}$:
...
We can solve for the set of parameter values that makes
...
is dynamic inefficiency, i.e., an overaccumulation of capital. 
Specifically, this is more likely the
...
The proof of the proposition is relegated to 
appendix X. But here we use Figure X
for a graphical exposition of possible Pareto improvements 
that moves the economy from an inefficient steady state 
