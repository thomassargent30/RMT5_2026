...
We now pause to stare at equation \Ep{stareatit101} and  think about alternative possible assumptions about what is predetermined at $t$ and
...
implied by decision rule \Ep{stareatit101}, an assertion that we'll soon prove.
 ...
 To construct a financial plan that supports constant consumption, we must let the consumer trade the one-period state-contingent securities 
 called Arrow securities like those to be studied in chapter \use{recurge}.  Thus,  temporarily we take $\bar c$ as given, substitute \Ep{guess101} into
...
In the reverse engineering exercise we are now pursuing, we suppose that equation \Ep{guess102} describes the consumer's optimal portfolio
of one-period Arrow securities issued at $t-1$ that are to be paid off at time $t$ in Markov state $z_t$.
...
We pause here to remark that model \Ep{guess102}-\Ep{guess103} has extinguished the history dependence of both debt and consumption exhibited by 
...
\Ep{guess103}  is obviously not history dependent, while the debt decision given by \Ep{guess102} dependents on $z_t$ but not earlier 
Markov states. To understand what underlies  these outcomes,  we begin by noting that, 
\index{history dependence}%
...
list of one-period state-contingent securities that  pay off amounts next period that depend on next period's information vector $z$.
  ...
  so that they will  induce the consumer to want to smooth consumption completely
...
associated with a permanent income model.} 
In the
