...
be denoted $s^t = [s_0, s_{1}, \ldots, s_{t-1}, s_t]$.\NFootnote{Later in this chapter and in other chapters too,
we shall reverse the recording order and  denote a history as $s^t = [s_t, s_{t-1}, \ldots, s_1, s_0]$.} The unconditional
...
allocations and prices. We discuss related consequences of heterogenous beliefs and   heterogenous discounting in
...
parameters describing the initial distribution of wealth and  the realization of
...
deliveries are to be made.  The consumer's budget constraint is
...
agent $i$ becomes 
$$ \sum_{t=0}^\infty \sum_{s^t}  q_t^0(s^t)  c_t^i(s^t)   \leq  q_0^0(s_0) a_0^i(s_0) \,+\,
...
$$ - a_0^i(s_0) \leq 
 \sum_{t=0}^\infty \sum_{s^t}  {q_t^0(s^t) \over 
                                q_0^0(s_0)} y_t^i(s^t)
...
\subsection{Interpretation of trading arrangement}
  In the  competitive   equilibrium with Arrow-Debreu timing, all trades occur at $t=0$.  Deliveries occur after $t=0$, but no more trades.
...
that each consumer faces a single  budget constraint that restricts  trades across all commodities indexed by  dates and histories.
...
\medskip
\noindent{\bf 2.}  Use \Ep{eq3} for any consumer $i$ to solve for the
price system $q_t^0(s^t)$.
\medskip
\noindent{\bf 3.}  For $i =2, \ldots, I$, check the budget constraint
...
We are free to normalize the price system by setting 
$\mu_i$
%%$\mu_i \alpha_i^{-\gamma}$ 
for one consumer to an arbitrary positive number.
...
This formula asserts that consumer $i$'s fixed consumption share $\alpha_i$ equals its share of aggregate wealth evaluated at the competitive  equilibrium price vector.
...
In this example, 
%we assume that a common  one-period utility function is 
%$\frac{c^{1-\gamma}}{1-\gamma}$.
there are two consumers named $i=1,2$.  Their endowments are $y_t^1 = y_t^2 = .5 $ for $t=0,1$ and
$y_t^1 = s_t $ and $y_t^2 = 1-s_t$ for $t \geq 2$.  The event space $S = \{0,1\}$ and $s_t$ is governed by
...
for $t > 2$.  This specification implies that $\pi_0(1) = 1, \pi_1(1,1) =1$, $\pi_t(1, 1,  \ldots,1, 1, 1) = .5$ and $\pi_t(0, 0, \ldots,  0, 1 ,1 )=.5$
for all $t > 2$, where $s^t = [s_t, s_{t-1}, \ldots, s_1, s_0]$. Thus, for $t \geq 2 $, there are just two possible paths of $s^t$, each with probability
$.5$, namely, $s^t = [ 1, 1, \ldots, 1, 1, 1]$ and $s^t = [0, 0, \ldots, 0, 1, 1]$.  Along the first path, the endowment
of agent $1$ is $y_t^1 = 1$ for $t \geq 2$ and $y_t^1 = .5 $ for $t=0, 1$. Along the second path, the endowment of agent $1$
is $y_t^1 = 0$ for $t \geq 2$ and $y_t^1 = .5 $ for $t=0, 1$.
...
for all $t$ and all $s^t$ that occur with positive probability.  Therefore, an equilibrium price vector is $q_t^0 (s^t) = .5 \beta^t$ for
both possible $s^t$ sequences.
%$q_1^0(1,1) = \beta, q^0_2(0,1,1)= q^0_2(1,1,1)= .5 \beta^2$ and $q_t^0(1, 1, \ldots, 1, 1) = q_t^0(0, 0, \ldots, 1, 1) = .5 \beta^t$
%for $t > 2$.
If we use these prices to compute the value of agent $i$'s endowment, we obtain $.5 \sum_{t=2}^\infty \beta^t + .5 (1 + \beta) = .5 \sum_{t=0}^\infty \beta^t 
 = {\frac{.5}{1-\beta}}$.  We obtain the same value of agent $2$'s endowment.
%
% $\sum_t \sum_{s^t} q_t^0(s^t) y_t^i(s^t)
%=\sum_t \beta^t .5 [.5 + .5 + 0 + \ldots + 0] +  \sum_t \beta^t .5 [.5 + .5 + 1 + \ldots +1] = 2 \sum_t \beta^t .5 [.5 + .5 + \ldots + .5] = .5 \sum_t \beta^t = {.5 \over 1 - \beta}$. %\frac{.5}{1-\beta}$.
  Agent $i$'s budget constraint is satisfied when he consumes a constant
...
endowment and consumption streams for a complete markets economy that originally had convened at $t=0$.
...
$t$, history $s^t$ financial wealth that is implicit in a time $0$ Arrow-Debreu
equilibrium. If markets are reopened at date $t$ after history
$s^t$, no trades  occur: facing   price system
...
the consumer  in the sequential economy with  equilibrium  tail wealth
...
Let $q_\tau^t(s^\tau)$ be an Arrow-Debreu price, denominated  in
...
At time $t$, a consumer chooses $\tilde c^i_t(s^t)$
...
%$\tilde a^i =\{ \tilde a^i_{t+1}(s^{t+1})\}_{t=0}^{\infty}$
$\tilde a^i =
\{\{\tilde a^i_{t+1}(s^t,s_{t+1})\}_{s_{t+1}}
                            \}_{t\geq0,\,s^t}$
and the associated borrowing limits are
%$A^i =\{A^i_{t+1}(s^{t+1})\}_{t=0}^{\infty}$.
$A^i =\{A^i_{t+1}(s^{t+1})\}_{t\geq0,\,s^{t+1}}$.

 \medskip
\noindent{\sc Definition:}  A {\it distribution of wealth} is
a vector $\vec {\tilde a}_t(s^t) = \{\tilde a_t^i(s^t)\}_i$ satisfying
...
an initial distribution of wealth $\vec {\tilde a}_0(s_0)$, 
borrowing limits $\{A^i\}_i$ satisfying 
\Ep{cecm_debt1}, 
%%for all $i$, for all $t$, and for all $s^t$,
an allocation $\{\tilde c^i\}_i$, asset portfolios 
$\{\tilde a^i\}_i$, and pricing
kernels $\tilde Q_t(s_{t+1} | s^t)$ such that
%\noindent
{\bf (a)} for all $i$,
given $\tilde a^i_0(s_0)$, the borrowing 
limits $A^i$ and the pricing kernels, 
the allocation $\tilde c^i$ and asset portfolios
$\tilde a^i$ solve the consumer's problem;
and
%\noindent
{\bf (b)} for all realizations of $\{s^t\}_{t\geq0}$,
allocations and   portfolios
$\{\tilde c^i_t(s^t), 
\{\tilde a^i_{t+1}(s^t, s_{t+1})\}_{s_{t+1}}\}_i$
satisfy
$\sum_i \tilde c^i_t(s^t) = \sum_i y_t^i(s^t)$ and
$\sum_i \tilde a_{t+1}^i(s^t, s_{t+1}) = 0$ 
for all $s_{t+1}$.
...
$\vec {\tilde a}_0(s_0)$ of the sequential-trading economy should be a zero vector, a natural guess because
...
initial wealth vector enables consumer $i$ to finance $\{c^i_t(s^t)\}$
...
$\tilde a^i_0(s_0)=0$, substitution of equation \Ep{cecm_Wcost} into
...
Hence, consumer $i$ does not want to increase consumption at time $t$ by
...
The price system is $Q_0(1\big|1) =\beta$, 
%$Q_0(0\big|1) =0$, 
$Q_1(1\big|1,1) = Q_1(0\big|1,1) = .5 \beta$  and
$Q_t(1\big|1,1,1,\ldots,1) = Q_t(0\big|1,1,0,\ldots,0) 
= \beta $ for $t \geq 2$.
Also, $\Upsilon^i_0(1) = \Upsilon^i_1(1,1)= 0$ for $i=1,2$.  For $t \geq 2$,
$\Upsilon_t^1 (1,1,1, \ldots,1)= \sum_{\tau \geq t} \beta^{\tau - t}(.5 -1) = {-.5 \over 1-\beta}$ and
$\Upsilon_t^2(1,1,1, \ldots,1) = \sum_{\tau \geq t} \beta^{\tau - t}(.5 -0) = {.5 \over 1-\beta}$; and
the reverse outcomes  
$\Upsilon_t^1 (1,1,0, \ldots,0)= {.5 \over 1-\beta}$ and
$\Upsilon_t^2 (1,1,0, \ldots,0)= {-.5 \over 1-\beta}$.
Therefore, in period $1$, the first  consumer trades Arrow securities in amounts $a_2^1(1,1,1) = {-.5 \over 1-\beta},
a_2^1(1,1,0) = {.5 \over 1-\beta}$, while the second consumer trades  Arrow securities in amounts 
$a_2^2(1,1,1) = {.5 \over 1-\beta},
a_2^2(1,1,0) = {-.5 \over 1-\beta}$   After period $2$, the consumers perpetually roll over their debts or assets of either
...
wealth $\vec {\tilde a}_0(s_0)$ is revisited whenever 
the Markov state $s_t$ returns to $s_0$, so that 
$\vec {\bar a}(s_0) = \vec {\tilde a}_0(s_0)$.
\medskip
...
state $s$).\NFootnote{When we compute an equilibrium
...
it would have no effect on an equilibrium  allocation if we were impose that
even in such a current state $s$ consumers 
 have
end-of-period holdings $\{\bar a^i(s')\}_i$ 
...
$\vec {\tilde a}_0(s_0)$. Given those primitives 
...
$\vec {\tilde a}_0(s_0)$. The following example
...
who would like to borrow, i.e.,  $a^2(0)<0$, since he 
has no endowment in period 0, $y^2(s_0)=y^2(1)=0$.
A loan allows consumer 2 to consume
$-Q(0 | 1)\, a^2(0)$ in period 0. It is reasonable 
...
holding of Arrow security $a^2(0)<0$.
...
loans to consumer 2, $a^1(0)=-a^2(0)=a >0$.
...
$\bar c^2$ so long as state $s'=0$ persists. 
Without having to borrow, consumer 2 does this 
by maintaining an unchanged positive holding of 
Arrow securities, $a^1(0)=a>0$. 
...
indebtedness $a^2(0) <0$ does not violate his natural 
debt limit, $-a^2(0) \leq \bar A^2(0)$.
%% where $a^2(0)=-a$. 
...
the natural debt limit, $\bar A^2(0) \geq -a^2(0)$
where $a^2(0)=-a$ as given by \Ep{aREC}, holds:
$$ \EQNalign{
\bar A^2(0) &= {1 \over
     1 - \beta \pi(0|0) - \beta \pi(1|0) 
...
be strictly positive, which  in turn requires that the sum of
...
true:
$$
%%\beta \left(\pi(0|0) + \pi(1|0){\beta \pi(0|1) \over 
%%                              1-\beta \pi(1|1)} \right)
...
%\NFootnote{To verify that
%natural debt limit $\bar A^2(0)$
%is strictly positive, we need to show that the sum of
%the two negative terms in the denominator
%of \Ep{debtlim0_REC} is less than one in absolute value:
%$$
%%\beta \left(\pi(0|0) + \pi(1|0){\beta \pi(0|1) \over 
%%                              1-\beta \pi(1|1)} \right)
%\beta \left(\pi(0|0) + \pi(1|0){\beta [1-\pi(1|1)] \over 
...
$$ v^i(a, s) = \max_{\hat c, \{\hat a(s'),\, \forall s'\}} 
\left\{ u_i(\hat c) + \beta
...
\medskip
\noindent{\bf (b)} For all $i$, the value function
...
\medskip
\noindent  
Condition ({\bf a}) asserts that the decision 
...
decisions. Condition ({\bf b}) confirms that each
...
and future endowments. Condition ({\bf c}) 
...
the counterpart to constraint \Ep{eq10}, the consumer's budget
...
$$
The realization of $s_{t+1}$ determines which element of the portfolio of
one-period-ahead claims $\{z^i_{t,1}( s_{t+1})\}$  pays off at
time $t+1$,
and also the capital gains earned and losses inflicted on  holdings of longer horizon
...
consumer also trade  an ex-dividend ``Lucas (1978) tree''.
...
Recall formula \Ep{eq14} for the price $Q_j(s_{t+j} | s_t)$ of a $j$-period ahead Arrow security in Markov state
...
$$ R_{j,t}(s_{t+1},s_t) = {\frac{p_{j-1,t+1}(s_{t+1}) - p_{j,t}(s_t)}{p_{j,t}(s_t)}} .$$}
$$ p_{j,t}(s_t) = \sum_{s_{t+j} } Q_j(s_{t+j} | s_t) . $$
...
$$  \bmatrix{ v_{1,t} \cr v_{2,t} \cr \vdots \cr v_{I,t} } =
     \bmatrix{ d_{1,t+1} & d_{1,t+2} & \cdots & d_{1,t+\tau} \cr
      ...
      \bmatrix{ p_{1,t}(s_t) \cr p_{2,t}(s_t) \cr \vdots \cr p_{I,t}(s_t) }   $$
or
$$ V_t = D_t P_t . $$
...
consumes the entire endowment and the other type consumers nothing.
...
where $\epsilon\in(0,0.5)$ is an arbitrarily small number.
...
\subsection{Recursive version of Pareto problem}
 We radd heterogeneous beliefs to the section \use{sec:RecursivePareto} recursive  formulation of a
Pareto problem.
