...
Then the   marginal condition \Ep{eq3b} or \Ep{eq3} can be regarded as restricting
the price system $q_t^0(s^t)$ as a function of the equilibrium allocation assigned
to consumer $i$, for any $i$. But to exploit this fact in computation, we need a way first to compute
...
putting the price system in units of time $0$
 ...
 $\mu_i  =  u'_i[c_0^i(s_0)] $.
An alternative for setting the units of the price 
system would be to normalize one of the
multipliers $\{\mu_i\}_i$, say $\mu_1=1$, as
we do in the equilibrium computation 
in section \use{sec:Negishi}. Clearly, the price 
system and the multipliers are interrelated,
as evident in the marginal condition \Ep{eq3b} 
or \Ep{eq3}. On the one hand, fixing the
units of the price system also determines the 
absolute values of the multipliers, i.e., the
marginal utilities of relaxing the budget
constraints expressed in that particular price 
system. On the other hand, fixing the 
absolute values of the multipliers would 
also determine in which units the price system is 
expressed, in order for the multipliers to 
emerge as the marginal utilities of relaxing 
the budget constraints expressed in that
particular price system. 

The reason for normalizing either the 
price system or the multipliers is that the
equilibrium computation only determines relative 
prices. Absolute prices in a competitive equilibrium 
are determined after picking a numeraire, in 
which the price system is expressed. The analogue 
in the Pareto problem is that only relative Pareto
weights matter.
...
at  history $s^t$. % associated with the Arrow-Debreu
 That  allocations for the planning
problem and the competitive equilibrium are identical reflects 
...
consider an initial redistribution scheme as follows.
...
In order for the redistribution scheme to be feasible,
...
right side of \Ep{Redist_eq2} and can therefore not
...
Pareto problem. Thus, the efficient allocation has 
then agent $i$ consuming nothing and his entire 
endowment is allocated to other agents.
...
a function only of the realized state $s_t=s$:
$\vec {\bar a}(s)$ satisfying $\sum_i \bar a^i(s)=0$.
Thus, for each state $s'$ that can be realized in the next 
period, i.e., $\pi(s'|s)>0$, $\{\bar a^i(s')\}_i$ are the 
...
wealth $\vec {\tilde a}_0(s_0)$ will reemerge whenever 
the Markov state $s_t$ returns to $s_0$, 
...
continue  a trading strategy that insures against future
...
state $s$).\NFootnote{Computationally,
when solving for the equilibrium set of 
beginning-of-period wealth distributions,
$\{ \vec {\bar a}(s) \}_s$,
we can proceed as if the end-of-period 
asset portfolios are identical across
all current Markov states $s$. Because 
if there is any state $s'$ that
cannot be realized after a current
state $s$, i.e., $\pi(s'|s)=0$, the
equilibrium price for the corresponding Arrow
security is zero, $Q(s'|s)=0$. Hence,
it would be immaterital for our calculation 
of an equilibrium if we impose that consumers 
even in such a current state $s$ have
end-of-period holdings $\{\bar a^i(s')\}_i$ 
of that particular Arrow security since
at a zero price, there would be neither any
cost to the purchasers nor any payments
to the issuers of that security.}
...
consumers ``play the hand that they are dealt'' 
...
in any state $s$ so that they end up bearing
no idiosyncratic risk but only aggregate risk.
...
is also back to the exact same situation that 
prevailed in the first period, including a
beginning-of-period wealth distribution equal to
the initial wealth distribution
$\vec {\tilde a}_0(s_0)$. The following example
illustrates the recursive nature of the wealth
distribution.
...
Let the stochastic event $s_t$ take the values 
$s\in\{0,1\}$, 
with the initial state $s_0=1$ and transition 
...
the pricing kernel in \Ep{MarkovQ} becomes
$Q(s'|s) = \beta \,\pi(s'|s)$.

In period 0 when the initial state is $s_0=1$, the 
...
a period whenever the realized state is $s=1$, and 
...
turn imply what the time-invariant end-of-period 
asset portfolios of the two consumers must be. 
Specifically, there is no issuance of Arrow
securities that pay off next period contingent on 
the realized state being $s'=1$ (since the 
beginning-of-period wealth distribution is the 
zero vector when the realized state is $s'=1$).
Thus, the only possible issuances of Arrow
securities would be ones that pay off next period
contingent on the realized state being $s'=0$,
i.e., there could be traded quantities of 
securities $\{\bar a^i(0)\}_i$ at the price 
$Q(0 | s)$ where the price depends on the 
state $s$ in the period of the issuance.

Starting in period 0, it is trivially consumer 2
who would like to borrow, $a^2(0)<0$, since he 
has no endowment in period 0, $y^2(s_0)=y^2(1)=0$.
The loan allows consumer 2 to consume
$-Q(0 | 1)\, a^2(0)$ in period 0. It is sensible 
that the repayment of the one-period loan is
contingent on next period's state being
$s'=0$, i.e., the state in which consumer 2 
receives the endowment $y^2(0)=1$, while the
loan is forgiven if next period remains in
state $s'=1$. This equilibrium outcome is 
critical for an efficient risk sharing since
otherwise consumer 2 would become ever more
indebted as long as state $s'=1$ persists.
Furthermore, according to proposition 5, 
when state $s'=0$ is eventually realized, 
consumer 2 still keeps the same negative 
holding of Arrow security $a^2(0)<0$.
Consumer 2 is de facto ``rolling over''
...
consumption if he were to not ``roll over'' 
the indebtedness. Eventually, when state $s'=1$ 
is realized, the debt is forgiven and the 
economy reverts back to the beginning-of-period 
wealth distribution as given by
the zero vector. Amazingly, the preceding 
transactions in Arrow securities since period 0 
will have enabled consumer 2 to enjoy a constant 
consumption level, $\bar c^2$.

Likewise, the counterparty to these past Arrow
security transactions, consumer 1, has
also enjoyed a constant consumption level,
$\bar c^1$. It started in period 0 with
consumer 1 extending the above state-contingent 
loans to consumer 2, $a^1(0)=-a^2(0)=a >0$.
Any repayment in that succession of 
one-period loans was contingent on the
realized state in next period being $s'=0$, the
state in which consumer 1 has no endowment,
$y^1(0)=0$. When that eventually happens, 
the loan repayment $a$ enables consumer 1 
to continue to finance the consumption level 
$\bar c^2$ as long as state $s'=0$ persists. 
Without having to borrow, consumer 2 does this 
by maintaining an unchanged positive holding of 
Arrow securities, $a^1(0)=a>0$. 

After this expos\'{e} of equilibrium outcomes based
on proposition 5, we can derive the equilibrium
consumption levels, $\bar c^1$ and $\bar c^2$, from
the consumers' budget constraints and the market
...
consumer 1 or 2, we can substitute out for the 
...
$$
where we invoke the equilibrium price kernels,
$Q(0|s) = \beta \,\pi(0|s)$.
...
\EQNalign{ \bar c^1 &= { 1-\beta\,\pi(0 \vert 0) \over
    ...
    1 + \beta \bigl[\pi(0 \vert 1)-\pi(0 \vert 0)\bigr]}\,.    
                                          \EQN allocREC;b \cr} $$
These expressions have the implications that one would
expect, for example, given any $\pi(0|0)$, as the 
transition probability $\pi(0|1)$ goes to zero, the 
limits of $\bar c^1$ and $\bar c^2$ are 1 and 0,
respectively. This follows from the initial Markov 
state being $s_0=1$ in which consumer 1 receives 
the entire endowment. Another example is deterministic
...
As described in section \use{sec:PO}, suppose that the
economy in example 5 in section \use{sec:example5}
starts off with an initial lump-sum redistribution.
...
for some feasible value of $a_0$. Specifically, 
feasibility of the initial redistribution imposes a 
bound on $a_0$ as determined by the borrowing limit
of the consumer who receives the negative lump-sum
transfer. Let consumer $i$ be the one who receives
a negative transfer, the bound on the absolute value
of $a_0$ then becomes $-|a_0|\leq \bar A^i(1)$. That 
is, the initial indebtedness of consumer $i$ cannot 
...
distribution will prevail at the beginning of
a period whenever the realized state is $s=1$, i.e.,
...
on $a_0$ is simplified when probabilities are interchanged:
 $\pi(0|1)-\pi(0|0)
= [1-\pi(1|1)]-[1-\pi(1|0)]
= \pi(1|0)-\pi(1|1)$.}
...
at the start of the economy causes a permanent increase
(decrease) in the consumption of the consumer who receives
the positive (negative) initial transfer, and that change
in consumption is the annuity value of the transfer.



\subsection{Recursive formulation of optimization and equilibrium}
The finding that the pricing kernel $Q(s'|s)$ and the endowment $y^i(s)$
are both functions only of a Markov state $s$ enables us to provide a
...
supposition that the value function on the right
side of Bellman equation \Ep{eq11} 
correctly captures future consequences of those 
decisions. Condition ({\bf b}) confirms that each
consumer uses the appropriate 
value function.
...
expected utility given the initial wealth
and the process generating prices
...
According to proposition 5, a recursive competitive
equilibrium exists when endowments are governed by 
a Markov process. Specifically, given the pricing 
...
decision rules will then prescribe that:
...
g^i(\bar a^i(s),s,s') &=\cases{\bar a^i(s'), 
                     ...
                     0, & otherwise.\cr}  \EQN EQpolicy2;b 
                                                 \cr}$$
In the circular reasoning of a competitive equilibrium, 
this confirms that the equilibrium pricing kernel is 
the one in \Ep{MarkovQ}. In particular, use the 
...
extreme end points of either a type 1 or a type 2 person 
consuming all of the endowment and the other type getting
nothing. Specifically, we consider initial promises $v$ 
...
where $c \in [0,1]$ and $w_{s'} \in W$ for all $s'$. 
...
The corresponding assignment to the type 1 person is
current consumption $1-c$ and continuation values
$\{P(w_{s'})\}_{s'}$. In this section, a sufficient
set $W$ of admissible future state-contingent discounted
values of utility promised to a type 2 person is $V$ as 
specified above, $W=V$.
...
the type 2 person. The recursive formulation brings out that,
because $P'(w_{s'}) = P'(v)$,
...
to the current realization of shocks to endowments or preferences.\NFootnote{In those settings, the recursive
formulation of the Pareto problem will become one of
a `dynamic program squared'.  We call it that because the 
state variable $v$ that appears in the Bellman equation
for $P(v)$ itself will then satisfy another Bellman equation.
For example, the consumer on the receiving end of the
promised value might be deciding whether or not to remain
within the insurance arrangement or to elope into autarky,
...
For the case of heterogeneous beliefs, we 
revisit the example of a recursively formulated
Pareto problem in section \use{sec:RecursivePareto}.
In addition to Inada condition \Ep{eq_Inada}, we 
assume that the utility functions satisfy 
$\lim_{c \rightarrow 0} u_i(c) = -\infty$. This
facilitates our analysis with the following 
implications for the Pareto frontier. On the left 
side, the Pareto frontier $P(v)$ plateaus 
out and converges to $u_1(1/(1-\beta))$ as 
$v \rightarrow -\infty$ and hence, the slope $P'(v)$
converges to zero. That is, at arbitrarily low 
expected discounted utilities $v$, a type 2 person
is hardly consuming anything and hence, any further 
declines in that person's utility, regardless of how 
large of a decline, can only involve miniscule 
quantities of goods changing hands from a type 2 to
a type 1 person. Thus, the expected  
discounted utility $P(v)$ of a type 1 person
hardly changes in response to an arbitrarily low 
$v$ declining further toward $-\infty$. On 
the right side of the Pareto frontier, the same 
argument applies but in reverse. Namely, the Pareto
...
Under the assumption of heterogeneous beliefs,
...
$$ u_2(c) + \beta \sum_{s'} w_{s'} \pi^2(s') \geq v .
                                      \EQN heterogbel13 $$
First-order conditions with respect to $w_{s'}$ 
and the envelope condition $P'(v) = -\theta$ no longer
imply that $P'(w_{s'})=P'(v)$ but rather that 
$P'(w_{s'}) \pi^1(s') =P'(v) \pi^2(s')$ for all $s'$. 
Recall that $-P'(v)$ serves as a relative Pareto weight
on the type 2 person, so the recursive formulation  
brings out how that Pareto weight changes with
the realization of next period's state $s'$:
$$
-P'(w_{s'})= {\frac{\pi^2(s')}{\pi^1(s')}} 
              \left(-P'(v)\right) .     \EQN heterogbel14 $$
This is the i.i.d.\ version of the general case 
for updating Pareto weights under heterogeneous 
beliefs in expression \Ep{heterogbel3}. 
In particular, the relative Pareto weight on the 
type 2 person increases (decreases) if a state $s'$
is realized which the type 2 person had thought
to be more (less) likely to happen as compared to 
the beliefs of the other type 1 person. 

To characterize what happens to the relative
Pareto weight asymptotically as time goes to
infinity, we need to invoke the true probabilities.
...
a nonnegative martingale. And based on what we 
learnt in section \use{sec:heterogbel_example},
expectations of future values of $-P'(v)$ are
bounded from above, i.e., $E[-P(v_t)]<+\infty$.
The reason is that under our assumption of
a type 1 person knowing the true probabilities,
such a person will almost surely not become
impoverished and vanish from the economy as
...
spread out when the planner maximizes the
expected discounted utilities of agents who
do not share the same beliefs. The 
contradiction is avoided only for $v$, the
utility of the type 2 person with the less
accurate beliefs, converging to $-\infty$; 
therefore, the limit of
$-P'(v)$ must be zero.\NFootnote{If it had
been the type 1 person who had less accurate 
beliefs, we would simply have exchanged
places of the two types in the
