...
y' B z \over \partial z} = B' y$. See appendix \use{appendixLQA} of chapter \use{timeseries}.}
...
$$F_{j} =(Q+B'P_j B)^{-1}B'P_j A. \EQN 147;b$$
 Equation \Ep{147} can be  derived using the same approach that we used to derive equation \Ep{146}, 
...
in the form
$$ P_j = \sum_{k=0}^\infty \beta^k (A-BF_j)^{\prime k} (R + F_j' Q F_j)
                                (A - B F_j)^k.  $$
...
(See Kwakernaak and Sivan, 1972, for an extensive study of a continuous-time
...
\subsection{Certainty equivalence}\label{sec:certequiv}%
A notable feature of formula \Ep{163}  is: %that the feedback rule \Ep{163} is
...
It is   remarkable that, although through $d$ the
...
versions of the optimal  savings problem, there are no
precautionary savings. Compare outcomes from  section \use{sec:LQmodel} of chapter \use{timeseriesapp} and   chapters \use{selfinsure} and \use{incomplete}.} The
certainty
...
\subsection{Transforming states and controls to eliminate discounting}\label{sec:discounttrans}%
A pair of useful transformations allows us to convert a discounted problem into an undiscounted one.
Thus, suppose that we have a discounted problem with objective \Ep{148} and that the state transition equation
is again $x_{\toone}=Ax_t+Bu_t$.
...
obey the transition law $\hat x_{t+1} = \hat A \hat x_t + \hat B \hat u_t$.  Then a discounted optimal control problem
...
the following instances of   equations \Ep{145}, \Ep{146}:
$$\hat F=(Q+B'\hat PB)^{-1}\hat B'P \hat A \EQN 145a $$
  and
$$\hat P=R+\hat A'P \hat A-\hat A'P \hat B(Q+B'\hat P \hat B)^{-1} \hat B'P \hat A.\EQN 146b$$
...
\index{eliminating discounting!transformations}%
...
For several purposes,\NFootnote{For a planning problem in a linear quadratic economy,  the gradient of the value function
...
%\vfil\eject
\section{Stability and existence}\label{sec:ch4stable}%
...
linear optimal control.  (For continuous-time linear systems, these concepts are
...
$$\eqalign{2 Q u_t &+ 2B^\prime \mu_{t+1} = 0 \ ,\ t \geq 0 \cr \mu_t &= R x_t + A^\prime \mu_{t+1}\ ,\ t\geq 1.\cr}\EQN Lagr102 $$
Define $\mu_0$ to be the vector of shadow prices of $x_0$ and apply an envelope condition to
\Ep{Lagr101} to  deduce that
$$ \mu_0 = R x_0 + A' \mu_1 ,$$
which is a time $t=0 $ counterpart to the second equation of system  \Ep{Lagr102}.
...
Solve the first equation of \Ep{Lagr102} for $u_t$ in terms of $\mu_{t+1}$; substitute
into the law of motion $x_{t+1} = A x_t + B u_t$; arrange the resulting
equation and the second equation of \Ep{Lagr102} into the form
...
this system as
$$\pmatrix{x_{t+1}\cr \mu_{t+1}\cr}\ = M\ \pmatrix{x_t\cr\mu_t\cr} \EQN aLagr101 $$  %\EQN a
where
$$M\equiv L^{-1} N = \pmatrix{A+B Q^{-1} B^\prime A^{\prime-1}R &
-B Q^{-1} B^\prime A^{\prime-1}\cr -A^{\prime -1} R & A^{\prime -1}\cr} .\EQN Mdef $$

We seek to solve the difference equation system \Ep{aLagr101} for a sequence $\{x_t\}_{t=0}^\infty$
...
We acquire our wish for stability of the $\{x_t\}$ sequence from our  desire to maximize
...
any system of the form \Ep{aLagr101} if a solution exists, even
...
solves.    See chapter \use{linappro} for some applications
...
\section{More about a Lagrangian formulation}\label{sec:Lagrangian_more}%
We can use the  subsection \use{sec:discounttrans} transformations to solve a discounted optimal linear regulator problem using the Lagrangian and invariant subspace
methods introduced in section \use{lagrangianformulation}.   But for several purposes, it is useful  explicitly briefly to describe a Lagrangian for
a discounted problem. Thus, for the discounted optimal linear regulator problem, form the Lagrangian
$$\eqalign{{\cal L} &= - \sum^\infty_{t=0} \beta^t \biggl\{ x^\prime_t R x_t +
u_t^\prime Q u_t\cr
&+ 2 \beta \mu^\prime_{t+1} [A x_t + B u_t - x_{t+1}]\biggr\}, \cr} \EQN Lagr101d $$
where $2  \mu_{t+1}$ is a vector of Lagrange multipliers on the state vector $x_{t+1}$.
First-order conditions for maximization with respect
to $\{u_t,x_{t+1}\}_{t=0}^\infty$ are
$$\eqalign{2 Q u_t &+ 2  \beta B^\prime \mu_{t+1} = 0 \ ,\ t \geq 0 \cr \mu_t &= R x_t + \beta A^\prime \mu_{t+1}\ ,\ t\geq 1.\cr}\EQN Lagr102d $$
Define $2 \mu_0$ to be the vector of shadow prices of $x_0$ and apply an envelope condition to
\Ep{Lagr101d} to  deduce that
$$ \mu_0 = R x_0 + \beta A' \mu_1 ,$$
which is a time $t=0 $ counterpart to the second equation of system  \Ep{Lagr102d}.
Proceeding as we did above with  the undiscounted system \Ep{Lagr103}, we can rearrange the first-order conditions into the
system
$$ \bmatrix{ I & \beta B Q^{-1} B' \cr
             0 & \beta A' } \bmatrix{ x_{t+1} \cr \mu_{t+1} } =
   \bmatrix{ A & 0 \cr
             - R & I } \bmatrix{ x_t \cr \mu_t } \EQN Lagr103d $$
which in the special case that $\beta = 1$ agrees with equation \Ep{Lagr103}, as expected.

By staring at system \Ep{Lagr103d}, we can infer  identities that shed light on the structure of optimal linear regulator problems,
some of which will be useful in chapter \use{stackel} when we apply and  extend the methods of this chapter to study Stackelberg and Ramsey problems.

First, note that the first block of equation \Ep{Lagr103d} asserts that when  $\mu_{t+1} = P x_{t+1}$, then    $(I + \beta Q^{-1} B' P B P ) x_{t+1} = A x_t$, which  can be rearranged to
be
$$ x_{t+1} = (I + \beta B Q^{-1} B' P)^{-1}  A x_t , \EQN Lagr104d $$
...
$$ x_{t+1} = (A - BF) x_t . $$
But using  
$$F=\be (Q+\be B'PB)^{-1} B'PA ,  \EQN optF103 $$  it follows that $A - B F = (I - \beta B (Q+ \beta B' P B)^{-1} B' P) A $. Thus, our two expressions for the
closed loop dynamics will agree if and only if
$$ (I + \beta B Q^{-1} B' P )^{-1} =    (I - \beta B (Q+\beta  B' P B)^{-1} B' P) , \EQN Lagr105d $$
a matrix  equation that can be verified by applying a partitioned inverse formula.\NFootnote{Apply
the formula $(a - b d^{-1} c)^{-1} = a^{-1} + a^{-1} b (d - c a^{-1} b)^{-1} c a^{-1}$ for appropriate choices of the matrices $a, b, c, d$.
See the appendix to chapter 8 of Hansen and Sargent (2013) for other applications of partitioned inverse formulas in linear optimal control theory.}

Notice that an optimal $P$ obeys the following  discrete Lyapunov equation:
$$ P = (R + F' Q F) + \beta (A - B F)' P (A - BF)  \EQN Lagr106d $$
provided that the decision rule $F $ is the function of $P$ given by formula \Ep{optF103}.
Equation \Ep{Lagr106d} is thus an instance of formula \Ep{lasset7} for the quadratic matrix pinning down the 
value of using a fixed decision rule forever.  
Furthermore, the second equation of system \Ep{Lagr103d} implies the ``forward looking'' equation for the Lagrange multiplier $\mu_t = R x_t + \beta A' \mu_{t+1}$ whose
solution is $\mu_t = P x_t$, where
$$ P = R + \beta A' P (A - BF) .  \EQN Lagr107d $$
Formula \Ep{Lagr107d} is an instance of formula (3) from  exercise 2.25 for forecasting
a discounted  geometric sum of a quadratic form of a state vector $x_t$ with one-step transition
dynamics governed by $A - BF$. 


%A comparison of equations \Ep{Lagr106d} and \Ep{Lagr107d} is useful for bringing out features of the optimal value function for a discounted optimal linear
%regulator problem.
...
for computing  the probability distribution 
%mathematical expectation $E[ x_t \vert  y_{t-1}, \ldots,
%y_0]$ 
of a hidden state  vector $x_t$, 
conditional on 
...
$t$, history $y^t \equiv [y_t, \ldots, y_0]$
 is available to help infer likely  locations of
$x_t$ and  $x_{t+1}$.  The \idx{Kalman filter}
...
Equations
 \Ep{kalman3} are connected   to equations
...
a Riccati equation.   With  judicious 
matrix transpositions and reversals of time, the two systems of equations
\Ep{kalman3} and \Ep{147}
can be aligned.\NFootnote{Align $(A, B, R, Q, F, P)$ for a dynamic program 
with $(A', G', C C', R, K', \Sigma)$ for a Kalman filter. See Hansen and Sargent (ch.~4, 2008) for an account of how the LQ dynamic programming problem
  ...
  then judiciously transforms  first-order conditions into an associated optimal linear regulator.} %In Appendix \use{lincontrol} on dual filtering
...
For some applications of the Kalman filter in macroeconomics, sameee chapter \use{timeseriesapp}, especially sections \use{sec:Muth_reverse} and \use{app:Boyan}.\NFootnote{The
...
so that there is perfect foresight.  Equation (1) is a Philip Cagan-like 
demand function for currency, expressing real balances as
an inverse function of the expected gross rate of inflation.
\auth{Cagan, Philip}%

   Speaking of Cagan (1956), the government is running
a permanent real  deficit of $g$ per period, measured in goods,
all of which it finances by printing money.  The government's
...
\medskip
\noindent{\it Exercise \the\chapternum.9}     A planner chooses a contingency plan for
...
\medskip
\noindent{\it Exercise \the\chapternum.10}   A consumer values consumption, asset streams $\{c_t, k_{t+1}\}_{t=0}^\infty$ according to
...
\medskip
\noindent{\it Exercise \the\chapternum.15} \quad {\bf A forecasting problem}
\medskip
\noindent
Let $x_{t+1} = A x_t + C w_{t+1} $, where   $A$ is a stable matrix and $\{w_{t+1}\}_{t=0}^\infty$ is an i.i.d.~sequence of normal ${\cal N}\sim (0,I)$ random vectors.
Assume that the initial $x_0$ is drawn from a normal distribution ${\cal N}(0, \Sigma)$ where $\Sigma$ is a positive definite matrix
that solves $\Sigma = A \Sigma A'+ C C'$.  Let $\beta \in (0,1)$ be a discount factor and $R$ a positive definite matrix.  Consider the random process
process $\{y_t\}_{t=0}^\infty$ defined by
$$ y_t = \sum_{j=0}^\infty \beta^j A^{\prime j} R x_{t+j} . \leqno(1) $$

\medskip
\noindent {\bf a.} Please compute the mean and covariance matrix of $y_t$. (Please avail yourself of techniques learned in chapter \use{timeseries}
if you wish.)

\medskip
\noindent
{\bf b.} Please compute the covariance matrix between $y_t$ and $x_t$.

\medskip
\noindent{\bf c.} Please compute the linear least squares projection of $y_t$ on $x_t$.

\medskip
\noindent{\bf d.} Please interpret your answer to part {\bf c}  in light of system \Ep{Lagr103d} and equation \Ep{Lagr107d}.

\medskip
\noindent{\bf e.} Please interpret your answer to part {\bf c}  in light of exercise 2.25.


\medskip
\noindent{\it Exercise \the\chapternum.16} \quad {\bf Time to build}
\medskip
\noindent
A monopolist faces a linear inverse demand curve
$$ p_t = a_0 - a_1 Q_t $$
where $a_0 > 0, a_1 >0$, $p_t$ is the price of the good and $Q_t$ is the quantity produced by the monopolist.
The monopolist operates a linear technology $Q_t = K_t$, where $K_t$ is a capital stock obeying the law of motion
$$ K_{t+1} = \delta K_t + i_{t-2}, $$
 $\delta \in (0,1)$ is a depreciation factor, and $i_t$ is gross investment in starting a  new capital project. It takes several periods
 for investment to materialize in new capital.  The monopolist
has initial conditions $K_0 = 0, i_{-1} = 0, i_{-2} =0$.  The monopolist has time $t$ cost
$c_t = J_t K_t + d i_t^2$ where $J_t$ is variable cost of operating $K_t$ and $J_t = \rho_0 + \rho_1 J_{t-1}$, where $\rho_1 \in (0,1)$, and
$d >0$ measures adjustment costs. The monopolist chooses $\{i_t, K_{t+3}\}_{t=0}^T$ to maximize
$$ \sum_{t=0}^T \beta^t \left( p_t Q_t - c_t \right) $$
where $\beta \in (0,1)$ is a discount factor and $T>0$ is an integer that possibly equals $+ \infty$.

\medskip
\noindent{\bf a.} Assume that $T = +\infty$.  Please formulate the monopolist's problem as a linear-quadratic discounted dynamic programming problem, being careful to define
the state and control vectors.

\medskip
\noindent{\bf b.}  Now assume that $T= 50$.  Please formulate the monopolist's problem as a dynamic programming problem, being careful
  to describe the monopolist's optimal decision rules.  Are they time invariant? If not, how do they depend on time?


\medskip
\noindent{\it Exercise \the\chapternum.17} \quad {\bf Phelps and Pollak (1968) meet Howard again}
\medskip

\noindent This is a continuation of exercise 2.26.  
 A sequence of decision makers at dates $t= 0, 1, \ldots, $ chooses $\{x_{t+1}, u_t \}_{t=0}^\infty$ subject to
