...
he expects to have the option later to sell the asset  to another investor who will value the asset more highly.
...
Valuations $\hat p_a(s)$ and $\hat p_b(s)$ are displayed in the fourth and fifth rows of \Tbl{HKtab}. Even the temporarily more pessimistic investors who don't buy the asset think that it is worth
...
Under the true distribution, we can then compute the  expected rate of return from
...
$$
Given the dividends $d(1)=0$ and $d(2)=1$ and the
...
to be sold when the state changes to $2$ trivially has 
a higher expected return $\Omega(1)$ the sooner state $2$
can be expected to be realized, upon which the investment yields 
both a dividend $d(2)$ and  resale price $\overline p(2)$. 
In contrast, the continued collection of dividend $d(2)=1$
in state $2$ is more valuable than the receipt of the
resale price $\overline p(1)$ when state $1$ is
realized and hence, the expected return $\Omega(2)$ 
...
of the true probabilities $P(1,1)$ and $P(2,2)$,
respectively. Thus, the left (right) panel in
Figure \Fg{HK_rr} shows the true expected net 
return on assets purchased by agents of type
$a$ ($b$) in state $1$ ($2$). When the true
probability $P(1,1)$ ($P(2,2)$) coincides with
the subjective probability $P_a(1,1)$ ($P_b(2,2)$), 
 agents of type $a$ ($b$) earn an expected net rate
of return equal to $\beta^{-1}-1=1/3$ on their
...
from agents of type $b$ ($a$). From thereon,
...
\caption{Share of the true unconditional expected 
present value of the asset that agents of type $b$ 
are expected to receive, as a function of the true 
probabilities $P(1,1)$ and $P(2,2)$. The dashed curve 
is the isoquant for a share equal to $0.5$, and the 
solid lines demarcate four quadrants with the origin 
at the subjective beliefs $P_a(1,1)=0.5$ and 
$P_b(2,2)=0.75$.}
\infiglist{HK_share_b}
\endfigure

The solid lines in Figure \Fg{HK_share_b} demarcate four
quadrants with the origin at ($P_a(1,1)$, $P_b(2,2)$).
...
the two types of agents depends on how the distorted equilibrium prices create an
advantage for one or the other type of agent. As is 
to be expected, the smaller $P(1,1)$ (the greater 
$P(2,2)$) is the more of an advantage to agents of 
type $a$ ($b$).
...
in the very first period and never trade the asset again.
(This opportunity presumes  that behavior by  such a `deviating' 
agent would  not subvert  existence of a 
competitive equilibrium in which 
the collective of each type of agent
who continue 
%every agent who continues 
to trade the asset 
...
of type $a$ ($b$). As is abundantly clear 
...
probability $P(1,1)$ equals $0.95$. However, 
while agents of type $b$ receive an ever greater 
share as the true probabilities $P(1,1)$ and 
$P(2,2)$ approach $1$ and $0$, respectively,  
in the parameter region in 
the farmost left corner of Figure \Fg{HK_share_b},
it should be noted that the true unconditional
...
at the expense of agents of type $a$ are moderated
in absolute terms .
...
than $\beta^{-1}$ while agents of type $b$ earn a 
subpar rate of return. This results in agents of 
...
%%By way of contrast, 
the  Harrison-Kreps model of this appendix assumes 
risk-neutral agents and incomplete markets, including
a prohibition of short sales.  
...
subjectively expected gross rate 
...
the real Lucas tree. That agent would have
...
asset, the agent's liability in the next period
would then be to pay the higher price $\bar p(2)$ 
...
stream would be the natural debt limit and therefore, 
...
It follows that allowing for 
short sales puts the dynamics of the wealth 
distribution on the table. For example, if the 
transactions and the realizations of states could lead 
to outcomes where one type of agents can no longer 
afford to purchase the entire Lucas tree, it would 
mean that prices $\overline p$ in \Tbl{HKtab} cannot
...
such complications,  literature that builds on the
Harrison-Kreps analysis typically assumes that agents
have infinite wealth. Under that assumption,  short sales cannot be allowed since there 
would not exist an equilibrium with short sales permitted. Pessimistic agents 
...
tree falls below $\beta^{-1}$.\NFootnote{Scheinkman and Xiong (2003) use a continuous-time model inspired by the Harrison-Kreps
(1978) model to 
analyze how a Tobin transaction tax  substantially
reduces ``speculative trading'' but still brings only small reductions in 
...
Notice how compared to the homogeneous beliefs setting that leads to the pricing formula \Ep{HarrKrep1}, high 
trading volume occurs when the Harrison-Kreps pricing formula \Ep{HarrKrep3} prevails. Scheinkman takes this outcome as helping him to explain the  
high volumes observed   during ``famous bubbles''.}
...
described in appendix \use{app:compB} of chapter \use{recurge}, a model in which   both risk aversion and  heterogeneous beliefs 
...
Harrison-Kreps model with risk-neutral agents and their
linear utility functions. Furthermore, while trading 
...
approach zero when agents with the most
accurate beliefs eventually  consume the entire
aggregate endowment; agents with less 
accurate beliefs will  have sold  continuation endowment streams to finance earlier consumption 
and asset trades.
