...
Above we interpreted negative consumption as a 
...
the vectors $z^i_0$ implies that the cross-section average of non-financial income equals the mean of its  stationary distribution each period, denoted $Y$. It follows that
when we integrate over all agents' consumption as given
by expression \Ep{eq:c_null}, the goods market clears in
period $0$ in this closed economy,
$$ \int_0^1 c^i_0 \, di 
...
conditional expectation $E_0$ and the resulting
integration of $y^i_{j}$ over the continuum of agents on
the unit interval yields the cross-section average of 
non-financial income that equals the constant $Y$ in 
this stationary environment.
...
\check C $. 
The integration of $c^i_{t+1}$ over all
agents yields
$$ \int_0^1 c^i_{t+1} \,di 
...
of the i.i.d.~process for $w_{t+1}$ when
...
econom grows with the forecast horizon as shown in
\Ep{eq:spreadcons}. It follows that consumers in 
the incomplete markets economy attain lower expected 
utility than they do in the complete markets economy 
since consumers dislike
