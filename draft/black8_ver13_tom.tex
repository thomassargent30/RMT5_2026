...
generations model as a special case of the general equilibrium model
of chapter \use{recurge} with complete markets and all
...
Two surprising findings emerge from this analysis: competitive equilibria can be
Pareto inefficient, and intrinsically useless assets---fiat money and
unbacked government bonds---can be valued in equilibrium.

Complete markets with time $0$ trading in
an overlapping generations model strain
...
general equilibrium model of chapter \use{recurge}. Since we  assume that everyone can 
trade with anyone else at time 0, we will discover that such  outcomes 
do not depend on any inability of agents alive today to trade with agents not yet born
(as shown in section~\use{sec:time0trading}).
...
sequential trading to analyze: valued fiat currency;
unbacked government debt; deficit finance;
the real bills doctrine; and overaccumulation of capital in a growth model.
...
stationary endowments in which  $y^i_i=y_1$ and 
$y^{i-1}_i=y_2$
for all $i\geq 0$. Here  subscript $1$ on $y_1$ denotes
young and $2$ on $y_2$ denotes old. 
...
In contrast, excess supply of time $0$
goods cannot occur in the general equilibrium model of chapter \use{recurge}. Consequently, it doesn't  matter
...
are   valued in equilibrium, outcomes that are not possible in the general equilibrium model
of chapter \use{recurge}.
...
trading equilibrium, $\alpha_i$ will be the inverse of the 
...
$c^i_i = c_1$ and $c^i_{i+1} = c_2$ for all $i \geq 0$.
\medskip
\noindent Here the subscript $1$ on $c_1$ denotes young 
and $2$ on $c_2$ denotes old.
Note that we do not require that $c^{-1}_0 = c_2$.
...
which amounts to guessing $c_1$ and $c_2$ in a 
...
a consumption profile $(c_1,c_2)$ is 
...
allocation $c_1=c_2 =.5$ and $c^{-1}_0 = \epsilon$. 
...
equilibrium. We guess a stationary equilibrium
allocation $c_1=1-\epsilon$, $c_2 =\epsilon$,
...
\Ep{olgbud2} and market clearing conditions
...
to trade. Since all markets clear with equality,
equilibrium L is also a Walrasian equilibrium.
...
old that makes them better off.  We shall soon learn that  either fiat money or  unbacked, perpetual government debt 
can accomplish this purpose.\NFootnote{Karl Shell (1971)  studies why some competitive equilibria
...
\subsection{Dynamic (in)efficiency}%
...
endowments, $(y^i_i, y^{i-1}_i) = (y_1,y_2)$ for
all $i\geq 0$ and $y^i_t = 0$ otherwise,
multiple equilibria and dynamically inefficient
equilibria both exist
if and only if $y_2<y_1$. If $y_2 \geq y_1$,
there is a unique equilibrium that is also Pareto
efficient, as we found more generally for the general equilibrium model of chapter \use{recurge}.
...
pairs $(c_i^i, c_i^{i-1})$ of consumptions assigned to a young and an old person at time $i$.\NFootnote{Given our assumptions 
...
Following Gale (1973), for the important special case in which $(y_i^i, y_i^{i-1}) = (y_1, y_2)$ for all $i \geq 0$,
...
$c^{-1}_0 = y_2$. As for the continuation of the equilibrium 
allocation, there is a multiplicity of possible
outcomes since $y_2<y_1$. Specifically, we can initiate an
equilibrium computation by choosing
$c^0_0 \in ((y_1+y_2)/2,y_1)$.\NFootnote{By setting 
$c^0_0$ equal to one of the two boundary points, 
$(y_1+y_2)/2$ or $y_1$, we could recover one of the two section \use{sec:olg_stat} stationary equilibria.}
...
$$\EQNalign{  \psi(c^i_i, c^i_{i+1}) & = 0, \EQN equilib1;a \cr
              c^{i+1}_{i+1} + c^i_{i+1} & = y_1 + y_2. \EQN equilibr1;b \cr}$$
...
period $i=0$ is assured by our choice of $c^0_0<y_1$ and
the initial old agent consuming his endowment in any
equilibrium, $c^{-1}_0=y_2$, i.e., equation \Ep{olg_equib;b}
...
%of David Gale (1973).
Notice that  $y_t^t$ and  $c_t^t$  are on the horizontal axis while
$y_{t+1}^t$ and both $c_{t+1}^t$ and $c_{t}^{t-1}$ are on the vertical axis.
...
same across all equilibria, $c^{-1}_0 = y_2$.
...
The multiple equilibria found under time-0 trading motivate the following
sequential-trading formulation.
...
that at time $0$ issues either  a zero-dividend infinitely
durable asset in the form of an inconvertible fiat money or 
one-period bonds that it rolls over forever.  Both of these instruments -- i.e., money and bonds -- are ``unbacked'' by any government
promise to convert them into consumption goods. Under the auxiliary assumption 
...
$\Omega_0\geq 0$, will no longer be wasted but will instead be
...
underlies the outcome that the same sequence  $\{c^t_t, c^t_{t+1}\}_{t\geq0}$ 
arises  under both time $0$ trading and sequential 
trading. (Note that $c^{-1}_0$ has been omitted from this sequence.)  
...
In the following sections, we will activate various items
...
Here  $M_t$ is the nominal quantity of money at the end of period $t$,
...
the left side of budget constraint \Ep{Gov_bc} sums sources of  
...
at time $t=0$ from  initial conditions 
$M_{-1}= B_0 = 0$. We assume that government purchases
$\{g_t\}_{t \geq 0}$ provide no utility to agents.
...
with $M >0$ units of paper currency that
no one has promised to redeem  for goods.
This paper currency is  not ``backed'' by another asset.  Nevertheless, 
Samuelson showed that there  exists a   system of expectations about the value of paper currency that  can impart  positive value to
an unbacked currency. Currency will be valued today if people expect it
to be valued tomorrow.  In this way, Samuelson constructed a situation in which
currency is backed
by expectations without promises.
\index{expectations!without promises}

To cast this  Samuelson (1958) setup in terms of our
government formulation in \Ep{Gov_bc}, we assume that
the government at time $0$ gives  initial old agents a total of  $M$ units of currency. Consequently, the  government budget constraint
at time $0$ becomes 
%per our formulation \Ep{Gov_bc}:
$$ {M_0 \over  p_0} + \tau^{-1}_0 = 0,  \EQN Gov_bc_M $$
where $M_0=M$. 
%That is, the government hands over the
%currency $M$ to the initial old agents in period $0$.
 The
real value of the lump-sum transfer of $M$ that the government hands to
 initial old agents depends on the 
equilibrium price level $p_0> 0$, so that in effect 
$-\tau^{-1}_0 = M / p_0 \geq 0$. 
Thereafter, the government takes no further actions.
...
for agent $t\geq0$. However, the budget set for an initial old
agent $t=-1$ differs from that implied by equation \Ep{olgbud} and
is now given by  
...
problem for each generation $t\geq -1$.
\medskip
...
a saving function
$$ y_t^t - c_t^t = s(\alpha_t; y_t^t, y_{t+1}^t). \EQN savfn $$
Then the equilibrium conditions  are
$$ \EQNalign{ {M  \over p_t} & = s(\alpha_t; y_t^t, y_{t+1}^t), \EQN sameq1;a \cr
                \alpha_t & = {p_{t+1} \over p_t}, \EQN sameq1;b \cr }$$
where it is understood that
$ c_{t+1}^t = y_{t+1}^t + {M \over p_{t+1}} $.
Equation \Ep{sameq1;a} states that at time $t$   net  saving of generation $t$ (the expression on the right side) equals
 net dissaving of generation $t-1$ (the expression on the left side).  To compute an equilibrium, we solve the difference equations
\Ep{sameq1} for $\{p_t\}_{t\geq 0}$,  then get the allocation
from the consumer's budget constraints evaluated at equality
at an equilibrium level of real balances $M/p_t$.

As an example, suppose that $u(c) = \ln(c)$,
and that $(y^t_t, y^t_{t+1}) = (w_1, w_2)$ with $w_1 > w_2$.
  The saving function is
$ s(\alpha_t; w_1, w_2) = .5(w_1 - \alpha_t w_2)$.   Then
...
The similarities across equilibria discussed in the previous section motivate us to formulate a  proposition that describes how,
after an adjustment to the endowment and the consumption allocated
...
The converse of the proposition is true.
\medskip
...
%acquire value.
 ...
 \section{Unbacked government debt}\label{sec:unbackedD}%
We now study how to transform  the fiat monetary equilibria of the preceding section 
into equilibria with unbacked government debt. Instead of issuing fiat currency in period $0$, the government finances the lump-sum transfer 
...
= {1 \over R_0 R_1} \,{B_3 \over R_2 } = \cdots 
...
Two equilibria are called isomorphic if they share the same
consumption allocation and rates of return.
With a change  of variables we can transform the equilibria with
...
To establish that each of these  transformations generates an   equilibrium, we must show that the resulting rates of return and amounts of assets are the same as in a stipulated original equilibrium, as well 
as that government budget constraints are satisfied. Note that 
both transformations  explicitly impose the same rates of return, i.e., we
...
As for the mapping from an unbacked-debt 
...
While fiat-money equilibria and unbacked-debt 
equilibria are  isomorphic, there are economically substantive differences in terms
of what the government controls in each type of equilibrium.  In an unbacked-debt equilibrium,
...
But recall that in  our transformation of an unbacked-debt 
equilibrium into a fiat-money equilibrium we
began by saying: `pick some arbitrary nominal 
quantity of money $M$'; that is, the positive nominal quantity $M$ itself
has no significance. Instead, what matters is 
...
Thus, depending on what agents' beliefs are, the associated price level $p_0$ pins down the 
...
equilibrium that will prevail (see chapters \use{optax} and \use{fiscalmonetary}).
In the current context, such a FTPL would point to      
government budget constraint \Ep{Gov_bc_M}
as the determinant of price level $p_0$ and, 
consequently, of the value of $\kappa$ 
in equation \Ep{samsol1}. Thus,  the fiscal policy
...
In addition to the transfer to the initial old agents $-\tau^{-1}_0\geq 0$,
...
As in computation \Ep{Gov_bc_iter}, we solve equations \Ep{Gov_bc_Bdef} 
...
purchases do not enter agents' utility functions, agents' objective functions are
unchanged. Budget sets are also unaffected: 
the budget constraint of an initial old agent is given by 
...
where $\alpha_t=1/R_t$. We are led to the following:
\medskip
...
and consumption of the initial old agent is augmented by the
...
To illustrate the equilibrium dynamics of the sequence
...
$f(R_t)= s(1/R_t; w_1, w_2)$. In an equilibrium, 
savings of the young $f(R_t)$ 
...
This equation supports our earlier analogy that 
declining indebtedness in an unbacked-debt equilibrium 
can be likened to a government `amortizing' its
...
`amortization' on the left side is now
...
Notice that whenever a solution of equation \Ep{DEq2gSS} exists,  
there is at least one other (except at the very peak of
the curve $(1-R) f(R)$).
...
higher interest rate $\overline R$ imposes a smaller distortion on
the intertemporal price. As we will elaborate when we study   
 the isomorphic fiat-money equilibrium, $(1-R)$ can be 
...
equilibria and says nothing about how they
were reached. So let's turn to the analysis
of nonstationary equilibria.



\subsection{Nonstationary equilibria}\label{sec:DefBnonstat}%
To analyze nonstationary equilibria, we adopt the offer-curve 
approach of subsection \use{sec:olg_nonstat}. But instead of
a clearinghouse, we retain the services of  a Walrasian auctioneer
who seeks an equilibrium price vector that ensures that the market 
clearing conditions \Ep{DEq3} are satisfied. In the
...
of the offer curve and the new feasibility line in 
...
we deduce $c^0_0$ from the government's fiscal decisions.
Specifically, as conveyed by  
...
$\underline R$. Convergence will still occur as we
...
In summary, there is a continuum of solutions
to  difference equation \Ep{DEq2g}, as indexed by the
initial transfer 
...
in any period $T\geq 1$ remains bounded, as shown in expression \Ep{Gov_debtdef}, 
...
as we have learned, the initial 
transfer $-\tau^{-1}_0$ has a permanent impact when it determines whether
the economy converges to a stationary
...
since discounting with negative net interest rates magnifies
rather than dampens present values of future expenditures.
The present value budget constraint nevertheless holds because
bounded government indebtedness at any future
date, as just described, becomes unbounded in
present value. The negative present value
of the stream of future government expenditures
and the positive present value of outstanding unbacked 
government debt in the distant future both go to infinity 
at rates that allow the budget constraint to be satisfied.



\section{Deficit finance with fiat money}
Here we explore deficit finance with fiat money instead of the unbacked
debt studied in the preceding section. The government budget constraints 
are then
...
exchange for time $t$ consumption). The  term $(M_t - M_{t-1})/p_t$  on the right side
...
$$ M_t = \argmax_{m \geq 0} \left[ u(y_t^t - m/p_t)
       ...
       \quad \forall t \geq 0; \EQN Mdef_eq1 $$
...
This follows from the fact that the nominal money
supply and the price level enter only as ratios of one another in all
relevant equations.



\subsection{Isomorphic equilibria under deficit finance}\label{sec:iso_deffin}%
...
We invite the
reader to carry out the mapping in the opposite direction
...
% equilibrium characterization as that of an economy 
% with bond-financed deficits in equations \Ep{DEq2g}. Thus, the 
% stationary-equilibrium characterization \Ep{DEq2gSS}
% also remains the same:
% $$ (1-R)\, f(R) = g \quad \Longrightarrow \quad 
%    {(p_{t+1}/p_t) -1  \over p_{t+1}/p_t} \,
%          f\!\left({p_t \over p_{t+1}}\right) = g, \EQN DEq2mSS 
% $$
% where we have invoked $R_t=p_t/p_{t+1}$ to shed light on the
% earlier taxation analogy in subsection \use{sec:SSLaffer}.
% Recall that we interpreted $(1-R)$ as a `tax rate' which here,
% at low inflation, is approximately equal to the net inflation 
% rate, $(p_{t+1}/p_t) -1$, and interpreted $f(R)$ as a `tax base' 
% which here is agents' demand for real money balances that
% equals real money supply, $M_t /p_t$, in an equilibrium. 
% The government's printing of money causes inflation and, 
% hence, erodes the real value of money holdings. In a stationary
% equilibrium, agents will want to replenish nominal
% money holdings so that real balances remain unchanged.
% The taxation analogy is then that the government can
% be thought to levy a `tax rate', approximately equal to 
% the net inflation rate, on agents' real money holdings, 
% the `tax base'. But of course, the `tax rate' cannot exceed 
% $100 \%$ (i.e., a money holder cannot fare worse than money 
% becoming worthless) so the exact expression for the 
% `tax rate' in equation \Ep{DEq2mSS} is 
% $(1-R)= [(p_{t+1}/p_t) -1] / (p_{t+1}/p_t) <1$.


% %Substitute the equilibrium condition $M_t / p_t = f(R_t)$, where
% %$R_t=p_t/p_{t+1}$, into these equations to get
% %$$\EQNalign{ f(R_t) &= f(R_{t-1})R_{t-1} + g  \quad 
% %                             \forall t \geq 1\EQN money1;a \cr
% %             f(R_0) &=  g - \tau^{-1}_0. \EQN money1;b \cr}
% %$$
% %As one might have anticipated, the equilibrium characterization
...
% \Ep{samdiff1}, which we now reformulate using a lag operator
% $$\left( 1 - {w_2 \over w_1} L^{-1} \right) p_t = {2 \over w_1} M_t,
%                                                        \EQN samdiff2
% $$
% where we now allow for a time-varying forcing term
...
% to the earlier expression \Ep{samsol1}, but otherwise we need the infinite sequence
...
% to a stationary equilibrium. Given the constant inflation rate in the
% characterization above of a stationary equilibrium, $p_{t+1}/p_t=1/R$,
% a natural conjecture is that the nominal money supply grows at a
% constant rate, $M_{t+1} = zM_t$ for some $z>0$. According to
...
% to  $z w_2/w_1<1$.\NFootnote{As detailed in 
% section \use{sec:olg_optimality},  
...
% counterpart to the stationary solution of our difference 
% equation \Ep{samsol2z}. While Wallace replaces our unfunded
% government expenditures $g$ with lump-sum transfers to agents,
% that difference in assumptions does not matter for Wallace's 
% proofs of necessity and sufficiency for the existence of at
% least one monetary equilibrium. (But of course, the difference
% in assumptions matters for any welfare analysis.)}
...
% $$
% By setting $\kappa=0$, we obtain the price sequence that clears
% the money market in a stationary equilibrium in which the 
% nominal money supply changes at a given rate $z>0$. The
% associated inflation rate is
% $$
% {p_{t+1} \over p_t} = { {2 \over w_1} { M_{t+1} \over 1 - {w_2 \over w_1} z }
%            \over {2 \over w_1} { M_t \over 1 - {w_2 \over w_1} z } }
%                     = {M_{t+1} \over M_t} = z,           \EQN DEq2mSSPz
% $$
% and the real money supply is
% $$
% {M_t \over p_t} = { M_t \over {2 \over w_1} { M_t \over 1 - {w_2 \over w_1} z } }
%                 = {w_1 -w_2 z  \over 2}   \quad = f(1/z),  \EQN DEq2mSz
% $$
% where the last equality is the savings function evaluated at the rate
% of return $R=p_t/p_{t+1}=1/z$. 

% In a stationary equilibrium with money-financed government deficits equal to $g$,  
% the growth rate of the nominal money supply $z$ must be chosen to satisfy
% equilibrium condition \Ep{DEq2mSS}:
% $$   {(p_{t+1}/p_t) -1  \over p_{t+1}/p_t} \,
%          f\!\left({p_t \over p_{t+1}}\right) = g
% \quad \Longrightarrow \quad 
%    {z -1  \over z} \,
%          f\!\left({1 \over z}\right) = g. \EQN DEq2mSSz 
% $$
% As depicted in Figure \Fg{graph52f}, there are two values of $z$ 
% that satisfy equation \Ep{DEq2mSSz}: 
% a high-inflation-rate equilibrium with $z=1/ \underline R>>1$ 
% and a low-inflation-rate equilibrium with $z=1/ \overline R>1$. 
% Drawing on the finding in subsection \use{sec:iso_deffin} that 
% equilibria are isomorphic when the government finances its 
% deficits either by printing money or by borrowing at a 
% negative real net interest rate, we can now infer the 
% following based on the analysis of the latter 
% equilibria in subsection \use{sec:DefBnonstat}. The 
% low-inflation-rate stationary equilibrium is 
% attained in period $0$ by setting the initial 
% transfer $-\tau^{-1}_0$ equal to the critical value in 
% equation \Ep{tau_highR}. But if the initial transfer 
% is smaller, all monetary equilibria will converge
% to the high-inflation-rate stationary equilibrium. 
% As noted earlier, the ``bad'' Laffer-curve equilibrium is 
% stable.
...
% dynamics.  They find that under either a crude form
% of  adaptive expectations  or a least-squares learning scheme,
...
\subsection{Computing equilibria again}\label{sec:DefMnonstat}%
...
The stability of the bad Laffer-curve equilibrium is a
feature of perfect foresight dynamics. It is natural to ask whether better
outcomes can emerge from an alternative to perfect foresight dynamics. 
Bruno and Fischer (1990) and Marcet and Sargent (1989) analyze how the
system behaves under two different types of adaptive dynamics with imperfect foresight.
...
intuitive at $\overline{R}$ (larger deficits bring higher inflation).
...
endowments.  
%We shall follow Sargent and Wallace  (1982) and Smith (1988)
...
% good substitute for private IOUs.
We assume  that
within each generation born at $t  \geq 0$,
...
$w_2(j)$ when old.  
A young person in   consumer group $j$ born at time $t$ behaves according to  
$$  s_{jt} = f(R_t,j) = \argmax_{s} 
\left[ u(w_1(j)-s)+u(w_2(j) + R_t s)\right] .\EQN savingsprob $$
If we  assume that all
consumers of generation $t$  have
preferences  $U^t({\bf c}^t) = \ln(c_t^t) + \ln(c^t_{t+1})$,
the saving function is
$$ f(R_t,j) = .5\left( w_1(j) - {w_2(j)  \over R_t} \right).$$
At $t=0$, there are old people who are endowed in
the aggregate with $H$ units of an inconvertible currency.

For example, assume that $J=2$, that $(w_1(1), w_2(1))  =
 (\alpha, 0)$ and that $
(w_1(2), w_2(2))   = (0,  \beta)$, where $\alpha >0$, $\beta >0$.
The type 1 people are lenders, while type 2 people are borrowers.
...
A nonmonetary equilibrium consists of sequences of rates 
of return, $\{R_t\}_{t=0}^\infty$, and savings rates, 
$\{s_{jt}\}_{t=0}^\infty$ for $j= 1, \ldots , J$, that satisfy
...
date $t \geq 0$ the old receive goods from the young
in exchange for the currency stock $H$.\NFootnote{As before,
we assume that the initial old agents start out owning
the currency stock $H$. The distribution of currency across
the initial old agents is immaterial to our analysis here.}
...
An equilibrium with valued fiat money is a set of
sequences of price levels, $\{p_t\}_{t=0}^\infty$, rates 
of return, $\{R_t\}_{t=0}^\infty$, and savings rates, 
$\{s_{jt}\}_{t=0}^\infty$ for $j= 1, \ldots , J$,
such that (1) $p_t>0$,
(2) $R_t =p_t / p_{t+1}$,
(3) $s_{jt} = f( R_t,j)$,  and  (4)
$ \sum_{j=1}^J N_j f(R_t, j) = {H \over p_t}.$
...
%\subsection{Another Gale diagram}
\subsection{Lucas tree}\label{sec:olg_lucas_tree}%
To set up an environment with a ``Lucas tree,'' take the preference and endowment
structure to be the same as subsection \use{sec:olg_stat} and modify only one feature.
Endow the initial old with a claim
to a constant stream of $d >0$ units of consumption for each
$t\geq 0$.\NFootnote{This is a version of an example of Brock (1990). The `Lucas tree' refers to a colorful interpretation
...
$$ q_0^0 c_0^{-1} = d \sum_{t=0}^\infty q_t^0 + q_0^0 y_0^{-1}
            = d \sum_{t=0}^\infty q_t^0 + q_0^0 \epsilon. $$       
The offer curve of each young agent remains as before,
but now the feasibility line is
$$ c_i^i + c^{i-1}_i =  y_i^i +y_i^{i-1} + d = 1+d $$
for all $i \geq 0$.  Note that young agents are  endowed
...
old would be unbounded, prompting an unbounded demand for consumption---which is not
feasible.
...
$R = \alpha^{-1}$.  Here $p$ is the ex-dividend price of the
Lucas tree for all $t \geq 0$ in a sequential-trading equilibrium.

For the sake of argument, suppose that the Lucas
tree bears no fruit, $d=0$, but that the tree itself is durable.
What would be the price of such a tree? According to the above price 
formula based on `fundamentals', $p = \sum_{t=1}^\infty q_t^0 d $, 
the tree would be worthless, $p=0$. However, if the tree takes on the
same role as the constant quantity of fiat money in 
section \use{sec:Samuelson101}, the price
of the tree would be governed by the same valuation calculations that apply to fiat money. 
However, note that the price level in  
section \use{sec:Samuelson101} is the price of goods in terms of fiat
money; in our current context, that  would instead be the price of goods in 
terms of Lucas tree. Hence, the price of the Lucas tree (in terms of
goods) would be the inverse of the price level in 
section \use{sec:Samuelson101}.
...
\subsection{Population growth}  
  It is easy to extend our models to accommodate
growth in the population.  Let there be $N_t = n N_{t-1}$ identical young
people at time $t$, with $n > 0$. Then the extended 
budget constraint \Ep{Gov_bc} in period $t\geq0$ becomes
$$ {N_t m^t_t - N_{t-1} m^{t-1}_{t-1} \over  p_t} + N_{t-1} \tau^{t-1}_t + N_t \tau^t_t 
+ {N_t b^t_{t+1} \over R_t} = N_t g_t + N_{t-1} b^{t-1}_t,  $$
where $g_t$ is per-young-person government   purchases.
Dividing both sides of the budget constraint by
$N_t$ and rearranging gives
$$ \hat m_t + n^{-1} \tau^{t-1}_t + \tau^t_t + {b^t_{t+1} \over R_t} 
= n^{-1} \hat m_{t-1} {p_{t-1} \over p_t} + g_t + n^{-1} b^{t-1}_t,  
                                                              \EQN Gov_bc_pop $$
where $\hat m_i$ is real money balances of a young agent at 
time $i$, $\hat m_i \equiv m^i_i / p_i$.


For comparison, recall the high-interest-rate stationary
equilibrium with unbacked debt in section \use{sec:unbackedD} with
interest rate $R=1$. The counterpart in the present economy with 
population growth is a stationary equilibrium in which the amount
of bonds purchased by a young agent stays constant over time, 
$b^i_{i+1}=b$ for all $i$,
and satisfies the corresponding version of government budget 
constraint \Ep{Gov_bc_pop}
$$ {b \over R_t} = n^{-1} b.  $$
Evidently, the stationary high interest rate is now $R=n$.
Similarly, the counterpart to the stationary equilibrium with 
fiat money in section \use{sec:Samuelson101} now has  
constant per-young-agent real money holdings, 
$\hat m_i=\hat m$ for all $i$, that satisfy the 
corresponding version of government budget constraint 
\Ep{Gov_bc_pop}
$$ \hat m  = n^{-1} \hat m {p_{t-1} \over p_t}.$$
Thus, the stationary return on fiat money, 
$p_{t-1}/ p_t = n$, is the same as the return on 
unbacked debt, $R=n$, which was to be expected 
since the two equilibria can be shown to be isomorphic
(as we showed for $n=1$ in subsection \use{sec:iso_mb}).

%Updated%%%%%%%%%%%%%%%%%

It is the positive population growth rate $n>1$ that allows
the government to pay a gross interest rate greater than
one, $R=n>1$. The government finances the positive net 
interest rate by issuing larger amounts of one-period bonds
to satisfy the growing aggregate demand from an ever-increasing 
cohort of young agents where each agent wishes to hold
bonds $b^i_{i+1}=b$. In contrast, the government in the
monetary economy does nothing after having initially issued
a stock of fiat money $M$. Here it is deflation with falling
price levels that yield the same rate of return
$p_{i}/ p_{i+1} = n>1$.
...
\subsection{Gift-giving equilibrium}
...
$$ u(y^i_i - s_i) + u(y^i_{i+1} + s_{i+1}), \quad i \geq 0
  \EQN utility1 $$
...
$v_0$.   Equations   \Ep{folk1} act as the transition
laws that young agents face in choosing $s_i$ in
\Ep{utility1}.

  An initial condition $v_0$ and the rule \Ep{folk1} form a system
...
if for each $i \geq 0$, each young   agent chooses
to conform.
...
this equilibrium within equations \Ep{folk1}, set $v_0 \neq \overline v$.
...
person chooses not to give.
  ...
  For the second equilibrium, set $v_0 = \overline v$.   Here
...
zero. The welfare of the initial old agents at time $0$ is
strictly increasing in $c^{-1}_0$, and each one of them is endowed with
$y_2$ goods and $m^{-1}_{-1}> 0$ units of fiat money. Thus, the
beginning-of-period aggregate nominal money balances in the initial
period $0$ are $M_{-1}=N_{-1} m^{-1}_{-1}$.

For all $t \geq 0$, $M_t$, the post-transfer time $t$ stock of fiat
...
of a young agent born in period $t\geq 0$ are
...
> \theta_{\rm aut},   \quad  \forall t \geq 0 .            \EQN wall5
$$
...
$C = \{c^{-1}_0; (c^t_t,c^t_{t+1}), t\geq 0\}$ is feasible if
$$ \EQNalign{
N_t c_t^t + N_{t-1} c_t^{t-1} &\leq N_t y_1 + N_{t-1} y_2,
                                       \quad  \forall t \geq 0, \cr
\noalign{\hbox{\rm or, equivalently,}}
n c_t^t + c_t^{t-1} &\leq n y_1 + y_2, \quad  \forall t \geq 0.
                                                        \EQN wall7 \cr}
$$
...
\hat c^t_{t+1} = y_2 + \epsilon [\theta_{\rm aut} + f(\epsilon)]
               \equiv \hat c_2,    \quad  \forall t \geq 0. \EQN wall14
$$
...
for $t\geq 1$. By setting $\hat c^{-1}_0 = y_2 + n \epsilon$, feasibility
is also satisfied in period $0$ and the initial old generation is
...
$(c^t_t,c^t_{t+1}) = (c_1,c_2)$ for all $t\geq 0$, is given by
$$
...
at time $t\geq 0$ by $(\hat c_1, \hat c_2)$. It is also the case
...
$(\hat c_1, \hat c_2)$ and all generations born in period $t \geq 0$
...
$$
\overline c^{-1}_0 = y_2 + n(y_1 - \overline c^0_0)
                > y_2 + n(y_1 - \hat c^0_0) = \hat c^{-1}_0.
$$
...
\section{Dynamic inefficiency in a growth model}\label{sec:dynineffgrowth}%
The starting point of our analysis is a version of the optimal 
growth example with an infinitely-lived representative household 
in chapter \use{dynamicdp1}. Our decentralization of the allocation 
in a competitive equilibrium foreshadows our presentation of growth models
 in chapters \use{linappro} and \use{growth1}. We use the 
efficient equilibrium outcomes in models with infinitely-lived 
agents as a benchmark against which we can compare possibly
inefficient equilibrium
allocations that  entail overaccumulation of capital.



\subsection{Technology and input prices}\label{sec:growth_infin}% 
The planner in the optimal growth example of 
chapter \use{dynamicdp1} solves the following problem:
$$ \max_{\{c_t, k_{t+1}, l_t\}_{t=0}^\infty} \sum_{t=0}^\infty \beta^t \ln(c_t)  $$
where maximization is subject to
$$\EQNalign{&c_t + k_{t+1} = A \,  k_t^\alpha l_t^{1-\alpha}
                           \;\;\equiv F(k_t,l_t),                     \EQN growth1 \cr
            &c_t \geq 0,\; k_{t+1} \geq 0,\; l_t\in [0,1],  \cr
            &\hbox{\rm given}\, k_0 > 0,    \cr} $$
where  $A >0$, $\alpha \in (0,1)$, and $\beta \in (0,1)$.
We have added labor supply $l_t$ as a choice variable and an
input to the production function $F(k_t,l_t)$ in resource 
constraint \Ep{growth1}. The household is endowed with one unit
of time in each period that it  can  allocate to working, so
$l_t\in [0,1]$. Since the time spent working does not
enter the period utility function, $\ln(c_t)\equiv u(c_t)$,
the planner will  set $l_t$ equal to its highest 
value.
After plugging that optimal value $l_t=1$ into resource 
constraint \Ep{growth1}, the planner's optimization problem becomes 
identical to one formulated in 
chapter \use{dynamicdp1}.\NFootnote{As will become
evident, an important feature  of our augmented production function in 
equation \Ep{growth1} is that the exponents on capital 
and labor add up to one.  This  makes the 
technology have  constant returns to scale,
$\lambda F(k,l) = F(\lambda k, \lambda l)$ for any
$\lambda >0$. By setting $\lambda = 1/l$, we see that the 
capital-labor ratio is the determinant of average labor 
productivity, $F(k,l)/l=F(k/l,1)$, and, hence, the 
size of the economy {\it per se} 
does not matter. This explains why assuming that the  household's time endowment equals  one 
makes the production function, $F(k_t,1)=A \, k_t^\alpha$, 
identical to the one in the growth example of 
chapter \use{dynamicdp1} where capital is
the sole input; though, note that $k_t$ in the present
model is both the capital stock and the capital-labor 
ratio.\label{note:prodCRS}}


% with the same optimal solution for $\{c_t, k_{t+1}\}_{t=0}^\infty$.

As discussed  in chapter \use{dynamicdp1}, the optimal solution
satisfies an Euler equation,
$$ 1 = \beta {u'(c_{t+1})\over u'(c_t)} F_1(k_{t+1},1),  \EQN growth2$$
where $F_j(k,l)$ is the partial derivative with respect to
the $j$-th argument and we have invoked the optimal labor
supply $l_{t+1}=1$. When the economy has converged 
to a stationary value of capital, as given by 
$k_\infty = (A\beta \alpha)^{1 \over 1-\alpha}$
in chapter \use{dynamicdp1}, the associated
constant value of consumption can be deduced from resource
constraint \Ep{growth1}. Thus, the Euler equation 
in a steady state simplifies to  
$$F_1(k_\infty,1) = {1 \over \beta}. \EQN growth3$$

 A competitive 
equilibrium in a production economy with infinitely-lived 
agents is Pareto efficient (see chapter \use{growth1}).
There are several ways to formulate a competitive equilibrium, each with its own
versions of optimization problems that confront price-taking  households and firms (managers of the
production process).
% operating the production technology 
%in a market economy.
 Here we adopt a common
assumption that households own the capital 
and rent it to firms at a rental rate $r_t$, 
and sell their labor services at a wage rate 
$w_t$. Profit maximization by firms implies
that the marginal products of capital and labor equal the rental
rate and the wage rate, respectively.
Thus, in a competitive equilibrium,
% that is, the following
% first-order conditions for profit maximization hold
% in an equilibrium:
$$\EQNalign{  r_t &= F_1(k_t,l_t)=
  \alpha A \, \left(k_t \over l_t\right)^{-(1-\alpha)}
  \equiv r(k_t/l_t),     \EQN firm_FOC;a \cr
              w_t &= F_2(k_t,l_t)= 
  (1-\alpha) A \, \left(k_t \over l_t\right)^{\alpha}
  \equiv w(k_t/l_t),     \EQN firm_FOC;b \cr}$$    
where equilibrium labor supply is $l_t=1$.\NFootnote{As 
noted in footnote \use{note:prodCRS}, the production
function is homogeneous of degree 1 so, by Euler's 
theorem, the function equals the sum of its
partial derivatives multiplied by their
respective variables:
$$F(k,l) = F_1(k,l)\,k+F_2(k,l)\,l = r k + w l
         = \alpha F(k,l) + (1-\alpha)F(k,l),$$
where the second and third equalities invoke equilibrium
conditions \Ep{firm_FOC}. 
Thus, output is fully distributed as payments to
the two production factors. With our Cobb-Douglas
production function, those factor shares are constant over time and
equal to $\alpha$ for capital and $1-\alpha$ for labor.
For further details,
%and for an alternative market decentralization in
%which firms invest in and own the capital, 
see chapter \use{growth1}.\label{note:facshares}}%


In contrast to the efficient competitive equilibria 
in models with infinitely-lived agents,  an equilibrium allocation in a 
growth model with overlapping generations can fail 
to be Pareto efficient, a possibility that we  explore next.


\subsection{Agents' optimization and market clearing}
The initial old representative agent $i=-1$ in the overlapping 
generations model is endowed with $k_0$ units of capital.
The sole endowment of all other agents $i\geq 0$ is one unit 
of time when young that can be devoted to working, while old 
agents are unable to work. A young agent in period $t\geq0$
supplies labor $l^t_t$ and purchases goods for
current consumption $c^t_t$ and investment that will 
turn into capital $k^t_{t+1}$ next period (which will 
finance old-age consumption $c^t_{t+1}$). The agent
solves the following optimization problem:
$$ \max_{\{c^t_t, c^t_{t+1}, k^t_{t+1}, l^t_t\}} 
      \ln(c^t_t) + \beta \,\ln(c^t_{t+1})     \EQN olgmax $$
subject to
$$\EQNalign{&c^t_t + k^t_{t+1} \leq  w_t \, l^t_t, \EQN growth_bc;a \cr
            &c^t_{t+1} \leq  r_{t+1} \, k^t_{t+1}, \EQN growth_bc;b \cr
            &c^t_t, c^t_{t+1}, k^t_{t+1} \geq 0;\; l^t_t\in [0,1].  \cr}$$
Here we have added a discount factor $\beta>0$ to earlier
preference specification \Ep{olg1;a}.\NFootnote{In the case of 
infinitely-lived agents in the preceding subsection, we imposed the
restriction that $\beta\in (0,1)$ to ensure that an agent's lifetime 
utility is bounded. That is no longer necessary when an agent's life 
length is finite. Parameterizations with $\beta\geq 1$ are common 
in applied work using overlapping generations models; a larger discount factor generates larger savings, which helps to
match aggregate observations of high capital-output ratios.}
 Since
there is no disutility from working, the agent chooses to set $l^t_t=1$.

The operation of firms and the characterization of equilibrium 
factor prices in expression \Ep{firm_FOC} remain the same as in
the model with infinitely-lived agents. The same  Euler 
equation \Ep{growth2} emerges from the first-order conditions
of the present optimization problem \Ep{olgmax}:
$$ 1 = \beta \,{u'(c^t_{t+1})\over u'(c^t_t)}\,  r_{t+1} 
     = \beta\, {u'(c^t_{t+1})\over u'(c^t_t)}\, F_1(k_{t+1},1), 
                                                      \EQN olg_Euler_growth $$
where the second equality follows from equilibrium
condition \Ep{firm_FOC;a} and we have invoked equilibrium
labor supply $l_{t+1}=1$. But   
 Euler equation \Ep{olg_Euler_growth} now pertains
to a life-cycle optimization problem where the agent is
only concerned about the two periods $t$ and $t+1$. 

Under our assumption that $u(c)=\ln(c)$, the saving function is
$ s( w_t,  r_{t+1}) = 
 w_t - c^t_t= w_t - {1 \over 1+\beta} w_t
= {\beta \over 1+\beta} w_t$.\NFootnote{With logarithmic
preferences, the intertemporal elasticity of substitution is
equal to one, so when the rate of return on savings
rises, the incentive to consume later (substitution effect) is
exactly offset by the increased wealth that allows for higher
consumption today (income effect). Thus, variations in the rental rate on
capital $ r_{t+1}$ do not affect optimal savings.}
In an equilibrium, saving of the representative agent
$t\geq 0$  equals next period's capital stock $k_{t+1}$:
$$k_{t+1} =  s( w_t,  r_{t+1})
            = {\beta \over 1+\beta} w_t
            = {\beta \over 1+\beta} (1-\alpha)A k^{\alpha}_t,
                                                    \EQN olg_k
$$
where the third equality follows from equilibrium
condition \Ep{firm_FOC;b} and we have invoked equilibrium
labor supply $l_t=1$. The law of motion for capital in
equation \Ep{olg_k} converges to a steady state in 
which capital equals 
$\hat k = [{\beta \over 1+\beta} (1-\alpha)A]^{1 \over 1-\alpha}$. 


\subsection{Dynamic inefficiency and Pareto 
                               improvements}\label{sec:dynineff}%
In appendix \use{appendixgrowth}, we generalize our model by 
allowing for population growth, $N_t=n N_{t-1}$ with $n>0$, and 
by introducing a capital depreciation rate $\delta\in (0,1]$ 
that can be less than the present assumption of $100\%$ 
depreciation. With those extra features, the following 
proposition delineates when a stationary equilibrium
fails to be Pareto optimal.

\medskip
\noindent{\sc Proposition:}  Suppose a competitive equilibrium converges
to a steady state with capital-labor ratio $k^\star$ satisfying
$$ F_1(k^\star,1) < \delta +n-1 ,
$$
then the equilibrium allocation is not Pareto optimal.

\medskip


\noindent
In our setting with $\delta=n=1$, the condition for 
inefficiency translates into 
$F_1(k^\star,1)< 1$, which implies that 
$k^\star>[\alpha A]^{1 \over 1-\alpha} \equiv \bar k$.
We can compute a set of parameter values that make
the steady-state value $\hat k$, as derived above, exceed $\bar k$:
$$ \hat k > \bar k \quad \Longrightarrow \quad 
   {\beta \over 1+\beta} \, {1-\alpha \over \alpha} > 1, \EQN olg_param
$$
which requires that $(1-\alpha)>\alpha$ since
$\beta/(1+\beta)<1$. As one might have anticipated, 
parameterizations that would
lead young agents to choose relatively high savings 
can depress rates of return on capital so much that there
is ``dynamic inefficiency'' in the sense of an overaccumulation of capital. 
This outcome is more ``likely'' the
greater the discount factor $\beta$ is, i.e., when agents 
place a high weight on the utility of future consumption,
and the larger the labor share of output ($1-\alpha$) is 
relative to the capital share ($\alpha$), so that  young 
agents receive relatively high labor earnings 
(see footnote \use{note:facshares} about  factor
shares).

To prove the condition for inefficiency, we show
by example a possible Pareto improvement that moves the 
economy from an inefficient steady state to a Pareto 
optimal steady state in just one transition period. 
A government policy that implements such a Pareto
improvement in a competitive equilibrium has two
parts: (i) a one-time lump-sum tax and transfer scheme 
in the transition period, and (ii) the issue and eternal 
rollover of unbacked government debt.

Our welfare analysis starts with the resource constraint 
(and goods market clearing condition) in period $t\geq 0$:
$$ c^t_t + c^{t-1}_t + k_{t+1} = F(k_t,1),      \EQN olg_res $$
which, at a stationary allocation with capital $k$, can be
rearranged to read 
$$ c_1 + c_2   = F(k,1) - k 
                           \equiv C(k),   \EQN olg_SS $$
where $c_1$ and $c_2$ are the consumption of a young and 
an old agent, respectively. Figure \Fg{graphgrow}
depicts components of expression \Ep{olg_SS} as functions
of a stationary value of $k$. Hence, the stationary
aggregate consumption $C(k)$ on the solid line is the
difference between the two dashed lines:
aggregate output $F(k,1)$ minus the stationary
value of capital $k$. Since the slope of the latter
dashed diagonal line is one, consumption $C(k)$ attains
a maximum when the marginal product of capital has fallen
to one, which occurs at $\bar k$ as computed above.
The maximum sustainable level of consumption $C(\bar k)$ 
is known as the Golden Rule of capital 
accumulation.\NFootnote{Evidently, if the initial 
capital endowment of the infinitely-lived household in 
subsection \use{sec:growth_infin} is $k_0=\bar k$, i.e., the 
Golden-Rule level of capital, an efficient allocation
would gradually  reduce capital until it reaches  
$k_\infty$, the value prescribed by the steady-state Euler equation \Ep{growth3}.
The utility of temporarily higher consumption
outweighs the ultimate and permanently lower level
of consumption.}

\midfigure{graphgrow}
\centerline{\epsfxsize=3truein\epsffile{OLG_growth.eps}}
\caption{Output $F(k,1)$ and consumption $C(k)$
as functions of a stationary capital-labor ratio $k$. The 
Golden-Rule level of $k$ is $\bar k$, i.e., 
$C'(\bar k)=0$, and $k^\star$ is an example of
a dynamically inefficient steady state.}
\infiglist{graphgrow}
\endfigure


For the sake of argument, consider a steady-state 
equilibrium with capital $k^\star$ that exceeds $\bar k$, 
i.e., capital $k^\star$ is on the downward-sloping 
side of $C(k)$ as illustrated in Figure \Fg{graphgrow}.
Starting in some period $T$, the following change in
capital expands aggregate consumption in 
all periods $t\geq T$ and, hence, it would be feasible 
to increase consumption of all currently living and all 
future agents. Specifically, the planner in period $T$
permanently lowers next period's and all future 
periods' capital from $k^\star$ to $\bar k$. Thus,
the resource constraint in  period $T$
becomes
$$ c^T_T + c^{T-1}_T + \bar k = F(k^\star,1)
\;\;\;=C(k^\star) + k^\star,      \EQN olg_trans $$
which can be rearranged to read
$$ c^T_T + c^{T-1}_T
   =C(k^\star) + k^\star - \bar k > C(k^\star). \EQN olg_trans2 $$
Not surprisingly, aggregate consumption in period $T$ 
is increased by the reduction in capital investment,
i.e., by $k^\star - \bar k>0$. In all future periods $t>T$, 
aggregate consumption is also increased since 
$C(\bar k) > C(k^\star)$ in Figure \Fg{graphgrow}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Any steady-state equilibrium on the upward-sloping part
%of $C(k)$ is Pareto optimal
%young agents are already earning rates of return
%greater than or equal to a social rate of
%transformation of one

%Also the infinite horizon of the economy is essential 
%In the last period
%$$ c^T_T + c^{T-1}_T + 0 = F(1,\bar k) < F(1,k^\star),
%                                               \EQN olg_last $$
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


This change in capital can be attained in a competitive 
equilibrium with the following government 
policy.\NFootnote{We assume that the policy comes
as a surprise to the agents when implemented in period $T$. 
If the policy had been known in advance in period $T-1$, 
young agents in period $T-1$ would have changed their
savings because of a lump-sum transfer they were about 
to receive in period $T$. That would have altered 
the capital stock $k_T$.}
In the 
transition period $T$, an old agent receives a lump-sum 
transfer $\tau^{T-1}_T<0$ as computed below, while a young 
agent pays a lump-sum tax $\tau^T_T>0$ so that the disposable 
income becomes the same as the labor earnings of all future 
young agents whose wages will be suppressed under the 
targeted lower future capital $\bar k$,
$$\tau^T_T = w(k^\star) - w(\bar k)>0.     \EQN olgex1;a
$$
The government policy so far would simply set the current 
young and future agents back as if the economy had an 
imaginary capital equal to $\bar k$ already at the 
beginning of period $T$ 
and then, according to equilibrium law of motion 
\Ep{olg_k}, capital would 
eventually revert to its higher steady-state value $k^\star$
and rate of return $r(k^\star)<1$. 
So in order to freeze future capital at $k_{T+1}=\bar k$ 
and rate of return $r(\bar k)=1$, the government 
in period $T$ issues one-period bonds at a sale price 
equal to the bonds' face value, i.e., the 
debt policy sets a floor for the equilibrium gross interest 
rate, $R_T\geq 1$. Specifically, if an 
equilibrium without government bonds has an interest rate 
$R_T> 1$, there would be no demand for those bonds; 
but if sold in an equilibrium, a no-arbitrage 
argument ensures that not only bonds but also capital would
earn the same gross rate of return equal to one. Any issued
bonds are rolled over forever thereafter. Given our 
assumption $k^\star>\bar k$, the strictly positive quantity of 
bonds required to support the interest rate $R_T=1$ can be 
deduced from the market clearing condition for savings in 
period $T$,
$$  s(w(k^\star)-\tau^T_T,\,r(\bar k)) 
                  = s(w(\bar k),\, r(\bar k))
                  = \bar k + {B_{T+1} \over R_T}, \EQN olgex1;b
$$
where $r(\bar k)=1=R_T$. The government policy must also be 
budget feasible:
$$\EQNalign{
\tau^{T-1}_T &= -\left( \tau^T_T + {B_{T+1} \over R_T} \right),
                                              \EQN olgex1;c \cr
{B_{t+1} \over R_t} &= B_t, \quad \forall t\geq T+1, \EQN olgex1;d \cr}
$$
where equation \Ep{olgex1;c} is the budget
constraint in period $T$ (that residually determines
$\tau^{T-1}_T$) and equations \Ep{olgex1;d} are the
future budget constraints, all of which are evaluated at
the equilibrium interest rates $R_t=1$ for all $t\geq T$.
Evidently, the quantity of bonds stays constant over time
and can be computed from market clearing condition 
\Ep{olgex1;b},
$$\bar B = s(w(\bar k),\, 1) - \bar k
         = {\beta \over 1+\beta} w(\bar k) - \bar k >0, \EQN olg_barB
$$
where the strict inequality is implied by $\bar k$ being 
less than the original steady-state value $k^\star$, i.e., 
parameter restriction \Ep{olg_param} holds.\NFootnote{To prove
the strict inequality in expression \Ep{olg_barB},
substitute equation \Ep{firm_FOC;b} and
$\bar k =[\alpha A]^{1 \over 1-\alpha}$ into
that expression to obtain
$$\bar B = {\beta \over 1+\beta} (1-\alpha)A 
           [\alpha A]^{\alpha \over 1-\alpha} 
           - [\alpha A]^{1 \over 1-\alpha} 
         = [\alpha A]^{1 \over 1-\alpha} 
\left[ {\beta \over 1+\beta} \, {1-\alpha \over \alpha} -1\right] >0,
$$
where the strict inequality follows from parameter
restriction \Ep{olg_param}.}


This government policy results in a Pareto
improvement where all currently living and all future 
agents are strictly better off. The old agent at time $T$ is 
trivially better off when receiving additional income 
from the lump-sum transfer
that equals the sum of the lump-sum tax paid by the young 
agent in period  $T$ and the funds raised by the government 
from the first issue of one-period bonds in period $T$. 
All young agents $t\geq T$ are better off in 
spite of receiving a smaller income $w(\bar k)$ that is less
than their wage earnings $w(k^\star)$ in the original steady 
state. First, the fact that $C(\bar k) > C(k^\star)$ implies 
that the sum of young-age and old-age consumption is greater 
in the new steady state as compared to the original steady state. Second, 
because they derive all their income in young age and,
hence, are savers, they benefit from the higher interest 
rate in the new steady state as they attain a more
favorable intertemporal consumption profile --
consuming less in young age, $\bar c_1$, and more in old age, 
$\bar c_2$, as compared to the original steady state,
($c^\star_1$, $c^\star_2$):
$$
\bar c_1 
={1 \over 1+\beta} w(\bar k)\; <\; {1 \over 1+\beta} w(k^\star)
= c_1^\star,                                  \EQN olg_cbar;a
$$
where the strict inequality is directly implied by $\bar k < k^\star$
and that the wage is equal to the marginal product of labor
in equation \Ep{firm_FOC;b};
$$
\bar c_2 =s(w(\bar k),\,1) \; >\; 
r(k^\star) \, s(w(k^\star),\, r(k^\star)) = c_2^\star,
                                      \EQN olg_cbar;b
$$
where the strict inequality follows from parameter
restriction \Ep{olg_param}.\NFootnote{After 
substituting savings function 
$ s(w, r) = {\beta \over 1+\beta} w$ and factor-price 
expressions \Ep{firm_FOC} into 
expression \Ep{olg_cbar;b} and invoking
$\bar k =[\alpha A]^{1 \over 1-\alpha}$
and $k^\star = \hat k = 
[{\beta \over 1+\beta} (1-\alpha)A]^{1 \over 1-\alpha}$,
the resulting inequality
%expression \Ep{olg_cbar;b} becomes
%$${\beta \over 1+\beta} (1-\alpha)A 
%[\alpha A]^{\alpha \over 1-\alpha}
%> \alpha A  
%\left({\beta \over 1+\beta} (1-\alpha)A\right)^{\alpha \over 1-\alpha}
%$$
%which 
can be simplified and rearranged to read
$$
\left[{\beta \over 1+\beta} {1-\alpha \over \alpha}\right]^{1-{\alpha \over 1-\alpha}}
> 1,
$$
where the quantity inside square brackets is greater than one by 
parameter restriction \Ep{olg_param}; a restriction
that also implies that $\alpha < (1-\alpha)$ and,
hence, the quantity greater than one in
square brackets is raised to a strictly
positive power. Thus, the strict inequality here  holds and, hence, also
holds for expression \Ep{olg_cbar;b}.}



\subsection{Money and the real bills doctrine}

In accordance with subsection \use{sec:iso_mb},
we can transform the equilibrium
under the government policy with unbacked debt in the
preceding subsection into an equilibrium with
valued fiat money. The one-time lump-sum tax and
transfer scheme in period $T$ is the same but instead 
of issuing unbacked debt to be rolled over forever, 
the government once and for all issues a quantity of 
fiat money $\bar M$ in period $T$. This money is immediately  handed over
to the current old agent.  There 
is now a single government budget constraint,  
$$\tau^{T-1}_T = -\left( \tau^T_T + {\bar M \over p_T} \right).
                                              \EQN kM_bc
$$
After period $T$, the government takes no further actions.

Following the steps in subsection \use{sec:iso_mb},
to arrive at an  equilibrium with
fiat money, pick some arbitrary nominal 
quantity of money $\bar M$ and solve for
$p_T$ from $\bar M/p_T=B_{T+1}/R_T$. All
other prices, $\{p_t\}_{t>T}$, are then
recursively set according to 
$p_{t-1}/p_t=R_{t-1}$ for $t>T$. Since the
gross rate of return equals one in
the equilibrium with unbacked debt, the
price level stays constant in the monetary 
equilibrium and equals
$$
p_t = \bar p = {\bar M \over \bar B}, 
\quad \forall t\geq T,           \EQN groMp
$$
where the time-invariant debt level $\bar B$ in the
earlier equilibrium is given by \Ep{olg_barB}. The
new market clearing condition for savings
becomes
$$ s(w(\bar k),\, 1)
        = \bar k + {\bar M \over \bar p}. \EQN groMs
$$

Next, to study   
the real bills doctrine, we allow the government to issue additional
money after period $T$. We show that so long as the 
additional money is used to acquire capital, any expansions of 
the money supply
%, $\{M_t\geq\bar M\}_{t\geq T+1}$, 
will leave the price level 
%$\bar p$ 
and equilibrium allocation
%$\{\bar c_1, \bar c_2, \bar k\}$ 
unchanged. 
Let
$k^g_{t+1}\in[0,\bar k]$ be the capital held by the
government between periods $t$ and $t+1$. Under an assumption that these  open market 
operations leave the price level and allocation unchanged, the government budget constraints (and
constraints on open market operations) become
$$ {M_t - M_{t-1} \over  \bar p} + r(\bar k) k^g_t 
        = k^g_{t+1}, \quad \forall t \geq T+1, \EQN groMGbc $$
where $M_T=\bar M$ and $k^g_{T+1}=0$.
Equation \Ep{groMGbc} states that the government finances 
capital holding $k^g_{t+1}$ in two ways:
first, by injecting new money in the amount $M_t-M_{t-1}$,
and second, by rolling over the proceeds $r(\bar k) k^g_t$
from last period's capital holding where $r(\bar k)=1$.
Furthermore, the open market operations change the market 
clearing condition \Ep{groMs} for private agents' savings 
to become
$$ s(w(\bar k),\, 1)
        = \bar k - k^g_{t+1} + {M_t \over \bar p},
\quad \forall t \geq T+1, \EQN groMs2
$$
where the net of the first two terms on the right side is 
the capital held by private agents and the third
term is the aggregate real money supply.

 Effects of open market operations
like this have concerned monetary economists for centuries  under several guises.\NFootnote{One
...
that the notes $M_t$ be issued by the government.
We can also think of them as being issued by a private bank.}
We  state the following proposition:
\medskip
\noindent{\sc Irrelevance of Open Market Operations:}   
Sequences $\{M_t\geq \bar M, k^g_{t+1}\in [0, \bar k]\}_{t=T+1}^\infty$
 that satisfy the constraint
\Ep{groMGbc} on open market operations  are associated with the 
same equilibrium allocation $\{\bar c_1, \bar c_2, \bar k\}$ as
derived in subsection \use{sec:dynineff} and
price level $\bar p$ in equation \Ep{groMp}.


\medskip
\noindent{\sc Proof:} We can rearrange and manipulate the government 
budget constraint \Ep{groMGbc} for period $t=T+j$ as follows
$$\EQNalign{
  {M_{T+j} \over \bar p} &= k^g_{T+j+1} -k^g_{T+j} + {M_{T+j-1} \over  \bar p} \cr
   &= \sum_{i=0}^{j-1} \left(k^g_{T+j+1-i} -k^g_{T+j-i}\right) \, + {M_T \over  \bar p} 
   = k^g_{T+j+1} + {\bar M \over  \bar p},                       \EQN groMGbc2  \cr}
$$
where the first equality invokes $r(\bar k)=1$, the second equality solves the
equation backward to period $T+1$, and the third equality invokes initial
conditions $k^g_{T+1}=0$ and $M_T=\bar M$. Substituting \Ep{groMGbc2} 
into market clearing condition \Ep{groMs2} for period $t=T+j$ gives
$$ s(w(\bar k),\, 1)
        = \bar k - k^g_{T+j+1} + {M_{T+j} \over \bar p} 
        = \bar k  + {\bar M \over  \bar p},                       \EQN groMs3
$$
which is the same as market clearing condition \Ep{groMs}
in the economy with no open market operations. 
Evidently, it does not matter if the government or the
private sector owns the capital when the additional 
money issued for the government's acquisition has 
the same rate of return and is fully backed by the 
capital. Specifically, when the government
stops rolling over its capital holding, the constraint
\Ep{groMGbc} on open market operations prescribes that
the corresponding quantity of money is redeemed by the
government and de facto returns the ownership of capital 
to the private sector. Thus, the invisible hand 
ensures that the sum of private sector and government 
holdings of capital equals $\bar k$.  \qed
...
$ M_t - \bar M$ is sometimes called ``inside money'' 
that is 100\% backed by evidences of private indebtedness while
$\bar M$ is called ``outside money'' that is ``unbacked''.   
...
This chapter has shown that the overlapping generations structure can produce a
multiplicity of equilibria, dynamically inefficient capital accumulation, and
positive value for intrinsically worthless assets. The isomorphism between
fiat-money and unbacked-debt equilibria, and the irrelevance of open-market
operations when money is fully backed, are its central analytic contributions.
...
Auerbach  and  Kotlikoff (1987)  formulated a large-scale overlapping
generations model with capital, labor, production, and
various kinds of taxes. They used the model to study
a host of fiscal issues.\NFootnote{Rios-Rull (1994a) used a calibrated
overlapping generations growth model to examine the quantitative importance of
market incompleteness for insuring against aggregate risk. See Attanasio (2000)
for a review of theories and evidence about consumption within life-cycle
models.\auth{Attanasio, Orazio}}
...
monetary economics; for a range of applications, see that volume and McCandless
...
\appendix{A}{Dynamic inefficiency in a generalized growth model}\label{appendixgrowth}%

This appendix generalizes the growth model with 
overlapping generations in section \use{sec:dynineffgrowth}
by allowing for population growth, $N_t=n N_{t-1}$ with $n>0$, 
and for any rate of depreciation of capital, $\delta\in(0,1]$. 
By relaxing our earlier assumptions $n=\delta=1$, we 
can prove the proposition on suboptimal steady states
in subsection \use{sec:dynineff}, i.e., that a steady state with
a capital-labor ratio $k^\star$ satisfying
$F_1(k^\star,1) < \delta +n-1$ is Pareto suboptimal.

Our earlier resource constraint \Ep{olg_res} in period $t\geq 0$
is now replaced by
$$ N_t c^t_t + N_{t-1} c^{t-1}_t + K_{t+1} = F(K_t,N_t)
             + (1-\delta) K_t,      \EQN olg_res2 $$
where $K_i$ denotes the aggregate capital
stock accumulated by the end of period $i-1$ to be used in 
production in period $i$ (and in future periods after 
deducting for depreciation). Hence,  inputs to the 
production function are capital $K_t$ and labor 
supplied by the current young generation of size $N_t$.
Dividing both sides of resource constraint \Ep{olg_res2}
by $N_t$ yields
$$ c^t_t + n^{-1} c^{t-1}_t + n k_{t+1} = F(k_t,1)
             + (1-\delta) k_t,      \EQN olg_res3 $$
where $k_i\equiv K_i/N_i$ is the capital-labor 
ratio.\NFootnote{As shown in footnote \use{note:prodCRS},
$F(K,N)=N F(K/N,1)$. Note that our earlier interchangeable 
use of lowercase letter $k_t$ to denote both the 
capital stock and the capital-labor ratio in section 
\use{sec:dynineffgrowth} was dependent on the
representative young agent's time endowment being equal
to one. In our present setting,
we must distinguish between the lowercase letter $k_t$ 
denoting the capital-labor ratio and the uppercase letter 
$K_t$ denoting the aggregate capital stock.}
At a stationary allocation with capital-labor ratio $k$,
equation \Ep{olg_res3} can be rearranged to read
$$ c_1 + n^{-1} c_2   = F(k,1) - (\delta +n-1) k 
                           \equiv C(k),   \EQN olg_SS2 $$
where $c_1$ and $c_2$ are the consumption of a young and 
an old agent, respectively. We can now reinterpret 
Figure \Fg{graphgrow} to represent components of 
expression \Ep{olg_SS2} as functions
of a stationary capital-labor ratio $k$. While the upper
dashed line $F(k,1)$ would be the same as before, the slope 
of the straight dashed line would now be $(\delta+n-1)$.
The solid line is the difference between the two dashed 
lines, which represents aggregate consumption where the
factor $n^{-1}$ accounts for different relative cohort 
sizes of old versus young agents.

As before, a Pareto suboptimal steady state has a 
capital-labor ratio $k^\star$ that is on the 
downward-sloping side of $C(k)$, which here means that
$F_1(k^\star,1) < \delta +n-1 $. Following the steps in
subsection \use{sec:dynineff}, we can describe a
Pareto improvement that moves the economy from such 
an inefficient steady state to a Pareto optimal steady
state in one transition period. Specifically, 
the planner in period $T$ permanently lowers next 
period's and all future periods' capital-labor
ratio from $k^\star$ to $\bar k$, where $\bar k$ is 
the capital-labor ratio at which $C'(\bar k)=0$.
Thus, the resource constraint in period $T$
becomes
$$ c^T_T + n^{-1} c^{T-1}_T + n \bar k = F(k^\star,1)
+ (1-\delta) k^\star \;\;\;=C(k^\star) + n k^\star,  \EQN olg_tra $$
which can be rearranged to read
$$ c^T_T + n^{-1} c^{T-1}_T =C(k^\star) 
         + n (k^\star - \bar k) > C(k^\star),  \EQN olg_tra2 $$
where the strict inequality follows from 
$k^\star >\bar k$. Aggregate consumption is increased 
in period $T$ according to expression \Ep{olg_tra2}, 
as well as in all future periods $t>T$ since 
$C(\bar k) > C(k^\star)$ in our reinterpretation of 
Figure \Fg{graphgrow}.


The condition for a steady state to be Pareto 
suboptimal, $F_1(k^\star,1) < \delta +n-1$, involves 
the marginal product of capital $F_1(k^\star,1)$
being too low.\NFootnote{Under our assumption of  
constant returns to scale, $F_1(k,1)$ is 
the marginal product of capital for any pair $(K,N)$
whose ratio satisfies $K/N=k$;
$${\partial \, F(K,N) \over \partial \, K }
  = {\partial \, N F\left({K \over N}, 1\right) \over \partial \, K }
  = N {1 \over N} {\partial \, F\left(k, 1\right) \over \partial \, k }
  = F_1(k,1).
$$}
First, if $F_1(k^\star,1)$ is less than the rate of
depreciation $\delta$, it would mean that the marginal
unit of capital does not increase output enough
to replace its own depreciation. 
Second, the cutoff for suboptimality is 
raised if the population grows, $n>1$. This is also 
a kind of required replacement from a planner's 
perspective; namely, the marginal unit of capital should 
 not only exceed  its own depreciation rate
but also yield an additional $n-1$ so as to create 
the extra output needed to maintain that higher 
capital-labor ratio in the face of a growing labor 
force. If the marginal product of capital 
$F_1(k^\star,1)$ falls short of these requirements, it 
is efficient to reduce capital investments and instead 
set up a system of intratemporal transfers of goods from 
young to old agents. That can  be 
done by issuing either  unbacked government debt or fiat money.


%We invite the reader to design a government policy
%that attains this change in the capital-labor ratio
%and increases the lifetime utility of all 
%currently living and future agents.
