...
are set at time $0$,
before the people who are to execute  trades have been born.
...
are  equivalent between these  two trading arrangements, so insights discovered in one
...
transfers, and fiscal policy in sections \use{sec:Samuelson101}, \use{sec:deffinance},
...
is strictly increasing in consumption  $c$ of the one good.
  Agent $i$ orders consumption stream  $c^i = \{c_t^i\}_{t=1}^\infty$ according to the
...
Notice that agent $i \geq 1 $ only wants goods dated $i$ and $i+1$, while at agent born at $i=0$ before the
model starts at $i =1$ wants consumption only at time $0$.
Equations \Ep{olg1} tell us  that agent $i$ lives during
periods $i$ and $i+1$ and wants to consume only when he is alive.

Each consumer has an endowment sequence $y^i$ satisfying
$y_i^i \geq 0, y^i_{i+1} \geq 0, y_t^i = 0 \ \forall t \neq i \ {\rm or}
\ i+1$.
Thus, consumers are endowed with goods only when
they are alive. (In some exercises, we shall alter this assumption.)
...
time $0$  an ``auctioneer'' or  ``clearinghouse''  posts prices, monitors demands and supplies for goods in all periods,
and verifies that each person's single time $0$ budget constraint is satisfied.
An equilibrium price vector assures that for
periods $t\geq 2$ aggregate endowments in that period are consumed by living agents,  but it can turn out that 
in period $1$ the aggregate endowment exceeds  aggregate consumption so
 the time $0$ Arrow-Debreu  clearinghouse might
end up with some of the time $1$ good not yet allocated.  Presumably, after trading concludes, the manager
 of the artificial clearing house could give  any such excess supply of
goods in period $1$  to the initial old
...
Thus,
at the end of period $0$, there are complete markets  in time $t$ consumption goods
...
\noindent Evidently an allocation is feasible if for all $t \geq 1$,
...
\noindent This economy has many equilibria.    We describe two stationary
equilibria now and 
nonstationary equilibria later.  We can use a \idx{guess-and-verify method} to
...
and that the allocation is feasible.  At the equilibrium price vector
$q_t^0$, extensive
intergenerational trade  occurs
at time $0$.   Constraint \Ep{olg3} holds with  equality
for all $t \geq 2$ but with strict inequality for $t=1$; note that 
...
\medskip
\noindent To understand why we call them high- and low interest rate equilibria,
note that we can represent $q^0_t$ as
$$ q_t^0 = q_1^0  \exp(-r_{1,2}) \cdots \exp(-r_{t-1,t}) $$
where $R_{t,t+1}^{-1} = {\frac{1}{1+r_{t,t+1}}} \approx \exp(- r_{t,t+1})$
is the reciprocal of the gross interest rate between times $t$ and $t+1$. 
In Equilibrium H, $R_{t,t+1}^{-1} = 1$ for all $t \geq 1$ while in equilibrium L ,
$R_{t,t+1}^{-1} = \alpha > 1$ for all $t \geq 1$.
Soon we shall construct  equilibria in which $R_{t,t+1}^{-1}$ varies over time $t$.

\medskip
\subsection{Relation to welfare theorems}

Evidently, equilibrium H Pareto dominates
...
is peculiar because some of the time $1$ good is not consumed, leaving
 room to set up a giveaway program to the initial
old that makes them better off.  We shall soon learn how an institution of  either perpetual government debt or of fiat money
...
also reviews earlier studies that had sought to understand how  the welfare
...
\medskip
\noindent{\sc Definition:}  Person $i$'s {\it offer curve} is
...
Evidently, points $(c_i^i, c_{i+1}^i)$ on
 the offer curve simulataneously solve the following pair of equations:
...
We denote the offer curve  swept out by $\alpha_i \geq 0 $  by
...
the endowment vector.  At the intersection between the offer curve and a straight
...
By relabling axes, in the same graph we can plot a consumption possiblity frontier
$$ c_i^i + c_i^{i+1} \leq y_i^i + y_i^{i+1} $$
that bounds feasible allocations at time $i$.  Note that while the offer curve restricts a life-time consumption
profile $(c_i^i, c_{i+1}^i)$ for one person born at time $i$, the consumption possibility frontier involves 
pairs $(c_i^i, c^i_{i-1})$ of consumptions assigned to a young and old person at time $i$.
...
Following Gale (1973), for the important special case in which $(y_i^i, y_{i+1}^i) = (y_y, y_o)$ for all $i \geq 0$,
 we can use the offer curve and
a straight line depicting feasibility in the $(c^i_i, c^{i-1}_i)$ plane
to  compute equilibrium allocations and prices.  The assumption that  $(y_i^i, y_{i+1}^i) = (y_y, y_o)$ for all $i \geq 0$
means that the feasibility line is the same for all $i \geq 1$.  To compute an equilibrium allocation, we must  we choose a boundary value, for example,  
an initial condition
$c_1^1$. As we shall see, for each  different $c_1^1$, a different  competitive equilibrium prevails.
Thus, we  can  find an equilibrium allocation by solving  a pair of nonlinear difference equations.
  For $i \geq 1$, the equations
...
$$\EQNalign{  \psi(c^i_i, c^i_{i+1}) & = 0, \EQN equilib1;a \cr
              c^i_i + c^{i-1}_i & = y_y + y_o. \EQN equilibr1;b \cr}$$
Equation \Ep{equilib1;a} states that the allocation to a person born at time $i$ is on the offer curve. 
Equation \Ep{equilibr1;b}
asserts that the allocation at time $i$  is feasible. 
We take $c_1^1$ as an initial condition and then compute an equilibrium allocation via the following steps:


\medskip
\item{({\bf a})} Solve person $1$'s offer curve  \Ep{equilib1;a} for $c^1_2$.

\medskip
\item{({\bf b})} Solve feasibility condition \Ep{equilibr1;b} for $c_2^2$.
...
\medskip
\item{({\bf d})} An optional last step: solve feasibility condition \Ep{equilibr1;b} perhaps for a set of  $c_1^0$'s that the clearing house 
could give to the initial old if it wishes.


\medskip

\noindent After an allocation has been computed, an Arrow-Debreu  equilibrium price
...
pairs   $(c_t^t, c^t_{t-1})$ that can be allocated to people alive at time $t$.  The curved offer curve also goes through the endowment point $(y_t^t, y^t_{t+1})$
and plots pairs $(c_t^t, c^t_{t+1})$ that a person born at $t$ optimally chooses as a function of the gross interest rate $\alpha_t$ implicitly
sweeping out the offer curve.   

\medskip

\noindent{\it Example 1:}  \quad  Plain vanilla model with stationary endowment pattern:
  This is the situation illustrated
in Figure \Fg{graph22f}.  Start with
...
of the time $1$ good is thrown away.  This follows because
...
\medskip
\noindent{\it Example 2:} \quad Log utility:
...
an equilibrium $\alpha$ sequence.   So is any $\alpha_i$ sequence that  satisfies
...
autarkic equilibrium.    Figure \Fg{graph22f} suggests that   the stationary
$\alpha_i=1$ equilibrium is not stable, while the autarkic 
equilibrium is.\NFootnote{For this example,  a counterpart of difference equation
\Ep{alphadiff1} would be 
...
trading in which a durable asset, for example,  either government debt or unbacked fiat money, 
