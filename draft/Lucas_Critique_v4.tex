...
$P_{ij} = {\rm Prob}(s_{t+1} = j| s_t  = i) $ and $\pi_{0,i} = {\rm Prob}(s_0 = i)$.
We assume that $\{s_t, x_{t+1}, y_t\}_{t=0}^\infty$ is governed by  the Markov-switching linear state-space system
...
Distributions of $x_{t+1}, y_t$, and $z_t$ conditional on $x_t$ are {\it mixtures of normals}, with $\pi_{i,t}$   being the mixing probabilities over states $i$  at time $t$.              
...
The appearance of  time subscripts $t$ in the formulas for $\bar A_t, G_t, H_t$ are a consequence of  $\pi_{i,t}$ not necessarily
...
Under suitable convergence conditions on   $\pi_t$, iterations on these two recursions  converge, respectively, to 
