...
%\hbox{}
\footnum=0
\chapter{Matching Model Mechanics\label{mechanics_matching}}
...
\subsection{Nash bargaining model}\label{sec:ML1_Nash_elasticity}%
\noindent In the standard version of the Nash bargaining model, the equilibrium
...
and $\Psi^{\rm m.surplus}_n$ solve the Bellman equations \index{Bellman equation}%
$$\EQNalign{
...
\frac{r+s}{q(\theta)}\,+\,\phi\,\theta\,.
                                                \EQN fixed_a_theta
$$
Paralleling the steps of implicit differentiation in subsection
...
we compute the elasticity of market tightness 
$$
...
\subsection{Another version of alternating-offer wage bargaining}\label{sec:CET_AOB}%
Instead of assuming as in the preceding subsection that
alternating offers are made in successive periods and can
potentially continue indefinitely, Christiano, Eichenbaum and
Trabandt (2016) assume that bargaining proceeds within a
period and that there is a maximum number of rounds. 
Specifically, the firm and the worker take turns to extend 
offers across $M$ sub-periods, where $M$ is even. The firm 
begins and makes a wage offer $w^f_1$ in the first 
sub-period and, as long as there is no agreement,
continues to make offers $w^f_j$ in subsequent odd 
sub-periods $j=3,5,\ldots, M-1$. Likewise, the worker makes 
wage offers $w^w_j$ in even sub-periods $j=2,4, \ldots, M$, 
where an offer $w^w_M$ in the last sub-period is assumed to
be a take-it-or-leave-it offer, i.e., the match is broken up 
if the offer is not accepted.\NFootnote{We adopt the
assumption of the
preceding subsection that match destruction probabilities
are the same during bargaining and production. 
Thus, since there is no interperiod job destruction
during production, we assume no job destruction between
bargaining rounds within a period; and keep the same
job destruction probability $s$ between periods. As before,
the rationale for this symmetry is that outside values will 
then not affect wages which simplifies our 
derivations and, most importantly, makes our inference more
transparent. Specifically, since small asymmetries in
job destruction probabilities across bargaining
and production cannot be central to the elasticity of market tightness, we are justified not cluttering our equations. 
For a numerical illustration of the relatively small impact 
of variations in job destruction probabilites in the 
Hall-Milgrom alternating-offer bargaining model, see Figure
\Fg{figMLIHallMilgrom}.}

\auth{Christiano, Lawrence J.} \auth{Eichenbaum, Martin S.}%
\auth{Trabandt, Mathias}%
%%%%\index{alternating-offer bargaining}%

A sub-period wage rate refers to compensation for work
performed in one subperiod. For example, if the first
wage offer $w^f_1$ is accepted, as it will be in an 
equilibrium, the match produces output throughout the 
entire period and the worker's total compensation would 
be $M w^f_1$. Analogously, on a 
sub-period basis, a worker's output, the value of leisure 
and a firm's cost of delay in bargaining, are denoted   
$y_\triangle \equiv y/M$, $z_\triangle \equiv z/M$ 
and $\gamma_\triangle \equiv \gamma /M$, respectively.
We will now show that the determinant of the fundamental 
surplus fraction and hence, of the elasticity of market 
tightness with 
respect to productivity, is the combined value of leisure 
and a firm's cost of delay in bargaining, in accordance with 
our finding for the original Hall-Milgrom (2008) 
alternating-offer bargaining model in the preceding 
subsection.

Since it is optimal for both bargaining parties to make 
barely acceptable wage offers, the following indifference
conditions must hold across two successive 
equilibrium wage offers.\NFootnote{When indifferent 
between accepting and declining an offer, an agent is
assumed to accept.} 
In an odd sub-period $j$, the 
equilibrium wage offer $w^f_j$ extended by the firm
would be such that the worker is indifferent between 
accepting the offer or waiting until the next sub-period 
to make the equilibrium counteroffer $w^w_{j+1}$:
$$
(M-j+1)w^f_j = z_\triangle \,+\, (M-j) w^w_{j+1},
\hskip1cm \hbox{\rm for} \hskip.1cm j=1,3,\ldots , M-1,  
                                               \EQN CET_firmW
$$
where the left side is the worker's labor income for the
remaining number of subperiods $M-j+1$ when accepting and
working at the wage rate $w^f_j$, while the right side is the
worker's outcome upon declining the offer which consists
of the value of leisure $z_\triangle$ in the current subperiod
and subsequent labor income when working for one less
subperiod $M-j$ at the counteroffer wage rate $w^w_{j+1}$ 
(that the firm would accept, in accordance with the indifference
condition to which we turn next). Likewise, in an even 
sub-period $j$, the equilibrium wage offer $w^w_j$ extended 
by the worker would be such that the firm is indifferent 
between accepting the offer or waiting until the next 
sub-period to make the equilibrium counteroffer $w^f_{j+1}$:
$$
(M-j+1)\left[y_\triangle - w^w_j \right] = - \gamma_\triangle 
\,+\, (M-j) \left[y_\triangle - w^f_{j+1} \right],
\hskip.3cm \hbox{\rm for} \hskip.1cm j=2,4,\ldots , M-2,  
                                               \EQN CET_workerW
$$
where the left side is the firm's profits during the 
remainder of the period when accepting the 
wage offer, while the right side is the firm's outcome upon
declining the offer, i.e., the incurrence of a cost of delay 
in bargaining $\gamma_\triangle$ and the subsequent earning
of profits based on the firm's counteroffer (that the 
worker would accept, in accordance with indifference condition 
\Ep{CET_firmW}).

After a recursive substitution of wages in equations
\Ep{CET_firmW} and \Ep{CET_workerW}, the first wage offer 
can be expressed as 
$$
w^f_1 = \frac{z_\triangle + 
             \left( \frac{\displaystyle M}{\displaystyle 2}
                    -1 \right) 
              \left[z_\triangle + \gamma_\triangle + y_\triangle 
                       \right] \,+\, w^w_M}{M}\,.
                                                   \EQN CET_w1
$$
Regarding the worker's take-it-or-leave-it offer
$w^w_M$ extended to the firm with a zero 
outside value, the worker would ask for  
the entire output $y_\triangle$ in that sub-period $M$ and 
the expected present value of all future profits in the match, 
$$
w^w_M = y_\triangle + \sum_{i=1}^\infty \tilde \beta^i 
M \left(y_\triangle -w^f_1\right)
      = y_\triangle + \tilde \beta M 
                       \frac{y_\triangle -w^f_1}{1-\tilde \beta},
\EQN CET_wM
$$
where $\tilde \beta \equiv \beta (1-s)$. After substituting
\Ep{CET_wM} into \Ep{CET_w1}, the first wage offer can 
be expressed solely in terms of primitives, 
$$
w^f_1 = y_\triangle - \frac{1}{2} (1-\tilde \beta)        \left[y_\triangle - z_\triangle - \gamma_\triangle \right]
       - (1-\tilde \beta) \frac{\gamma_\triangle}{M}\,.
\EQN CET_w1_final
$$
By choosing an arbitrarily large even number of sub-periods 
$M\!$,
the last term in \Ep{CET_w1_final} can be made to vanish. Hence, 
a parameter restriction for the existence of an equilibrium 
is  $y_\triangle > z_\triangle + \gamma_\triangle$ or, 
equivalently, $y > z + \gamma$.

Following the steps of the preceding subsection, we
use expression \Ep{CET_w1_final} (without its last term, as
discussed) to substitute for the equilibrium wage, $w = M w^f_1$, 
in the no-profit condition \Ep{wage1} for vacancy creation,
$$
M\left(y_\triangle - \frac{1}{2} \frac{r+s}{1+r} 
           \left[y_\triangle - z_\triangle 
                             - \gamma_\triangle \right]\right)
\;=\;
y \,-\, \frac{r + s}{q(\theta)} \, c \,,
$$
where we have invoked $\beta = (1+r)^{-1}$. 
After some simplification, this equilibrium expression for market 
tightness becomes
$$
\frac{1}{2} \beta \left[y - z - \gamma \right]
\;=\;
\frac{c}{q(\theta)}\,.      \EQN CET_altbarg
$$
Using implicit differentiation,
we can compute the elasticity of market tightness 
$$
\eta_{\theta,y}
%% \;\equiv\; {d\, \theta \over d\, y} \; {y \over \theta}
\;=\; \frac{1}{\alpha} \; \;
      \frac{y}{y-z- \gamma}
\;=\; \Upsilon^{\rm sticky}\;
         \frac{y}{y-z-\gamma}\,.  \EQN CET_thetaelast
$$
It is notable that a firm's cost of delay in bargaining 
between sub-periods $\gamma_\triangle$ enters 
the fundamental surplus fraction in \Ep{CET_thetaelast}
as $M \gamma_\triangle = \gamma$. Based on expressions
\Ep{ML1_thetaelast3} and \Ep{CET_thetaelast}, we conclude
that the elasticities of market tightness are almost
identical in this alternative version
of the alternating-offer bargaining model and in the 
original model of Hall and Milgrom (2008).
...
intergenerational labor market externalities   like  those in the  heterogeneous jobs model of  section \use{Sec_matching_heterojobs}.
...
\Ep{M_OLG_cutoff} shows how   reservation productivities are
...
at age $T+1$. First-order conditions for  remaining
