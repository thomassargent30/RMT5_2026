...
This chapter puts tools from chapter \use{timeseries} to work to study some classic applications in
economics.  Section \use{Kfilterapplications} is devoted to applications of the Kalman filter.  We put the
Kalman filter to work on a  classic reverse engineering problem that John F. Muth (1960)  posed for the ``adaptive expectations''  model of
Milton Friedman.  Muth used other time series methods other than the Kalman filter  to attack the problem.  It turns out that the Kalman
filter allows a more concise formulation and solution of the problem. Section \use{Kfilterapplications}  also tells how the Kalman filter appears in
a classic paper of Jovanovic (1979). Section \use{sec:LQmodel} then puts several of our chapter \use{timeseries}  tools to 
study properties of a linear quadratic model of consumption smoothing, a workhorse benchmark model in macroeconomics
and public finance.  We use the model as a vehicle  to introduce complete and incomplete markets models of types that we shall
...
Critique of some {\it circa\/} 1970's econometric policy evaluation procedures while section \use{sec:lucascritique2} describes some  responses
to  Lucas's Critique.
\auth{Muth, John F.}%
...
some responses to the Lucas Critique.  Section \use{sec:lucascritique} already mentioned one major response, namely,
rational expectations econometrics, an estimation and interpretation strategy now widely used in modern applied macroeconomics. 
...
on the perspective that policy rule changes or ``regime changes''  don't occur in a vacuum but were already
contemplated during an historical 
sample period being modeled.  Thus, this view imagines  regime changes as being  governed by statistical processes known to agents inside a rational expectations model, so there is a precise sense in which government policy rule change
are anticipate and  responded to in advance by private agents.
All of these ideas adopt and elaborate a perspective of Sargent and Wallace (1976) that regards government policy rules  as being
determined by a  process that  leaves  econometricians and policy analysts with no ``free parameters'' to change,
disabling them from being able to offer policy advice.  This circle of ideas was embraced and extended by
...
\noindent We'll use chapter \use{timeseries} tools to represent and study these parts of Sims and Zha's description:

\item{a.} How to represent ``nonlinearities'' in the form of dependencies of state-space coefficients depending on a discrete  Markov variable that
stands in for a ``regime''.

\medskip
\item{b.} How agents who understand regime transition probabilities and who know which policy regime they are in at each moment
shape decisions to take account or prospective regime changes.

\medskip
\item{c.} How well a best-fitting linear model that ignores regime changes could approximate observed outcomes.  
...
We use a linear state space model with coefficients governed by a finite state Markov chain to represent the nonlinearities mentioned by
Sims and Zha that can capture policy regime changes.   
...
As for $x_t$, in the present example it is a state vector $x_t= \bmatrix{1 & m_t & m_{t-1} }'$  that lets us represent the following stochastic process for $\{m_t\}_{t=0}^\infty$ in the state-space form \Ep{MSLQ }:
