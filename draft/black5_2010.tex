...
This chapter describes a search problem that
%choice between
...
\auth{McCall, John}\auth{Jovanovic, Boyan}\auth{Stigler, George}\auth{Neal, Derek}%
 is  useful  for studying unemployment.
...
We seek to  convey some of the excitement that Robert E. Lucas, Jr. (1987, p.57)
...
the options they face and the pros and cons as they themselves see them.''


We describe techniques used in
the search literature and examples of search models. We
study ideas introduced in important papers by McCall
(1970) and Jovanovic (1979a).  These papers differ in the possibilities
with which they confront an unemployed worker.\NFootnote{Stigler's (1961)
 early paper studied a setting  different
...
McCall (1969)  studied  an unemployed worker who is searching for a job
...
$F(B)=1$ for $B<\infty$.\NFootnote{Appendix \use{app:nonneg} of this chapter describes useful
mathematical properties of nonnegative random variables.} The worker has the option of rejecting the offer, in
which case he or she receives $c$ in unemployment compensation this period  and
...
Offers rejected in the past cannot be recalled.

Let $y_t$ be the worker's income in period $t$.  We have assumed that $y_t=c$ if the
worker is unemployed and $y_t=w$ if the worker has accepted an offer to
work at wage $w$.  The unemployed worker wants to maximize the mathematical expectation of
$\sum_{t=0}^\infty \be^t y_t$ where $0<\be<1$ is a discount factor. Note that the worker {\it chooses\/}
...
and who behaves optimally.   The value function
$v(w)$ satisfies the Bellman
equation
$$v(w)=\max_{\rm accept, reject} \left\{ {w\over 1-\be}, c+\be\int_0^B v(w')
dF(w')\right\},\EQN 2.14$$
where  maximization is over  values of outcomes associated with   two actions: (1) {\it accept}
the wage offer $w$
...
$\be\int_0^B v(w') d F(w') $ of drawing a new offer and deciding optimally tomorrow.
...
\midfigure{powdmt4f}
\centerline{\epsfxsize=3true in\epsffile{mcall101a.eps}}
...
%reservation wage $\overline w$.
 The left side is the opportunity  cost of {\it rejecting\/} 
an offer $\overline w$.  The right side is the opportunity cost of {\it accepting\/} an offer $\overline w$, which equals the   probability that next period's wage draw $w'$  exceeds $\overline w$ times the  expected present value of $w' - \overline w$  conditional on drawing an offer $w'$ greater than $\overline w$. %next period.  
To see this,
%GGHH  Adding $\overline w\int_{\overline w}^B dF(w')$ to both sides and rearranging gives
rearrange and rewrite equation \Ep{2.16} to become
$$\overline w = c +  {\rm Prob}( w \geq \overline w){  \be\over 1-\be} \int_{\overline w}^B (w'-\overline w)
d \tilde F(w').\EQN 2.16a$$
where $\tilde F(w) = {\frac{F(w')}{{\rm Prob}( w \geq \overline w)}} $ is the cdf of $w$ conditional on $w \geq \overline w$.  The term after the $+$ sign on the right side of equation \Ep{2.16a} is the value that the worker assigns to an  option to wait for an offer better than $\overline w$.  It equals the probability that $w \geq \overline w$ times
the  discounted present value of a constant  continuation income stream that starts next period and that equals the  expected value of the conditional wage distribution $\tilde F$.  
Notice that \Ep{2.16a}  is one equation  in the unknown reservation wage $\overline w$, so while it might be useful for interpreting why the worker chooses to reject some offers that exceed $\overline w$, it is not a formula for $\overline w$
in terms of the underlying constituents $c, F, \beta$.  


% benefit
% of searching one more time in terms of the expected present value associated
% with drawing $w'>\overline w$.  Equation \Ep{2.16} instructs a worker to set $\overline w$
% so that the cost of searching one more time equals the benefit.

\subsection{Characterizing reservation wage}
Define the function on the right side of equation \Ep{2.16} as
$$h(w)={\be\over 1-\be} \int_w^B (w'-w) dF(w').\EQN 2.17$$
Notice that $h(0)= Ew \be /(1-\be)$, that $h(B)=0$, and that $h(w)$ is
differentiable, with derivative\NFootnote{To compute $h'(w)$,  apply Leibniz's rule to equation
 \Ep{2.17}.  Let
...
Here we exploit properties of nonnegative random variables summarized
in appendix \use{app:nonneg} of this chapter.
...
and
characterization (iii) or (v) of a mean-preserving increase in risk from appendix \use{app:nonneg}.
...
Under a mean-preserving increase in risk, the higher probability attached to  very
good wage offers increases the value of searching for a job while
the higher probability attached to very bad wage offers is not detrimental because
...
recall that the reservation wage $\overline w$ in \Ep{2.15} satisfies
 ...
 $$
 Suppose the worker has in hand an offer to work at wage $w$.
 Assuming that
 the worker behaves optimally after rejecting a wage $w$,
we can
...
unemployment compensation in the amount $c$ each period. A  worker receives a
time-invariant wage $w$ on a job until she is fired. Only previously employed workers are fired.
...
Let $\hat v(w)$ be the expected present value of income of a previously
unemployed worker who has offer $w$ in hand and who behaves optimally. Thus, if she
...
she is not fired  and gets a continuation value $\hat v(w)$ next period that this period is  worth  $\be \hat v(w)$;
...
A previously unemployed worker's value function $\hat v(w)$ after drawing $w$ this period  satisfies
 the Bellman  equation\NFootnote{If a worker who is fired at the beginning of a period were to have the opportunity to draw a new offer that same period,
...
the stationarity of the problem (i.e., the fact that $F, \alpha, c $ are all fixed) makes $\hat v(w)$ also be the continuation value associated with retaining this job next period.
  The Bellman equation has a  solution of the
...
reservation wage in expression \Ep{res_wage} that prevailed  when there was no risk of
...
(see section \use{quits}). So even though workers who face  a zero probability of being fired can mimic outcomes from 
a setting in which they would face a positive probability of being fired  by
occasionally ``firing themselves'' by  quitting into unemployment, they choose
not to do that because it  would lower the expected present value of their income stream. Since
employed workers who might be fired with positive probability are worse off than
employed workers who can never be fired,  it follows that $\hat v(w)$
lies strictly below $v(w)$ for all values of $w$. Even at wages $w$
that are rejected, the value function partly reflects a stream of future outcomes whose mathematical 
expectation is less favorable when a worker faces a chance of
being fired.
...
wage $\overline w$ is strictly lower  when there is a positive probability of being fired.
There is less of a reason to wait for high-paying jobs when a worker typically expects a job
 to last for a shorter period of time.
That is, a higher probability of being fired reduces the amount of time  that an unemployed worker should  optimally invest less time in search.
%when the
...
Solving the above difference equation for a stationary solution, i.e., imposing
$U_{t+1} = U_t = U$, gives
...
\EQN Ulake $$
Equation \Ep{Ulake} expresses a  stationary unemployment   rate in terms
...
encounter with a class of Bewley models that we  shall study  in chapter \use{incomplete}.
\index{Bewley models}
...
$U$ and $1-U$ that represent volumes of unemployment and employment, and flows
...
study  determinants of the  unemployment rate in
terms of the hazard rate  $\alpha$ of becoming unemployed and
the hazard rate  $1 - F(\overline w)$ of escaping unemployment.
...
Each period the worker receives one  offer to work forever at a wage $w$ drawn  from one of two possible 
 cumulative distribution functions $F$ or $G$, where $F(0)=G(0) = 0$ and $F(B) = G(B) = 1$ for $B > 0$. We assume that the distributions have densities $f$ and $g$, respectively, and that they have common support.
Nature draws from the same distribution, either $F$ or $G$, at all dates. The worker knows this but does not know whether it is $F$ or $G$.
...
is $h(w_0;\pi_{-1}) = \pi_{-1} f(w_0) + (1-\pi_{-1}) g(w_0) $.  Let $a \in \{ f, g\} $ be an index that indicates whether  nature chose permanently to draw from distribution $f$ or from distribution $g$. After drawing $w_0$, the worker uses Bayes' law to deduce that
the posterior  probability $\pi_0 = {\rm Prob}{(a = f) | w_0} $
%\newpage
%\noindent
 that the density is $f(w)$ is\NFootnote{The worker's initial beliefs induce a joint probability distribution
...
the probability that $w_{t+1}$ will  be drawn from  distribution  $F$ is
$$ \pi_t = \pi_t(w_t | \pi_{t-1}) \equiv { \pi_{t-1} f(w_t)/g(w_t) \over \pi_{t-1} f(w_t)/g(w_t) + (1-\pi_{t-1})} \EQN update1 $$
...
on $w_{t-1}, \ldots , w_0$ or equivalently conditional on the statistic  $\pi_{t-1}$ that completely summarizes information that history $w_{t-1}, \ldots , w_0$ contains about whether nature takes  
i.i.d.~ draws from cdf $F$ or cdf $G$.\NFootnote{Thus,  $\pi_{t-1}$ is a {\it sufficient statistic\/} for a parameter $s_{-1} \in \{0, 1\}$,  where $s_{-1} =1 $ if nature always draws from cdf $F$
and $s_{-1} = $ if nature always draws from cdf $G$.  We can regard $s_{-1}$ as a time-invariant hidden Markov state.} \index{sufficient statistic}%  %for $w_{t-1}, \ldots , w_0$.) 
Here we are computing $E(\pi_t | \pi_{t-1})$ under the subjective density described in the second
term in brackets.\NFootnote{It follows from the martingale convergence theorem (see appendix A %\use{app:Ach16}
of chapter \use{selfinsure}) that $\pi_t$ converges almost surely to a random variable  $\pi_\infty$ in $[0,1]$.  Practically, this means that  probability one is  attached to   sample paths
 $\{\pi_t\}_{t=0}^\infty$ that  converge pointwise.  Different sample  paths can   converge to different limiting values, so that  limit points of  $\{\pi_t\}_{t=0}^\infty$ as $t \rightarrow +\infty$ are realizations of a random variable that  has a non-trivial distribution. By studying equation \Ep{update1}, it can be  verified that the only possible limit points  $\pi_\infty$ are $1$ and $0$.
 The martingale property $E \pi_{t+1} | \pi_t = \pi_t$ then implies that the probability attached to sample paths in which $\pi_\infty = 1$ equals $ \pi_{-1}$. }
 \index{martingale!convergence theorem}%
 It is also possible to show that under the $f$ distribution 
 $$
 E(\pi_t | \pi_{t-1})  = \int [  { \pi_{t-1} f(w) \over \pi_{t-1} f(w) + (1-\pi_{t-1})g(w)  } ]
 f(w)   d w \geq \pi_{t-1}
 $$
 ...
 E(\pi_t | \pi_{t-1})  = \int [  { \pi_{t-1} f(w) \over \pi_{t-1} f(w) + (1-\pi_{t-1})g(w)  } ]
  g(w)   d w \leq \pi_{t-1} .
 $$
 These two inequalities can be shown to imply that when nature permanently draws from $f$, $\pi_t \rightarrow 1$,
 while when nature permanently draws from $g$, $\pi_t \rightarrow 0$.




Let $v(w_t,\pi_t)$ be the optimal value of the problem for a previously unemployed worker who has just drawn
$w$ and who updates $\pi$ according to \Ep{update1}.  The optimal value function $v(w, \pi_t)$ satisfies the Bellman equation
$$ v(w, \pi_t) = \max_{{\rm accept, reject}} \biggl\{ {w \over 1-\beta}, c
  + \beta \int v(w', \pi_{t+1}(w')) h(w'; \pi_t) d w' \biggr\} \EQN Bellman_unknown$$
where maximization is subject to \Ep{update1} and \Ep{distribut_t}.
The state vector is the worker's  current draw $w$ and his post-draw estimate of the probability that the distribution is
$f$.  The second term on the right side of \Ep{Bellman_unknown} integrates the value function evaluated at next period's state vector  with respect to the conditional probability
...
on $[0, 2]$ while $g$ is a beta distribution with parameters $(\alpha, \gamma) =  (3,1.2)$.\NFootnote{The beta distribution for $w$ is characterized by
...
\centerline{\epsfxsize=3true in\epsffile{two_densities_rev.eps}}
\caption{Two densities for wages,  a uniform $f(w)$ and  $g(w)$ that is  a beta distribution
with parameters $(\alpha, \gamma) = ( 3, 1.2)$.}
...
\centerline{\epsfxsize=3true in\epsffile{reservation_wage_pi_gray.eps}}
...
whether  nature is drawing from $f$ and not $g$.  Evidently,
...
\caption{Top panel: CDF of duration of unemployment; bottom panel: CDF of $\pi$ at time worker accepts wage
and leaves unemployment. In each panel, the lower solid line is the CDF when nature permanently draws from the uniform density $f$ while
...
than when $F$ prevails.  This  captures  what the interrogator of an unemployed  McCall worker in the passage of Lucas cited in the introduction to this chapter might have had in mind when he said `Maybe you are setting your sights too high'.
...
about to decide whether to accept or reject the offer. Let $\tilde p$ be  the buyer's reservation price. In equilibrium, it will turn out
that no seller will over a price higher than $\tilde p$.   The buyer's Bellman equation is then
$$ v(p) = \min_{\rm accept, reject} \Bigl\{ p, c + \int_{\underline p}^{\tilde p} v(p') d H (p')    \Bigr\} . \EQN BJ_Bellman $$
...
Next note that $E(p) = \int_{\underline p}^{\tilde p} p d H(p) = p H(p)|_{\underline p}^{\tilde p} - \int_{\underline p}^{\tilde p} H(p) d p  = \tilde p -
\int_{\underline p}^{\tilde p} H(p) d p$. Combining
$E(p) = \tilde p -\int_{\underline p}^{\tilde p} H(p) d p $ with equation \Ep{BJstilde} implies that
$$ \tilde p = c + E p, $$
...
$G(\underline p) = 0, G(\tilde p) = 1$.  We have assumed that  $G$ is differentiable.\NFootnote{Burdett and Judd (1983, p. 959, lemma 1) show that an equilibrium $G$ is differentiable when $q \in (0,1)$ and $\tilde p >0$.  Their argument
...
\item{1.} Consider the special case in which $q=1$ (and therefore $\hat q =1$).  Here, $\underline p = \tilde p$. Formula \Ep{BJ_Gformula} shows that the  distribution of prices
...
\section{Neal's model of career choice}
...
\centerline{\epsfxsize=3truein\epsffile{neal1newa_gray.eps}}
\caption{Optimal value function for Neal's model with $\beta=.95$. The value
function is flat in the reject $(\theta,\epsilon)$ region; increasing in only
$\theta$  in the keep-career-but-draw-new-job region; and increasing in
...
region, the worker retains his career but draws a new job.  The worker accepts
$(\theta,\epsilon)$ in the black region.}
\infiglist{Neal 2}
\endfigure
...
is flat with respect to $\epsilon$.
  ...
  An interesting feature of the model is that it  is possible
...
Another  interesting response to  Rothschild's questions about the source of the equilibrium wage (or price) distribution
comes  from matching models. Here  the main idea is to
...
productivities and utilities jointly generated by the activities of the match.
...
which matches form and  dissolve by viewing both parties to a match
...
workers, or men and women, or owners and renters, or lakes and fishermen.\NFootnote{We consider a simplified version of Jovanovic's
model of matching.  Prescott and Townsend, 1980, describe a discrete-time
version of Jovanovic's model that we have further simplified here.}
...
the firm both observe only $y=\th+u$, where the random noise $u$
%\newpage
%\noindent
  is
uncorrelated with $\th$.  Thus, in the first period, the worker-firm pair
receives only a noisy observation on $\th$ and therefore both worker and firm form only an error-ridden impression of the
...
$\th$, meaning that  both sides of the match learn about
...
$\sigma^2_{1}$. Using formulas for conditional distributions from section \use{sec:populationregression} or the  Kalman filtering formula from section \use{sec:Kalman100} in chapter \use{timeseries},
...
The worker can accept or reject
the offer.
...
&  \hskip1cm  \Biggl.    \be \int V(m'_1) dG \left( m'_1|\mu, K_0 \sigma^2_{0}\right)
                                                              \Biggr\}.\cr} \EQN eqn:Jovfnal101 $$
\autoparens

The expression on the right defines an operator $T$ that  maps continuous
functions $V$ into continuous functions $TV$.  Th
 functional equation  \Ep{eqn:Jovfnal101} can
...
Technical Appendix \use{functional} on functional analysis at the end of this book.
...
\appendix{A}{Nonnegative random variables}\label{app:nonneg}%
  This appendix  describes elementary properties of probability distributions
...
assume that $F(0)=0$ so  that $p$ is nonnegative.  We assume
...
nonnegative random variable:\NFootnote{To compute the variance $E (p - E(p))^2 = E(p^2) - (E(p))^2$, first compute $E p^2$.  By integrating by parts, we can verify that 
$$
E p^2 = \int_0^B p^2 d F(p) =  2 \int_0^B p (1 - F(p)) dp
.$$}
$$Ep=\int_0^B [1-F(p)]dp = B - \int_0^B F(p)dp .\EQN 2.2$$

Now consider two independent random variables $p_1$ and $p_2$ drawn from the same
...
.\EQN 2.3$$
Similarly, if $p_1,p_2,\ldots, p_n$ are $n$ independent random variables each drawn
from $F$, then  Prob$\{\max(p_1,p_2,\ldots, p_n)<p\}=F(p)^n$ and
...
the same mean.  Consider a class of non-negative random variables with identical means.  Index
the probability distributions for this  class of random variables by a parameter $r$ belonging to some set $R$.  For the $r$th
...
\appendix{B}{More numerical dynamic programming}\label{searchA}%
...
\noindent At the beginning of each period,
a worker who enters the period  unemployed chooses search effort $e$,   receives a wage offer
with probability $\pi \left( e\right) $,  and receives no offer with probability $1-\pi
\left( e\right) $. Exerting search effort---a measure of search
intensity---brings with it   one-period  cost $e$. The  wage offer $w$ is drawn
from distribution $F$. The worker chooses to accept or reject it. If the
offer is rejected or if  no offer is received, the worker receives unemployment
benefit $c>0$ this period  and starts next period  as a searching unemployed worker.
 If this period's  wage is accepted, the worker receives wage $w$
minus income tax $T\left( w\right) $ and enters next period   as  someone who was employed this  period. 
At the beginning of each period, a worker who was
employed in the previous period can be fired with probability $\alpha $, where  $%
0<\alpha <1$. If a previously employed worker is fired this period, 
 the worker receives unemployment benefits $c$ this period  and becomes a searching unemployed worker at the beginning
of  next period.

The worker chooses a policy about  whether to accept an offer or not
and how much effort to put into search when unemployed. The worker wants  to maximize%
$$
...
\medskip\noindent {\bf  a.}  Analyze the worker's problem. Establish that the optimal policy 
involves  a reservation wage. Find a condition  that restricts  the
optimal choice of $e$ and show that the reservation wage is independent of $%
e$.

\medskip\noindent{\bf  b.} Assume that $T\left( w\right) =\tau \left( w-a\right) $ where $%
0<\tau <1$ and $a>0$, i.e., the tax schedule consists of a flat tax rate $%
\tau $ and a lump-sum rebate $\tau a$. Show that an increase in $a$
decreases the reservation wage and increases the level of effort, thereby increasing
the probability of accepting employment.

\medskip\noindent{\bf  c.} Show   a change in $\tau $ that  has the opposite
effect.
...
\medskip
\noindent {\bf b.} Find an expression for the expected value of the purchase price net of all
search costs.
...
At the beginning of each period a worker employed at wage $w$ the previous period is exposed to  probability 
...
\medskip
\noindent {\it Exercise \the\chapternum.28} \quad {\bf A functional equation}
\medskip
\noindent
Please read the following functional equation:
$$
v(w) = \max \Bigl\{ w + \beta (1 - \pi) v(w) + \beta \int v(w') d F(w'), c + \beta \int v(w') d F(w') \Bigr\},
$$
where $c > 0$, $\beta \in (0,1)$, and $F$ is a cumulative distribution function for a bounded  nonnegative random variable.
Please completely describe a decision problem that is associated with this functional equation.



\medskip
\noindent {\it Exercise \the\chapternum.29} \quad {\bf Coupled functional equations}
\medskip
\noindent
Assume that $c > 0, \alpha \in (0,1), \gamma \in (0,1)$, and that $F$ is a cumulative distribution function for a bounded nonnegative random variable. 
Assume that $h_H > h_L > 0$.  Please read the following coupled functional equations:
$$
\eqalign{ v(w, h_L) & = \max \Bigl\{ w h_L + \beta \gamma v(w,h_H) + \beta (1-\gamma) v(w, h_L), c + \beta \int v(w', h_L) d F (w') \Bigr\} \cr 
          v(w, h_H ) & = w h_H + \beta \alpha \int v(w', h_L) d F(w') + \beta (1 -\alpha) v(w, h_H) 
}
$$
...
\medskip
\noindent {\it Exercise \the\chapternum.30} \quad {\bf Another  functional equation}
\medskip
\noindent
Please read the following equation:
$$
w(x) = \max \Bigl\{ {\frac{x}{1-\beta}}, d+ \beta \int w(s)[\alpha f(s)+ (1-\alpha) g(s)] d s\Bigr\}
$$
where $f \geq 0, g \geq 0, \int f(x) d x = 1, \int g(x) d x = 1, \alpha \in (0,1)$.
Please describe a decision problem associated with  this equation. 
Please interpret each of the objects $w,x,\beta,d, \alpha, s,f,g$. 



\medskip
\noindent {\it Exercise \the\chapternum.31} \quad {\bf A reverse engineering problem}
\medskip
\noindent  Please describe a decision problem whose optimal value is given by a function $v(w)$ that
...
\medskip
\noindent {\it Exercise \the\chapternum.32} \quad {\bf Another reverse engineering problem}
\medskip
\noindent
Please describe a decision problem whose optimal value is given by a function $v(w)$ that
satisfies the following equations: 
...
\medskip
\noindent {\it Exercise \the\chapternum.33} \quad {\bf Bayes Law again}
\medskip
\noindent Let $f:[0, B] \rightarrow {\bf R}^+$ be a known probability density function for a nonnegative random variable
...
\medskip
\noindent {\it Exercise \the\chapternum.34} \quad {\bf Quits again}
\medskip
\noindent
