...
% next two lines to add blank page  (FRV 12 Feb 2018)
...
(1987a, ch.~IX) for a more comprehensive account of lag operators.  By
using lag operators, we extend Hall's results to allow arbitrary
fiscal policy paths.}  \index{shooting algorithm}%

After studying our single closed-economy model, we study an extension of our model in the spirit of Mendoza and Tesar (1998) in which 
two economies that trade physical capital and financial assets, but not labor.
...
(1-n)$.\NFootnote{See Hansen (1985) for a comparison of these two specifications.  Both  specifications
...
Government purchases of goods at time $t$ are $g_t \geq 0$ and a government expenditure
plan is a sequence $g = \{g_t\}_{t=0}$.  A government tax plan is a 4-tuple of sequences
$ \{\tau_{ct}, \tau_{kt}, \tau_{nt}, \tau_{ht} \}_{t=0}^\infty$,
where $\tau_{ct}$ is a tax rate  on consumption at time $t$, $\tau_{kt}$ is a tax rate on rentals of
capital at time $t$, $\tau_{nt}$ is a tax rate on wage earnings at time $t$, and $\tau_{ht}$ 
is a lump sum tax on a consumer at time $t$. 
...
We say that a government expenditure plan and tax plan that satisfy a
...
Besides making computations easier in some settings,
including lump sum taxes is convenient because it lets us include an experiment 
that underlies Prescott's (2002) analysis that attributed Europe's depressed economic activity 
to its high tax on labor: in Prescott's experiment, $g_t=0 $ for all $t\geq 0$ and the government returns revenues from  labor taxes
lump sum to households. Thus, in Prescott's  setting,
the government's budget restraint \Ep{ricg} reduces to 
$$
 - \sum_{t=0}^\infty  q_t   \tau_{ht}  \leq 
        \sum_{t=0}^\infty  q_t \Bigl\{ \tau_{ct}  c_t
  +
\tau_{kt} (\eta_t -\delta) k_t +  \tau_{nt} w_t
      n_t \Bigr\}  .
...
A graph of $r_{t,t+s}$ against $s$ for $s=1, 2, \ldots, S$ is called a (real) yield curve at $t$. \index{expectations!theory of the term structure}%
...
that the price vector $\{q_t\}_{t=0}^\infty$ of zero coupon bonds must be such that one-period holding period returns are equated across zero coupon bonds of
all maturities.  Note also how the price system $\{q_t\}_{t=0}^\infty$ contains forecasts of one-period holding period returns on zero coupon bonds of all maturities
at all dates $t \geq 0$. \index{holding period yields}%
...
The Arrow-Debreu budget constraint \Ep{govt_budget_AD} automatically imposes a `no-Ponzi scheme' condition on the path of
government debt $\{B_t\}$.
...
$$
The model of this chapter asserts that one payment stream $\{\xi^t_s\}_{s=t+1}^\infty$  that satisfies \Ep{term_structure_payments} is 
equivalent with any other. The model pins down  the total value of the continuation government bond payment stream $\{\xi^t_s\}_{s=t+1}^\infty$ at each $t$, but it leaves the
...
\noindent The sequence of period-by-period net returns on  government debt $\{ r_{t,t+1} B_t\}_{t=0}^\infty$ is independent of
the government's choice of payments sequences $\{\{\xi^t_s\}_{s=t+1}^\infty\}_{t=0}^\infty$.
...
policy is  an expenditure plan $\{g_t\}_{t=0}^\infty $  and a   tax plan $ \{\tau_{ct}, \tau_{kt}, \tau_{nt}, \tau_{ht} \}_{t=0}^\infty$ that
...
and a price system for which, given the price system and the government
...
The term on the right side of equation \Ep{ric1a} is often called  the ``user cost of capital''. As we saw from studying the representative firm's  optimization problem,   $\eta_{t+1}$ is the object that the firm wants to equate to the marginal product of capital at time $t+1$. 
...
and  a depreciation
cost.\NFootnote{The right side of equation \Ep{ric1a} is a discrete-time version of a continuous-time
 formula for the user cost of capital derived by Hall and Jorgenson (1967).}
\auth{Hall, Robert E.} \auth{Jorgenson, Dale}%
...
The definition of a competitive equilibrium and  concavity
...
and then use $\sum_{t=0}^\infty q_t \tau_{ht}$ as a slack variable that we set to balance the government's budget.  We now attack this system of difference
equations.
...
\subsection{Computing an equilibrium path with a shooting algorithm}
...
$k_0$, assuming a possibly time-varying path of government policy variables.\NFootnote{We recommend a suite of computer programs
...
\subsection{Another computational approach}

For a given government plan, we want to compute  a sequence that $\{c_t, k_t\}_{t=0}^{S}$
that satisfies the difference equations  \Ep{ric7} and \Ep{tom100}, together with appropriate initial and terminal conditions.
To do so, we can proceed as follows.

\medskip
\noindent{\bf 1.} 
Given an initial government plan $z_0$, 
compute residual for restriction \Ep{steadstk}  on  $k_0$:
$$
\varepsilon_{k_0} = 1 - \beta \left[ (1 - \tau_{k0}) \left(f'(k_0) - \delta \right) + 1 \right]
$$

\medskip
\noindent{\bf 2.} Define residuals from feasibility  condition \Ep{ric7}  for $t = 1, \dots, S-1$:
$$
\varepsilon_{tk} = k_{t+1} - f(k_t) + (1 - \delta)k_t - g_t - c_t
$$
\medskip

%2.Initialize a sequence of initial guesses $\{\hat{c}_t, \hat{k}_t\}_{t=0}^{S}$

\noindent{\bf 3.} Define  residuals from consumption Euler equation \Ep{tom100} 
for $t = 0, \dots, S$ as 
$$
\varepsilon_{ta} = \beta u'(c_{t+1}) {\frac{(1 + \tau_{ct})}{(1 + \tau_{ct+1})}} \left[(1 - \tau_{kt+1})(f'(k_{t+1}) - \delta) + 1 \right] - u'(c_t) $$
...
\noindent{\bf 4.}
Comnpute residual from imposing that  $k_S$ is a steady steady:
$$
\varepsilon_{R_{kS}} = k_S - k_{S-1}
$$


\medskip
\noindent{\bf 5.}
Apply a gradient descent algorithm that chooses $\{c_t, k_t\}_{t=0}^{S}$ to minimize the norm of the  vectorized list of  residuals $ \{\varepsilon_{ta}, \varepsilon_{R_{tk}}\}_{t=0}^S.$
\index{gradient descent algorithm}%
...
\section{Effects of taxes on equilibrium allocations and prices}
   ...
   We use the model  to analyze  effects of
government expenditure and tax sequences.
The household can affect its payments of a {\it distorting\/} tax
by altering a decision. The household cannot affect its payments of a {\it nondistorting\/}
tax.
...
Suppose that  distorting taxes are all zero and that
only lump-sum taxes are used to raise government revenues.  Then
the equilibrium allocation is identical with  one that solves a
...
with first-order conditions and feasibility conditions for
a planning problem.
...
\Ep{formula1;g}.
\medskip
\noindent{\bf 4.}  {\bf Capital taxes are distorting.}
...
We continue to study the special case with $U(c,1-n)=u(c)$.
  Figures \Fg{fig_g} through \Fg{fig_tk} apply a  %\Fg{experiment01f}, \Fg{experiment02f}, %\Fg{experiment03f},
%  \Fg{experiment06f},
...
zero and consider perturbations  that we describe in the experiments
...
state version of the Euler equation \Ep{ric8}),  consumers choose to make the
capital stock
...
in Milton Friedman's permanent income theory. Variation over time
...
Figure \Fg{fig_g_gamma} compares responses to a foreseen increase in $g$ at $t=10$ for two economies,
...
This shows up in the equilibrium  outcomes
 in figure \Fg{fig_g_gamma}. When $\gamma=.2$, consumption is much less smooth than when $\gamma=2$, and  is closer to being
...
and this leads to smaller fluctuations in $\bar R$ and $\eta$.  These two experiments reveal the   dependence  
on the magnitude of $\gamma$
of the strength of both the `feedforward' anticipation effect
and the `feedback' transient effect that wears off initial conditions.  We discuss this more later in section \use{sec:lambda_2} with the aid of equation \Ep{lambda_gamma}.
...
calendar time $t$ is
on the $x$ axis. In this model, $q_t = \beta^t c_t^{-\gamma}$ and $r_{t,t+1} = - \log \beta \Bigl({\frac{c_{t+1}}{c_t}}\Bigr)^{-\gamma}$, so the term structure of interest rates
...
\medskip\noindent{\bf Foreseen jump in $\tau_{ct}$.} Figure \Fg{fig_tc}
...
to apply a shooting algorithm. But for models with  larger state
spaces, it can be more difficult to apply a shooting algorithm. For those
...
structure.} (1) a ``feedback'' part that portrays  transient
responses of the system to an initial condition $k_0$ that deviates
from an  asymptotic steady state, and (2) a ``feedforward'' part
that shows  current effects of foreseen
...
i.e., $\lambda_i = \zeta_0^{-1}$ where  $\phi(\zeta_0) = 0$.\NFootnote{The Matlab roots
 command {\tt roots(phi)}
finds zeros of a polynomial $\phi$, but you must arrange  the polynomial
...
$(1-\lambda_1 L) = -\lambda_1 L( 1-\lambda_1^{-1} L^{-1})$.\NFootnote{Notice that
 $(1-\lambda_1 L)^{-1} =  %- \lambda_1^{-1} L^{-1} (1-\lambda^{-1} L^{-1})^{-1}$ as
...
The term $\lambda_2 k_t$ is often called
...
are often called  ``feedforward'' parts. They depend on the
infinite {\it future\/} of the exogenous $z_t$ (which for us
contain   components of government policy) and measure the
...
representation \Ep{newsys1;c} for the endogenous state variables.\NFootnote{This finding is 
 the technical foundation of the 1976 ``Lucas Critque'' discussed in sections \use{sec:lucascritique} and
 \use{sec:lucascritique2} of chapter \use{timeseriesapp}.}
 \auth{Lucas, Robert E., Jr.}%
...
\centerline{\epsfxsize=3truein\epsffile{lambdaogamma_gray.eps}}
...
concluded that \Ep{ric21} does not hold (``workers are off their labor supply curve'').
}
%
...
linearly homogeneous production function described above. We assume that
...
{Y_t\over n_t A_t}$.  We say that $k_t$ and $y_t$ are measured in  
units of ``effective  labor'' $A_t n_t$. We also let $c_t = {C_t
...
measured in  units of effective labor, then proceed as before.
...
from $1.02$ to $1.025$ in the  gross rate if  growth in productivity  $\mu_t$ at
$t=10$. This figure and also Figure \Fg{fig_pr_2}
 now measure $c$ and $k$ in  units of effective labor.  The
...
\Fg{fig_pr_2} smooth. Consumption per  unit of effective  labor
...
We  shut down productivity  growth by setting the gross productivity growth rate  $\mu =1$, but we
     allow a 
...
Now we have to carry along equilibrium conditions
  for the intertemporal  evolution of capital and for the
...
It is helpful to use these equations to write \Ep{tom_e_2001_old}
as
...
to {\it lower\/}
consumption per capita by the amount of the {\it increase\/} in government purchases per capita.  Now,  the consequence 
is to leave unaltered  both  steady state  consumption per capita and the steady state capital/labor ratio.  This is accomplished
 ...
 These asymptotic outcomes  follow immediately  from  our steady state equations.  The increase in
...
\Ep{eqlbrm_new_a} then dictates that  steady-state consumption per capita also remains unaltered.
...
accompanied by an adjustment in the present value of lump sum taxes that balances the government's budget.
...
economic activity has been depressed relative to the U.S. because its tax rate on labor has  been higher.  Prescott's numerical
...
To interpret these outcomes, we begin by noting that with  preference specification \Ep{hansen_rogerson_1},
...
to its initial value, the real wage falls by the entire amount of the decrease in $(1-\tau_n)$.  The decrease in the after-tax wage after $t=10$ makes it relatively more attractive to work before $t=10$ than after.  As a consequence,
...
Mertens and Ravn (2011) use such forces  to  offer an interpretation of contractionary contributions that the Reagan tax cuts made
...
We now study a  two-country  model with a structure like those used  Backus, Kehoe, and Kydland (1992))  and  Mendoza and Tesar (1998).
We paste two of our one-country models  together and allow the countries  freely to trade goods and  claims on future goods, but not labor.  %This means that there is international trade
%in goods, capital, and financial claims to future goods, but not in labor.
 We must  be careful in how we specify taxation of earnings by non residents. %In particular,
...
\auth{Kydland, Finn E.}
 ...
 Objects for  the first country are denoted without asterisks, while those for the second country
...
A consumer in country one can hold capital in either country and  pays taxes on rentals from   holdings of foreign capital to the foreign country  at the rate set  by the foreign country.
At time $0$, residents in both countries can purchase consumption at date $t$ at a common Arrow-Debreu price $q_t$. Let $\tilde k_t$ be capital in country 2 held by a representative   consumer of country 1. Temporarily, in this paragraph only, let $k_t$  denote the amount of domestic capital owned by the domestic country consumer. (In all other paragraphs of our exposition of the two-country model,
...
of capital in country 1.) Let $B_t^f$ be the amount of time $t$   goods  that  the representative  domestic consumer raises by issuing a one-period IOU to the  representative foreign consumer;
    so $B_t^f >0 $ indicates that the domestic consumer is borrowing from abroad at time  $t$ and $B_t^f < 0$ indicates that the domestic consumer is lending abroad at time $t$.  For $t \geq 1$ let $R_{t-1,t}$ be the gross return on a one-period loan from period $t-1$ to period $t$. Define $R_{-1,0} \equiv 1$ and let the
...
Adopting this way of resolving portfolio indeterminacy is convenient because it  economizes on the number of initial conditions we must specify.
...
Equilibrium requires that the following two country-specific Euler equations be satisfied for $t \geq 0$:
...
$$\EQNalign{ f'(\overline k) &= \delta + {\frac{\rho}{1-\tau_k}} \EQN steadst_2100\cr
             ...
             $$
Given  steady state capital-labor ratios $\overline k$ and $\overline k^*$, the following two equations
...
time $0$. In the following experiments,  we shall typically  initialize international debt at zero:
$ B_{-1}^f = 0, $
...
which says that at time $0$ the domestic country finances imports of physical capital from abroad by borrowing from the foreign country $^*$.
...
\Ep{twocountry5}, and \Ep{twocountry6}.  The shooting algorithm  `aims' for   $(\overline k, \overline k^*)$ that satisfy  steady-state
...
equate rates of returns on bonds and capital.

\newpage

The production function is $f(k) = A k^\alpha$. Parameter values are $\beta=.95, \gamma =2, \delta = .2, \alpha = .33, A=1$;
 $g$ is initially
$.2$  in both countries and all distorting taxes are initially $0$.
In the first two experiments,  taxes are  lump sum in both countries.  In the third experiment we activate
...
steady state in the domestic economy.}
\infiglist{fig1_kinit}
\endfigure


\subsubsection{Opening International Trade}
In our
first example, we study the transition dynamics  when  in period one
newly produced output and stocks of capital both   suddenly  become internationally mobile. The  economies are initially
identical
...
has a capital stock that is half as large.  This means that initially, before trade is opened at $t=1$, the
...
foreign interest rate $R_{0,1}^*$ exceeded the domestic interest rate $R_{0,1}$, and that consequently the foreign consumption growth rate initially  exceeded the
domestic consumption growth rate. The disparity of interest rates before international trade is opened is a force for physical capital to flow from the domestic
country to the foreign country  when trade is opened at $t=1$.
Figure \Fg{fig1_kinit} presents  transition dynamics. When countries
...
Before international  trade had opened, rental rates on capital and interest rates differed across countries because
marginal products of capital and  consumption growth rates differed.  When trade opens unexpectedly  at
time $1$ and  capital is reallocated across countries to equalize returns,  the interest rate in the domestic country
jumps. Because $\gamma =2$, this means that consumption $c$ in the domestic country  must temporarily fall.
% The opposite is true for the foreign economy.
\newpage
\noindent Notice also that figure \Fg{fig1_kinit} shows an
...
differences account for a steady-state cross-country  consumpton difference that equals  $2 \rho
\overline{B^f}$.
...
Opening trade in goods and capital at time $1$ benefits consumers in both economies.
 By
 allowing capital flows, the foreign country achieves convergence to
a steady-state consumption level at an accelerated rate. This steady-state
consumption rate is lower than what it would have been  had the economy remained
closed;  the  reduction in long-run consumption is more than compensated
by the rapid increase in consumption and output in  the short-run. In contrast, domestic consumption
falls in the short-run as trade in physical and financial capital allows domestic consumers to accumulate
foreign assets that ultimately  support  greater steady-state consumption.

This experiment shows the importance of studying transitional dynamics for
welfare analysis. Focusing only on steady-state consumption
would lead to the false conclusion that opening markets is  detrimental for the 
poorer foreign  economy.


\midfigure{fig1_g-2}
\centerline{\epsfxsize=3truein\epsffile{fig1_g.eps}}
\caption{Response to  increase in domestic $g$ at time $10$ foreseen at time $1$.
...
domestic economy from $.2$ to $.4$ that is announced ten periods in advance. We start both economies
 from a steady-state with $B_{0}^f = 0$. When the new domestic $g$ path  is
revealed  at time $1$, consumption smoothing motives induce domestic households to increase
...
domestic capital in the short-run,  which would be  only physical asset in a closed economy version of this experiment.
 In an open economy, domestic consumers  have anohter  way to save, namely,  by lending abroad.
 The no-arbitrage conditions connect adjustments of the two types of saving: the   increase
   in
savings  by domestic households will reduce the equilibrium return
on bonds and capital in the foreign economy in order to eliminate  arbitrage
opportunities in equilibrium. Confronting  the revised  interest rate path that now begins with lower
interest rates, foreign households increase their rates of consumption and
investment in physical capital. These increases  are funded
by  increases in foreign consumers' external debt. After the announcement of
the increase in $g$,  paths for consumption (and capital) in both countries follow
similar  patterns because no-arbitrage conditions equate their intertemporal  ratios of marginal utilities of consumption.
 Both countries  accumulate capital until the
increase in acually $g$ occurs. Then   domestic households begin consuming some of their
 capital. By the  no-arbitrage conditions,  when  $g$  actually  increases both countries reduce their
investment rates. The domestic economy starts
running current-account deficits  to help pay for  the increase in $g$. Coincidentally,  foreign households begin repaying some of  their external debt by
reducing their  capital stock. Figure \Fg{fig1_g-2} also plots gross investment  $x$.  When the increase in $g$ actually occurs at $t=10$,  gross investment drops sharply. After
$t=10$, all variable converge smoothly to a new steady state in which  the
domestic economy persists with positive asset holdings $- B^f$. Consequently,  the foreign country ends up  with lower steady state consumption
than it had  in  the initial steady state.  In the new steady state, minus the sum of  {\it decreases\/} of
...
The experiment  it shows how the
consequences of the foreseen increase in domestic $g$ will be distributed across time and countries. They are distributed  through time by accumulating or reducing the
capital stock; they are distributed across  countries by their running international current-account deficits and
surpluses.
\midfigure{fig1_tauk}
...
steady-state values (dotted-dashed line).}
\infiglist{fig1_tauk}
\endfigure
...
decline in order to  equalize  after-tax
returns across countries. % despite a higher domestic tax rate on returns from capital.
Domestic households  reduce their
capital stock by increasing their rate of consumption. Consequent higher equilibrium
world interest rates  induce
foreign households to increase consumption. Prior to the  increase in $\tau_k$, the domestic country runs
a current account deficit. When $\tau_k$ actually  increases,  
...
the return on bonds occurs as  the capital returns tax $\tau_k$ in the domestic country 
reduces the after-tax return on capital. %, and because the foreign economy has
%a higher capital stock. 
The foreign country  funds this large purchase of
capital with a sharp increase in its  external debt, generating  a current account deficit.
After $\tau_k$ has increased, the two economies smoothly converge to a
new steady state in which  consumption rates are lower in both countries and  differences in their  capital stocks equate after-tax returns to capital.  Despite
 having  perpetually positive liabilitiesm   steady-state consumption in the foreign economy is higher than in the domestic countrys. This occurs because foreign output is larger because foreign  capital stock held abroad is also larger.

This experiment illustrates how, via the no-arbitrage conditions,  both   countries are affected by the shock to the domestic economy, 
 and how  fluctuations in capital stocks  smooth intertemporal  adjustments  of  consumption in both countries.
...
interpret them,  not as computational errors as we did  in discussing  the procedure
recommended in section
\use{sec:accuracy}, but instead  as measures of additional shocks that have to be added to the basic model to make it fit  data  better.
In the basic stochastic growth model described  in chapter
\use{growth1}, the technology shock  is the only shock, but it alone 
cannot  account for the discrepancies that emerge in
fitting all of the model's Euler equations.   A
message of  Ingram, Kocherlakota, and Savin (1994) and  Hall
(1997) is
 that more shocks are required to fit  their  data.
...
using likelihood function-based methods.   Christiano, Eichenbaum, and Evans (2003) and
Christiano, Motto, and Rostagno (2003) also  add  shocks and  measure their importance.
Greenwood, Hercowitz, and Krusell (1997) introduced  a technology shock
...
of economies, like  ones described here, in which distortions prevent equilibrium allocations from
being optimal.  They describe ways of attaining locally more
