...
For pure exchange stochastic economies, chapter \use{recurge}  described
...
with sequential trading was to account for how individuals' financial wealths
evolve as time passes in a time $0$ trading economy.
In a time $0$ trading economy, individuals make no 
 trades other than those executed in period $0$, but  present
values of those portfolios change as time passes and as uncertainty gets
resolved.
So in period $t$ after  history $s^t$, we used the Arrow-Debreu
prices to compute the value of an individual's time-$0$-purchased
claims to current and future goods net of his outstanding liabilities.
We could then show that these financial wealth levels (and the
associated consumption choices) could also be attained in a sequential-trading
economy in which  markets only in one-period Arrow securities  open  each period.


In section \use{sec:arrowsecurities} of  chapter \use{recurge} we also demonstrated how to obtain a
recursive formulation of an equilibrium with sequential trading.
This required us to assume that individuals' endowments
were governed by a Markov process. Under that assumption
we could construct a state vector
...
This transformation of price systems was easy in
    the pure exchange economies of chapter \use{recurge}
because in equilibrium,  the relevant state variable, financial wealth, is a
...
parts of the aggregate state are endogenous in the sense that they
emerge from a
{\it history}
of
...
injecting various frictions into the environment.} %
We also formulate  a
recursive competitive equilibrium with trading in Arrow securities
by using a version of
  the ``Big $K$, little $k$'' device that we encountered earlier in chapters \use{recurpe} and \use{recurge}.

\index{growth!stochastic}
...
Here we spell out basic ingredients of the stochastic growth model:
preferences, endowments, a technology, and information. The
environment resembles that of  chapter \use{linappro} except
that we now allow for a stochastic technology level.
In each period $t\geq 0$, there is a realization of a stochastic
event $s_t \in {\bf S}$. Let the history of events up to time $t$
be denoted $s^t = [s_t, s_{t-1}, \ldots, s_0]$. The unconditional
probability of observing a particular sequence of events $s^t$ is
given by a joint probability measure $\pi_t(s^t)$. For $\tau > t$, write a conditional
probability as $\pi_\tau(s^\tau\vert s^t)$, which is the probability
...
In each period, a representative household is endowed with one
...
investment good augments a capital stock that  depreciates at
the rate $\delta$. Negative values of $\xst$ are permissible, which
means that the capital stock can be reconverted back into the
consumption good.
...
about positive but diminishing marginal products
$$
...
$$
Another property of the linearly homogeneous
function $F(k,n)$ is that its first derivatives are homogeneous of degree 0
and thus  are functions only of the ratio $\hat k$. In
particular, we have
$$\EQNalign{
...
\section{Lagrangian formulation of a planning problem}
A planner chooses an allocation
...
To solve the planning problem, we
form the Lagrangian
...
\autoparens
where $\mu_t(s^t)$ is a process of nonnegative Lagrange multipliers  on
...
problem with a less decentralized arrangement with only the first of
our two types of firms, and in which the decision for making
physical investments is assigned to the household. We assign the investment
decision to a second type of firm because we want to price more
items, in particular,  capital stocks.}
...
firm and sells labor services to a type I firm that operates
...
The household chooses consumption $c$, labor supply $1-n$ processes to maximize
...
where $\eta>0$ is a Lagrange multiplier on the budget constraint
\Ep{hh2}.
...
$t$ and each possible history $s^t$, the firm enters into
...
and the firm's maximization problem can then be decomposed into two parts.
...
It wants to set  $n_t(s^t)$  to zero or
...
imply the following restrictions on  equilibrium prices:
...
Thus, the type II firm bears of manages the risks associated with technology
...
the type II firm's objective function can be written 
...
%capital. 
The profit-maximizing levels of the capital stocks
$k^{II}_{t+1}(s^{t}) \ \forall t \geq 0, \ \forall s^t$ in expression \Ep{ff_IIk} are equal to
zero or infinity depending on whether the associated  terms in
curly brackets are strictly negative or strictly positive,
respectively. However, for any term  in curly brackets
in \Ep{ff_IIk} that is zero, the firm would be
indifferent to the level of $k^{II}_{t+1}(s^{t})$, since profits
 then equal  zero for all choices of $k^{II}_{t+1}(s^{t})$.
In an equilibrium, $k^{II}_0$ and $k^{II}_{t+1}(s^{t})$ are
strictly positive and finite, so each expression in curly brackets
in \Ep{ff_IIk} must equal zero, and hence equilibrium prices must satisfy the restrictions
...
allocation of labor services and capital. But this leaves undetermined
...
But this leaves undetermined  equilibrium values of 
particular prices. To restrict equilibrium prices and quantities, we bring to bear
the representative household's first-order conditions \Ep{hhh1}.
...
To take advantage of  the equivalence of allocations, it is standard to compute
a competitive equilibrium allocation by first solving the
planning problem.
We can then compute equilibrium  prices by substituting
...
determined. In order to pin down a nominal  price level, we also
have to normalize by picking a numeraire. Such a normalization of prices is
...
\subsection{Implied financial wealth dynamics}
Even though trades occur  only at time $0$ in the Arrow-Debreu
market structure, we can study how the representative household's continuation financial
...
Thus,  we change  numeraires so that
...
In chapter \use{recurge} we computed  implied financial wealths of  households at time $t$ after history
$s^t$.
Here we compute counterparts except that instead of accounting for continuation values of endowments
we account for continuation values of earnings from labor.
Now a household's net claim
...
goods. Thus, the representative household's financial wealth, or the value of all its
...
terms, and  finally, the fifth equality acknowledges that $q_t^t(s^t) = 1$
and that each term in curly brackets is zero by virtue  of equilibrium price
...
market structure that sustains the allocation from
the time $0$ trading competitive equilibrium.
...
$\tilde Q_t(s_{t+1} | s^t)$ is the price of one unit of
...
on the realization of $s_{t+1}$.   Let $\tilde a_t(s^t)$ denote 
claims to time $t$ consumption
that the household brings into time $t$ at history $s^t$. Thus,
...
time $t+1$ consumption,
one component of the vector for each  possible value of 
$s_{t+1}$.


To rule out Ponzi schemes, we must impose borrowing constraints on
the household's asset position. We could compute state-contingent
{\it natural debt limits,\/} where a counterpart to the chapter \use{recurge} present
value of the household's endowment stream would be the present
value of the household's time endowment. Alternatively, we could  just
...
well enough for the following reason. As long as the household cannot 
run a Ponzi scheme that leaves its consumption possibilities unbounded, 
 forces in equilibrium will ensure that the representative
...
Let $\eta_t(s^t)$ and $\nu_t(s^t;s_{t+1})$ be  nonnegative
Lagrange multipliers on the budget constraint \Ep{seq_hh0} and on the
borrowing constraint with an arbitrary debt limit of zero,
respectively, for time $t$
and history $s^t$. A Lagrangian can then be formed as \offparens
$$ \eqalign{ L =
...
$\{\tilde c_t(s^t), \, \tilde n_t(s^t)\}_{t=0}^\infty$ because 
 Inada conditions on the utility function
ensure that the household will  set neither $\tilde c_t(s^t)$
nor $\ell_t(s^t)$ equal to zero so that $\tilde n_t(s^t)<1$. The Inada
conditions on the production function guarantee that the
household will always find it desirable to supply some labor,
$\tilde n_t(s^t)>0$. At interior solutions,
 first-order conditions for maximizing $L$ with respect to
...
conjecture that the arbitrary debt limits of zero do not bind so that the Lagrange multipliers $\nu_t(s^t;s_{t+1})$
are all equal to zero.  After
setting those multipliers to zero in equation \Ep{seq_hhFOC;c}, the
...
If conditions \Ep{seq_f1c} were violated, the type I firm would
either make infinite profits by hiring infinite capital
and labor, or else would make negative profits for any positive
output level and therefore shut down.  If conditions \Ep{seq_f1c}
...
two-period optimization problem in which a type II firm decides how
...
The key is to guess that  prices in the sequential
equilibrium satisfy
...
$$
An additional set of guesses posits that the representative household
chooses asset portfolios 
$\tilde a_{t+1}(s_{t+1},s^t) = \Upsilon_{t+1}(s^{t+1})$ for all
$s_{t+1}$. While verifying that the household can afford these asset
portfolios together with the prescribed quantities of consumption
and leisure, we will find that the required initial wealth is 
$$
\tilde a_0 = [r^0_0(s_0) + (1-\delta) ] k_0 = p_{k0} k_0,
$$
i.e., at the beginning of period $0$ the household in the sequential equilibrium  owns
the initial capital stock, which it then sells to a type II firm at the
same competitive price that previals in the time $0$ trading equilibrium.
...
Thus, the firm of type II is entirely owned by its creditor, namely, the
household. The firm earns zero profits.
...
immediately from fact that next period the household's desired
beginning-of-period wealth  is 
$\tilde a_{t+1}(s^{t+1})$ and equals $\Upsilon_{t+1}(s^{t+1})$,
as given by \Ep{seq_wealth}. Thus,  equilibrium prices entice the
representative household to enter each future period with
a strictly positive net asset level that is equal to the value
of the type II firm. We have thus confirmed 
 our earlier conjecture that the arbitrary
debt limit of zero does not bind in the household's optimization
problem.
...
Following an approach taken in chapter \use{recurge},
we have verified that 
equilibrium allocations are identical in an Arrow-Debreu
...
function of the history of events $s^t$ that in turn are
governed by some arbitrary probability measure $\pi_t(s^t)$.
At this level of generality,
 prices $\{\tilde Q_t(s_{t+1} | s^t),\, \tilde w_t(s^t),\,
\tilde r_t(s^t)\}$ and the capital stock $k_{t+1}(s^t)$
in the
sequential-trading economy all depend on  history $s^t$. That is,
...
social planning problem and the sequential-trading competitive equilibrium,
...
nonstochastic and hence $\pi_0(s_0)=1$ for a particular
$s_0\in{\bf S}$. Sequences of probability measures $\pi_t(s^t)$
on histories $s^t$ are induced by the Markov
process via the factorizations
...
Our specialization of the technology process enables us to
...
last period's technology level, denoted $A$, in order both to determine
...
does not call for  conceptual changes relative to  what we did in
chapter \use{recurge}. We are merely adding another state
variable that is governed by a function of exogenous stochastic events;
it  depends on  no endogenous outcomes.


However,  we must expand the aggregate state vector to include 
an {\it endogenous\/} component of the state of the economy, namely, the
beginning-of-period capital stock $K$. With a state
vector $X\equiv \left[ K \;  A \; s\right]$, we are ready to construct
 recursive versions of the planning problem
and of a sequential-trading competitive equilibrium. This state vector is
a complete summary of the economy's current position. It is
all that  a planner needs to compute an optimal
allocation and also  all that  an ``invisible hand'' needs
to construct competitive Equilibrium prices.
...
use the
 state vector $X$
from the planner's problem as a state vector
with which to construct  Arrow securities in a competitive economy with sequential
trading.    Then we define a competitive equilibrium
and show how  prices in a  sequential competitive equilibrium are encoded in
...
\section{Recursive formulation of  planning problem}
...
problems.  We shall eventually equate big and little letters, but not until after we have
formulated the household's and firms' problems
...
$X = \left[ K \;  A \; s\right]$, we denote   optimal
policy functions as
...
First-order conditions for the planner's problem can be represented
...
To do this, we  use a ``Big $K$, little $k$'' trick of the type used in
chapter \use{recurpe}. \index{Big $K$, little $k$}%


\subsection{``Big $K$, little $k$'' device}
...
that is useful for forecasting  prices and that agents
regard as beyond their control when posing and  solving their optimum
problems. Values of little $k$ are chosen by firms and consumers.
While we distinguish $k$ and $K$
 when posing  decision problems of the household and firms,
...
state-contingent payoffs are defined.  We proceed by guessing that an
...
Temporarily, we also take as given an arbitrary candidate for the law of
motion for $K$:
$$ K' = G(X). \EQN Kmotion $$
...
In chapter \use{recurge} in which the state  consisted only of the exogenous variable $s_t$, simply setting $\hat \pi = \pi$ imposed rational 
expecatations.  Here we have to impose more because the state $X$  now  includes additional components, some of which 
are endogenous.  
...
Here $a$ represents the financial wealth of the household denominated in
units of current consumption goods and $\overline a(X')$ represents
next period's financial wealth denominated in units of next period's consumption
good.
...
$$ \EQNalign{ \overline u_c(a,X) & \equiv u_c(\sigma^c(a,X), 1-\sigma^n(a,X)),
 ...
 \overline u_\ell(a,X) & \equiv  u_\ell(\sigma^c(a,X), 1-\sigma^n(a,X)).
   \EQN hh10;b \cr  } $$
Then we can represent first-order conditions for the household's
problem as
...
\subsection{Firm of type I}
From subsection \use{sec:FirmI}, recall  the static optimum problem
of a type I firm in a sequential equilibrium. In a recursive
...
of a type II firm in a sequential equilibrium. In a recursive
...
impose equilibrium conditions on these objects and convert 
them into outcomes of the analysis.\NFootnote{An important
...
to be determined by  equilibrium interactions of households
...
also of $\hat \Pi$ and thereby impose rational expectations.
...
is to be regarded as instructing us to find a fixed point of
a mapping from a perceived $G$ and a price
system  to an actual $G$.
A fixed point  requires that the perceived law of motion
...
capital  are equal. Item e, which imposes rational expectations only about the exogenous part of the state $s$, and 
 equality of the perceived and actual
$G$ imply that $\hat \Pi  = \Pi$. Thus, items d and e together impose
rational expectations.


\medskip
\subsection{Verification via a  planning problem}
...
for consumption and labor supply  match those chosen by the
...
Here we are again exploiting  consequences of the welfare
...
chapter has been to configure an endogenous state vector in terms of
...
This appendix is a variation on the theme that  single agent models can often
be reinterpreted as general equilibrium models.

\subsection{Reinterpreting the single-agent model}\label{sec:pricesystemLQ}%
We cast the single-agent linear quadratic  permanent income model
of section \use{sec:LQmodel} of chapter \use{timeseriesapp} as a competitive equilibrium
with time $0$ trading of a complete set of history-contingent 
securities.\NFootnote{The model is an instance of a  class of linear-quadratic presented 
by Hansen and Sargent (2013). See chapter 5 of that book.}
\auth{Hansen, Lars P.}%
...
linear combination of  $z_t$.  As shown in chapter \use{timeseriesapp},
...
Equations \Ep{kc_solnk}, \Ep{kc_solnc}, \Ep{eq:pLQrepres} together with the equation $r_t^0 = \alpha$ 
to be explained
below in equation \Ep{eqnMPK}
...
As we saw in section \use{sec:LQmodel} of chapter \use{timeseriesapp} and also in
...
fluctuations in his consumption?   In the complete markets economy of this appendix, the consumer believes that
 it is possible 
 completely to smooth consumption over time and across histories by purchasing and selling
history contingent claims. But at equilibrium prices, the consumer prefers to accept
 fluctuations in consumption over time and across histories.\NFootnote{See Hansen and Sargent (2013, ch.~7) for formulas
 for the term structure of interest rates on risk-free claims in a class of models of which the present model is an 
 instance.}
