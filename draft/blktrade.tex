...
\section{Moral hazard and difficult enforcement}
...
\newpage
\noindent where $\delta \in (0,1)$ and $u(\cdot)$ is increasing, strictly concave,
...
the Bellman equation \index{Bellman equation}%
...
The optimal value  $W(Q)$ for a borrower with goods $Q$ satisfies the Bellman  equation  \index{Bellman equation}%
...
to add a promised value as an independent state 
variable.\NFootnote{Tsyrennikov (2013) extends Atkeson's model to a small open economy, showing that moral hazard and limited enforcement frictions together can rationalize observed repayment patterns—specifically, why net repayments occur after low output realizations. Tsyrennikov demonstrates that moral hazard alone can generate crisis-like dynamics and counter-cyclical interest rates, while limited enforcement adds little once moral hazard is present.}
...
\auth{Bassetto, Marco} \auth{Huo, Zhen} \auth{R\'ios-Rull, Jose-Victor}
...
\NFootnote{Bassetto, Huo, and R\'ios-Rull (2024) introduce an ``organizational equilibrium'' concept that they use to  model  desirable policies that are implemented gradually by a sequence of decision-makers, each tempted to delay costly reforms. Cooperation can emerge without full commitment, producing policy paths that improve steadily but fall short of the first-best outcome. Their  equilibrium is characterized by symmetric treatment of all agents, optimality among gradual paths, and a "no-delay" condition that prevents excessive generosity to the initial decision-maker. This captures the observed slow progress in areas like trade liberalization and climate policy.}
...
\auth{Worrall, Tim}\auth{Thomas, Jonathan}\auth{Kocherlakota, Narayana R.}% \newpage
...
\newpage
  ...
  Depending on  specifications of functional forms, preferences, and the joint distribution of shocks, case (3) may or may not be possible.  When it is, one can use the model to calculate waiting times to breakup of a union.  Many currency unions have broken up in the past, an observation
...
Although substantive details differ, mechanically
...
contract for Atkeson's (1988) model.  Following Atkeson, we work with the
...
\auth{Atkeson, Andrew}
\medskip
\item{1.}  First, solve the Bellman equation \Ep{bellmanaut} and
\Ep{foncautark}
...
\item{2.}  Solve the Bellman equation for the full-insurance
...
$\gamma_S=.4$ and  payoff functions  \Ep{bppayoff1;a} and
\Ep{bppayoff1;b}, with
