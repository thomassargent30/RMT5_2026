...
\item{{\bf ii.}} every state $e_i \in \tilde X$ is visited infinitely often.
\enddefinition
...
where $\beta \in (0,1)$ validates the Neumann series represenation $(I -\beta P)^{-1}
...
periods and not on time $t$. In this case, an autocovariance  matrix sequence $\{\Sigma_{t+j,t}\}_{j=0}^\infty$ is  called an {\it
...
Suppose that the eigenvalues of $A$ not associated with
a constant are bounded above in modulus  by unity.  Using
the lag operator $L$ defined by $L x_{t+1} \equiv x_t$,
express equation \Ep{diff1} as
$$ (I - A L) x_{t+1} = C w_{t+1} . \EQN diff2 $$
...
that if you don't want to deviate for one period,  you would never
...
This section presents the concept of  population linear regression.  \index{regression!population}% \index{regression equation}%
...
A Bayesian statistician breathes life into these equations % \Ep{eqn:Bayes101}
 by adopting  special interpretations of the probability
densities $f(z_1)$, $f(z_2)$,  and $f(z_2|z_1)$ and noting that equations \Ep{eqn:Bayes101} and \Ep{eqn:Bayes101a}
...
A Bayesian statistician interprets equation \Ep{condmeansurprise} in a useful way. The equation
...
think of $A, C$ as being  the matrix functions $A(\theta), C(\theta)$.}
...
\index{filter!linear}%
 ...
 observed variables.\NFootnote{See Hamilton (1994), Canova (2007), DeJong and Dave (2011), and section \use{sec:estimation2} below.}
...
Specification \Ep{kalf2}, \Ep{kalf3} implies that the conditional distribution of $y_0$ is
...
$y^{t-1}$ that has a convenient recursive representation. The Kalman filter attains this by constructing recursive formulas for objects $\hat x_t, \Sigma_t$
 that appear in the following generalization of \Ep{kalf4}
...
distribution of $y_t$ conditioned on $y^{t-1}= [y_{t-1}, \ldots, y_]$ for $ t \geq 1$.  The sufficient statistics and also the innovation
...
time-invariant Kalman filter, allowing for correlation between  $w_{t+1}$ and $v_t$ Also see exercise \the\chapternum.22.}
\mtlb{kfilter.m}%
...
the forecasting scheme; I have to find a stochastic process
...
To illustrate several  key ideas   of this chapter,
...
enjoy bliss consumption forever.  The purpose of imposing this
...
$u(c_t) =  -.5 (c_t - \gamma)^2$,
where $\gamma >0$ is a bliss level of consumption. Then
...
`\idx{precautionary savings}' divergence.}
...
where again, the last equation comes from applying to decision rule \Ep{sprob9} the appropriate version  our formula \Ep{discount1} for forecasting geometric sums of future values of the random variable $y_t$.


\subsection{Another representation}
Another  way to  understand the system under the  optimal decision rules for $b_t, c_t$  is to recognize that there is a point
...
is a sharp way to summarize the  LQ permanent
...
$$ \EQNalign{  E_t \sum_{j=0}^\infty \beta^j z_{t+j} &  =
   (I-\beta \check A)^{-1} z_t \cr
    E_{t+1} \sum_{j=0}^\infty \beta^j z_{t+j+1} & = (I -\beta \check A)^{-1} z_{t+1} \cr
...
the  forward direction  the first-order linear difference equation that is  the consumer's one-period budget constraint using the method
 described in appendix \use{appa1} of this chapter.  
...
identity contemporaneous covariance matrix, all  eigenvalues of $\check A$ are strictly less than ${\frac{1}{\sqrt{\beta}}}$ in magnitude,
 and
$\check G$ is a selection vector that describes $y$ as a particular
...
We now pause to stare at equation \Ep{stareatit101} and  think about alternative  assumptions about what is predetermined at $t$ and
what is free to adjust to shocks to $z_t$, i.e., alternative assumptions about what are {\it states\/}  and what are {\it outcomes\/} at $t$.
...
By making other assumptions about what variables in \Ep{stareatit101}  are set before time $t$ (they are  ``predetermined'' at time $t$)
and what variables can  be adjusted  at time $t$, we can put the consumer into a system of markets that provide him more opportunities
to trade. To show this, we begin by constructing another  consumption plan that satisfies both conditions \Ep{sprob5c} and \Ep{sprob6c} as well
as a special case of  equation \Ep{stareatit101} in which 
$$ c_t = \bar c  = c_0 , \EQN guess101 $$
where  $c_0$ is given by equation  \Ep{eqn:lucascritiquecons} for  the  optimal time $0$ consumption
in the section \use{sec:LQmodel}  incomplete markets version of the model.    Because it  completely smooths consumption and has
...
implied by decision rule \Ep{stareatit101}, an assertion that we'll soon prove.

 To construct a financial plan that supports constant consumption, temporarily take $\bar c$ as given, substitute \Ep{guess101} into
...
%where $\bar b_0$ is an initial level of debt that the household owes in state $z_0$ at time $0$.  

In effect,  we have backed ourselves into  a model in which each period the consumer  trades a complete
list of one-period state-contingent securities that  pay off amounts next period that depend on next period's information vector $z$. 
  To make this work,  prices of those securities must induce the consumer to want to smooth consumption completely
...
These prices will turn out to  imply that the price at time $t$ of one unit of consumption for sure at time $t+1$ is $\beta$ for all $t \geq 0$, just as it is in the section \use{sec:LQmodel}  model  in which
...
where $q(z_{t+1} | z_t)$ is a pricing kernel for one-period  state contingent claims and $b_t(z_{t+1})$ is   a vector of state-contingent debts due in state $z_{t+1}$
 that are chosen at time $t$.
We exogenously set the pricing kernel to 
$$ q(z_{t+1} | z_t ) = \beta \phi(z_{t+1} | z_t) , \EQN kernel101 $$
where $\phi(z_{t+1} \ z_t)$ is the probability density of $z_{t+1}$ conditional on $z_t$.  Kernel \Ep{kernel101} discounts state-contingent prices
of claims on consumption at date $t+1$ by the product of the discount factor $\beta$ and  conditional probabilities of   events
at which  claims pay off. This special   pricing kernel
implies that the price of a one-period risk-free bond is 
$$ \int q(z_{t+1} | z_t) d z_{t+1} = \beta \int \phi(z_{t+1} | z_t) d z_{t+1} = \beta,  \EQN noarbitrageLQ101 $$
so that the gross interest rate on one-period risk free loans is  $\beta^{-1}$.\NFootnote{It would be redundant to add a risk-free one-period bond to the state-contingent one period securities that the consumer can trade in  our  complete markets model.  If that redundant asset were to 
be added, a no-arbitrage profits argument like the one to be applied repeatedly in chapters \use{recurge}, \use{assetpricing1}, and \use{assetpricing2} would imply formula \Ep{ noarbitrageLQ101}.}

With \idx{pricing kernel}  \Ep{kernel101}, the  
consumer's time $t$ one-period budget constraint \Ep{CMbudget101}  becomes
...
in which  $b_{t+1}$ is a scalar to be chosen at time $t$. 
...
$$ \eqalign{ E_0 c_t & = c_0 \cr
              E_0(c_t  - c_0)^2 & = t h h' . } $$
Because the consumer dislikes variance in consumption, he prefers the completely smoothed   consumption stream  that he chooses in the complete markets version of the model to the random walk consumption stream that he chooses in the incomplete markets version of the
 model.
...
the objects that are determined within an  equilibrium.  
 


\index{Lucas Critique}%
\section{The Lucas Critique}\label{sec:lucascritique}%
 We can concisely describe reasoning that led Robert E. Lucas, Jr., (1976) to criticize a class of  procedures that were widely used in applied work at the
...
where $A$ is a stable matrix, $\delta \in (0,1)$ is a discount factor,
...
forecasts of       discounted sums of current and future $y_t$ vectors, forecasts that equal the appropriate conditional expectations
...
$d_t = H E_t \sum_{j=0}^\infty \delta y_{t+j}$ where $E_t$ denotes a mathematical expectation conditional on time $t$ information.
Lucas (1976) offered three examples that can be  fit into this framework. In those examples the decisions $d_t$ were either
  a choice of a consumption rate or  an   investment rate or
or a labor supply amount (i.e., Lucas's Phillips curve example), and $y_t$ was a stochastic process (e.g., a tax rate or inflation process), forecasts
of which influenced time $t$ decisions $d_t$.\NFootnote{A variety of other examples  that we'll
encounter later in this book also fit.}
...
Lucas (1976) described and criticized a collection of  studies that had used the following faulty logic.

\medskip

\item{a.}  A researcher does not understand or care that    $S$ satisfies restriction \Ep{lucas101;d} but just wants to know  the decision rule for $d_t$ .
...
\item{c.}  Mistakenly as it will turn out, the researcher  assumes that  the  decision rule $d_t = S x_t$ that he has estimated
 ...
 arbitrary hypothetical alterations  of the right-hand side variables $\{x_t\}$, including ones that  no longer would be  described by the historical law of motion
 \Ep{lucas101;a}-\Ep{lucas101;b}.    He believes that he can use that estimated  decision rule to predict how
systematic variations in $\{x_t\}$ will induce systematic variations in   $\{d_t\}$.

\medskip

\item{d.} Holding the historically estimated decision rule $S $ fixed, the researcher simulates outcomes $\{d_t\}_{t=T+1}^{\tilde T}$
under alternative rules for generating
 time paths  for
 $\{x_t\}_{t=T+1}^{\tilde T}$\NFootnote{For example, by using
 alternative state-dependent  ``policy rules'' to  generating them.} and then evaluates the
resulting $\{d_t, x_t\}_{t=T+1}^{\tilde T}$
...
of a state-space system to which he applies a linear-quadratic control theory to be described in chapter \use{dplinear} to  recommend a
...
\item{f.} When  decision makers recognize that $\tilde A, \tilde G$ and not $A, G$ will describe future $y_t$'s, they adjust the decision rule $S$  to become  $\tilde S = H \tilde G (I - \beta \tilde A)^{-1}$. By failing to understand that,
...
Lucas (1976) used this  line of reasoning to  characterize an influential  1940s-1960s econometric policy evaluation
...
Lucas said that the method is flawed because it  ignores the key cross-equation restrictions \Ep{lucas101;c}. Altering the policy-generating
matrices from the $G, A$ pair that prevailed historically to new pair  $\tilde G, \tilde A$ that describes a  hypothetical policy ``experiment'' that we want to study
 ...
 should be expected  systematically to alter how $y_t$ depends on $x_t$, as
 described by
...
$ \Sigma_{xx}, \Sigma_{dx}$ and therefore the population regression matrix of $d_t$ on $x_t$ that will characterize outcomes
under the new  regime. Thus,  structure \Ep{lucas101}, and in particular the cross-equation restriction \Ep{lucas101;c},
...
A concise way to say this is that the joint probability distribution of the $\{d_t, x_t\}$ process and the  probability distribution
...
that any change in policies will systematically affect the structure of econometric models.}{``Econometric Policy Evaluation: A Critique,'' 1976}
 ...
 section \use{sec:LQmodel} when we found that the LQ permanent income model of consumption smoothing implies an optimal decision rule
  ...
  As an essential part of a better theory of econometric policy evaluation,
   Lucas (1976) called for the construction of a rational expectations econometrics that incorporates  the kinds of cross-equation
restrictions  that system \Ep{lucas101} encodes.\NFootnote{See  Sargent (1981) and the introductory essay in Lucas and Sargent (1981), a volume containing
 early contributions to  rational expectations econometrics.  See and Hansen and Sargent (1991a) for
further
...
\index{distribution!stationary}%
 is given by the formula\NFootnote{To verify  formula \Ep{blackspectrum}, first note that for $ j \geq 0$, $E x_t x_{t+j}' = \Sigma_\infty A^{j \prime}$
 and that
...
Formula \Ep{blackspectrum} is valid if and only if premultiplying the expression on the right side of the last line by
$(I - A^e {-i \om})$ and postmultiplying it by $(I - A ' e^{i \om}) $ produces an object that equals $C C'$.  This equality follows from
...
This appendix describes the concept  of ergodicity in the context of Markov chains and
uses it to state  laws of large numbers.
...
\medskip
\noindent {\bf Note:} Exercise 8.24 applies a very similar analysis to the one used to solve part {\bf b} of  exercise \the\chapternum.2.
...
\noindent{\it Exercise \the\chapternum.29}\quad  {\bf Forecasting a discounted sum}
\medskip
...
\noindent{\it Exercise \the\chapternum.31}\quad  {\bf Invariant functions}
\medskip
\noindent
...
\medskip
\noindent {\it Exercise \the\chapternum.32}\quad  {\bf Muth and Kalman again}
...
\medskip
\noindent {\it Exercise \the\chapternum.35}\quad  {\bf Howard and Kalman}
...
\medskip
\noindent {\it Exercise \the\chapternum.37}\quad  {\bf Linear least squares}
...
\medskip
\noindent {\it Exercise \the\chapternum.38}\quad  {\bf Covariance between state and measurement noises}
...
\medskip
\noindent {\it Exercise \the\chapternum.39}\quad  {\bf Regression}
