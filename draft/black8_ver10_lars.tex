...
\section{Additional topics}  %% to be revised
...
structure to be the same as section \use{sec:olg_stat} and modify only one feature.
...
\section{Dynamic inefficiency in a growth model}
...
$$\EQNalign{&k_{t+1} + c_t = A \, l_t^{1-\alpha} k_t^\alpha 
                           \;\;\equiv F(l_t,k_t),                     \EQN growth1 \cr
            &c_t \geq 0,\; k_{t+1} \geq 0,\; l_t\in [0,1],  \cr
            &\hbox{\rm given}\, k_0 > 0,    \cr} $$
where  $A >0$, $\alpha \in (0,1)$, and $\beta \in (0,1)$.
We we have added labor supply $l_t$ as a choice variable and an
input to the production function $F(l_t,k_t)$ in resource 
constraint \Ep{growth1}. The household is endowed with one unit
of time in each period that it  can  allocate to working, 
...
equation \Ep{growth1} is that the exponents on labor
and capital add up to one.  This  makes the 
technology have  constant returns to scale,
$\lambda F(l,k) = F(\lambda l, \lambda k)$ for any
$\lambda >0$. By picking $\lambda = 1/l$, we see that the 
capital-labor ratio is the determinant of average labor 
productivity, $F(l,k)/l=F(1,k/l)$, and, hence, the 
size of the economy {\it per se} 
does not matter. This explains why our assumption 
of the household's time endowment being equal to one 
directly yielded the  simplification of the
production function described in equation \Ep{growth1}, where
$k_t$ in expression $F(1,k_t)=A \, k_t^\alpha$ is both 
the capital stock and the capital-labor 
...
$$ 1 = \beta {u'(c_{t+1})\over u'(c_t)} F_2(1,k_{t+1}),  \EQN growth2$$
where $F_j(l,k)$ is the partial derivative with respect to
...
equation in a steady state simplifies to  
$$F_2(1,k_\infty) = {1 \over \beta}. \EQN growth3$$
...
$$F(l,k) = F_1(l,k)\,l+F_2(l,k)\,k= w l+ r K
         = (1-\alpha)F(l,k) + \alpha F(l,k),$$
where the second and third equality invokes equilibrium
...
equal to $1-\alpha$ for labor and $\alpha$ for capital.
For further details,
...
infinitely-lived agents in the preceding section, we imposed the
...
in applied work using overlapping generations models; larger preferences for 
 future utilities generate larger savings which help to 
match aggregate observations on high capital-output ratios.}
 Since
there is no disutility from working, the person chooses to set $l^t_t=1$.
...
$$ 1 = \beta \,{u'(c^t_{t+1})\over u'(c^t_t)}\,  r_{t+1} 
     = \beta\, {u'(c^t_{t+1})\over u'(c^t_t)}\, F_2(1,k_{t+1}), 
...
Under our assumption $u(c)=\ln(c)$, the saving function is
$ s( w_t,  r_{t+1}) = 
...
$t\geq 0$  equals to next period's capital stock $k_{t+1}$:
$$k_{t+1} =  s( w_t,  r_{t+1})
...
In appendix X, we generalize our model by 
...
to a steady state with capital $k^\star$ satisfying
$$ F_2(1,k^\star) < n-1 +\delta,
$$
...
\noindent
In our setting with $n=\delta=1$, the condition for 
inefficiency translates into 
$F_2(1, k^\star)< 1$, which implies that 
...
relative to the capital share ($\alpha$), i.e., when young 
agents receive relatively high labor earnings 
(see footnote \use{note:facshares} on factor
shares).
...
$$ c^t_t + c^{t-1}_t + k_{t+1} = F(1,k_t),      \EQN olg_res $$
which, at a stationary allocation with capital $k$, can be
rearranged to read 
%$$ c^\star_1 + c^\star_2   = F(1,k^\star) - k^\star 
%                           \equiv C(k^\star),   \EQN olg_SS $$
%where $c^\star_1$ and $c^\star_2$ are the consumption of a
$$ c_1 + c_2   = F(1,k) - k 
...
aggregate output $F(1,k)$ minus the stationary
...
accumulation.\NFootnote{Needless to say, if the initial 
capital endowment of the infinitely-lived household in 
subsection \use{sec:growth_infin} is $k_0=\bar k$, i.e., the 
Golden-Rule level of capital, the efficient allocation
would be to gradually and optimally reduce capital until 
reaching $k_\infty$ as
given by steady-state Euler equation \Ep{growth3}.
...
\caption{Output $F(1,k)$ and aggregate consumption $C(k)$
as functions of a stationary capital value $k$. The 
Golden-Rule level of capital is $\bar k$, i.e., 
...
the resource constraint in the transition period $T$
becomes
$$ c^T_T + c^{T-1}_T + \bar k = F(1,k^\star)
...
equilibrium with the following government policy. In the 
...
$R_T> 1$, there will be no demand for those bonds; 
but if sold in an equilibrium, a no-arbitrage 
argument ensures that not only bonds but also capital will 
...
substitute equation \Ep{firm_FOC;a} and
...
in the new steady as compared to the original steady state. Second, 
...
%$$
%which 
can be simplified and rerranged to read
$$
...
positive power. Thus, the strict inequality here is
confirmed to hold and, hence, also
...
fiat money $\bar M$ in period $T$ (that is handed over
to the current old agent as were the funds
raised from the first issue of one-period bonds in 
the earlier equilibrium with unbacked debt). Thus, there
...
to arrive at an isomorphic equilibrium with
...
gross rate of return is equal to one in
...
Next, the government is allowed to issue additional
money after period $T$ so that we can study   
the real bills doctrine. Specifically, it will be
shown that any expansions of 
the money supply
...
additional money is exchanged for capital. Let 
$k^g_{t+1}\in[0,\bar k]$ be the capital held by the
government between periods $t$ and $t+1$. Under the 
supposition of such an irrelevance of open market 
operations, the government budget constraints (and
...
assures that the sum of private sector and government 
