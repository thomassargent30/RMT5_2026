...
We interpret the Harrison-Kreps analysis as a general
equilibrium in an endowment economy with a perishable good.
Besides the risky asset in the form of a Lucas tree, we
assume that each agent receives a constant endowment of the
good in each period that is large enough for each type of
agents to be able to purchase the entire Lucas tree. 
...
he expects to have the option to sell the asset later to another investor who will value the asset more highly.
...
\subsection{Returns under the true probabilities}
...
We can then compute the true expected rate of return of
...
dividend in state $1$, a purchase of the asset in state $1$
to be sold when the state changes to $2$ trivially has 
a higher expected return $\Omega(1)$ the sooner state $2$
is expected to be realized, upon which the investment yields 
both a dividend $d(2)$ and a resale price $\overline p(2)$. 
...
in \Ep{HK_return} with respect to $P(2,2)$ can be 
simplified to read
$$
...
restriction on the price $\overline p(1)$. Namely,
the recursion in \Ep{HarrKrep2} makes the equilibrium
prices dependent on the dividend process. While the agents
have heterogeneous beliefs about the underlying Markov
process and no one may know the true probabilities, there 
is yet a consensus about dividends being received only 
in state $2$. Thus, the understood interruption of 
the dividend stream in state $1$ implies that both
...
and hence, a strictly positive derivative above 
follows.}
...
the agents of type $a$ ($b$) earn an expected rate
...
prices $\overline p(1)$ and $\overline p(2)$ are
the ones that would prevail if all agents knew the
true Markov transition matrix $P$. Hence, for
equilibrium prices to correctly reflect the 
dividend process and thereby yield expected gross 
rates of return on asset investments equal to 
$\beta^{-1}$, it suffices that the agents who are in
possession of the asset in a particular state to have 
correct beliefs about the persistence of that state, 
while it does not matter what those agents think 
about the transition probabilities from the other 
state when they choose not to purchase the asset.


\subsection{Welfare under the true probabilities}

Heterogeneous beliefs have no effect on aggregate 
consumption 
in our endowment economy, but they do affect the 
distribution across the two types of agents through 
the equilibrium prices of and the trades in the 
Lucas tree. 

Given a particular true Markov transition matrix $P$
and associated invariant distribution $\pi$, we
evaluate the welfare implications of agents' 
subjective beliefs as follows. Suppose that 
the two types of agents enter the economy randomly,
in accordance with the invariant distribution $\pi$, 
with each type owning half of the asset. If the initial
state in that first period is $1$ ($2$), agents of 
type $a$ ($b$) purchase the other half of the asset
from agents of type $b$ ($a$). And from thereon,
the asset changes hands as analyzed above. 
Excluding any dividend in the first period, the
true expected present values of 
...
$$
By summing over $\Gamma_a$ and $\Gamma_b$, we arrive at
the true unconditional expected present value of the asset,
$$
...
asset that agents of type $b$ are expected to receive, 
under the assumptions that agents enter the economy 
randomly, in accordance with the associated invariant
distribution $\pi$, and that each type of agents 
initially owns half of the asset. The dashed curve 
...
ownership of the asset and the observation above that 
...
advantage for one or the other type of agents. As 
...
purchases less than $\beta^{-1}$. So from that 
perspective it seems that purchasing assets is 
...
As always, the smaller $P(1,1)$ (the greater 
$P(2,2)$) is the more of an advantage to agents 
...
This happens because agents of type $a$ are
paying too much for their purchases of the 
asset and hence, large capital losses are
often befallen them when selling the
...
probability $P(1,1)$ equals $0.95$. Though, 
while agents of type $b$ receive an ever greater 
share as the true probabilities $P(1,1)$ and 
$P(2,2)$ approach $1$ and $0$, respectively, in 
the farmost left corner of Figure \Fg{HK_share_b},
it should be noted that the true unconditional
expected present value of the asset is also plummeting 
in that parameter region. Thus, the exploding 
relative spoils received by agents of type $b$ 
at the cost of agents of type $a$ are tempered
in absolute terms since the Lucas tree yields
less dividends in that parameter region.
...
the prohibition against short sales.  
...
to satisfy the natural debt limits for all conceivable
...
be that the state changes to $2$ already next period 
...
endowments are not subject to any uncertainty, the
...
tree falls below $\beta^{-1}$.\NFootnote{An example
of a continuous-time model inspired by the Harrison-Kreps
(1978) model is Scheinkman and Xiong (2003), who
analyze how a Tobin transaction tax can substantially
reduce ``speculative trading'' while having only a
limited impact on the size of the ``bubble'' or on
price volatility. Scheinkman (2014) discusses a
research agenda that interprets the Harrison-Kreps 
model as a model of a bubble -- a situation in which  an asset price  exceeds what every investor thinks
is merited by the asset's underlying dividend stream.
Also, compared to the homogeneous beliefs setting that leads to the pricing formula \Ep{HarrKrep1}, high 
trading volume occurs when the Harrison-Kreps pricing formula \Ep{HarrKrep3} prevails. Scheinkman takes this as a strength of the model because he observed high volume  during ``famous bubbles''.}
\auth{Scheinkman, Jose}%
\auth{Xiong, Wei}%

By way of contrast, wealth distribution  effects 
are center stage in the model of  heterogeneous beliefs 
described in appendix \use{app:compB} of chapter \use{recurge}. Here, not only heterogeneous beliefs but
also risk aversion cause trade in Arrow securities in a 
sequential-trading competitive equilibrium.
The Arrow securities are in zero net supply so 
any trade means that some agents must be going short in 
those assets. But because of the strict concavity of 
utility functions there will be no bang-bang solutions
to agents' asset holdings, in contrast to the 
Harrison-Kreps model with risk-neutral agents and hence
...
chapter \use{recurge}, the volumes will eventually
taper off and go to zero when the agents with the most
accurate beliefs will asymptotically consume the entire
aggregate endowment, i.e., the agents with less 
accurate beliefs will {\it de facto} have sold off 
the continuation streams of their endowments to the 
agents with the most accurate beliefs.
