...
complete markets with time 0 trading to better understand
possible peculiarities in an overlapping generations model 
such as multiplicity of equilibria and Pareto inefficient
equilibrium allocations, which could not happen in the 
chapter \use{recurge} general equilibrium model. This will 
also demonstrate that the existence of such peculiarities 
in an overlapping generations model does not hinge upon 
agents alive today not being able to trade with future
generations since we assume that everyone can 
trade with anyone else at time 0.\NFootnote{To be 
explained in section \use{sec:time0trading}, under
time $0$ trading, we will 
switch from a Walrasian equilibrium to an alternative 
...
$N$ is needed for the analysis of a competitive equilibrium
in which agents take equilibrium prices as given. 
For expositional simplicity
we will refer to a representative agent in each generation.
Thus, there is an infinity of (representative) agents,
...
$$
Furthermore, we will mostly focus on the case of 
stationary endowments when $y^i_i=y_y$ and 
$y^{i-1}_i=y_o$
for all $i\geq 0$. Here thee subscript $y$ denotes
young and $o$ denotes old. 


\section{Dynamic (in)efficiency}\label{sec:dynineff}%
Pareto inefficient equilibria in this
chapter arise from intertemporal inefficiencies.
Therefore, such equilibria in an overlapping
generations model are commonly refered to as being
...
all $i\geq 0$ and $y^i_t = 0$ otherwise, a
multiplicity of equilibria and dynamically inefficient 
equilibria will exist 
if and only if $y_o<y_y$. If $y_o \geq y_y$, 
there is a unique equilibrium that is also Pareto 
efficient, as in the chapter \use{recurge} general 
equilibrium model.
...
is left to perish because no agent desiring those 
goods can afford to buy them. Hence, the market
...
The peculiarity with excess supply of time $0$
goods cannot happen in the chapter \use{recurge}
general equilibrium model. Hence, it would not matter
if we compute a Walrasian equilibrium or use our
...
as we characterized in chapter \use{recurge}.
``Walras Law'' holds in that economy. Restated in
terms of our present setup, Walras Law would
say that if all time $0$ markets for goods dated 
$t\geq 1$ clear so must the remaining market $t=0$.
But as we will demonstrate, Walras' Law can be 
violated in an overlapping generations model.
So here it matters that we adopt the alternative
equilibrium concept with a clearinghouse which
enables us to analyze a richer set of possible 
outcomes rather than being stymied by the 
restrictiveness of a Walrasian equilibrium. 
In fact, inspired by those additional 
outcomes that emerge under the alternative 
equilibrium concept with a clearinghouse,
we will later study sequential trading equilibria 
in which fiat money and unbacked government debt 
might be valued in an overlapping 
generations model. Outcomes that could never
happen in the chapter \use{recurge} general 
equilibrium model.
...
price of one good to be delivered at time $t$ in terms
of the good at time $0$.}
...
terms of the good dated $i$.\NFootnote{If we had used 
our notation in chapter \use{recurge}, $\alpha_t$ would 
have been written as $q^t_{t+1}$.}
Later, in a sequential 
trading equilibrium, this will be the inverse of the 
one-period gross interest rate $R_i$ between 
periods $i$ and $i+1$.

Regarding the initial old agent $i=-1$, who is alive only
...
$q^0_0 c^{-1}_0 \leq q^0_0 y^{-1}_0$ and hence, the optimal
...
In fact, as explained above, the equilibrium
consumption of the initial old agent $i=-1$ will
be $c^{-1}_0 = y^{-1}_0 = \epsilon$ in all
equilibria, both stationary and nonstationary
equilibria.
...
a consumption profile $(c_y,c_o)$ would be 
...
excess supply of goods dated $0$,
...
desiring those goods can afford to buy them.
...
As maintained by Shell, ``the double infinity of traders and
...
is an indifference curve with a tangent at
that point which is a straight line that can
...
consume his endowment, any optimally
...
$c^{-1}_0 = y_o$. Regarding the remaining equilibrium 
...
$c^0_0 \in ((y_y+y_o)/2,y_y)$.\NFootnote{When setting 
$c^0_0$ equal to one of the two boundary points, 
$(y_y+y_o)/2$ or $y_y$, the resulting equilibria would
be the counterparts to the two stationary equilibria 
in section \use{sec:olg_stat}.}
...
Applying the algorithm in the preceding section, we start with
...
%asset.}
The candidate time $0$ equilibrium allocation is feasible in 
accordance with footnote \use{note:feasible0}, but  the
...
equilibrium. For all the other choices of $c^0_0$,
the economy will converge to the {\it low\/}-interest-rate
...
(.5, 1-\epsilon)$ when
using the algorithm in section \use{sec:olg_nonstat}. 
...
around next period.
Hence, the unique equilibrium price system is the one
that eradicates all incentives to trade,
...
for one difference. Any goods that the clearinghouse 
...
is critical for the described equivalence of 
