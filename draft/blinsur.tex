...
\subsection{Autarky}
We first study the fate of an unemployed worker
...
optimally.  It satisfies the Bellman equation %for $V^u$  is 
...
public information about an unemployed worker's search effort.  An insurance agency can observe and control
...
satisfy the Bellman equation \index{Bellman equation}%
...
at least $V$.   Let $\theta$ be a Lagrange multiplier
...
\subsection{An incentive problem}
...
relies on  the agency's ability to control {\it both\/} the unemployed
...
for exerting the proper search effort in earlier periods.\NFootnote{See page \use{weak_inequality} for discussion of a related issue that occurs
in a model of Thomas and Worrall (1990).}
...
\centerline{\epsfxsize=3truein\epsffile{hugoreplnew_gray.eps}}
...
supply effort while  working and to search while unemployed.  The planner uses history dependence
...
what Zhao interprets broadly as a ``replacement rate'' like ones we observe in some 
...
\subsection{A lifetime contract}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We can represent the
Bellman equation for $C(v,s)$ in terms of the following two-part
optimization:\index{Bellman equation}%
$$\EQNalign{ \Psi(v,s,a) &  = \min_{c(z), w(z,s')}
  \Biggl\{ \sum_z \pi_z(z;s,a) \Bigl(-z + c(z)
    \cr &
...
which implies that
  ${1 \over u'(c_e(t))} > C_v(v_u(t+1),u) = {1\over u'(c_u(t+1))}. $ \qed
...
Formulate the Bellman equation for $C(V)$. \index{Bellman equation}%
\medskip
%\noindent {\bf Answer:}
