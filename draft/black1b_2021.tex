...
identity contemporaneous covariance matrix, all  eigenvalues of $\check A$ are strictly less than ${\frac{1}{\sqrt{\beta}}}$ in magnitude,
 and
$\check G$ is a  vector used to  determine $y$ as a 
...
We now stare at equation \Ep{stareatit101} and  consider  assumptions about what is predetermined at $t$ and
...
implied by decision rule \Ep{stareatit101}, an assertion that we'll soon prove.
 ...
 To construct a financial plan that supports constant consumption, we let the consumer trade one-period state-contingent securities. (These are examples of the  
  Arrow securities  to be studied in chapter \use{recurge}.)  Thus,  temporarily we take $\bar c$ as given, substitute \Ep{guess101} into
...
In our reverse engineering exercise, we suppose that equation \Ep{guess102} describes the consumer's optimal portfolio
of one-period state-contingent securities issued at $t-1$ that pay  off at time $t$ in Markov state $z_t$.
...
Note that model \Ep{guess102}-\Ep{guess103} has extinguished the history dependence of both debt and consumption exhibited by 
...
Markov states. To understand   these outcomes,  we begin by describing how  we have backed ourselves into  a model in which each period the consumer  trades a complete
list of one-period state-contingent securities that  pay off amounts next period that depend on next period's information vector $z$.
  To make this work with a consumption rate that is constant across time and Markov states $z$, we  reverse engineer prices of those securities
...
where $q(z_{t+1} | z_t)$ is a  kernel for pricing bundles of  one-period  state contingent claims and a function  $b_t(z_{t+1})$ describes    a bundle of state-contingent debts due at time $t+1$ 
 that are chosen at time $t$.   The function  $b_t(z_{t+1}): \bbR \rightarrow \bbR$ maps $z_{t} \in 
 \bbR$ into a $z_{t+1}$-contingent payment at time $t+1$.  In state $t$  at time $t$, the price  of a
 claim  on consumption
 at time $t+1$ when realization $z_{t+1} \in A \subset \bbR$ is $\int_A  q(z_{t+1} | z_t ) b_t(z_{t+1}) d z_{t+1}$.
We arbitraily set the pricing kernel to
$$ q(z_{t+1} | z_t ) = \beta \phi(z_{t+1} | z_t) , \EQN kernel101 $$
where $\phi(z_{t+1} | z_t)$ is the probability density of $z_{t+1}$ conditional on $z_t$. We regard it as somehow determined outside the model, just 
as earlier we took a one-period gross risk-free interest rate $R = \beta^{-1}$ to have been determined outside the consumption-smoothing model we have studied above.  Kernel \Ep{kernel101} discounts state-contingent claims on consumption at date $t+1$ by the product of the time  discount factor $\beta$ and  conditional probabilities of  the  events
at which  claims pay off. 
Sometimes the kernel $q(z_{t+1} | z_t)$ is called  a state-price density,
and the function $b_t(z_{t+1})$ is said to describe a bundle of one-period ahead Arrow securities.  % Where $\Phi_t(\,\cdot\,)$ is the standardized normal cumulative distribution function, 
An Arrow security issued at time $t$, Markov state $z_t$  that  pays off $1$ unit of consumption at time $t+1$ whenever the realized time $t$ state $z_{t+1}$ lies in interval $(z_{t+1}, z_{t+1}+d z_{t+1})$ is priced  at time $t$ by $d q(z_{t+1} | z_t)$. 
...
%$z_{t+1}$-contingent payoffs %The price at time $t$ of  bundle $ \Pi$ %(\varepsilon_{t+1})$ of  
 ...
 For example, consider a claim whose payoff at time $t+1$  is $1$  for all $z_{t+1}$, %at $t+1$ 
 so that $\Pi(z_{t+1})=1$.
Pricing kernel \Ep{kernel101}
implies that the price of  this  one-period risk-free bond is
$$ \int q(z_{t+1} | z_t) d z_{t+1} = \beta \int \phi(z_{t+1} | z_t) d z_{t+1} = \beta,  \EQN noarbitrageLQ101 $$
so  the gross interest rate on one-period risk free loans is  $\beta^{-1}$.\NFootnote{It would be redundant to add a risk-free one-period bond to the state-contingent one period securities that the consumer can trade in  our  complete markets model.  If that redundant asset were added, a no-arbitrage profits argument like  one that we shall apply  repeatedly in chapters \use{recurge}, \use{assetpricing1}, and \use{assetpricing2} would imply formula \Ep{ noarbitrageLQ101}.}
...
\appendix{A}{Infinite-order VAR generated by Markov switching}\label{appendixSimsZha}
