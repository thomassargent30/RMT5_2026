...
\item{1.} Describe the preferences, technology, and endowments of a dynamic
...
\item{2.} Set up a competitive market in some particular asset that
...
\item{3.} Equate the consumption that appears in the Euler equation
...
\subsection{Example 2: finite-state version}
...
After consolidating all future budget constraints, we
 find that
$$\EQNalign{
&c_t(s^t)+\tau_t(s^t) +
\sum_{j=1}^\infty
...
expression \Ep{exptheory} for such an economy would therefore be one in which the
covariance term is zero.

We now compute the expected value of future
...
Roberds (1996) that use safe interest rates as discount factors
...
\appendix{A}{Harrison-Kreps heterogeneous beliefs\label{app:compHarrKrep}}

This appendix sketches a model of Harrison and Kreps (1978) that features risk-neutral agents, heterogeneous
beliefs, incomplete markets, and short sales constraints.
%, and possibly (leverage) limits on an investor's ability
%to borrow  in order to finance purchases of a risky asset.
%The  model   simplifies by  ignoring  alterations in the 
%distribution of wealth among investors having different 
%beliefs about  fundamentals driving
%asset payouts.\NFootnote{Such wealth distribution  effects 
%are center stage in the models of  heterogeneous beliefs 
%described in  appendices  to
%chapter \use{recurge}.}
\auth{Kreps, David M.}%
\auth{Harrison, J. Michael}%
There is a single risky asset in fixed supply that we
set to unity.  Shares in the asset,  a ``Lucas tree'', are  perfectly divisible. 
%fixed number $A$ of shares of an asset.  Each share 
The asset emits a stream of dividends $\{d_t\}$  governed by a  Markov chain defined
on a state space $S \in \{1, 2\}$.
The dividend obeys
$$ d_t = d(s_t) = \cases{0 & if $s_t = 1$; \cr
                       1 & if $s_t =2$.\cr}  $$
Two types $h=a, b$ of agent, 
in equal and large numbers,
are distinguished only by their beliefs about a Markov transition matrix $P$ with typical element $P(i,j) = {\rm Prob}(s_{t+1} = j| s_t = i)$.
Agents of type $a$ believe  that the transition matrix is 
...
right to sell the asset at time $t+1$.  Both  types of
risk-neutral agents have the same fixed discount
factor $\beta \in (0,1)$.  In our numerical example, we'll set $\beta = .75$, just as Harrison and Kreps did.

We shall interpret the Harrison-Kreps model as a general
equilibrium for an endowment economy with a perishable good.
Besides trading the  risky asset just described,
each agent receives a constant endowment of the
good in each period that is large enough for each type of
agent to be able to purchase the entire Lucas tree. 
Short sales of the Lucas tree are prohibited. 



% We'll eventually  study the consequences of   two different 
%assumptions about the number of shares $A$ relative to the 
%resources that
%our two types of investors can invest in the stock. One 
%possibility  is to  follow Harrison and Kreps and to assume  
%that both types of investor  have access to enough  
%resources
%(either wealth or the capacity to borrow) to  purchase the 
%entire stock of the asset.  The alternative  assumption is 
%that no single type of investor
%has sufficient resources to purchase the entire stock of the 
%asset, so  both types of investor  always hold some of the 
%asset.
%  Short sales of the asset are prohibited.\NFootnote{With 
%the specified  preferences, if there were no constraint on 
%short sales, there would exist no equilibrium.
%Pessimistic agents would be willing to sell unbounded 
%amounts of the asset at the equilibrium prices  computed in 
%\Ep{HarrKrep2}.}
...
spells of temporary optimism and
...
All agents know the same pricing   function that maps the state $s_t$  into the equilibrium price $p(s_t)$.
...
%\medskip
%
%\item{3.} There are two types of agent with different 
%beliefs, but because of limited wealth  and/or limited 
%leverage, both types of agent  hold the asset each period.

\medskip

\subsection{Single belief prices}
We'll start by pricing the asset under homogeneous beliefs.
Suppose that there is only one type of agent, either of type $a$ or $b$.
%, and that this investor  always ``prices the asset''.  
Let $p_h = \bmatrix{ p_h(1) \cr p_h(2) }$ be the equilibrium
price vector when all agents are of type $h$.  The price today equals the expected discounted value of tomorrow's dividend plus tomorrow's price of the asset:
...
\caption{Row 1: equilibrium price function $p_a$  under homogeneous beliefs $P_a$. Row 2: equilibrium price function $p_b$ under homogeneous beliefs $P_b$. Row 3: equilibrium price function $\overline p$
under heterogeneous beliefs.
% with optimistic marginal investors. 
Row 4: type $a$ agents are  willing to pay $\hat p_a$  for asset. Row 5: type $b$ agents are willing to pay $\hat p_b$ for the asset.}
%Row 6: equilibrium price function under
...
%There are several possibilities.  The first is when 
Recall that both  types of agent have sufficient wealth to purchase all of the asset themselves. 
%In this case 
Therefore, the marginal investor who prices the asset is the more optimistic type, so that the equilibrium
...
for the price vector $ \overline p$ and iterating to convergence on an operator that maps a guess $\overline p^j$ into a guess $\overline p^{j+1}$ defined by
...
asset in state $s_t$.  It is instructive to compare these prices with
 equilibrium prices for the homogeneous belief economies that solve \Ep{HarrKrep1} under beliefs  $P_a$ and $P_b$.
...
of the asset in each possible state.   Each buyer of  the asset pays more than he believes
its future dividends are worth because
he has an option  to sell the asset  to another investor who will value the asset more highly.

Agents of type $a$ are willing to pay 
...
Valuations $\hat p_a(s)$ and $\hat p_b(s)$ are displayed in the fourth and fifth rows of \Tbl{HKtab}. Temporarily more pessimistic investors who don't buy the asset now also think that it is worth
more than they think future dividends are worth.


\subsection{Returns under  ``true'' probabilities}\label{sec:returnsundertrue}%

\noindent
In the spirit of the Blume and Easley model of appendix \use{app:compB} of chapter \use{recurge}, we'll now add a  third Markov process that we'll
interpret as ``true'' in the sense that it actually governs the data, unbeknownst to the agents inside the model. We'll use this true distribution to  evaluate outcomes. 

Let $P$ denote the true Markov transition matrix 
%%with $P(s,s)<1$ for $s=1,2$, and 
with an associated invariant
distribution $\pi = \bmatrix{ \pi(1) & \pi(2) }$.
Under the hypothesized true distribution, we can then compute the  expected rate of return from
purchasing the asset in state $s$ and selling it when the
state changes to $s' \neq s$,
$$\EQNalign{
\Omega(s) &= {\sum_{t=0}^{\infty} \beta^{t}P(s,s)^{t}
      \left(P(s,s)d(s) + \left[1-P(s,s)\right] 
            \left[d(s')+\overline p(s')\right]\right) 
      \over \overline p(s) }                             \cr
          &= {P(s,s)d(s) + \left[1-P(s,s)\right] 
            \left[d(s')+\overline p(s')\right] 
      \over \left[1-\beta P(s,s)\right] \overline p(s) },
                    \quad s = 1,2.       \EQN HK_return  \cr}
$$
For  the dividends $d(1)=0$ and $d(2)=1$ and the
equilibrium prices $\overline p(1)$ and $\overline p(2)$ 
in \Tbl{HKtab}, $\Omega(1)$ ($\Omega(2)$) is decreasing
(increasing) in $P(1,1)$ ($P(2,2)$). Because there is no
dividend in state $1$, a purchase of the asset in state $1$ that is 
to be sold when the state changes to $2$  has 
a higher expected return $\Omega(1)$ the sooner state $2$
can be expected to be realized, when owning the asset yields 
both a dividend $d(2)$ and  resale price $\overline p(2)$. 
In contrast, continuing to  collect   dividend $d(2)=1$
in state $2$ is more valuable than the receipt of the
resale price $\overline p(1)$ when state $1$ is
realized. Consequently, the expected return $\Omega(2)$ 
is increasing in the expected persistence of
state $2$.\NFootnote{The derivative of $\Omega(2)$ 
with respect to $P(2,2)$ in \Ep{HK_return}  can be 
simplified to 
$$
{\partial \,\Omega(2) \hfill \over \partial P(2,2)} =
{1 - \overline p(1) (1-\beta) \over
[1-\beta P(2,2)]^2 \overline p(2) } > 0,
$$
where the inequality follows from an equilibrium
restriction on the price $\overline p(1)$, in particular, from how
the recursion in \Ep{HarrKrep2} makes the equilibrium
prices dependent on the dividend process. While  agents
have heterogeneous beliefs about the underlying Markov
process and no one may know the true probabilities, agents agree that  dividends
arrive only 
in state $2$. Thus, agreement that no dividends arrive in 
 state $1$ implies that both
prices are bounded from above by the present value of
an hypothetical uninterrupted flow of dividend 
$d(2)=1$ forever: 
$\overline p(s) < \sum_{t=1}^{\infty} \beta^t d(2)
                = \beta / (1-\beta)$, for $s=1,2$.
It follows that ${\partial \,\Omega(2) \hfill \over \partial P(2,2)}$ is strictly
positive.}
%is strictly positive,
%$\partial \Omega(2)/\partial P(2,2) >0$.}
The two panels in Figure \Fg{HK_rr} depict
$\Omega(1)-1$ and $\Omega(2)-1$ for different values
of  true probabilities $P(1,1)$ and $P(2,2)$. The left (right) panel in
Figure \Fg{HK_rr} shows the true expected net 
return on assets purchased by agents of type
$a$ ($b$) in state $1$ ($2$). When  true
probability $P(1,1)$ ($P(2,2)$) coincides with
subjective probability $P_a(1,1)$ ($P_b(2,2)$), 
 agents of type $a$ ($b$) earn an expected net rate
of return equal to $\beta^{-1}-1=1/3$ on 
purchases of the asset in state $1$ ($2$).

\midfigure{HK_rr}
\centerline{\epsfxsize=2.0truein\epsffile{HK_rr_a.eps}, \epsfxsize=2.0truein\epsffile{HK_rr_b.eps}}
\caption{True expected net rates of return on
asset investments by agents of type $a$ in the
left panel and type $b$ in the right panel. The 
returns are functions of the persistence of the 
state in which investments are made, i.e.,
the true probabilities $P(1,1)$ and $P(2,2)$, 
respectively.}
\infiglist{HK_rr}
\endfigure
\medskip
...
When both $P(1,1)=P_a(1,1)$ and $P(2,2)=P_b(2,2)$,
an implication of the recursion in 
\Ep{HarrKrep2} is that the resulting equilibrium
prices $\overline p(1)$ and $\overline p(2)$ equal those
 that would prevail if all agents knew the
true Markov transition matrix $P$. Hence, for
equilibrium prices   to price  the 
dividend process correctly and thereby yield expected gross 
rates of return on asset investments equal to 
$\beta^{-1}$, it suffices that  agents who own the asset in a given state  have 
correct beliefs about the persistence of that state; 
it does not matter what those agents think 
about  transition probability from the  
state in which they do not own the asset.


\subsection{Welfare under  true probabilities}
Heterogeneous beliefs cannot affect  aggregate 
consumption in our endowment economy,  but by affecting  asset trades and the  equilibrium price of  the 
Lucas tree, they do affect the 
distribution of consumption across the two types of agent. 

For a true Markov transition matrix $P$
and the associated invariant distribution $\pi$, we
evaluate the welfare implications of agents' 
subjective beliefs as follows. 
We suppose that each  type of agent enters the economy owning one half of the Lucas tree and arrives at a moment when 
the state has been randomly drawn from the invariant distribution $\pi$.   If the initial
state in that first period is $1$ ($2$), agents of 
type $a$ ($b$) purchase the other half of the asset
from agents of type $b$ ($a$). After that,
the asset changes hands as described above. 
Excluding any dividends that had arrived  in the first period, under the true distribution the
 expected present values of 
all payoffs derived from those asset transactions
for agents of type $a$ and $b$, respectively, are:
$$\EQNalign{
\Gamma_a &= 0.5 \left(-\pi(1) \overline p(1) 
            +\pi(2) \overline p(2) \right) \cr
         &\;\;
            +\beta {\pi(1) P(1,2) [1+\overline p(2)]
                   -\pi(2) P(2,1) \overline p(1)
             \over 1-\beta}\,,         \EQN HKshare;a   \cr
\Gamma_b &= 0.5 \left(\pi(1) \overline p(1) 
            -\pi(2) \overline p(2) \right)  \cr
          &\;\;
            +\beta {-\pi(1) P(1,2) \overline p(2)
                    +\pi(2)[ P(2,1) \overline p(1)
                            +P(2,2) \cdot 1 ]
  \over 1-\beta}\,. \hskip1cm \hbox{ } \EQN HKshare;b   \cr}
$$
By summing over $\Gamma_a$ and $\Gamma_b$, we deduce that under the true distribution 
the unconditional expected present value of the asset is 
$$
\Gamma = \Gamma_a+\Gamma_b 
       = \beta 
         {\pi(1) P(1,2) \cdot 1 +\pi(2) P(2,2) \cdot 1 \over
          1-\beta }.                      \EQN HK_fundvalue
$$
Figure \Fg{HK_share_b} depicts $\Gamma_b/\Gamma$ for
different values of the true probabilities $P(1,1)$ 
and $P(2,2)$. Thus, for each pair $(P(1,1),P(2,2))$, 
the figure shows the share of the true unconditional 
expected present value of the 
asset that agents of type $b$ can be expected to receive
under our  assumption that each type of agent  
initially owns half of the asset and that  the state that prevails when an  agent first enters the economy 
is drawn from invariant 
distribution $\pi$. The dashed curve 
in Figure \Fg{HK_share_b} is the isoquant for a share 
equal to $0.5$, i.e., each type of agents is expected 
to receive one half of the true unconditional 
expected present value of the asset. Naturally,
the isoquant goes through the point where
the true probabilities satisfy $P(1,1)=P_a(1,1)$ and
$P(2,2)=P_b(2,2)$. This outcome follows from the two
types of agents entering the economy with equal 
ownership of the asset and the observation in subsection \use{sec:returnsundertrue} that 
the asset is priced correctly whenever 
the true probabilities satisfy $P(1,1)=P_a(1,1)$ and
$P(2,2)=P_b(2,2)$.
%, i.e., as if all agents 
%knew the true Markov transition matrix $P$. 


\midfigure{HK_share_b}
\centerline{\epsfxsize=3truein\epsffile{HK_PVshare_b.eps}}
\caption{Share ${\frac{\Gamma_b}{\Gamma}}$ of the true unconditional expected 
present value of the asset that agents of type $b$ 
are expected to receive, as a function of  true transition 
probabilities $P(1,1)$ and $P(2,2)$. The dashed curve 
is the isoquant for a share equal to $0.5$, and the 
solid lines demarcate four quadrants with  origin 
at the subjective beliefs $P_a(1,1)=0.5$ and 
$P_b(2,2)=0.75$.}
\infiglist{HK_share_b}
\endfigure

Solid lines in Figure \Fg{HK_share_b} demarcate four
quadrants with  origin at ($P_a(1,1)$, $P_b(2,2)$).
In the first quadrant, $P(1,1)<P_a(1,1)$ and 
$P(2,2)>P_b(2,2)$, i.e., both types of agents earn true
expected gross rates of return on asset purchases 
greater than $\beta^{-1}$. How the true unconditional
expected present value of the asset is split between
the two types of agents depends on whether  distorted equilibrium prices create an
advantage for one or the other type of agent. As could  
 be expected, the smaller is $P(1,1)$ (the greater is  
$P(2,2)$) the bigger is the  advantage that goes to agents of 
type $a$ ($b$).

In the second quadrant, $P(1,1)>P_a(1,1)$ and 
$P(2,2)>P_b(2,2)$, i.e., while agents of type $b$
continue to earn a true expected gross rate of 
return on asset purchases greater than $\beta^{-1}$,
agents of type $a$ now earn a subpar rate of 
return. So not surprisingly, agents of type $b$
receive an expected share greater than $0.5$ of 
the true unconditional expected present value of 
the asset throughout the second quadrant.


In the third quadrant, $P(1,1)>P_a(1,1)$ and 
$P(2,2)<P_b(2,2)$, i.e., both types of agents
earn true expected gross rates of return on asset 
purchases less than $\beta^{-1}$. 
Hence, under the true
distribution,  a single individual could attain a
higher expected lifetime utility by selling his  holdings of the asset
in the very first period and never trading  the asset again.
(This opportunity presumes  that behavior by  such a `deviating'
agent would  not subvert  existence of a
competitive equilibrium in which, for each type of agent,
the population that continues
to trade the asset
can afford to purchase the entire Lucas tree.)
%Of course, in an equilibrium 
%there will be no such deviations when agents
%maximize their expected lifetime utility
%under their respective subjective beliefs.
The division of the true unconditional expected 
present value of the asset between the two types
of agents is determined by how their subjective
probabilities distort equilibrium prices.
As always, the smaller is $P(1,1)$ (the greater is
$P(2,2)$) the more the advantage  tilts to agents 
of type $a$ ($b$). As evident 
in the third quadrant, the advantage to agents 
of type $b$ can be so large that they receive 
more than $100\%$ of the true unconditional 
expected present value of the asset.
This happens because agents of type $a$ 
pay too much for  the 
asset and hence, large capital losses 
often befall them when they sell the
asset, as reflected in the left panel of
Figure \Fg{HK_rr} where the true expected net 
rate of return on asset investments by agents 
of type $a$ drops below $-70\%$ when the true 
probability $P(1,1)$ equals $0.95$. However, notice that 
while agents of type $b$ receive  ever greater 
shares as the true probabilities $P(1,1)$ and 
$P(2,2)$ approach $1$ and $0$, respectively,  
in the parameter region in 
the farmost left corner of Figure \Fg{HK_share_b},
 the true unconditional
expected present value of the asset  also plummets. Thus, because
 the Lucas tree yields
smaller dividends in that parameter region, the exploding 
relative spoils received by agents of type $b$ 
at the expense of agents of type $a$ are reduced 
in absolute terms .

In the fourth quadrant, $P(1,1)<P_a(1,1)$ and 
$P(2,2)<P_b(2,2)$, i.e., agents of type $a$ earn 
a gross rate of return on asset purchases greater 
than $\beta^{-1}$ while agents of type $b$  rate of return less than $\beta^{-1}$. This results in agents of 
type $b$ receiving a share of the true unconditional 
expected present value of the asset less than one 
half. In the lowest corner of Figure \Fg{HK_share_b},
the share becomes negative and falls below 
$-20\%$ at $(P(1,1),P(2,2))=(0.05,0.05)$, with an  
associated true expected net rate of return on asset 
investments by agents of type $b$ equal to $-10\%$ in 
the right panel of Figure \Fg{HK_rr} (whereas
agents of type $a$ earn $65\%$ on their
investments in the left panel of Figure \Fg{HK_rr}).


\subsection{Reflections on the Harrison-Kreps model}

Please notice two 
key differences in the assumptions of the Harrison-Kreps model presented here  and the Blume and Easley model of appendix \use{app:compB} of chapter \use{recurge}. 
The chapter \use{recurge} model assumes risk averse agents 
and complete markets, while
%%; it focuses on the dynamics of continuation wealth in a 
%%competitive equilibrium.  There is zero volume in the 
%%sense that no  trades occur after date $0$.
%%By way of contrast, 
the  Harrison-Kreps model here  assumes 
risk-neutral agents, incomplete markets, and no  short sales.  
%%By assuming that  both types
%%of agent always have "deep enough pockets" to purchase all 
%%of the asset, the model  takes wealth dynamics off the 
%%table.   The Harrison-Kreps model
%%generates  high trading volume when the state changes 
%%either from 1 to 2 or from 2 to 1.
If short sales were allowed in the Harrison-Kreps model,
agents of type $a$ ($b$) would like to go short in the 
asset in state $2$ ($1$) because  their
subjective  expectations of the  gross rate 
of return on the asset is less than $\beta^{-1}$ in that state. An
agent who goes short  creates shares in 
a `synthetic' Lucas tree that mimics  payoffs of 
the actual  Lucas tree. Because an agent who sell short borrows, that  agent has 
to satisfy  natural debt limits for all conceivable
realizations of that underlying asset. 
For example, if the current state is $1$, there
would be no `synthetic' dividends to be paid out by an 
agent of type $a$ who has gone short in the asset. 
And the worst that could happen to that agent would 
be that the state  changes to $2$  next period immediately
after the short sale. That is, after having
received the price $\bar p(1)$ for the synthetic
asset next period, the agent would be obligated  to pay the higher price $\bar p(2)$ 
to the holder of that claim. Since we assume that
agents have a constant endowment of
the good in each period, the present value of that
stream would be the natural debt limit and consequently  
the upper bound for that liability. Given that those 
endowments are not subject to any uncertainty, 
risk-neutral agents would be willing to exchange 
shares of ownership to those endowment streams at 
an intertemporal price of the gross interest rate 
$\beta^{-1}$, thereby implementing risk-free
borrowing and lending with individual endowment 
streams as collateral.

Allowing for 
short sales thus  puts  dynamics of the wealth 
distribution on the table. For example, if 
transactions and  realizations of states could lead 
to situations in which one type of agents can no longer 
afford to purchase the entire Lucas tree, it 
means that prices $\overline p$ in \Tbl{HKtab} cannot
be the equilibrium prices. Instead, we would have to 
solve for equilibrium prices as functions of both the 
state and the wealth distribution. To avoid 
such complications,  papers  that build on the
Harrison-Kreps model  typically assume that agents
have infinite wealth. Under that assumption,   there 
 exist no equilibrium when  short sales are permitted. Pessimistic agents 
would be willing to sell unbounded amounts of the 
synthetic asset whenever their subjectively expected 
gross rate of return on the underlying Lucas 
tree falls below $\beta^{-1}$. So short sales cannot be allowed.\NFootnote{Scheinkman and Xiong (2003) use a continuous-time model inspired by the Harrison-Kreps
(1978) model to 
analyze how a Tobin transaction tax  substantially
reduces ``speculative trading'' but nevertheless  brings only small reductions in 
price volatility and  ``bubbles''. Scheinkman (2014) discusses a
research agenda that interprets the Harrison-Kreps 
model as a model of a bubble defined as a situation in which  an asset price  exceeds what every investor thinks
is merited by the asset's prospective dividends.
Notice how compared to the homogeneous beliefs setting that leads to  pricing formula \Ep{HarrKrep1}, high 
trading volume occurs when the Harrison-Kreps pricing formula \Ep{HarrKrep3} prevails. Scheinkman uses  this outcome  to explain   
high trading volumes observed   during ``famous bubbles''.}
\auth{Scheinkman, Jose}%
\auth{Xiong, Wei}%



By way of contrast, wealth distribution  effects 
are center stage in the model of  heterogeneous beliefs 
described in appendix \use{app:compB} of chapter \use{recurge}, a model in which    risk aversion and  heterogeneous beliefs 
motivate trade in Arrow securities in a 
sequential-trading competitive equilibrium.
Arrow securities are in zero net supply, so 
 trade  means that some agents must be selling them short. 
 The strict concavity of 
utility functions rules out  bang-bang solutions
to agents' portfolio problems, in contrast to the 
Harrison-Kreps model with its  risk-neutral agents. Furthermore, while trading 
volumes in Arrow securities can initially be substantial 
in the economy in appendix \use{app:compB} of 
chapter \use{recurge},  volumes 
approach zero when agents who have  the most
accurate beliefs eventually  consume the entire
aggregate endowment; agents with less 
accurate beliefs will  have already sold their  continuation endowment streams to finance their earlier consumption 
and asset trades.
...
%Outcomes differ when the more optimistic type of investor 
%has insufficient wealth -- or  insufficient ability to 
%borrow enough  -- to
%hold the entire stock of the asset. In this case, the asset 
%price must adjust to attract  pessimistic investors.  
%Instead of equation \Ep{HarrKrep2}, the equilibrium price 
%satisfies
%$$ \EQNalign{ \check p(s) &  = \beta \min\Bigl\{  P_a(s,1)  
%\check  p(1) + P_a(s,2) ( 1 +   \check  p(2)) , \cr
%                   &    \quad \quad \quad \quad \quad         
%P_b(s,1)  \check p(1) + P_b(s,2) ( 1 + \check p(2))  \Bigr
%\} , \EQN HarrKrep4  \cr }
%                                                        $$
%and the marginal investor who prices the asset is always the 
%type that values it {\it less\/} highly than does the other 
%type. Now the marginal investor is always
%the (temporarily) pessimistic type.   Notice from the sixth 
%row of \Tbl{HKtab} that the pessimistic price $\underline p$ 
%is lower than the homogeneous belief prices $p_a$  and $p_b$ 
%in both states.


%When  pessimistic investors price the asset according to 
%\Ep{HarrKrep4},  optimistic agents think that the asset is 
%underpriced.  If they could, optimistic agents
%would willingly borrow at the one-period
%gross interest rate $\beta^{-1}$ to  purchase more of the 
%asset. Constraints on leverage prohibit them from doing so.

%When  optimistic investors price the asset as in 
%\Ep{HarrKrep3}, pessimistic agents think that the asset is 
%overpriced and would like to sell the asset short.  
%Constraints
%on short sales prevent that.

%Scheinkman (2014) interprets  the  Harrison-Kreps model as a 
%model of a bubble -- a situation in which  an asset price  
%exceeds what every investor thinks
%is merited by the asset's underlying dividend stream. 
%Scheinkman stresses these features of the Harrison-Kreps 
%model:

%\medskip

%\item {1.}  Compared to the homogeneous beliefs setting that 
%leads to the pricing formula \Ep{HarrKrep1}, high volume 
%occurs when the Harrison-Kreps pricing formula 
%\Ep{HarrKrep3}
%prevails.
%Type $a$ agents sell the entire stock of the asset to type 
%$b$ agents every time the state switches from $s_t =1$ to 
%$s_t =2$.  Type $b$ agents sell the asset to
%type $a$ agents every time the state switches from $s_t = 2$ 
%to $s_t =1$.  Scheinkman takes this as a strength of the 
%model
%because he observed high volume  during ``famous bubbles''.

%\medskip

%\item {2.}  If the {\it supply\/} of the asset can be 
%increased sufficiently either physically (more ``houses'' 
%are built) or artificially (ways are invented to short sell 
%claims on houses), bubbles  end
%when the supply of the asset has grown enough to outstrip  
%optimistic investors' resources for purchasing the asset.

%\medskip
%\item {3.} If optimistic investors  finance  purchases by 
%borrowing, tightening leverage constraints can extinguish a 
%bubble.

%\medskip
%Scheinkman extracts insights about effects of   financial 
%regulations on bubbles. He emphasizes how limiting short 
%sales and limiting
...
%Please notice
%key differences in the assumptions of the Harrison-Kreps 
%model presented in this appendix and the Blume and Easley 
%model of appendix \use{app:compB} of chapter \use{recurge}.
%The chapter \use{recurge} model assumes complete markets and  
%risk averse consumers; it focuses on the dynamics of 
%continuation wealth in a competitive equilibrium.  There is
%zero volume in the sense that no  trades occur after date 
%$0$.
%By way of contrast, the  Harrison-Kreps model of this 
%appendix assumes  incomplete markets,  risk-neutral 
%consumers, and restrictions on short sales.  By assuming 
%that  both types
%of agent always have "deep enough pockets" to purchase all 
%of the asset, the model  takes wealth dynamics off the 
%table.   The Harrison-Kreps model
%generates  high trading volume when the state changes either 
