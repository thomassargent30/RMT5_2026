...
$$ {\rm Prob}(x_{t+k} = e_j \vert x_t = e_i) =
     P^{(k)}_{ij}  .$$
The unconditional probability distributions of $x_t$ at $t \geq 1$ are
determined by
...
\medskip
\definition{absorbing} A collection set of states $\tilde X \subset X = \{e_1, \ldots, e_n\}$ is said to be
an {\it absorbing subset\/} if both of the following conditions hold:
\item{{\bf i.}} for any nonnegative integer $T$,
if $e_i \in \tilde X$ is visited at time $t=T$, then only states $e_j \in \tilde X$ are visited at time $t \geq T$.%starting from $e_i \in \tilde X$ at time $t=0$, only states $e_j \in \tilde X$ are visited at $t \geq 1$.
...
that a probability equals the mathematical expectation of an indicator function.}  In section \use{sec:HMM}
...
we  use decision rule $F_0$ from time $1$ onward,  so that
...
of $z_1$ on all $k$ components of $z$.
...
and covariance matrix $\Sigma$. As a source, for example,  we can take an instance of the state-space  a state-space system \Ep{statesp1},  with
...
that in general is not linear in $z_2$ when $z$ is not distributed according to a multivariate normal distribution.\NFootnote{There exist other classes
of joint probability densities for which conditional expectations are linear least squares projections, for example, the multivariate Laplace distribution,
another member
 of the class of elliptical distributions of which the multivariate normal distribution is also a member.}
...
\section{The Lucas Critique}\label{sec:lucascritique}%
 We can concisely describe how Lucas (1976) criticized a class of  procedures that were widely used in applied work at the
 time that he wrote.
...
$$ \EQNalign{ x_{t+1} & = A x_t + C w_{t+1}  \EQN lucas101;a \cr
             ...
             S & = H G I(I - \delta A)^{-1}  \EQN lucas101;d \cr} $$
where $A$ is a stable matrix, $\delta \in (0,1)$ is a discount factor,
   ...
   $\{x_t\}_{t=0}^\infty$ process.  As usual, $w_{t+1}$ is the time $t+1$ vector in  an i.i.d.~sequence of ${\cal N}(0,I)$ random vectors.
   Here $d_t$ is a vector of decisions chosen by people an econometrician wants to model.
...
forecasts of       discounted sums of current and future $y_t$ vectors, forecasts that equal the appropriate conditional expectations
   (please see equation \Ep{discount1}). To fix ideas, take a univariate autoregression for $\{y_t\}$, namely,
...
$d_t$ as a function of time $t$ information $x_t$ about future $y_{t+j}$'s.
...
$\bmatrix{  \Sigma_{dd} & S\Sigma_{dx} \cr \Sigma_{xd} S' & \Sigma_{xx}}$, where $\Sigma_{xx}$ solves  the discrete Lyapunov equation $\Sigma_{xx}
= A \Sigma_{xx} A' + C C'$ and $\Sigma_{dx} = S \Sigma_{xx}$.  It immediately follows that  $S$  equals a matrix of population regression coefficients of $d_t$ on $x_t$
...
Lucas (1976) described and criticized a collection of  studies that had used the following logic.

\medskip

\item{a.}  A researcher does not understand that    $S$ satisfies restriction \Ep{lucas101;d} but wants to know  the decision rule for $d_t$ .
...
\item{c.}  Mistakenly as it will turn out, the researcher  assumes that  the  decision rule $d_t = S x_t$ that he has estimated
 ...
 arbitrary hypothetical alterations  of the right-hand side variables $\{x_t\}$ including ones that are no longer well  described by the law of motion
...
under alternative rules for generating
 ...
 alterative state-dependent  ``policy rules'' to  generating these.} and then evaluates the
resulting $\{d_t, x_t\}_{t=T+1}^{\tilde T}$
outcomes according to a social  welfare function.   %, that are functions of the joint sequence $\{d_t, x_t\}$.


\medskip

\item{e.} After
studying how the  welfare function varies with the hypothetical rule for setting the $\{x_t\}_{t=T+1}^{\tilde T}$ sequence,
the researcher recommends either that rule for setting $\{x_t\}$ recursively or else  a particular a continuation $\{x_t\}_{t=T+1}^{\tilde T}$
sequence that yields desirable outcomes. For example, the researcher holds $S$ fix and uses it as a component
of a state-space system to which he applies the linear-quadratic control theory to be described in chapter \use{dplinear} to  recommend a
law of motion for $x_t$ that manifests itself as a new pair of matrices $\tilde A, \tilde G$  that in the future
will replace $A, G$ in the state-space representation \Ep{lucas101}.

\medskip

\item{f.} By failing to understand that the decision rule $S$ under the new policy will be $\tilde S = H \tilde G (I - \beta \tilde A)^{-1}$,
the researcher is disappointed to find that the regression of that he estimated on the historical data no longer describes data generated
under the policy $\tilde G, \tilde A$ that he has recommended.


\medskip

In essence, Lucas (1976) used this  six part analysis to  characterize an influential  1940s-1960s econometric policy evaluation
research program that  applied optimal control techniques to Keynesian and monetarist
econometric models in order to create  recommendations for improved  macroeconomic policies.
Lucas argued  that the procedure is flawed because it  ignore the key cross-equation restrictions \Ep{lucas101;c}. Altering the policy-generating
matrices from $G, A$ that prevailed historically to new ones  $\tilde G, \tilde A$ that describe  hypothetical policy ``experiments'' that we want to study
  amounts to a   ``regime change'' that
 will systematically change the dependence of $y_t$ on $x_t$
 described by
...
%in
 ...
 A
 change in the decision maker's  environment $G, A$   will change the moment matrices
$ \Sigma_{xx}, \Sigma_{dx}$ and therefore the population regression matrix of $d_t$ on $x_t$ that will characterize outcomes
under the new post-recommendation regime. Thus,  structure \Ep{lucas101}, and in particular the cross-equation restriction \Ep{lucas101;c},
implies that $S$ will vary {\it systematically\/} with any such hypothetical changes
...
of $d_t$ conditional on  $x_t$ process both  depend on the $A,C$ matrices.
...
that any change in policies will systematically affect the structure of econometric models.}{``Econometric Policy Evaluation: A Critique,'' 1976}
 ...
 series relevant to the decision makers.''  We shall encounter more examples of  such restrictions  on optimal decision rules in
 section \use{sec:LQmodel}, in chapter \use{dplinear},  and  also in other chapters.
...
restrictions  that system \Ep{lucas101} encodes.\NFootnote{See the introductory essay in Lucas and Sargent (1981), a volume containing
 early contributions to  rational expectations econometrics.  Also, see Sargent (1981) and Hansen and Sargent (1991a) for
further
...
\index{filter!linear}%
 to build up the likelihood function of the
 observed variables.\NFootnote{See Hamilton (1994), Canova (2007), DeJong and Dave (2011), and section \use{sec:estimation2} below.}
 In section \use{sec:Kalman100}, we apply the population regression formulas of section \use{sec:populationregression} to derive the Kalman filter.
  %\NFootnote{Also see chapter \use{dplinear}
% and Appendix \use{lincontrol} (see Technical
% Appendixes).}
 Then in section \use{sec:estimation2}, we use the innovations representation associated with the Kalman filter as a device that tells us how to
...
Equation \Ep{kalf10new} is a matrix Riccati difference equation that  it restricts a sequence of conditional
...
The random vector  $a_t = y_t - E[y_t | y_{t-1}, \ldots, y_0]$ is called the  `innovation' for $y_t$ with respect to the
...
$[y_{t-1}, y_{t-2}, \ldots, y_0]$\NFootnote{This is an instance of a  Gram-Schmidt procedure. Please see appendix \use{appc1}.}
\index{Gram-Schmidt process}%
...
\medskip
\appendix{A}{Ergodicity}\label{appergodicMC}%
...
The basis vectors $v_1, v_2, \ldots, v_m$ are elements of a sequence of least squares errors.
\index{Gram-Schmidt process}%

%When the sequence of vectors or random variables  $x_1, x_2, \ldots, x_m$ is  ordered in time,
We can  use a Cholesky decomposition to describe
...
\appendix{E}{Differentiating matrix forms}\label{appendixLQA}%
...
(If necessary, please consult  formulas for differentiating quadratic forms presented in appendix \use{appendixLQA} of chapter \use{dplinear}.)
