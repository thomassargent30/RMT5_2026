...
the $x_t$ process,
we note that    the initial condition states that   $x_0$ is drawn from a distribution with mean $ \mu_0 = E x_0$ and
covariance $\Sigma_0 = E (x- E x_0) (x - E x_0)'$.
...
with $w_{t+1} \sim N(0,I)$ and the assumption that $x_0 \sim N(\mu_0, \Sigma_0)$, then
the $\{x_t\}_{t=0}^\infty$ process is wide-sense stationary (also known as  just plain stationary).  Stationarity
implies that all moments, not just means and covariances,  are independent of calendar time.
...
periods and not on time $t$. In this case, an autocovariance  matrix sequence $\{\Sigma_{t+j,t}\}_{j=0}^\infty$ is often  also called an {\it
...
discount factor} process.  The price of a perpetual claim on
...
First-order conditions for this problem can be rearranged to attain
...
%space systems under assumptions described above.
 Although
 the idea and our formulas  apply  to a much wider class of probability distributions,  until  subsection \use{sec:nonnormal}
...
$f(z_1 | \mu_1, \Sigma_1)$ and $f(z_2 | \mu_2, \Sigma_2)$, respectively, where $f( z_i |\mu_i, \Sigma_i)$ denotes a multivariate normal
density with mean vector $\mu_i$ and covariance matrix $\Sigma_i$. %first and second moments  $\mu_i, \Sigma_i$.
...
$$\beta = \Sigma_{12} \Sigma_{22}^{-1} \EQN leastsq12 $$ is an $(N- k) \times k$ matrix
 of population {\it regression coefficients \/} of $z_1 -\mu_1$ on $z_2- \mu_2$.
...
of $z_1$ on all $k$ components of $z$.  So equation \Ep{regresslinear} asserts that
...
$x_t$ and $y_t$ at various times $t$.  Of course, there are as many  regressions \Ep{eqn:regression_nonsense}
  as there are ways of selecting
$z_1, z_2$.   Thus, we have shown how a linear state-space model (e.g., a triple $A, C, G$,
together with an initial distribution for $x_0$) implies a population
regression.    Inferring what a given
regression tells about a model is often challenging. Many regressions tell little about a model, and what little they
...
A Bayesian statistician breathes life into these equations % \Ep{eqn:Bayes101}
 by adopting particular interpretations of the probability
...
the density of a potentially observed  random vector $z_2$ conditional on  $z_1$,  the density $f(z_1)$ as a ``prior''
 probability density
...
A Bayesian statistician has  an interesting interpretation of equation \Ep{condmeansurprise}. The equation
 asserts that the conditional mean $E z_1 | z_2 \equiv \hat \mu_1$ equals the unconditional mean $\mu_1$ before observing $z_2$
plus  $\beta (z_2 - \mu_2)$, the systematic part of the regression  of the unobserved surprise $z_1 - \mu_1$ in $z_1$ on an observed surprise
$z_2 - \mu_2$ in $z_2$.
A Bayesian statistician  interprets $\mu_1$ as the mean of her prior distribution for $z_1$ and $\hat \mu_1$ as the mean of her
posterior distribution for $z_1$ after observing $z_2$, while $\Sigma_{11}$ would be her prior covariance matrix and $\hat \Sigma_{11}$ would be her
posterior covariance.

\subsection{Non normal distributions}\label{sec:nonnormal}%
We have noted that for a multivariate normal   random vector $z$, a conditional expectation is linear in the conditioning variable,
and that it minimizes the mean squared error.  Now let's make the weaker assumption that $z$ is an $N \times 1$ random vector
...
conditional expectation'  to distinguish it from the conditional expectation  $ E z_1 | z_2$
...
that $E[x_1 | y_0] = A\hat x_0$ and that
...
$[y_{t-1}, y_{t-2}, \ldots, y_0]$\NFootnote{This is an instance of a  Gram-Schmidt procedure.}
...
\appendix{B}{Constant in the state}\label{appb1}%
...
=0$ is $\mu_2=0$ because  $1$ is {\it not\/} an eigenvalue of
$\tilde A$. It follows that the matrix $A=
   \left[\matrix{1 & 0 \cr 0 & \tilde A \cr}\right]$
on the right side of \Ep{statesp10} has one eigenvector associated
with a single unit eigenvalue: $(A  - I)\left[\matrix{\mu_1 \cr
...
of $A$. This eigenvector is an unconditional mean of $x_0$ that makes the process
covariance stationary.%
\NFootnote{That the  eigenvalues of $\tilde A$ are less than unity
in modulus implies that starting from any $\mu_0$, $\mu_t
\rightarrow \mu$. To understand this, assume that the
...
where $ \rho \in [0, 1)$ and $c$ are scalars, $\{w_t\}_{t=0}^\infty$ is an i.i.d.~scalar process with $w_t \sim N(0,1)$,
 ...
 $v_t \sim N(0,r)$ with $r >0$, and $x_0$ is a random initial value distributed as $x_0 \sim N(0, {\frac{c^2}{1 - \rho^2}})$. (Note: Muth (1960)
...
covariance generating function in the form of the $z$-transform of the autocovariance sequence $\{c_y(\tau)\}_{\tau =-\infty}^\infty$,
...
$$ \eqalign{ S_y(z) & = r + \left({\frac{c}{1-\rho z}}\right) \left({\frac{c}{1-\rho z^{-1}}} \right) \cr
                    ...
                    & = {\frac{ c^2 + r(1 + \rho^2) - r \rho z - r \rho z^{-1}}{(1 -\rho z)(1 -\rho z)}}. } $$
...
$$ S_y(z) =  {\frac{\omega ( 1 - (\rho -k )z)( 1 - (\rho - k)z^{-1} )}    {(1 -\rho z)(1 -\rho z)}} .  $$     
...
\use{dplinear}.  Equation   (1) as an instance of a more general spectral  {\it factorization identity\/}. Also, please note the steady state Kalman filter generates a Kalman 
gain $k$ that is a key to  achieving the factorization. 
\index{factorization identity}%
...
\medskip
\noindent{\bf b.}  Please plot the spectral density $S_a(\omega)$ against $\omega \in [0, \pi]$ and compare it with those of the scalar processes
...
where $\{w_{t+1}\}_{t=0}^\infty $ is an i.i.d.~sequence with $w_{t+1} \sim N(0,I)$ for all $t$
and $\{v_t\}_{t=0}^\infty$ is an i.i.d.~sequence with $v_t \sim N(0, R)$ for all $t$; and $E w_t' v_s = 0$
for all $t$ and $s$.
...
The shock $a_t$ is i.i.d.~with distribution $a_t \sim N (0, \Omega)$.
