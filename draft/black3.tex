...
where $\otimes$ is the Kronecker product.\NFootnote{If A is an $m \times n$ matrix and B is a $p \times q$ matrix, then the Kronecker product $A \otimes B$ is the %mp-by-nq 
$m p \times nq $ block matrix
...
\use{assetpricing2}.
\auth{Hansen, Lars P.}
 \auth{Tallarini, Thomas D.} \auth{Alvarez,
