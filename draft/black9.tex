...
value of their endowment sequences.  These results have qualified counterparts
...
influential paper of Barro (1974) that reignited interest in Ricardian equivalence.}
...
that $c \geq 0$.  There is no uncertainty.
...
a household to lend but not borrow.  The alternative
is to  permit a household to borrow, but only an amount
...
subject to a given initial condition for initial assets $b_0$, the budget constraints \Ep{ricardo2},  and $b_{t+1} \geq 0$ for all $t$. 
First-order
...
\quad   Assume $b_0 = 0$ and endowment path
...
does not necessarily indicate  that the consumer is borrowing constrained. We say that he is borrowing constrained if the Lagrange multiplier
on the constraint $b_t \geq 0$ is not zero.}
\medskip

\noindent{\it Example 2:} \quad  Assume $b_0 =0$ and endowment path
...
the period with the highest annuity value for the continuation
...
policy satisfies the government budget    constraints \Ep{ricardo7}, and
...
is that the original consumption plan $\{\bar c_t\}_{t=0}^\infty$, but adjusted
...
In particular, notice that the government's budget set at time $0$ does
...
This proposition depends on imposing  on the household  only the natural debt limit,
which is weaker than a no-borrowing constraint.
...
and  not fall below zero. This means that under the no-borrowing constraint,
...
$\{\bar \tau_t\}_{t=0}^\infty$ equilibrium or the new $\{\hat \tau_t\}_{t=0}^\infty$ equilibrium eliminate
...
be the tax   sequence in the initial equilibrium, and let
...
\section{Linked generations interpretation}
  Much of the
preceding analysis  applies to a setting
...
role of savings $b_{t+1}$ in the previous model.  What was a positive
savings outcome $b_{t+1} >0$  in the previous version of the
...
and shifts taxes  from the future toward the present. A
large enough shift of taxes toward the present  will require  that the government be a lender
in early periods. But  households are not allowed to become borrow, so
the government must lend abroad and not to domestic tax payers. That breaks Ricardian equivalence.

Readers might be able to  anticipate
a proof of Ricardian
equivalence in the chapter \use{assetpricing1} general equilibrium model.
...
Third, at those same interest rates, it can be shown that households
...
Finding  Ricardian equivalence in the infinitely lived agent model
is a useful starting point for isolating alternative assumptions under
...
continue to hold if agents care about their descendants
and there is an operational bequest motive.
 Bernheim and Bagwell (1988)
take this argument further and formulate a model in which all
agents are interconnected through links across and within families.
  They show how those linkages can become extensive enough  to render neutral all redistributive
...
variety of taxes.
\auth{Bassetto, Marco} \auth{Kocherlakota, Narayana R.}\auth{Kimball, Miles S.} \auth{Barsky, Robert}
