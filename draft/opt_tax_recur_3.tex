...
where $\pi_t(s^t)$ is a joint density over $s^t$ induced by $\pi_0, \Pi$.
 ...
 $$
c_t(s^t) + g(s_t) = n_t(s^t).                        \EQN TSs_techr
$$
The government faces a sequence of budget constraints whose time $t \geq 0$ component is
 $$ g(s_t) =   \tau^n_t(s^t)  n_t(s^t)
              + \sum_{s_{t+1}} p_{t+1}(s_{t+1} | s^t) b_{t+1}(s_{t+1} | s^t)
               - b_t(s_t | s^{t-1}),   \EQN TS_govr $$
where  the technology pins down the pre-tax wage rate to unity, $p_{t+1}(s_{t+1}|s^t)$ is the competitive equilibrium price of
...
$$c_t(s^t) + \sum_{s_{t+1}} p_{t+1}(s_{t+1} | s^t) b_{t+1}(s_{t+1} | s^t)
=  \left[1-\tau^n_t(s^t)\right]  n_t(s^t) + b_t(s_t | s^{t-1}) .
         \EQN TS_bcr
$$
First-order conditions for the household's  problem imply

$$ 1 - \tau_t^n(s^t) = {\frac{u_\ell(s^t)}{u_c(s^t)} }  \EQN LSA_taxr $$
...
\subsection{Finding the state is an art}
...
$$   x_t(s^t)  = u_c(s^t) [ n_t(s^t) - g(s_t)]  - u_\ell (s^t) n_t(s^t) + \beta \sum_{s_{t+1}\in {\cal S}} \Pi (s_{t+1}| s_t) x_{t+1}(s^{t+1}).
 \EQN LSA_budget1r $$
...
We can think of $x_t$ as indexing a competitive equilibrium with distorting taxes.  Equation \Ep{LSA_budget1r} shows implicitly how $x_t$ depends
on future government policy, via  marginal utilities $u_c$ and $u_\ell$  linked to government policies through the household's first-order necessary
...
\subsection{Intertemporal delegation}
...
not $x_0$, as a state variable.
%L%Furthermore,  the Ramsey planner cares about $(c_0(s_0), \ell_0(s_0))$,
%L% while continuation Ramsey planners do not.
...
%L%preferences,
authorities and responsibilities across time    express  continuation Ramsey planners' obligations to
...
$$ V(x, s) = \max_{n, \{x'(s')\}} \left\{u(n-g(s), 1-n) + \beta \sum_{s'\in S} \Pi(s'| s) V(x'(s'), s') \right\}\EQN LSA_Bellman1 $$
where  maximization over $n$ and the  $S$ elements of $x'(s')$ is subject to the
single implementability condition %L%for $t \geq 1$
$$ x = u_c\left[n-g(s)\right] -  u_\ell n + \beta \sum_{s' \in {\cal S}} \Pi(s'|s) x'(s')   \EQN LSA_Bellman1cons $$
coming from constraint \Ep{LSA_budget1}.
Here $u_c$ and $u_\ell$ are today's  marginal utilities.\NFootnote{Equation \Ep{LSA_Bellman1cons} expresses a competitive equilibrium $x$ today
in terms of a current period allocation $(c,n)$ and a continuation competitive equilibrium $x'(s')$ for each state $s'$ tomorrow. See
...
$$ \eqalign{ n_t & = f(x_t, s_t), \quad t \geq 1 \cr
             x_{t+1}(s_{t+1}) & = h(s_{t+1}; x_t, s_t), \quad s_{t+1} \in  {\cal S}, \, t \geq 1.} \EQN RRpolicyt $$
...
The Bellman equation for the time $0$ Ramsey planner is \index{Bellman equation}%
$$ W(b_0, s_0) = \max_{n_0, \{x'(s_1)\}} u(n_0 - g_0, 1 - n_0) + \beta \sum_{s_1 \in {\cal S}} \Pi(s_1| s_0) V( x'(s_1), s_1)  \EQN LSA_Bellman2 $$
where  maximization over $n_0$ and the $S $ elements of $x'(s_1)$ is subject to the time $0$ implementability condition
$$   u_{c} b_0 = u_{c} \left[n_0 - g_0\right] - u_{\ell} n_0  + \beta \sum_{s_1\in {\cal S}} \Pi(s_1 | s_0) x'(s_1) \EQN Bellman2cons $$
coming from constraint \Ep{LSA_budget1} at time $0$.
Associated with a value function $W(b_0, s_0)$ that solves Bellman equation \Ep{LSA_Bellman2} are $S +1 $
...
\Ep{RRpolicy0} for a Ramsey planner versus  $(x_t, s_t)$ in the policy functions \Ep{RRpolicyt} for   time $t \geq 1$ continuation  Ramsey planners.
...
$E_t \sum_{\tau = t}^\infty \beta^{\tau - t} u(c_t, \ell_t)$, where  consumption and leisure processes are evaluated along the original time $0$ Ramsey plan.
...
Attach a Lagrange multiplier $\Phi_1$ to constraint \Ep{LSA_Bellman1cons} and a Lagrange multiplier
...
$$ \beta \Pi(s'|s) V_x (x'(s'), s') - \beta \Pi(s' | s) \Phi_1 = 0 \EQN LSARxt $$
for $x'(s'), s'\in{\cal S}$,  and
$$ (1 + \Phi_1) (u_c - u_\ell ) + \Phi_1 \left[ n (u_{\ell \ell} - u_{\ell c}) + (n-g(s)) (u_{cc} - u_{c \ell})  \right] = 0  \EQN LSARnt $$
for $n$.
Given $\Phi_1$, equation \Ep{LSARnt} is one equation to be solved for $n$ as a function of $s$ (or of $g(s)$).
Equation \Ep{LSARxt} implies $ V_x(x'(s'), s')= \Phi_1$, while an envelope condition is $V_x(x,s) = \Phi_1$, so it follows that
$$ V_x(x'(s'), s') = V_x(x,s) = \Phi_1 . \EQN LSAenv $$


For the time $0$ problem on the right side of the Ramsey planner's Bellman equation \Ep{LSA_Bellman2}, the first-order conditions
are
...
%$$
\offparens
$$ \EQNalign{  (1 + \Phi_0) (u_{c} - u_{\ell}) & + \Phi_0 \bigl[ n_0 (u_{\ell \ell} - u_{\ell c} ) +  (n_0 - g(s_0)) (u_{cc} - u_{c \ell}) \Bigr] \cr &
  \quad \quad \quad -  \Phi_0 (u_{cc} - u_{c \ell}) b_0  =  0 .\EQN LSAn0}  $$
\autoparens
...
the three special cases in which  (a) $b_0 = 0$, or (b) $u_{cc}=u_{c \ell}=0$, i.e., preferences are quasi-linear  in consumption,  or
...
The presence of the extra term $\Phi_0 [u_{cc}(s_0) - u_{c \ell}(s_0)]\,b_0 $ in the first-order condition at time $0$ expresses the incentive for the Ramsey planner
to manipulate Arrow-Debreu prices in order to affect $u_{c}(s_0) b_0 = x_0$.
...
\subsection{State variable degeneracy}
...
\subsection{Symptom and source  of time inconsistency}

While the marginal utility adjusted level of government debt $x_t(s^t)$ is  a key state variable for the continuation Ramsey planners at  $t \geq 1$,
it is  not a state variable at time $0$.  The time $0$ Ramsey planner faces $b_0$,  not $x_0 = u_{c}(s_0)\, b_0$, as a state variable.
...
the {\it value\/} of government debt as measured by $u_{c}(s_0)\, b_0$.  In contrast, time $t \geq 1$ continuation Ramsey planners
are obligated {\it not\/} to alter  values of  debt, as measured by $u_{c}(s^t)\, b_t(s_t | s^{t-1})$, that they inherit from an earlier Ramsey planner or continuation Ramsey planner.
...
feature marginal utilities of consumption  $u_c(s^t)$
that, given $\Phi$, for $t \geq 1$  depend only on $s_t$, but that for $t=0$ depend on $b_0$ as well.  This means that $u_c(s^t)$ will be a time invariant
...
$p_{t+1}(s_{t+1} | s_t) = p(s_{t+1}|s_t)$ will be the {\it same\/} time invariant functions of $(s_{t+1}, s_t)$ for $t \geq 1$, but a different function $p_0(s_1|s_0)$
...
to manipulate Arrow security prices and, through them, the burden of initial government debt $b_0$. (For an illustration, see
section \use{sec:LucasStokey_ex1_b0} of chapter \use{optax}.)



%
%
% \subsection{Absence of history dependence}
...
\subsection{Recasting state variables}

In the AMSS setting, the government  faces a sequence of  budget constraints
$$   \tau_t^n(s^t) n_t(s^t) - T_t(s^t) +  b_{t+1}(s^t)/ R_t (s^t)
=  g(s_t) + b_t(s^{t-1})  ,$$
...
$$ b_t(s^{t-1}) + (1-\tau^n_t(s^t)) n_t(s^t) = c_t(s^t) + b_{t+1}(s^t)/R_t(s^t)  .  \EQN eqn:AMSSapp1  $$
As we saw in section \use{sec:AMSS},  the household's first-order conditions are
$u_{c}(s^t) = \beta R_t(s^t) E_t u_{c}(s^{t+1})$ and $(1-\tau_t^n(s^t)) u_{c}(s^t) = u_{\ell}(s^t)$.  Using these to eliminate
$R_t(s^t)$ and $\tau_t^n(s^t)$ from \Ep{eqn:AMSSapp1} gives
$$ b_t(s^{t-1}) + { \frac{u_{\ell}(s^t)}{u_{c}(s^t)}}\, n_t(s^t) = c_t(s^t) + {\frac{\beta \left[E_t u_{c}(s^{t+1})\right] b_{t+1}(s^t)}{u_{c}(s^t)}}
  \EQN eqn:AMSSapp2a $$
or
$$ u_{c}(s^t) b_t(s^{t-1}) + u_{\ell}(s^t) n_t(s^t) = u_{c}(s^t) c_t(s^t) + \beta \left[E_t u_{c}(s^{t+1})\right] b_{t+1}(s^t) . \EQN eqn:AMSSapp2 $$
For the purpose of posing a recursive version of the Ramsey problem, define
$$ x_t(s^t) \equiv \beta b_{t+1}(s^t) E_t u_{c}(s^{t+1}) = u_{c} (s^t) {\frac{b_{t+1}(s^t)}{R_t(s^t)}} \EQN eqn:AMSSapp3 $$
and represent  household budget constraint \Ep{eqn:AMSSapp2} at time $t$, history $s^t$  as
% the value of government debt
% $ {\frac{b_{t+1}(s^t)}{R_t(s^t)}}$ at time $t$.}
$$ {\frac{u_{c}(s^t) x_{t-1}(s^{t-1})}{\beta E_{t-1} u_{c}(s^t)}} = u_{c}(s^t) c_t(s^t) - u_{\ell}(s^t) n_t(s^t) + x_t(s^t)   \EQN eqn:AMSSapp4 $$
for $t \geq 1$.

\subsection{Measurability constraints}
...
$$ b_t(s^{t-1})  = c_t(s^t) -  { \frac{u_{\ell}(s^t)}{u_{c}(s^t)}} \,n_t(s^t) + {\frac{\beta \left[E_t u_{c}(s^{t+1})\right] b_{t+1}(s^t)}{u_{c}(s^t)}} .
  \EQN eqn:AMSSapp2b $$
...
adds  to  the single implementation condition \Ep{TSs_cham15r} imposed  by the Lucas and Stokey model.\NFootnote{These measurability constraints put the equilibrium allocation in a ``marketable subspace'' in the sense of Duffie and Shafer (1985).}
...
$$ {\frac{u_c(s) x_- }{\beta  \sum_{\tilde s} \Pi(\tilde s|s_-) u_c(\tilde s) }} = u_c(s) (n(s) - g(s)) - u_\ell(s) n(s) + x(s) . \EQN eqn:AMSSapp6 $$
...
$(x_{t-1}, s_{t-1}) = (x_-, s_-)$ as given and chooses $(n(s_t), x(s_t)) = (n(s), x(s))$ for $s \in  {\cal S}$ before $s_t$ is realized.
...
subject to
$$ u_{c}(s_0) b_0 = u_{c}(s_0) (n_0-g_0) - u_{\ell}(s_0) n_0 + x_0 . \EQN eqn:AMMSSapp101 $$
...
\subsection{Martingale replaces state-variable degeneracy}

Let $\mu(s) \Pi(s|s_-)$ be a Lagrange multiplier on constraint \Ep{eqn:AMSSapp6} for state $s$.  The continuation Ramsey planner's first-order condition
with respect to $x(s)$ is
$$ \beta V_x(x(s),s) = - \mu(s) .\EQN eqn:AMSSapp7  $$
Applying the envelope theorem to  Bellman equation \Ep{eqn:AMSSapp5} gives
$$ V_x(x_-,s_-) = - \sum_s \Pi(s|s_-) \mu(s) {\frac{u_c(s)}{\beta \sum_{\tilde s} \Pi(\tilde s|s_-) u_c(\tilde s) }} . \EQN eqn:AMSSapp8 $$
...
$u(c,\ell)= c + H(\ell)$.   In this case,  $V_x(x,s)\leq 0$  is a non-positive martingale.  By the martingale convergence
...
$$ (1+\mu(s) ) (1 - u_\ell(s)) + \mu(s) n(s) u_{\ell \ell}(s) =0  .$$
Since $\mu(s) = - \beta V_x(x(s),x) $ converges to zero, in the limit $u_\ell(s)= 1 =u_c(s)$, so that
%L%$\tau(x(s),s) =0$
the tax rate on labor converges to zero. In the limit, the government
...
\specsec{Remark:} Along a Ramsey plan, the state variable $x_t = x_t(s^t)$  becomes a function of the history $s^t$ and also
...
and equal to
the Lagrange multiplier on the single time $0$ implementability condition present in the original version of that model cast in terms of choice of sequences.
