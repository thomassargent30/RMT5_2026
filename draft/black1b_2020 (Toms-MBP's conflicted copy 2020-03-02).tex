...
This chapter uses tools from chapter \use{timeseries} to present classic applications.
  Section \use{Kfilterapplications} uses
...
Milton Friedman.  Muth used   methods other than the Kalman filter  to attack the problem.  It turns out that the Kalman
filter allows a more concise formulation. Section \use{Kfilterapplications} describes  how the Kalman filter makes an appearance  in
a classic paper of Jovanovic (1979). Section \use{sec:LQmodel} puts several of our chapter \use{timeseries}  tools to
study properties of  a workhorse benchmark model in macroeconomics
...
encounter often in later chapters. Section \use{sec:lucascritique} uses our chapter \use{timeseries} to present Robert E. Lucas's (1976)
Critique of some {\it circa\/} 1970's econometric policy evaluation procedures, while section \use{sec:lucascritique2} describes   responses
to  Lucas's Critique.
\auth{Muth, John F.}%
...
restrictions  that system \Ep{lucas101} encodes.\NFootnote{See  Sargent (1981) and the introductory essay in Lucas and Sargent (1981), a volume containing
 early contributions to  rational expectations econometrics.  See  Hansen and Sargent (1991a) for
further
...
\section{Interpretations and responses to the Lucas Critique}\label{sec:lucascritique2}%
Section \use{sec:lucascritique} already mentioned a principal response to the Lucas Critique,  namely,
rational expectations econometrics, an estimation and interpretation strategy now widely used in  applied macroeconomics.
...
that are based
on the perspective that changes in  government  decision rules  or ``regimes''   don't occur in a vacuum but were
understood during the historical
sample period being modeled econometrically.  Thus, this view imagines  government decision rules as being  governed by statistical processes
 that are known to the agents inside a rational expectations model.  So  changes in government decision rules
are anticipated and  properly responded to in advance by private agents.
These ideas adopt and elaborate a perspective of Sargent and Wallace (1976) that regards government policy rules  as being
determined by a  process that  leaves  econometricians and policy analysts with no ``free parameters'' to change,
disabling them from being able to offer policy advice. Thus, Sargent and Wallace argued that
...
Sargent and Wallace (1976), p.~181}
 ...
 This perspective  was embraced and extended by
 Christopher A. Sims (1982) and Sargent (1984).
 We use tools encountered in chapter  to \use{timeseries} express these ideas.
\auth{Sims, Christopher A.}%
\auth{Zha, Tao}%

In describing their work with vector autoregressions with regime shifting coefficients and volatilities, Sims and Zha (2006)  link
...
\noindent Here we'll use chapter \use{timeseries} tools to represent and study these parts of Sims and Zha's description:

\medskip
\item{a.} How to represent ``nonlinearities'' in the form of dependencies of state-space coefficients that are functions of  a discrete  Markov state variable that
...
\medskip
\item{c.} How a best-fitting fixed-coefficient linear model that ignores regime changes can approximate observed outcomes.
...
We  can capture policy regime changes with  a linear state space model with coefficients governed by a finite state Markov chain to represent the nonlinearities mentioned by Sims and Zha.\NFootnote{Hamilton (1989) introduces this  type of model and labels it  as ``non-stationary''.  If  we assume that 
chain $\pi_0$ is  a stationary distribution, then the model is actually stationary, a property that we exploit  below.}
...
We assume that $\{s_t, x_{t}, y_t\}_{t=0}^\infty$ is governed by  the linear state-space system with   Markov-switching matrices  
...
We   want to compute the conditional  mathematical expectations
 $$ z_t = E ( \sum_{t=0}^\infty \beta^t y_{t+j} ) \Bigl| x_t, s_t  $$
  and
 $$ \tilde z_t =  E ( \sum_{t=0}^\infty \beta^t y_{t+j} ) \Bigl| x_t . $$
...
This is the vehicle that Sims and Zha mention when they say of the time-varying system that ``its behavior could be well approximated as linear and non-time-varying''.
...
where $E_t (\cdot)$ is a mathematical expectation conditioned on time $t$ information, which will be either $x_t$ or $(x_t, s_t)$, where $(x_t, s_t)$ are defined as follows.
...
\noindent{\bf ADD DESCRIPTION OF CAGAN MODEL AND ALSO CITE CAGAN AND SARGENT.}
...
%We let $\pi_t$ be the distribution over the finite Markov state $s_t$. Note that the
 We start knowing that the distributions  conditional on $(x_t, s_t)$ are multivariate normal:
...
we can deduce conditional (on $x_t$) means
$$ \eqalign{ E x_{t+1} | x_t & = \bar A_t x_t \cr
                    E y_t | x_t & = \bar G_t x_t      \cr
                    E z_t | x_t & = \bar H_t x_t . }$$
The appearance of  time subscripts $t$ in the formulas for $\bar A_t, G_t, H_t$ are a consequence of  $\pi_t$ not
being a stationary distribution. % for Markov chain $P$.

Having computed conditional means,  let's compute conditional (on $x_t$) covariances.
Since
$$ x_{t+1} - \bar A x_t = \cases{ (A_1 - \bar A_t) x_t  + C_1 w_{t+1} & if $s_t =1$ \cr
                                                    (A_2 - \bar A_t) x_t  + C_2 w_{t+1} & if $s_t =2$ } $$
it follows that
$$ \eqalign{ E (x_{t+1} - \bar A_t x_t ) (x_{t+1} - \bar A _tx_t ) ' | x_t & = \sum_i \pi_{t,i} [ (A_i - \bar A_t) x_t x_t' (A_i - \bar A_t)' + C_i C_i' ] \cr
       & \equiv \Sigma_t (x_t) ,  \cr } \EQN Sigmatxt $$ 
...
A time vector autoregression of the form \Ep{Vartimeinv}, namely,
...
\section{Conclusions}
The examples in this chapter are the foundations of much  theoretical and empirical work
at the core of modern applied macroeconomics.\NFootnote{For example, see
