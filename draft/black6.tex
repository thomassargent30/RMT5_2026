...
Sargent (1987a, ch.~XIV).  The recursive competitive equilibrium concept
...
In the course of the exposition, we
 ...
 There is a unit measure of  identical firms whose profit functions
make them want to forecast the aggregate output decisions
of other firms just like them in order to choose
their own output.  Output of firm $\omega \in \Omega = [0, 1]$ is $y_t(\omega)$ at time $t$.  Aggregate output 
at time $t$ is $Y_t = \int_0^1 y_t(\omega) d \omega$.  It will turn out that firms make identical decisions 
$y_t = y_t(\omega)$ at time $t$.     The output of any single firm has a negligible effect on
...
of aggregate output as unaffected by their own output decisions.
  Thus, each  competitive firm sells  output $y_t$
...
where $a_0 >0, a_1 > 0 $ and $Y_t = \int_0^1 y_t(\omega) d \omega$ is the market-wide level of
output.
The firm believes that market-wide output
follows the law of motions
...
values of $Y$.  To state a firm's optimization problem completely requires that
...
Then with a unit measure of  identical firms, setting $Y_t = y_t$ makes
 the actual law of motion for
output for the market
$$ Y_{t+1} = h(Y_t, Y_t). \EQN comp9a $$
...
\medskip
\noindent{\bf b.}  The law of motion $H$ satisfies $H(Y)= h(Y,Y)$.
...
more generally in section  \use{sec:rce22}.
   How might we find an equilibrium?  The mapping ${\cal M}$ is not a contraction and  there is no guarantee that  iterations
   ...
   for a method that Krusell and Smith (1998) use to approximate  a rational expectations equilibrium of an incomplete-markets economy.
...
An approach to computing an equilibrium is to seek to match the Euler equations
of the market problem with those for a planning problem that can
be posed as a  dynamic programming problem.
Optimal quantities from  the planning problem are 
...
The first term is the area under the inverse demand curve.
The planning problem is to choose a production
plan $\{Y_{t+1}\}_{t=0}^\infty$ to maximize
$$   \sum_{t=0}^\infty \beta^t S(Y_t, Y_{t+1}) \EQN comp11 $$
subject to an initial condition $Y_0$.
Let $V(Y_0)$ be the optimal value function for the planning problem. It satisfies
...
Applying the Benveniste-Scheinkman formula from chapter \use{dynamicdp1} %\Ep{130}
  gives
$$ V'(Y) = a_0 - a_1 Y + d(Y' - Y) . \EQN comp15 $$
Substituting this into equation \Ep{comp14} and rearranging gives\NFootnote{It is fruitful to note that the second order difference equation 
\Ep{comp16} takes the form of the second order difference equation in appendix B of chapter \use{timeseries} that has solution \Ep{second_order_solution}.}
$$ \beta a_0 + d Y_t - [\beta a_1 + d(1+ \beta)]Y_{t+1} + d \beta Y_{t+2} =0 .
         \EQN comp16 $$

Return to equation
\Ep{comp7} and set $y_t = Y_t$ for all $t$. 
Notice that with $y_t=Y_t$, equations \Ep{comp16} and \Ep{comp7} are identical.
The Euler equation for the planning problem is the same  second-order difference equation
that we derived by first finding the Euler equation of the representative firm and substituting into
it the condition  $Y_t =  y_t$ that ``makes a representative firm be representative''.  Thus, if
 it is appropriate to apply  the same terminal conditions for these two difference equations, which it is,
then we have verified that  a quantity sequence $\{Y_{t+1}\}_{t=0}^\infty$ that solves the planning problem is also  a competitive  equilibrium
quantity sequence.
Setting $y_t = Y_t$ in
...
by forming the optimal linear regulator problem associated with  Bellman equation \Ep{comp12}.    The optimal policy
...
As  an example of a recursive competitive equilibrium, we formulate   a schooling model of the type used by Sherwin Rosen.
  \auth{Rosen, Sherwin}%
...
discounted optimal linear regulator problem, i.e., a linear-quadratic dynamic programming problem of the type studied in chapter \use{dplinear}. 
%We ask the reader to verify that it  suffices to take $ X_t = \bmatrix{\theta_t \cr N_{t+3}}$ as the state for the planner's problem.
...
To introduce the concept of a Markov perfect equilibrium, it is instructive to consider a dynamic model of duopoly.
...
market price. \NFootnote{Thus, we no longer assume that  firms are price takers, as they would be in a competitive equilibrium.
A consequence of departing from the price-taking framework is that the market
outcome  no longer maximizes  welfare, measured as the sum of consumer and
producer surplus. See exercise {\it \the\chapternum.4\/} for the case of a monopoly.}
The  one-period return
function of firm $i$ is
$$ \pi_{it} = p_t q_{it} - .5 d (q_{i t+1} - q_{it})^2.  \EQN game1 $$
There is a demand curve
$$ p_t = a_0 - a_1 (q_{1t} +  q_{2t}) .  \EQN game2 $$
Substituting the demand curve into equation \Ep{game1} lets us express
the return as
$$ \pi_{it} = a_0 q_{it} - a_1 q_{it}^2 - a_1 q_{it}q_{-i,t}
     - .5 d (q_{it+1} - q_{it})^2 , \EQN game3 $$
where $q_{-i,t}$ denotes the output of the firm other than $i$.
Firm $i$  chooses a decision rule that sets
$q_{it+1}$ as a function of $(q_{it}, q_{-i,t})$  and that  maximizes
$$ \sum_{t=0}^\infty \beta^t \pi_{it} .$$
Temporarily assume that  the maximizing
decision rule is $q_{it+1}  = f_i(q_{it}, q_{-i,t})$.


Given the function $f_{-i}$, the Bellman equation
of firm $i$ is
$$ v_i(q_{it}, q_{-i,t}) = \max_{q_{it+1}} \left\{
    \pi_{it} + \beta v_i(q_{it+1}, q_{-i,t+1}) \right\},  \EQN game4 $$
%or
%$$ v_i(q_{it}, y_{-i,t}) = \max_{y_{it+1}} \left\{ (a_0 y_{it} - a_1 y_{it}^2 - a_1 y_{it}y_{-i,t}
...
$$ q_{-i,t+1} = f_{-i}(q_{-i,t}, q_{it}). \EQN game5 $$
Note the interconnectedness of  the two problems for $i=1,2$.
...
\medskip
\noindent{\bf b.}  Policy function $f_i$ attains the right side
of  Bellman equation \Ep{game4}.
\medskip

  The adjective Markov denotes that the equilibrium decision
rules depend on the current values of the state variables
$q_{it}$ only, not other parts of their histories.   Perfect means `complete', i.e., that
...
future states, including many states  that will  not be realized along an equilibrium quantity path that emerges as  we iterate
 forward on the pair of Markov perfect equilibrium
policies $(f_1, f_2)$.
...
$$ v_i^{j+1}(q_{it}, q_{-i,t}) = \max_{q_{i,t+1}} \left\{
    \pi_{it} + \beta v_i^{j}(q_{it+1}, q_{-i,t+1}) \right\}, \EQN game4 $$
where the maximization is subject to
$$ q_{-i,t+1} = f^j_{-i}(q_{-i,t}, q_{it}). \EQN game5 $$

  Computing these iterations can be challenging.\NFootnote{See Levhari and
...
In this section, we show how an  optimal linear regulator
can be used to compute  a Markov perfect equilibrium for a model like that in the previous section.
That model is
...
by all players.\NFootnote{We intend that item (b) includes a complete description of a timing protocol specifying which player
chooses what when.}

  The optimal linear regulator is a good tool for formulating  and computing an equilibrium of a 
dynamic game.  A standard equilibrium
concept called subgame perfection  requires
...
that have a tractable mathematical structure.
...
s = t_0, \ldots, t\}$.
 ...
 contributes the adjective
``Markov'' to the phrase ``Markov perfect equilibrium.''
...
$$
This is an \idx{optimal linear regulator} problem that  can be solved
by working backwards. Evidently, player 2's problem is also an
optimal linear regulator problem.


Player 1's optimal decision rule satisfies
...
equation subject to  terminal condition $P_{1t_{1}} = 0$:
{ \ninepoint
$$\eqalign{& \, P_{1t}
...
Player 2's optimal decision rule satisfies
...
equation subject to terminal condition $P_{2t_1} = 0$:
...
Q_2)^{-1} B_{2t}^T P_{2t+1} (A_t - B_{1t} F_{1t}).\cr} \EQN orig-6$$
 ...
 Sequences of equilibrium decision rules  $\{F_{1t}, F_{2t}; t =
...
\Ep{orig-4}, \Ep{orig-5}, and \Ep{orig-6} to ``work backwards'' from time
...
unknowns in the matrices $F_{1t}$ and $F_{2t}$.
 ...
 Notice how $j$'s decision rule matrix $F_{jt}$ is a function of $\{F_{is},
s \geq t, i \neq j \}$. Thus, agent $i$'s choice of $\{F_{it}; t = t_0, \ldots,
t_1 - 1\}$ influences agent $j$'s choice of decision rules. In the
Markov perfect
...
of an infinite horizon game by driving $t_0 \rightarrow - \infty$.
...
\appendix{A}{A model of Judd}\label{appaJudd}%
This appendix describes the Markov perfect equilibrium of an
...
\medskip
\noindent{\bf d.} Within a Markov perfect equilibrium, does firm $1$ believe that its choice of $y_{1,t+1}$ influences $y_{2,t+j}$
for $j = 1, 2, \ldots$? Please explain your answer.
...
\medskip
\noindent{\it Exercise \the\chapternum.8} \quad {\bf An inverse problem}
\medskip
\noindent  Where $x \in {\bf R}$, $f >0, d >0 $,   $\beta \in (0,1)$, and $g \in  {\bf R} \rightarrow {\bf R}$, 
...
\medskip
\noindent{\it Exercise \the\chapternum.9} \quad {\bf Another inverse problem}
\medskip
\noindent   Where $x \in {\bf R}$, $f >0, d >0 $, and   $\beta \in (0,1)$, for what intertemporal problem is the following  a Bellman equation?
...
\medskip
\noindent{\it Exercise \the\chapternum.10} \quad {\bf Yet another inverse problem}
\medskip
\noindent  Where $x \in {\bf R}$, $f >0, d >0 $, $\sigma >0$,    $\beta \in (0,1)$, and $\phi$ is a univariate normal density function for a random
...
\medskip
\noindent{\it Exercise \the\chapternum.11} \quad {\bf An inverse problem}
\medskip
\noindent  Assume that  $F$ and $G$ are distinct  right continuous functions each mapping 
$[0, B]$ into $[0,1]$, that $\pi \in (0,1)$, and  that $\beta \in (0,1)$. 
For what intertemporal problem is the following a Bellman equation?
...
\medskip
\noindent{\it Exercise \the\chapternum.12} \quad {\bf Another inverse problem}
\medskip
\noindent Where $a_0 >0, a_1> 0, \gamma > 0$ and $\beta \in (0,1)$ suppose that
...
\medskip
\noindent{\it Exercise \the\chapternum.13} \quad {\bf Yet another inverse problem}
\medskip
\noindent Where $a_0 >0, a_1> 0, \gamma > 0$ and $\beta \in (0,1)$ suppose that
$$\eqalign{ p & = a_0 - a_1 Q \cr
           \pi &  = p q - \gamma(\hat q - q)^2 } $$
...
\medskip
\noindent{\it Exercise \the\chapternum.14} \quad {\bf Big $K$, little $k$ again}
\medskip
\noindent  Where $a_0 >0, a_1> 0, \gamma > 0$ and $\beta \in (0,1)$ suppose that
$$\eqalign{ p & = a_0 - a_1 Q \cr
           \pi &  = p q - \gamma(\hat q - q)^2 } $$
...
\medskip
\noindent{\it Exercise \the\chapternum.15} \quad {\bf Perfection?}
\medskip
\noindent  There are two firms indexed by $i = 1,2$.  A subscript $i$ indicates a  firm $i$ object  and a subscript $-i$ indicates
...
\medskip
\noindent{\it Exercise \the\chapternum.16} \quad {\bf Monopoly}
\medskip
\noindent Consider the following model of a monopolist.  Let  $S_t$ be sales at time $t$, $Q_t$ be production output at time $t$,
...
\medskip
\noindent{\it Exercise \the\chapternum.17} \quad {\bf A functional equation}
\medskip
\noindent  Let $C(Q) = c_1 Q + c_2 Q^2$ where $c_1, c_2 > 0$; $d(I, S) = d_1 I + d_2 I^2 + d_3 (S - I)^2$ where $d_1>0, d_2 >0, d_3 >0$; 
