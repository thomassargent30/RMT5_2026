...
economic growth.   We begin by describing a
 benchmark exogenous  growth model in which sustained growth
is driven by exogenous growth in labor productivity.
Then we turn our attention to several   endogenous growth
 models in which sustained growth of labor   productivity
...
We  facilitate comparison by using the same production
function  and simply changing the meaning
of one  argument.


\auth{Romer, Paul M.}%
In the spirit of Arrow's (1962)
\auth{Arrow, Kenneth J.}%
model of learning by doing, Romer (1986) presents an endogenous
growth model in which the accumulation of capital (or knowledge) is
...
The competitive equilibrium growth rate is less than  the
socially optimal one.

Another approach assumes
...
Romer (1987) constructs a model in which agents can choose to engage
...
by  smaller
supplies of  intermediate inputs and a lower growth rate than is
socially optimal.
...
a ``core'' of capital goods that is produced without  direct or
...
experience that he assumes gets embodied in capital goods,
while Romer (1986) postulates spillover effects of
firms' investments in knowledge.
In both analyses, the productivity of a given firm depends on
 an aggregate state variable, either the economy's stock of
...
This chapter has focused on the mechanics of endogenous
growth models, with only limited  motivation for assumptions.
...
reproducible, or that  a ``core'' of capital goods is  produced without
 direct or indirect use of nonreproducible factors.

Much effort in the endogenous growth literature has been expended to specify an appropriate
 technology. Even though growth is an endogenous outcome
...
$A_t^\nu$. The laissez-faire growth rate can exceed
the socially optimal rate because of how the new production function rearranges input
producers' market power, measured by the parameter $\alpha$, and the economy's
returns to specialization, measured by the parameter $\nu$.
...
excessive because of a {\it business-stealing\/} effect, where agents
\newpage \noindent fail to
