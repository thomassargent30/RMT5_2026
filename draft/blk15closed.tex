...
today in anticipation of future exchanges of goods constitutes a
...
antecedent to our villager-moneylender environment.
  Their counterpart to our moneylender is  a risk-neutral firm that forms a long-term relationship
...
relationship with a firm.  The firm seeks to maximize the
...
profits of $y_t - c_t$
(when compared to hiring a worker in the spot market for labor).
...
random spot market wage. Hence, they require that in a self-enforcing wage contract
 neither firm nor worker ever wants to
renege.

Kocherlakota
\auth{Kocherlakota, Narayana R.}%
(1996b) studies a model similar to Thomas and Worrall's.\NFootnote{The working paper of Thomas and Worrall
...
impediments to enforcement represented by his participation
constraints. \auth{Thomas, Jonathan}
\auth{Worrall, Tim}


To create a stationary stochastic environment, Kocherlakota assumes  two-sided lack of commitment.
In our section \use{sec:moneylender1} model of villagers facing a moneylender,
imperfect risk
...
villager economy of section \use{sec:moneylender1}.   After replacing the argument in the function in 
\Ep{Pdef;a} by the expected utility of the type 1 agent, Kocherlakota
writes the Bellman equation as \index{Bellman equation}%
...
period's endowment. In contrast, in our formulation above, in the current
...
Kocherlakota's single state variable in terms of our state vector,
$$
...
is related to our upper bounds $\{\overline x_j\}_{j=1}^S$ by
$$
...
v(c_{\triangle}) &=
\left( 1- \beta \sum_{s\in {\bf S}^o(c_{\triangle})} \Pi_s \right)^{-1}
\left\{  u(c_{\triangle}) \sum_{s\in {\bf S}^o(c_{\triangle})} \Pi_s  + \sum_{s\in {\bf S}^-(c_{\triangle})}
\Pi_s \Bigl[u(\overline c_s)  + \beta v(\overline c_s) \Bigr] \right.\cr
%\noalign{\vskip.2cm}
                      &\left. 
                      \quad \quad \quad +    \sum_{s\in {\bf S}^+(c_{\triangle})}
\Pi_s \Bigl[u(\underline c_s) + \beta v(\underline c_s)
                                   \Bigr]\right\}. \hskip1.3cm \EQN Koch_value2 \cr}
$$ }
%$$\EQNalign{
...
state variable of Kocherlakota (1996b) that we  described in the
preceding section.\NFootnote{See Krueger and Perri (2003b) for
another analysis of a two-state example.} Thus, at the end of a
...
a  value promised to such a consumer and let $c^+$ be the
...
In  our two-state example, there are only two
...
1-\overline c$ with continuation value $P(w^+)=w^-$. Given
implicit initial  consumption level $c_{\triangle}$, we can solve for the initial welfare assignment $\{v,
P(v)\}$.

\newpage

We can  use  updating rules \Ep{c_update} to get
...
One can argue that the alternative decentralization simply
converts one set of participation constraints into another.
For both specifications, we have a substantial departure from a decentralized
equilibrium under full commitment. When we
remove the assumption of commitment, we assign a very demanding
task to the ``invisible hand'' who now must not find market-clearing
prices but must also check participation/solvency
...
economy is \index{Bellman equation}%
...
From a general equilibrium perspective, when sellers of a state-contingent security are constrained with respect to the quantities that they can issue, it follows that the price is bid up when unconstrained buyers are competing for a smaller volume of that security. This tendency of lowering the yield on individual Arrow securities explains Alvarez and Jermann's result that interest rates are \newpage \noindent lower, when compared to a corresponding complete markets economy, a property shared with the Bewley economies studied in chapter 18.\NFootnote{In exercise {\it \the\chapternum.4,\/} we ask the reader
...
Further research about how to model the consequences of default in
...
recursive contract.  Formulate the Bellman equation. \index{Bellman equation}%
