...
%Lars%
The {\it choice\/} variables at $t$, $c_t$ and $b_{t+1}$, are
subject to both budget constraint \Ep{sprob2} and an upper 
bound on the amount of borrowing $R^{-1}b_{t+1}$. Because
without {\it borrowing limits\/} the consumer would  
optimally run a Ponzi scheme, whereby the consumer
chooses arbitrarily large consumption financed by
unlimited amounts of debt. 
At interior solutions for consumption,
first-order conditions for maximizing \Ep{sprob1} subject to
\Ep{sprob2} and borrowing limits are\NFootnote{We shall study how to derive this first-order condition
...
For reasons of tractability we assume
the quadratic  utility function
$u(c_t) =  -.5 (c_t - \gamma)^2$,
where $\gamma >0$ is a bliss level of consumption, and
we allow the consumer to produce rather than to
consume goods when choosing a negative
...
repayments next period.\NFootnote{For another 
example of intertwined consumption and production 
activities, see Kiyotaki och Wright's (1993)
search model of money and differentiated goods
in chapter \use{search2}. It is assumed that each 
consumer can produce a good from which other 
consumers but not he himself can derive utility. 
Hence, the consumer will use his produced good to 
exchange it for some other preferred good or for 
money in future bilateral trade opportunities.}
\auth{Kiyotaki, Nobuhiro} \auth{Wright, Randall}%
For this linear quadratic savings problem, 
we impose the following condition on the
consumption, borrowing plan:
$$  % \lim_{T \rightarrow +\infty}
 E_0 \sum_{t=0}^\infty \beta^t b_t^2 < +\infty. \EQN sprob3 $$
This condition suffices to rule out Ponzi schemes. Furthermore,
the quadratic utility function implies that Euler equation
...
To our help we
will exploit the fact that the solution to this linear quadratic
optimization problem satisfies {\it certainty equivalence\/},
to be shown in chapter \use{dplinear}, by which the optimal
...
inspired to proceed as follows. At time $t$, consider one possible path 
of realizations for the entire future of endowments
$\{y_{t+j}\}_{j=0}^\infty$ and associated sequentially
...
Imposing $\lim_{T\rightarrow +\infty} \beta^T b_{T+1} =0$ suffices to impose \Ep{sprob3} on the debt path. Then, do that
same operation for {\it all\/} possible realizations of the 
future on the right side of expression \Ep{sprob6} and sum 
over all those paths, each
one weighted by the conditional probability of it occuring:
$$ b_t = \sum_{j=0}^\infty \beta^j (E_t y_{t+j} - E_t c_{t+j}) ,  \EQN sprob6_subst $$
where the weights -- conditional probabilities -- sum to one
so the left side remains the same across expressions
\Ep{sprob6} and \Ep{sprob6_subst}.\NFootnote{Equation 
\Ep{sprob6_subst} holds in general for any savings
problem, i.e., the consumer will always satisfy his 
...
and Paxton (1994) and Storesletten, Telmer, and  Yaron (2004) for evidence that cross section distributions of consumption spread out with age.} We'll reconsider this outcome in the next several  sections of this chapter, and also in chapters \use{recurge}, \use{selfinsure}, \use{incomplete}, \use{socialinsurance} and
...
By adapting ideas of   Truman Bewley (1977, 1980) to the current setting, there is a way to fill out 
the  consumer's environment  that makes  $R = \beta^{-1}$ an equilibrium outcome.  
To do this, we imagine that our consumer is one of a  continuum of unit measure of  {\it ex ante} identical, but {\it ex post} heterogeneous consumers who trade a single risk-free one-period bond whose price is $\beta$.  
The consumers are {\it ex ante} identical in their preferences and in their stochastic processes for 
their non-financial incomes.  But they are {\it ex post} heterogeneous in  realizations of their non-financial incomes from time $0$ on.  We assume that their initial
non-financial incomes are independent draws from the stationary distribution  of their common statistical model \Ep{sprob15}, and that their initial financial wealth levels $b_0$  all equal  $0$. Our assumption about the initial distribution of non-financial incomes implies that the cross-section average of non-financial income equals the mean of its  stationary distribution each period.   
Since we are now analyzing a closed economy, it follows that the cross-section mean of consumption equals the stationary mean of non-financial income each period.\NFootnote{As in the derivations
above, any consumers' production of the good are here reported
as negative levels of consumption and hence, the extra consumption of the recipients of those produced goods are 
netted out in the cross-section mean of consumption.} 
...
$$ \eqalign{ E_0 c_t & = c_0 \cr
              E_0(c_t  - c_0)^2 & = t h h' . } \EQN eq:spreadcons $$
Because the consumer dislikes variance in consumption, he would prefers a completely smoothed   consumption stream  in which $c_t = c_0$ for all $t \geq 0$. Furthermore, such an allocation 
would be physically feasible since the cross section average of our continuum of consumers actually equals $c_0$ for all $t \geq 0$.  If before time $0$ everyone could get together and agree in advance to 
simply to share society's non-financial income equally, each person would prefer that allocation to the 
risky allocation they attain in our closed economy in which each period they trade one-period risk-free bonds.   In the next section, we describe an environment that our consumers would prefer to find themselves living in, one  with the same physical opportunities but with additional markets.  In particular, we add enough securities to the economy so that, by trading them, our {\it ex ante} identical consumers can  completely insuring themselves against random fluctuations in their
non-financial incomes. 
...
This view interprets  government decisions as  random  processes
 ...
 Aents forecast   government decisions optimally  under those processes.
