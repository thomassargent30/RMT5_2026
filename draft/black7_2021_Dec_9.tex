...
$s^\tau$, and being restarted from a distribution of  financial wealth  implied by the tails of each consumer's
...
limits, objects that were  only implicit in the time $0$ budget constraint \Ep{eq2} in the Arrow-Debreu economy.
...
time $t-1$ and history $s^{t-1}$ faces one such borrowing constraint for each possible
realization of $s_t$.
...
that consumer $i$ brings into time $t$ in history $s^t$. Suppose that
$ \tilde Q_t(s_{t+1} | s^t)$
...
after such a binding constraint,  it would be easy
...
As evident from Lagrangean \Ep{seqLagrangean}, solving a
consumer's optimization problem under sequential trading
with Arrow securities is no less onerous than the
optimization problem under time 0 trading with Arrow-Debreu
securities. While there are fewer securities traded at 
each date under sequential trading, a portfolio choice of  
one-period claims today can only be made by
simultaneously solving for all future portfolios of
...
adopt that very same numeraire. 
...
$\vec {\tilde a}_0(s_0)$ of the sequential-trading economy should be a zero vector. This is a natural guess, because
...
insight about how the sequential-trading competitive equilibrium
...
We conclude that equilibrium outcomes have
inherited the Markov property since the 
...
beginning-of-period wealth distribution that  equal 
...
quantity of the Arrow security $a$:
$$
...
expressions \Ep{allocREC} agrees with  \Ep{alloc} 
...
To solve for the beginning-of-period wealth
...
either of the budget constraint of consumer $i$, 
we can solve for the consumption level $\bar c^i$:
$$ 
\EQNalign{ \bar c^1 &= { 1-\beta\,\pi(0 \vert 0) \over
    1 + \beta \bigl[\pi(0 \vert 1)-\pi(0 \vert 0)\bigr]}
...
Our finding that an equilibrium  pricing kernel $Q(s'|s)$ and  endowments $y^i(s)$
are  functions only of a Markov state $s$ enables us to provide a
...
a set of borrowing limits $\{\bar A^i \}_i$
satisfying \Ep{borrowingconA},
...
\medskip
\noindent{\bf (c)} Consumption and asset
...
for all $t$ and $s'$ for all realizations of 
$\{s_t\}_{t=0}^\infty$.
